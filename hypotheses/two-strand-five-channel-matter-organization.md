...
Verdict: **null across the bracket** $\chi_w\in\{300,1000,3000\}$; no coupling binds the pair—$\chi_w=0$ is the bit-exact no-op (T1), the exploratory sub-critical arm $\chi_w=0.3$ (pre-bracket record `20260806_232104_two_strand_binding`) tracks control, and every bracket coupling collapses:
...
- (iii) **Yin-excess pair initialization**—the framework's attractive branch from the $G_{\mathrm{eff}}$ sign derivation (buoyancy is self-repulsive for the Yang-excess pair).
...
| ID | Quantity | Status |
|----|----------|--------|
| E1 | Binding: $d_0$, $K_d$, $T_d$, $\mu_d$, $\gamma_d$ | Open; TS1 null: the pair escapes at the lock timescale (d 9.90 → 15.73 cells at t = 40, §3.3), so no finite-$d_0$ binding coefficients are projectable from the existing PDE. The wake aggregation scratch layer is null as well (§3.4): no binding window—sub-critical no effect, super-critical Keller–Segel collapse; design-iteration candidates (ρ-weighted source, sub-critical feedback bound, Yin-excess init) remain open |
