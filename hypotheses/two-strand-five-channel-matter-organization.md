...
The open route is the wake sourced between the ridges ($S_W\propto|\nabla\varepsilon|^2$, the one source geometry in which the mass-like flux pulls the pair inward), a new model term. The steady-negative-attractor representation change is measured as §3.7: the mirror target $E_Y=\varphi^{-1}E_I$ realizes the unique constant-free $\Pi<0$ fixed point ($\Pi/\rho=-\varphi^{-3}$), while the sign-opposite half-space conversion $-\lambda(1-q)|\varepsilon|$ is a clamp-floor artifact with no interior fixed point. The Yin-excess branch's longer-window fate is measured rather than open: the t = 80 continuation (§3.5) ends in coalescence at t $\approx$ 47, so the branch supplies no finite-$d$ equilibrium.
...
E1 status: the Yin-excess branch changes the lock-timescale outcome from escape to persistence at t = 40—the first finite-separation survival under the existing PDE—but the continuation (below) shows the branch is a transient that ends in coalescence, not an equilibrium. E1 (a finite-$d_0$ bound state with projected $K_d$, $T_d$) remains open; the steady-Yin-excess representation change is measured separately as §3.7, and the remaining route is the wake sourced between the ridges (§3.4), a new model term.
...
E1 status: the derived negative-attractor representation change of §3.4 now exists and is measured—the mirror manifold is the unique constant-free steady $\Pi<0$ state, realized to 1e-13—but it does not bind the pair (coalescence at t = 20.7) and it is not a rest state of the expansion ($H\to\lambda$). E1 (a finite-$d_0$ bound state with projected $K_d$, $T_d$) remains open; the remaining route is the wake sourced between the ridges, a new model term.
