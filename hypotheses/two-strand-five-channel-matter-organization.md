...
Protocol: `two-fluid/run_two_strand_binding_suite.py` (committed; run record `20260807_binding_suite` is generated output under `runs/`, regenerated by the script). The smallest transport-capable scratch layer on the canonical solver (which is untouched): a damped-diffused wake field driven by the anti-phase source, $\partial_t W = -W/\tau_W + S_W/\tau_W + (\ell^2/\tau_W)\nabla^2 W$ with $S_W=(1-q)\varepsilon^2$, $\tau_W=1/\lambda=20$, $\ell=\mathrm{SIG}=5$ cells, coupling the strand densities mass-like through $\partial_t E_a \supset -\chi_w\,\mathrm{div}(E_a\,\nabla W)$; $\chi_w$ is the single protocol parameter (no framework constant registered). Acceptance: T1 $\chi_w=0$ bit-exact vs canonical, T2 monotone binding across the bracket, T3 sep0 one-string preservation, T4 no clamp pathologies.

Verdict: **null across the bracket** $\chi_w\in\{300,1000,3000\}$; no coupling binds the pair—$\chi_w=0$ is the bit-exact no-op (T1) and every bracket coupling collapses:

1. **The wake peaks at the $\varepsilon$-extrema, displaced ~0.5 cells outward from the density centers** by the anti-phase subtraction ($\varepsilon=E(g_1-g_2)$): the flux sharpens each hump toward its true center ($d$: 9.90 → ~13.0 cells pre-collapse) and then drags the pair apart—the wrong direction for binding.
2. **The $\varepsilon$-compression feedback is super-critical:** $\partial_t\varepsilon\supset-\chi_w\,\mathrm{div}(\varepsilon\nabla W)$ scales as $\varepsilon^3$ (Keller–Segel-type self-focusing). At $\chi_w=300$ the pair amplitude grows $A_+$: 0.517 → 0.642 and the fields collapse to NaN at t = 0.6; higher couplings collapse earlier and drift faster (NaN at t = 0.3 / 0.2, $d$ = 13.05 / 13.20 for $\chi_w=1000$ / 3000). TS1 at $\chi^\* = 1000$ reads **escaped** (collapse before any binding).
3. **There is no coupling window in which the pair binds:** sub-critical couplings are inert (the measured wake transfer is ~1000× below the design estimate—ifftn normalization plus $\varepsilon$-fade), super-critical couplings collapse. E1 remains open; the wake mechanism as specified does not supply $d_0$.

Open design-iteration candidates (not implemented; each is a distinct mechanism change, none registers a framework constant):

- (i) **$\rho$-weighted source** $S_W=(1-q)\,\rho\,\varepsilon^2/\langle\rho\rangle$—re-centers the W-lobes on the density maxima, removing the outward drag;
- (ii) **sub-critical feedback bound**—cap the $\varepsilon$-compression (the collapse is winner-take-all by construction).

The third candidate—the **Yin-excess pair initialization**, the framework's attractive branch from the $G_{\mathrm{eff}}$ sign derivation (buoyancy is self-repulsive for the Yang-excess pair)—is implemented and measured as §3.5.

T1 passed (all field/history diffs exactly 0 at t = 4 and t = 40); T3 holds bit-exactly on the field diffs (sep0 $W_{\max}\sim10^{-26}$, flux inert; the $\varepsilon$-digit reads $1.7\times10^{-12}$ vs the $10^{-12}$ threshold—float accumulation, not contamination); T4's only pathology is the collapse itself.
...
E1 status: the Yin-excess branch changes the lock-timescale outcome from escape to persistence—the first finite-separation survival under the existing PDE—but it is a transient-driven contraction, not an equilibrium with derived coefficients: $d(t)$ shows no turnaround and the Yin excess itself is erased by conversion at t $\approx$ 21. E1 (a finite-$d_0$ bound state with projected $K_d$, $T_d$) remains open; the natural next probes are the t > 40 continuation (does the contraction merge or stall?) and candidate (ii) (a sub-critical feedback bound that could hold $\Pi<0$).
...
| ID | Quantity | Status |
|----|----------|--------|
| E1 | Binding: $d_0$, $K_d$, $T_d$, $\mu_d$, $\gamma_d$ | Open; TS1 null: the pair escapes at the lock timescale (d 9.90 → 15.73 cells at t = 40, §3.3), so no finite-$d_0$ binding coefficients are projectable from the existing PDE. The wake aggregation scratch layer is null as well (§3.4): no binding window—sub-critical no effect, super-critical Keller–Segel collapse. The Yin-excess init candidate is measured (§3.5): the pair persists at finite separation through t = 40 (d 9.90 → 7.51, slow contraction, no turnaround)—the first non-escape at the lock timescale—but the branch is transient (conversion erases $\Pi<0$ at t ≈ 21) and no equilibrium coefficients are projectable; the $\rho$-weighted source and sub-critical feedback bound candidates remain open |
...
| E3 | Wake coupling $g$, damping length $\ell$ | Open; the wake aggregation candidate was instantiated with $g=\chi_w$ (protocol parameter), $\ell=\mathrm{SIG}$, $\tau_W=1/\lambda$ and tested null (§3.4); coefficients remain unprojected |
| E4 | Ridge width $\sigma_{\mathrm{ridge}}$ | Open; the run uses 5.0 cells as initialization, not a derived width |
| E5 | Twist: $\Omega$, $\mathrm{Tw}$ | Tested at t = 4: initialized half-twist persisted (Tw 0.500 → 0.499, band `persisted`); zero-twist arm null; the parity-odd scratch-layer generation candidate ($\chi_{\mathrm{circ}}$, `two-fluid/scratch_twist_chi.py`) is implemented and its t = 4 ramp is null (max $\|\Delta\mathrm{Tw}\|=3.3\times10^{-4}$ over $\chi\in\{\pm0.25,\dots,\pm2\}$, no clamp engagement, §3.2); t = 40 lock leg: no lock—Tw tracks the seed (0.448 at $2\pi/N$, 0.911 at $4\pi/N$, both ~9% decay) and the $\chi=1$ response is $\pm5\times10^{-4}$ (null); P_parallel relation unmeasured |
...
- **Tested (PDE/gate), wake-binding scratch layer (2026-08-07):** the smallest transport-capable E1/E3 candidate—wake field $S_W=(1-q)\varepsilon^2$, $\tau_W=1/\lambda$, $\ell=\mathrm{SIG}$, mass-like flux $-\chi_w\,\mathrm{div}(E_a\nabla W)$—is null across the bracket $\chi_w\in\{300,1000,3000\}$ and inert below it: $\chi_w=0$ is a bit-exact no-op vs the canonical solver (T1), the sep0 one-string is preserved bit-exactly (field diffs 0 at t = 40, T3), but no coupling binds the pair—sub-critical no effect, super-critical $\varepsilon$-compression collapse (NaN at t = 0.6/0.3/0.2); TS1 at $\chi^\* = 1000$ reads escaped (§3.4, `two-fluid/run_two_strand_binding_suite.py`, run record `20260807_binding_suite`).
- **Tested (PDE/gate), Yin-excess pair branch (2026-08-07):** the smallest initialization-only change—the canonical two-lobe state with $E_Y\leftrightarrow E_I$ exchanged, $\Pi=E_Y-E_I<0$ in every ridge (representable under the positivity floor: min fields 0.57, $\Pi_{\mathrm{tot}}=-4.27\times10^4$; a mere $\varepsilon$-sign flip is insufficient, §3.5 Y1)—turns the lock-timescale outcome around: the pair persists at finite separation (d 9.90 → 7.51 cells, back-20% mean 7.72, no merge, no escape) while the Yang-excess counterfactual escapes (15.73); the Yin excess is transient (erased by conversion at t ≈ 21.3) and the t = 40 state is a slow contraction with no turnaround, so E1 equilibrium coefficients remain unprojected (§3.5, `two-fluid/run_two_strand_yin_excess_suite.py`, run record `20260807_014428_two_strand_yin_excess`).
