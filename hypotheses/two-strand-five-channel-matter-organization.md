...
T1 passed (all field/history diffs exactly 0 at t = 4 and t = 40); T3 passes on the field diffs (exactly 0 at both windows, the wake inert below float resolution at sep0; the $\varepsilon$-digit at t = 4 reads $4.3\times10^{-13}$, float accumulation, below the $10^{-12}$ threshold); T4's only pathology is the collapse itself.
