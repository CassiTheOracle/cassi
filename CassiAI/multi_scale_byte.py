...
class MultiScaleByteEmbedder(nn.Module):
    ...
    def __init__(
        ...
        # Scale factor to bound output after LayerNorm.
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # Bound output to prevent unbounded field accumulation with no_reset=True.
        # tanh keeps values in [-1, 1]; output_scale learns the dynamic range.
...
class ByteEmbedder(nn.Module):
    """Simple per-position byte embedder — no context mixing.

    Maps each byte independently to a d-dimensional bounded complex source.
    The PDE must propagate context across positions; this embedder provides
    pure token identity, no pre-smoothed n-gram features.
    """

    def __init__(self, d_out: int, byte_embed_dim: int = 64):
        super().__init__()
        if d_out <= 0:
            raise ValueError(f"d_out must be positive, got {d_out}")
        if byte_embed_dim <= 0:
        ...
        self.proj = nn.Linear(byte_embed_dim, d_out)

        # LayerNorm stabilizes the distribution before tanh bounding.
        self.norm = nn.LayerNorm(d_out)

        # Learned output scale — bounds the source to prevent unbounded
        # field accumulation with persistent state (no_reset=True).
        self.output_scale = nn.Parameter(torch.tensor(0.5))

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Encode byte tokens to per-position features.

        Args:
            x: byte ids of shape [B, N], dtype torch.long.

        Returns:
            Tensor of shape [B, N, d_out] bounded to ±output_scale.
        """
        if x.dtype == torch.uint8:
            x = x.long()
        if x.dtype != torch.long:
        ...
        # Pure per-position projection: no cross-position mixing.
        out = self.proj(z)  # [B, N, d_out]
