#!/usr/bin/env python3
"""
Train an Observer head on top of frozen Qwen3.5-4B.
...
class ObserverHead(nn.Module):
    def __init__(self, d_model: int, vocab_size: int, bottleneck: int = 640):
        super().__init__()
        hid = int(d_model / PHI)
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        if val_loss < best_val:
            best_val = val_loss
            torch.save(observer.state_dict(), "experiments/observer_4b_best.pt")
            print("Saved best observer")

        # Periodic save
        torch.save(observer.state_dict(), f"experiments/observer_4b_epoch{epoch + 1}.pt")
