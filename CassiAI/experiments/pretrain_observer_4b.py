#!/usr/bin/env python3
"""
Pre-train the ObserverHead on frozen Qwen3.5-4B hidden states.

The base model is completely frozen. The observer receives hidden states
and learns to predict the next token. This gives it a strong initialization
before it has to compete in the harmony gate.

After pre-training, the observer checkpoint can be loaded into the full
CassiTrainableModel for end-to-end fine-tuning.
"""
...
class ObserverHead(nn.Module):
    def __init__(self, d_model, vocab_size, bottleneck=1024):
        super().__init__()
        hid = int(d_model / PHI)
        ...
        self.logits_proj = nn.Sequential(
            nn.Linear(d_model, bottleneck), nn.LayerNorm(bottleneck), nn.GELU(),
            nn.Linear(bottleneck, bottleneck), nn.LayerNorm(bottleneck), nn.GELU(),
            nn.Linear(bottleneck, vocab_size, bias=False),
        )
        # Scale down final layer for stability
        nn.init.xavier_uniform_(self.logits_proj[-1].weight, gain=0.1)
...
def train_epoch(base_model, observer, loader, optimizer, grad_accum=4, grad_clip=1.0, steps_per_epoch=500):
    observer.train()
    total_loss = 0.0
    total_acc = 0.0
    steps = 0
    ...
    for batch_idx, batch in enumerate(loader):
        ...
        with torch.amp.autocast('cuda'):
            ...
            # Track accuracy
            preds = obs_logits.argmax(dim=-1)  # [B, L]
...
def validate(base_model, observer, loader, val_steps=50):
    observer.eval()
    total_loss = 0.0
    total_acc = 0.0
    total_tokens = 0
...
def generate_test(base_model, observer, tok, prompt, max_new=40):
    observer.eval()
    ...
    for _ in range(max_new):
        ...
        # Blend base and observer at 50/50 for generation test
...
def main():
    ...
    observer = ObserverHead(d_model, vocab_size, bottleneck=256).to(DEVICE).to(torch.bfloat16)
    ...
    print(f"Observer trainable params: {trainable:,}")
    ...
    for epoch in range(args.epochs):
        ...
        val_metrics = validate(base_model, observer, loader, val_steps=args.val_steps)
        print(f"Val  loss={val_metrics['loss']:.4f}  acc={val_metrics['acc']:.2f}%")

        if val_metrics["loss"] < best_val_loss:
            best_val_loss = val_metrics["loss"]
            torch.save(observer.state_dict(), "experiments/observer_pretrained_best.pt")
            print("Saved best observer")

        torch.save(observer.state_dict(), f"experiments/observer_pretrained_epoch{epoch + 1}.pt")
    ...
    print("Observer pre-training complete.")
    print(f"Best checkpoint: experiments/observer_pretrained_best.pt (val_loss={best_val_loss:.4f})")
