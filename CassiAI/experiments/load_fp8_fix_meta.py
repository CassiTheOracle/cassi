"""
Fix meta-device tensors before dispatch, then load and dispatch.
"""
import os
import gc
import torch
...
# Step 2: Initialize any meta-device tensors with zeros/empty
print("Step 2: Initializing meta tensors...")
for name, param in model.named_parameters():
    if param.device.type == "meta":
        new_param = torch.empty_like(param, device="cpu")
        if name.endswith(".weight") or "embed" in name:
        ...
        setattr(module, parts[-1], torch.nn.Parameter(new_param))
...
# Step 3: Copy decoder weight_scales to encoder
print("Step 3: Copying decoder weight_scales to encoder...")
for i in range(n_layers):
    ...
    for dec_name, dec_mod in dec_layer.named_modules():
        if hasattr(dec_mod, 'weight_scale') and dec_mod.weight_scale.device.type != 'meta':
            ...
            if hasattr(enc_mod, 'weight_scale') and enc_mod.weight_scale.device.type == 'meta':
                # Need to replace the meta buffer
                parts = f"model.encoder.language_model.layers.{i}.{dec_name}".split(".")
                module = model
                for part in parts[1:]:  # skip 'model'
                    module = getattr(module, part)
                module.register_buffer('weight_scale', dec_mod.weight_scale.clone())

print("Meta tensors initialized.")

# Step 4: Dispatch
print("Step 4: Dispatching model...")
gc.collect()
