"""Run A-A-B-B on source-bound CassiCosmos chronology.

This is the real-source counterpart to the controlled generation-two holdout.
It loads three receipt-backed CassiCosmos attractor worlds without synthesizing
trajectories: one development world, one chronological A regime, and one
chronological B regime.  The generation-two guard, ridge, threshold, and
non-worse decision are unchanged.
"""
...
SCHEMA = "cassifi.generation2-real-chronological-holdout.v1"
SLICE_SCHEMA = "cassifi.generation2-real-chronological-holdout-slice.v1"
EXPERIMENT_ID = "generation2-real-chronological-holdout-v1"
OUTPUT = "CassiFI/_diag/generation2-real-chronological-holdout-v1.json"
...
class RealChronologicalHoldoutError(RuntimeError):
    """Raised when the real chronological release cannot be measured."""
...
def _source_manifest(workspace: Path) -> list[dict[str, Any]]:
    ...
    for relative in ANALYSIS_SOURCES:
        ...
        if not path.is_file():
            raise RealChronologicalHoldoutError(
                f"analysis source is absent: {relative}"
            )
        rows.append(
            {
                "path": relative,
...
def _load_world(workspace: Path, relative_root: str) -> RealWorld:
    # The morphology adapter is the source-bound extractor for the recorded
    # CassiCosmos bundles.  Materialize all rows while its activation is held,
    # then the field sees only immutable CovariantFrame values.
    with _activated():
    ...
    if not frames:
        raise RealChronologicalHoldoutError(f"world has no chronological frames: {relative_root}")
    if any(frame.target is None for frame in frames):
        raise RealChronologicalHoldoutError(f"world has a target-free frame: {relative_root}")
    ...
    if source_summary.get("domain_status") != "BOUNDED":
        raise RealChronologicalHoldoutError(
            f"world is not bounded: {relative_root} ({source_summary.get('domain_status')})"
        )
    return RealWorld(
        relative_root=relative_root,
...
def _targets(
    ...
    try:
        ...
        raise RealChronologicalHoldoutError(
            f"forecast requested a target outside the source manifest: {exc}"
        ) from exc
...
def _nrmse(prediction: np.ndarray, target: np.ndarray) -> float:
    ...
    if target_rms == 0.0:
        raise RealChronologicalHoldoutError("chronological holdout target is exactly zero")
...
def _split(
    ...
    if first_stop >= second_stop or second_stop >= len(frames):
        raise RealChronologicalHoldoutError(
            f"{world.relative_root} has no usable chronological holdout"
        )
...
def _holdout_metrics(
    ...
    if not np.array_equal(aligned.ticks, isotropic.ticks) or not np.array_equal(
        ...
        raise RealChronologicalHoldoutError(
            "guarded and isotropic forecasts changed evaluated rows"
        )
    target = _targets(world.target_lookup, aligned.ticks, aligned.identity_ids)
    target_rms = _rmse(np.zeros_like(target), target)
    if target_rms == 0.0:
        raise RealChronologicalHoldoutError(
            f"chronological holdout has zero target: {world.relative_root}"
        )
...
def _run_actor(workspace: Path, analysis_sources: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "scope": (
            "one independent field over receipt-backed CassiCosmos chronology "
            "with A-A-B-B training"
        ),
...
def _aggregate(slice_result: Mapping[str, Any]) -> dict[str, Any]:
    a_holdout = slice_result["regime_A"]["holdout"]
    b_holdout = slice_result["regime_B"]["holdout"]
    delta = float(b_holdout["guarded_vs_isotropic_delta"])
    return {
        "actor_count": 1,
        "regime_count": 2,
        ...
        "A_holdout": {
            "world_count": 1,
            "source_root": slice_result["regime_A"]["source_root"],
        ...
        "B_holdout": {
            "world_count": 1,
            "source_root": slice_result["regime_B"]["source_root"],
            ...
            "guarded_vs_isotropic_delta": delta,
            "positive_or_equal": delta <= NONWORSE_TOLERANCE,
            "release_support": int(
                slice_result["regime_B"]["after_second"]["guard_unique_regime_support"]
            ),
            "anisotropic_basis_after_first": float(
                slice_result["regime_B"]["after_first"]["basis_coefficients"][1]
            ),
            "anisotropic_basis_after_second": float(
                slice_result["regime_B"]["after_second"]["basis_coefficients"][1]
            ),
        },
        "controls_passed": bool(slice_result["controls_passed"]),
        "slice_digest": _digest(slice_result),
    }
...
def run(workspace: Path) -> dict[str, Any]:
    ...
    slice_result = _run_actor(workspace, analysis_sources)
    aggregate = _aggregate(slice_result)
    if not slice_result["controls_passed"]:
        failed = [
            name
            for name, row in slice_result["controls"].items()
            if row["status"] != "PASS"
        ]
        raise RealChronologicalHoldoutError(
            f"real chronological controls failed: {failed}"
        )
    return {
        "schema": SCHEMA,
        "experiment_id": EXPERIMENT_ID,
        "status": "PASS",
        "scope": (
            "receipt-backed CassiCosmos attractor sources; "
            "one independent field; chronological A-A-B-B rollout"
        ),
