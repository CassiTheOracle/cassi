"""Independently verify the frozen L40 ordered-recall capacity board."""
...
VERIFICATION_SCHEMA = "cassi.l40.ordered-recall-capacity-verification.v1"
PREREGISTRATION = ROOT / "designs" / "L40-ORDERED-RECALL-CAPACITY-PREREG.md"
DEFAULT_BOARD = ROOT / "_diag" / "l40-ordered-recall-capacity" / "l40-board.json"
DEFAULT_OUTPUT = ROOT / "artifacts" / "l40-ordered-recall-capacity"
DEFAULT_REPORT = DEFAULT_OUTPUT / "L40-ORDERED-RECALL-CAPACITY-REPORT.md"
DEFAULT_JSON = DEFAULT_OUTPUT / "l40-verification.json"
...
PROFILE_NAMES = ("l31-cyclic", "l39-ordered")
PROFILE_IDENTITIES = (
    {
        "name": "l31-cyclic",
    ...
    },
    {
        "name": "l39-ordered",
        "layout_profile_id": "cassi.qi-cyclic-chromatic-coordinate-native.v1",
        "operator_profile_id": "cassi.qi-ordered-relational-chromatic-recall.v1",
        "projection_profile_id": "cassi.qi-cyclic-chromatic-projection.v1",
    },
)
EXPECTED_SOURCES = {
    "designs/L40-ORDERED-RECALL-CAPACITY-PREREG.md",
    ...
    "verification/run_l40_ordered_recall_capacity.py",
    "verification/verify_l40_ordered_recall_capacity.py",
}
EXPECTED_ARRAYS = {
    "schema_id",
    "profile_names",
...
class L40VerificationError(RuntimeError):
    """L40 evidence violates its frozen contract."""


def need(condition: bool, message: str) -> None:
    if not condition:
        raise L40VerificationError(message)
...
def verify_histories(sequences: np.ndarray) -> None:
    need(
        np.array_equal(sequences, heldout_sequences()),
        "held-out history tensor mismatch",
    )
    for depth_index, depth in enumerate(DEPTHS):
        for history in range(BATCH_SIZE):
            active = sequences[depth_index, history, :depth]
            need(
                len(set(int(value) for value in active)) == depth,
                "history repeats a symbol",
            )
            need(
                not any(int(value) in EXPOSED for value in active),
                "history uses exposed symbol",
            )
            need(
                bool(np.all(sequences[depth_index, history, depth:] == -1)),
                "history padding mismatch",
            )
        for history in range(0, BATCH_SIZE, 2):
            ...
            if depth > 1:
                need(
                    not np.array_equal(even[:-1], odd[:-1]),
                    "paired prefixes collide",
                )
...
def sibling_trace(board_path: Path, value: Any) -> Path:
    ...
    name = item.get("path")
    need(isinstance(name, str) and name == "l40-traces.npz", "trace sibling name mismatch")
    path = board_path.parent / "l40-traces.npz"
...
def analyze(arrays: Mapping[str, np.ndarray]) -> tuple[dict[str, Any], str]:
    ...
    baseline = profile_metrics["l31-cyclic"]
    ordered = profile_metrics["l39-ordered"]
    ordered_depth2 = ordered["depths"]["2"]
    if (
        ordered["retained_capacity"] >= 2
        and ordered["history_observable_capacity"] >= 2
        and ordered["retained_capacity"] > baseline["retained_capacity"]
        and ordered["history_observable_capacity"]
        > baseline["history_observable_capacity"]
        and ordered_depth2["oldest_mean_reciprocal_rank"] >= 0.05
        and ordered_depth2["paired_same_tail_distance_median"] >= 0.01
    ):
        outcome = "SUPPORTS"
    elif (
        ordered["history_observable_capacity"] < 2
        and ordered["retained_capacity"] <= 1
        and ordered_depth2["oldest_mean_reciprocal_rank"] < 0.05
        and ordered_depth2["paired_same_tail_distance_median"] < 0.01
    ):
        outcome = "CONTRADICTS"
...
def verify_board(
    ...
    need(
        board.get("profiles") == list(PROFILE_IDENTITIES),
        "profile identities mismatch",
    )
    ...
    if not allow_smoke_device:
        ...
        need(
            device.get("name") == "AMD Radeon RX 7900 XTX",
            "canonical GPU mismatch",
        )
    ...
    need(
        arrays["clamp_counts"].shape == (len(PROFILE_NAMES), len(DEPTHS)),
        "clamp shape mismatch",
    )
    ...
    for depth_index, depth in enumerate(DEPTHS):
        need(
            bool((arrays["scores"][:, depth_index, :, depth:] == 0.0).all()),
            "score padding is nonzero",
        )
        need(
            bool((arrays["available"][:, depth_index, :, depth:] == 0).all()),
            "availability padding is nonzero",
        )
        need(
            bool(
                (arrays["white_coherence"][:, depth_index, :, depth:] == 0.0).all()
            ),
            "coherence padding is nonzero",
        )
        need(
            bool((arrays["dynamic_energy"][:, depth_index, :, depth:] == 0.0).all()),
            "energy padding is nonzero",
        )
        need(
            bool(
                (arrays["input_energy_drift"][:, depth_index, :, depth:] == 0.0).all()
            ),
            "drift padding is nonzero",
        )
    ...
    arm = mapping(
        mapping(board.get("arms"), "arms").get("canonical"), "canonical arm"
    )
    need(
        arm.get("profile_count") == len(PROFILE_NAMES),
        "profile count declaration mismatch",
    )
    need(
        arm.get("history_count_per_depth") == BATCH_SIZE,
        "history count declaration mismatch",
    )
    declared_energy = arm.get("maximum_dynamic_energy")
    declared_drift = arm.get("maximum_absolute_input_energy_drift")
    need(
        isinstance(declared_energy, (int, float)),
        "maximum energy declaration missing",
    )
    need(
        isinstance(declared_drift, (int, float)),
        "maximum drift declaration missing",
    )
    assert isinstance(declared_energy, (int, float))
    assert isinstance(declared_drift, (int, float))
    need(
        math.isclose(
            float(declared_energy), maximum_energy, rel_tol=0.0, abs_tol=0.0
        ),
        "maximum energy declaration mismatch",
    )
    need(
        math.isclose(float(declared_drift), maximum_drift, rel_tol=0.0, abs_tol=0.0),
        "maximum drift declaration mismatch",
    )
...
def report_text(verdict: str, payload: Mapping[str, Any]) -> str:
    lines = [
        "# L40 Ordered Recall Capacity — Verification",
        "",
        f"**Outcome: `{verdict}`**",
    ]
    profiles = mapping(
        mapping(payload.get("metrics", {}), "metrics").get("profiles", {}),
        "profiles",
    )
    if profiles:
        lines.extend(
            [
                "",
                "| Profile | Retained depth | History-observable depth | Depth-2 oldest MRR | Depth-2 paired distance |",
                "|---|---:|---:|---:|---:|",
            ]
        )
        for name in PROFILE_NAMES:
            ...
            depth2 = mapping(
                mapping(values.get("depths"), "depths").get("2"), "depth 2"
            )
            lines.append(
                f"| `{name}` | {values['retained_capacity']} | "
                f"{values['history_observable_capacity']} | "
