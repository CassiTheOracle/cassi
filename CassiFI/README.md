...
| [`cassi_emergent_field.py`](cassi_emergent_field.py) | Two-fluid field (2D or 3D) whose own medium density adapts to Qi intensity, plus the brain's note codec. One sound loop plays a batch of drives, each on its own quiet fast field over the shared medium: listening reads it, playing thickens it, and the medium's whole state is its imprint `rho - 1`. An idea is an even-brightness chord over thousands of the field's standing notes, composed from its name; two chords played together at one pitch leave their link as a medium imprint, and many pairs sound in one play; Yang and Yin are driven in golden proportion, so the counterflow stays silent and nearly every cell is a clean note (about 244k on a 64³ field); recall bows a chord, subtracts the exact response of an even medium at the same mean thickening and divides out each note's own answer; as the medium fills, the brain lowers its pitch with the mean (`w / sqrt(1 + level)`) and retunes against that even reference, so the readout sees only the uneven imprint where links live (about 4,000 links held cleanly on 64³, clarity falling as sqrt(notes / links)). On ROCm the 3D float32 field steps as one fused GPU kernel per step, compiled at first use with hiprtc (about 4× faster), and torch steps it elsewhere. Chord phases are a 32-bit counter hash of the name's key and the note, identical on every device and made on the GPU. `SequenceMemory` gives each idea a sending and a receiving chord, so stored steps replay chains forward, trace them backward, branch at forks, and are erased by anti-phase replay. Its `describe` links an idea's receiving chord to a phrase that sounds all of the idea's words at once, and `recall` sounds a phrase and hears the described ideas that share its words; their resemblance follows the phase-only law of the word cosine, (π/4)·c·₂F₁(½,½;2;c²), and the thinking brain (`../CassiQwen/cassi_thinking_brain.py`) uses it to let each new idea recall the earlier ideas it restates. `TemporalMedium` holds an owner temporal memory's episodes (a chord of nested history voices → observation, each deeper voice twice as loud, so a memory met once is still heard through its full context); recall plays each voice separately and blends the answers, deeper shared history weighing more, and reports how confidently it knows; `choose` imagines every action's futures several steps ahead and acts toward goal observations, or explores what it knows least; it sleeps by replaying recognised experiences before a uniform fade. A medium keeps about 400 notes per link; when its links crowd it the owner regrows it by half its side and replays the whole experience into the larger lattice. Owner surface: `grow_temporal_medium`, `predict_temporal_medium`, `choose_temporal_medium_action`, `rest_temporal_medium`; admissions and revocations sync automatically |
...
| [`run_memory_consumer_path.py`](run_memory_consumer_path.py) | One declared consumer on the owner's canonical page: the target direction's deposit read back through the owner's own `read_packet_deposit` against a declared floor of half the captured deposit, one owner write of the remembered direction or the declared fallback, and the act's own read-frame increment as the statistic — with the identity control on a byte-identical page with the read suppressed, the no-memory and mismatch controls, the mutated-policy firing control, and the owner-side read taken across the act |
| [`run_owner_surface_options.py`](run_owner_surface_options.py) | Two additive default-off options on one declared profile: the evidence reading of the written-direction read with its retry replay, second-identity and no-declared-channel controls, the leaf-class diff over the four declared surfaces, and the exposure scan; and the coupled input relation with its authority over the declared readout on both beta settings, its persistence, its per-tick cost against one owner write, and its eleven-amplitude route identity against the owner write page, plus the enumerated overlap lattice with its source-level mechanism |
| [`test_memory_consumer_path.py`](test_memory_consumer_path.py) | Fast regression for the memory consumer path receipt: the declared arms and their act increments, the retrieval floor comparison, the page and evidence-clock invariants of the reads, the owner-side read across the act, the separation and pursuit margins with a firing control, the orthogonality of the declared directions, and receipt-digest reproducibility and mutation sensitivity |
