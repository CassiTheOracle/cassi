...
| [`cassi_emergent_field.py`](cassi_emergent_field.py) | Two-fluid field (2D or 3D) whose own medium density adapts to Qi intensity, plus the brain's note codec. An idea is an even-brightness chord over thousands of the field's standing notes, composed from its name; two chords played together at one pitch leave their link as a medium imprint; recall bows a chord, subtracts the exact blank response and inverts each note's Yang/Yin transfer. `SequenceMemory` gives each idea a sending and a receiving chord, so stored steps replay chains forward, trace them backward, branch at forks, and are erased by anti-phase replay. `TemporalMedium` holds an owner temporal memory's episodes (context chord → observation), generalizes to unseen contexts, and sleeps by replaying recognised experiences before a uniform fade. Owner surface: `grow_temporal_medium`, `predict_temporal_medium`, `rest_temporal_medium`; admissions and revocations sync automatically |
