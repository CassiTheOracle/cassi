...
| [`cassi_emergent_field.py`](cassi_emergent_field.py) | Two-fluid field (2D or 3D) whose own medium density adapts to Qi intensity, plus the brain's note codec. An idea is an even-brightness chord over thousands of the field's standing notes; two chords played together at one pitch leave their link as a medium imprint; recall bows a chord, subtracts the exact blank response and inverts each note's Yang/Yin transfer. `SequenceMemory` gives each idea a sending and a receiving chord, so stored steps replay chains of thought forward, trace them backward, and branch at forks |
