...
class QiIngressJournal:
    ...
    def append(self, packet: BoundaryPacket) -> JournalReference:
        ...
        with self._append_lock():
            ...
            if packet.ingress_journal_sha256 != previous:
                if previous != ZERO_SHA256:
                    current = self._read_json(previous)
                    if (
                        current.get("packet_sha256") == packet.packet_sha256
                        and current.get("previous_head_sha256")
                        == packet.ingress_journal_sha256
                    ):
                        return self._reference_from_head(current, previous)
                raise UniversalDataError("packet ingress journal parent does not match HEAD")
            prior_stream_rows = [
                row
                for row in self.replay()
                if row.source_stream_id == packet.identity.source_stream_id
            ]
            if (
                prior_stream_rows
                and packet.source_sequence <= prior_stream_rows[-1].source_sequence
            ):
