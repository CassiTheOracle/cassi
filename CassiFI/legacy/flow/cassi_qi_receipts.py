...
_RECEIPT_ENVELOPE_CORE_KEYS = frozenset(
    {
        "schema",
        "receipt_id",
...
def _enum_values(value: Any, context: str, max_fanout: int) -> list[Any]:
    values = _array(value, context)
    _require(
        0 < len(values) <= max_fanout,
        "FANOUT_LIMIT_EXCEEDED",
        f"{context} cardinality is invalid",
    )
    for index, item in enumerate(values):
        _canonical_scalar(item, f"{context}[{index}]")
    _require(
        all(
            not (type(left) is type(right) and left == right)
            for index, left in enumerate(values)
            for right in values[index + 1 :]
        ),
        "SCHEMA_LITERAL_MISMATCH",
        f"{context} has duplicates",
    )
    return values


def _enum_member(value: Any, values: Sequence[Any], context: str) -> None:
    _canonical_scalar(value, context)
    _require(
        any(type(value) is type(candidate) and value == candidate for candidate in values),
        "SCHEMA_LITERAL_MISMATCH",
...
def _descriptor(record_value: Any, context: str, max_fanout: int, depth: int) -> None:
    ...
    if kind == "enum":
        _require(set(record) == {"type", "values"}, "UNKNOWN_KEY", f"{context} enum descriptor fields are not exact")
        _enum_values(record.get("values"), f"{context}/values", max_fanout)
        return
    if kind == "string":
        allowed = {
            "type",
...
def _validate_indexed_receipt_contract(registration: Mapping[str, Any]) -> None:
    ...
    envelope = _RECEIPT_ENVELOPE_CORE_KEYS | frozenset({registration["self_hash_field"]})
    _require(envelope <= required, "MISSING_REQUIRED_KEY", "indexed receipt schema omits a required envelope field")
    _require(not (envelope & nullable), "FORBIDDEN_NULL", "indexed receipt schema makes an envelope field nullable")
    ...
    for name in ("contract_root_sha256", "profile_sha256", registration["self_hash_field"]):
...
def _validate_value(value: Any, descriptor: Mapping[str, Any], context: str, depth: int) -> None:
    ...
    if kind == "enum":
        _enum_member(value, _array(descriptor.get("values"), f"{context} enum"), context)
        return
    if kind in {"string", "finite-f64-bits"}:
        ...
        elif descriptor_format in {"identifier-v1", "id"}:
            _identifier(value, context)
        elif descriptor_format == "json-pointer":
...
def _receipt_id(record: Mapping[str, Any], registration: Mapping[str, Any]) -> str:
    material = {
        name: value
        for name, value in record.items()
        if name not in {"receipt_id", registration["self_hash_field"]}
    }
    return _hash(material, f"{registration['schema']}.receipt-id", "receipt identifier")
...
def _validate_receipt_envelope(
    ...
    *,
    ...
    if "contract_root_sha256" in record:
        root = _digest(record["contract_root_sha256"], "receipt contract_root_sha256")
        if expected_root is not None:
            _require(root == expected_root, "HASH_DOMAIN_MISMATCH", "receipt contract root does not match the selected root")
    if "profile_sha256" in record:
        profile = _digest(record["profile_sha256"], "receipt profile_sha256")
        if expected_profile is not None:
            _require(profile == expected_profile, "HASH_DOMAIN_MISMATCH", "receipt profile does not match the selected profile")
    if "consumed_semantic_subhashes" not in record:
        return
    rows = _array(record["consumed_semantic_subhashes"], "receipt consumed semantic subhashes")
    ...
    for row in rows:
        ...
        name = _normalise_parent_name(parent.get("name"), "receipt semantic parent")
        found.append(name)
...
def _entry(raw_entry: Any) -> dict[str, Any]:
    ...
    raw_parents = tuple(_field_names(entry.get("semantic_parent_names"), "registered semantic parent names"))
    parents = _registered_parents(raw_parents)
    ...
    indexed_envelope = object_class == "indexed-receipt"
    ...
    registration = {
        ...
        "semantic_parent_field_names": raw_parents,
...
def _maybe_resolved_context(
    ...
    *,
    ...
    known = registration["field_contract"]["known"]
    if not {"contract_root_sha256", "profile_sha256", "consumed_semantic_subhashes"} & known:
        return None, None, {}
...
def _parent_records(names: Sequence[str], values: Mapping[str, str]) -> list[dict[str, str]]:
    ...
    for field_name in names:
        name = _normalise_parent_name(field_name, "registered semantic parent")
        _require(name in values, "SEMANTIC_PARENT_SET_MISMATCH", f"selected profile omits semantic parent {name}")
        result.append({"name": field_name, "sha256": _digest(values[name], f"selected profile semantic parent {name}")})
    return result
...
def _payload(
    ...
    *,
    ...
    reserved = reserved_keys & set(result)
...
def build_receipt(
    ...
    **fields: Any,
    ...
    """Build one root/profile-resolved object from an indexed receipt schema."""

    registration = _registration(schema, schema_registry)
    root_id, profile_id, parent_values = _maybe_resolved_context(
        registration,
        contract_root=contract_root,
        contract_root_sha256=contract_root_sha256,
        profile=profile,
    ...
    automatic: dict[str, Any] = {"schema": registration["schema"]}
    if "receipt_id" in known:
        automatic["receipt_id"] = "0" * 64
    if "contract_root_sha256" in known:
        automatic["contract_root_sha256"] = root_id
    if "profile_sha256" in known:
        automatic["profile_sha256"] = profile_id
    if "consumed_semantic_subhashes" in known:
        automatic["consumed_semantic_subhashes"] = _parent_records(
            registration["semantic_parent_field_names"],
            parent_values,
        )
    automatic[self_hash_field] = "0" * 64
    record = {
        **automatic,
        **_payload(
            payload,
            fields,
            reserved_keys=frozenset(automatic),
        ),
    }
    _validate_receipt_shape(record, registration)
    ...
    if "receipt_id" in record:
        record["receipt_id"] = _receipt_id(record, registration)
    record[self_hash_field] = _receipt_self_hash(record, registration)
    _validate_receipt_shape(record, registration)
    _validate_receipt_envelope(
        record,
        registration,
    ...
    if "receipt_id" in record:
        _require(_receipt_id(record, registration) == record["receipt_id"], "RECEIPT_ID_MISMATCH", "built receipt_id does not match")
    _require(_receipt_self_hash(record, registration) == record[self_hash_field], "SELF_HASH_MISMATCH", "built receipt self hash does not match")
    return record
...
def validate_receipt(
    ...
    *,
    ...
    if "receipt_id" in record:
        declared_receipt_id = _digest(record["receipt_id"], "receipt receipt_id")
        _require(_receipt_id(record, registration) == declared_receipt_id, "RECEIPT_ID_MISMATCH", "receipt_id does not match")
    self_hash_field = registration["self_hash_field"]
    declared_self = _digest(record.get(self_hash_field), f"receipt {self_hash_field}")
...
def receipt_fixture_payload(
    ...
    *,
    ...
    """Return the canonical minimal payload for one indexed receipt."""
    ...
    automatic = _RECEIPT_ENVELOPE_CORE_KEYS | frozenset({registration["self_hash_field"]})
    payload = {
        name: value
...
def _builder(kind: str):
    def build(payload: Mapping[str, Any] | None = None, **kwargs: Any) -> dict[str, Any]:
        schema = RECEIPT_SCHEMAS[kind]
        prepared = None if payload is None else dict(payload)
        if prepared is not None:
            self_hash_field = _registration(schema, kwargs.get("schema_registry"))["self_hash_field"]
            if self_hash_field != "self_sha256":
                prepared.pop(self_hash_field, None)
        return build_receipt(schema, prepared, **kwargs)
