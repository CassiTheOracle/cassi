...
def _derive(w2: Mapping[str, Any], w3: Mapping[str, Any], schedule: Mapping[str, Any]) -> dict[str, Any]:
    ...
    with localcontext() as context:
        ...
        h = _dec(h_tag, name="h")
        h_min_tag = dynamics["h_min_s"]
        h_max_tag = dynamics["h_max_s"]
        h_min = _dec(h_min_tag, name="h_min")
        h_max = _dec(h_max_tag, name="h_max")
        if not (Decimal(0) < h_min == h <= h_max):
            _fail("W3 schedule is not the lower endpoint of its accepted clock interval")
        ...
        if any(value < 0 for value in gamma_values):
            _fail("W3 damping coefficients must be nonnegative")
        ...
        for stage in stages:
            ...
            if "propagation" in name or "spectral" in name:
                ...
                bound = max((Decimal(2) + (Decimal(3) * abs(kappa_values[scale]) * differential_domain_cap ** 2 * duration_max) ** 2).sqrt() for scale in range(scale_count))
                branches = [[] for _ in rows]
            else:
                bound = Decimal(1)
                branches = [[] for _ in rows]
            stage_bounds.append(bound)
            stage_records.append({"ordinal": int(stage["ordinal"]), "name": name, "duration_s": stage["duration_s"], "duration_interval_s": {"minimum": _tag_dec(duration_min), "maximum": _tag_dec(duration_max)}, "branch": "duration-uniform-energy-norm-damped-oscillator" if "propagation" in name or "spectral" in name else "identity-or-force-kick", "branches_by_scale": branches, "amplification": _outward(Decimal(1), bound)})
        ...
        for scale, row in enumerate(rows):
            ...
            damping_by_scale.append(weight * gamma_values[scale] * mass * velocity_cap ** 2 * h_max)
        ...
        return {
            ...
            "derivation_formulae": {"laplacian": "lambda(k)=-(kx^2+ky^2), k_axis=2*pi*n_axis/L_axis", "spectral_maximum": "max over every signed W2 FFT bin on each active (Ny,Nx) sheet", "frequency": "Omega_D,s(k)=sqrt(omega_D,s^2+c_D,s^2*(kx^2+ky^2))", "damped_branches": "exact underdamped/critical/overdamped analytic 2x2 branch classification", "half_amplification": "duration-uniform energy-normalized Frobenius enclosure over the complete accepted clock interval", "force_kick": "metric-normalized Frobenius enclosure with cubic-force Jacobian norm <=3*abs(kappa_D,s)*D_domain^2 at the maximum accepted stage duration", "raw_admission": "amplitude_cap/product(stage_amplification)", "metric": "W_s=dx_s*dy_s*I and ||I_s R_s||_(W_s)=1", "projection": "complete-signed-frequency injection/restriction with metric-adjoint projection", "energy": "sum_s w_D*N_s*dx_s*dy_s*(|V_D|^2+(c_D,s^2*k2_max,s+omega_D,s^2)*|D|^2+abs(kappa_D,s)*|D|^4/2)/2", "workspace": "state bytes plus active and oversampled complex workspaces, bounded by semantic workspace.byte_cap", "rounding": "outward-nextafter-f64 after Decimal derivation"},
