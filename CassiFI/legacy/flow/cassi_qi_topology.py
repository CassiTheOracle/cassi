...
G3N_CERTIFICATE_DOMAIN = "cassi.qi-flow-w3n-certificate.v1"
...
def topology_codeword_phase(kind: str, shape: tuple[int, int]) -> tuple[tuple[float, ...], ...]:
    """Return the canonical selected-sheet phase witness for one protected sector."""
    ny, nx = (int(shape[0]), int(shape[1]))
    if ny < 2 or nx < 4:
        raise TopologyError("topology codewords require at least a 2x4 periodic sheet")
    if kind == "uniform":
        rows = [[0.0 for _ in range(nx)] for _ in range(ny)]
    elif kind in {"cycle-positive", "cycle-negative"}:
        sign = 1.0 if kind == "cycle-positive" else -1.0
        rows = [[sign * 2.0 * math.pi * float(x) / float(nx) for x in range(nx)] for _ in range(ny)]
    elif kind == "vortex-antivortex":
        rows = [[0.0 for _ in range(nx)] for _ in range(ny)]
        rows[0][1] = 2.0 * math.pi / 3.0
        rows[0][2] = -2.0 * math.pi / 3.0
    else:
        raise TopologyError(f"unknown topology codeword {kind!r}")
    return tuple(tuple(0.0 if value == 0.0 else float(value) for value in row) for row in rows)
...
def _codeword_sector(phase: tuple[tuple[float, ...], ...]) -> tuple[dict[str, Any], float, float]:
    ny, nx = len(phase), len(phase[0])
    dx = [[_wrapped_phase(phase[y][(x + 1) % nx] - phase[y][x]) for x in range(nx)] for y in range(ny)]
    dy = [[_wrapped_phase(phase[(y + 1) % ny][x] - phase[y][x]) for x in range(nx)] for y in range(ny)]
    raw_n_x = [sum(row) / (2.0 * math.pi) for row in dx]
    raw_n_y = [sum(dy[y][x] for y in range(ny)) / (2.0 * math.pi) for x in range(nx)]
    raw_p = [[(dx[y][x] + dy[y][(x + 1) % nx] - dx[(y + 1) % ny][x] - dy[y][x]) / (2.0 * math.pi) for x in range(nx)] for y in range(ny)]
    ...
    max_edge = max(abs(value) for row in dx + dy for value in row)
    return sector, max_edge, integer_residual
...
def _codebook_payload(
    ...
    *,
    ...
    cell_area = _metric_cell_area(geometry, slow_scale)
    parents = _w2_parent_identities(geometry)
    endpoint_radius = 2.0 ** -45
    phase_radius = 2.0 ** -38
    codewords = []
    for kind in ("uniform", "cycle-positive", "cycle-negative", "vortex-antivortex"):
        phase = topology_codeword_phase(kind, shape)
        ...
        branch_margin = math.pi - delta_topo - max_edge - phase_radius
        integer_margin = delta_topo_int - integer_residual - integer_radius
        amplitude_margin = rho_ring - endpoint_radius - rho_topo
        torus_charge = sum(value for row in sector["p"] for value in row)
        if min(branch_margin, integer_margin, amplitude_margin) <= 0.0 or torus_charge != 0:
            raise TopologyError(f"topology codeword {kind!r} has no positive guard margin")
        witness = {
            "selected_scale": int(slow_scale),
            "shape_yx": list(shape),
            "amplitude": _f64_tag(rho_ring, name=f"{kind} amplitude"),
            "phase_f64": [[_f64_tag(value, name=f"{kind} phase") for value in row] for row in phase],
        }
        codeword = {
            "codeword_id": kind,
            "control_label": {
            ...
            "witness": witness,
            "witness_sha256": _sha256(witness, "cassi.qi-flow-w4r-topology-codeword-witness.v1"),
            "sector_vector": sector,
            "guard_disks": {
                "method": "binary64-directed-operation-count.v1",
                "endpoint_radius": _f64_tag(endpoint_radius, name="endpoint disk radius"),
                "edge_phase_radius": _f64_tag(phase_radius, name="edge phase radius"),
                "amplitude_margin": _f64_tag(amplitude_margin, name="amplitude margin"),
                "branch_margin": _f64_tag(branch_margin, name="branch margin"),
                "integer_margin": _f64_tag(integer_margin, name="integer margin"),
                "torus_net_charge": torus_charge,
            },
        }
        codewords.append(codeword)
    hashes = [row["witness_sha256"] for row in codewords]
    if len(hashes) != len(set(hashes)):
        raise TopologyError("topology codebook contains duplicate field witnesses")
    return {
        ...
        "selected_resolution": {
            ...
            "shape_yx": list(shape),
            "metric_diagonal": [_f64_tag(cell_area, name="selected metric diagonal") for _ in range(shape[0] * shape[1])],
            "geometry_identities": parents,
        },
...
def load_w4r_topology_profile(
    *,
    ...
    metric_values = tuple(_metric_cell_area(geometry, slow_scale) for _ in range(shape[0] * shape[1]))
    metric_payload = retention.get("metric_diagonal")
    if metric_payload is not None:
        if not isinstance(metric_payload, (tuple, list)):
            raise TopologyError("retention metric diagonal must be a site-sized sequence")
        configured_metric = tuple(_f64(value, name="metric diagonal", positive=True) for value in metric_payload)
        if len(configured_metric) != len(metric_values):
            raise TopologyError("retention metric diagonal has the wrong dynamic site count")
        if configured_metric != metric_values:
            raise TopologyError("retention metric diagonal disagrees with the sealed W2 sheet metric")
    parents = _w2_parent_identities(geometry)
    parents.update({
...
def _validate_profile(profile: QiTopologyProfile, geometry: Any) -> None:
    ...
    for key in ("w2_profile_sha256", "w2_contract_root_sha256", "w2_geometry_contract_sha256", "w2_operator_semantic_sha256"):
        ...
        now = current.get(key)
        if not _is_sha256(bound) or not _is_sha256(now) or bound != now:
            raise TopologyError(f"topology profile {key} is missing or disagrees with current W2 geometry")
...
def _validate_numerical_certificate(certificate: Mapping[str, Any], geometry: Any) -> Mapping[str, Any]:
    ...
    if _sha256(body, G3N_CERTIFICATE_DOMAIN) != self_hash:
    ...
    for key, expected in checks.items():
        ...
        if not _is_sha256(expected) or not _is_sha256(actual) or actual != expected:
            raise TopologyError(f"G3N certificate W2 ancestry missing or mismatched at {key}")
