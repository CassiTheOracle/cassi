...
def _resolve_duration(transport: Any, numerical_certificate: Mapping[str, Any], duration_s: float | None) -> float:
    parameters = transport.pinned_parameters
    duration = finite_float(parameters.h if duration_s is None else duration_s, name="carrier duration")
    h_min = finite_float(getattr(parameters, "h_min_s", parameters.h), name="W3 h_min")
    h_max = finite_float(getattr(parameters, "h_max_s", parameters.h), name="W3 h_max")
    certified = numerical_certificate["offline_derivation"]["inputs"]
    certified_min = finite_float(certified["h_min_s"], name="G3N certified h_min")
    certified_max = finite_float(certified["h_max_s"], name="G3N certified h_max")
    if certified_min != h_min or certified_max != h_max:
        raise CarrierError("G3N numerical certificate clock interval does not match the W3 profile")
    if not (certified_min <= duration <= certified_max):
        raise CarrierError("carrier duration is outside the certified W3 clock interval")
    return duration
...
def _transition_v4_carrier_split(
    ...
    *,
    ...
    try:
        ...
        duration = _resolve_duration(validated_transport, numerical_certificate, duration_s)
        ...
        receipt: dict[str, Any] = {
            ...
            "composition_derivation_sha256": composition_derivation["self_sha256"],
            ...
            "composition": {
                ...
                "registered_coordinate_work_bound": _canonical_zero(registered_coordinate_work_bound),
                "wave_energy_delta": _canonical_zero(damping_work),
                "total_coupled_closure": _canonical_zero(energy_closure),
                "registered_total_coupled_integrator_bound": _canonical_zero(registered_total_coupled_integrator_bound),
...
def build_composition_derivation(*, carrier_profile: QiCarrierProfile, numerical_certificate: Mapping[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        certificate_inputs = numerical_certificate["offline_derivation"]["inputs"]
        duration_min = finite_float(certificate_inputs["h_min_s"], name="G3N h_min")
        duration_max = finite_float(certificate_inputs["h_max_s"], name="G3N h_max")
    except Exception as exc:
        raise CarrierError("composition derivation has no sealed raw envelope/clock interval") from exc
    if not (0.0 < duration_min <= duration_max):
        raise CarrierError("composition derivation has an invalid sealed clock interval")
    ...
    work_bound = (curvature + 1.0) * duration_max * duration_max * 128.0
    derivation: dict[str, Any] = {
        ...
        "inputs": {
            ...
            "duration_s": duration_max,
            "duration_interval_s": {"minimum": duration_min, "maximum": duration_max},
        },
        "bounds": {
            "c_abs": c_cap,
