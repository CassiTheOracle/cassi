...
class QiCrossScaleProfile:
    ...
    def __post_init__(self) -> None:
        ...
        reserved_operator_fields = {"family", "p_operator_sha256", "p_adjoint_sha256"}
        overlapping_operator_fields = reserved_operator_fields.intersection(operators)
        if overlapping_operator_fields:
            fields = ", ".join(sorted(overlapping_operator_fields))
            raise CrossScaleError(f"operator_identity cannot override authoritative fields: {fields}")
...
def _validate_profile_geometry(profile: QiCrossScaleProfile, surface: PeriodicSheetGeometry) -> None:
    ...
    base_payload = getattr(getattr(surface.profile, "base_profile", None), "payload", {})
    base_coupling = base_payload.get("scale_coupling", {}) if isinstance(base_payload, Mapping) else {}
    expected_p = base_coupling.get("p_operator_sha256", getattr(surface.profile, "operator_semantic_sha256", profile.p_operator_sha256))
    if profile.p_operator_sha256 and profile.p_operator_sha256 != expected_p:
