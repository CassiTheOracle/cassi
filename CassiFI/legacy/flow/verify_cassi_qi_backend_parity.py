...
def _verify_trajectory_receipt(
    ...
    *,
    ...
    """Verify the branch-shaped receipt emitted by QiCandidateTrajectoryReceipt.

    Trajectory comparisons intentionally use ``branches`` rather than the
    six-field ``terms`` sequence used by a termwise parity receipt.  Keeping
    this check separate prevents a verifier from silently accepting a
    mismatched receipt shape.
    """
    _require(payload.get("schema") == BACKEND_PROBE_SCHEMA, f"{label} trajectory schema mismatch")
    _verify_hash_field(payload, "receipt_sha256", BACKEND_PROBE_SCHEMA, label)
    _require(payload.get("profile_sha256") == profile_sha, f"{label} profile identity mismatch")
    _require(payload.get("guard_sha256") == guard_sha, f"{label} guard identity mismatch")
    _require(payload.get("executed") is executed, f"{label} execution flag mismatch")
    ...
    _require(
        ...
        f"{label} candidate count invalid",
    )
    _require(isinstance(branches, list) and len(branches) == candidate_count, f"{label} branch count mismatch")
    for index, branch in enumerate(branches):
        _require(isinstance(branch, dict), f"{label} branch {index} malformed")
        _verify_term(branch, f"candidate-{index}", f"{label}.branch-{index}")
...
def _verify_gate(root: Path, index: Mapping[str, Any]) -> dict[str, Any]:
    ...
    _verify_trajectory_receipt(trajectory_receipt, "trajectory", profile_sha=index["profile_identity"]["oracle"]["profile_sha256"], guard_sha=guard_sha, executed=trajectory.get("executed") is True)
