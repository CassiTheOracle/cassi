"""Focused W6 reciprocal-link, FFT2 Hodge, and W5 wiring contract checks.

Core tests use only the fixed W2 geometry and in-memory state tensors.  The
single public-transition smoke uses W1-W5 parent profiles plus a deterministic
in-test carrier split, without artifacts, runners, or filesystem discovery.
"""
...
class CrossScaleCoreTests(unittest.TestCase):
    ...
    def test_operator_identity_rejects_authoritative_field_collisions(self) -> None:
        common = {
            "scale_count": self.profile.scale_count,
            "link_pairs": self.profile.link_pairs,
            "g_D": self.profile.g_D,
            "g_C": self.profile.g_C,
            "phi": self.profile.phi,
            "w_D": self.profile.w_D,
            "w_C": self.profile.w_C,
            "c_D": self.profile.c_D,
            "c_C": self.profile.c_C,
            "metric_cell_areas": self.profile.metric_cell_areas,
            "enabled": self.profile.enabled,
            "law_id": self.profile.law_id,
            "operator_family": "canonical-family",
            "p_operator_sha256": "canonical-p",
            "p_adjoint_sha256": "canonical-adjoint",
            "tolerances": self.profile.tolerances,
            "parent_identities": self.profile.parent_identities,
            "active_site_counts": self.profile.active_site_counts,
        }
        for key in ("family", "p_operator_sha256", "p_adjoint_sha256"):
            with self.subTest(key=key), self.assertRaisesRegex(CrossScaleError, "authoritative fields"):
                QiCrossScaleProfile(
                    **common,
                    operator_identity={key: "forged", "extra_identity": "retained"},
                )

        candidate = QiCrossScaleProfile(
            **common,
            operator_identity={"extra_identity": "retained"},
        )
        self.assertEqual(candidate.payload["operator"]["extra_identity"], "retained")
    ...
    def test_public_transition_w6_integrated_delegates_once_to_w5(self) -> None:
        (
            geometry_profile,
            transport_profile,
            carrier_profile,
            ...
            cross_scale_profile,
            state,
        ) = _RuntimeFixtures.build()
        with patch.object(conversion, "_transition_v4_carrier_split", _fake_carrier_split):
            result = transition_w6_integrated(
                state,
                ...
                numerical_certificate={},
                cross_scale_profile=cross_scale_profile,
                duration_s=conversion_profile.h_min,
                conversion_enabled=False,
                epsilon_ema_enabled=False,
            )
        self.assertTrue(result.committable, result.failure_reason)
        self.assertEqual(result.receipt["extra_conservative_law_id"], cross_scale_profile.law_id)
        self.assertEqual(result.receipt["energy"]["extra_conservative_law_id"], cross_scale_profile.law_id)
        self.assertEqual(result.receipt["topology"]["force_evaluations"], 2)
        self.assertEqual(result.receipt["conversion"]["q_evaluations"], 1)
        self.assertEqual(result.receipt["ema"]["invocations"], 1)
        self.assertFalse(result.receipt["additional_state"])
