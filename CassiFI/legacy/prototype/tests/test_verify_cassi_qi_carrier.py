...
class CompactControlIndexTests(unittest.TestCase):
    ...
    def test_w3n_w2_identity_is_preserved_exactly(self) -> None:
        ...
        for candidate in root.iterdir():
            ...
            if (
                ...
                # Byte-exactness cannot separate duplicated W3N runs; the
                # producer's parent authority is independent verification.
                try:
    ...
    def test_clock_interval_endpoints_are_committable_and_bound_uses_maximum(self) -> None:
        import math

        import run_cassi_qi_carrier as producer
        from cassi_qi_carrier import build_composition_derivation, load_w4_carrier_profile, transition_v4_carrier
        from cassi_qi_field import QiFlowStateV3
        from cassi_qi_geometry import load_w2_geometry_profile
        from cassi_qi_profile import canonical_json_loads, finite_float, load_development_profile
        ...
        parent, _ = producer._discover_w3n(geometry, transport)
        certificate = canonical_json_loads((parent / "certificate" / "certificate-root.json").read_bytes())
        carrier = load_w4_carrier_profile(geometry=geometry, transport=transport)
        inputs = certificate["offline_derivation"]["inputs"]
        h_min = finite_float(inputs["h_min_s"], name="certified h_min")
        h_max = finite_float(inputs["h_max_s"], name="certified h_max")
        derivation = build_composition_derivation(carrier_profile=carrier, numerical_certificate=certificate)
        self.assertEqual(finite_float(derivation["inputs"]["duration_s"], name="composition duration"), h_max)
        self.assertEqual(
            [
                finite_float(derivation["inputs"]["duration_interval_s"]["minimum"], name="composition h_min"),
                finite_float(derivation["inputs"]["duration_interval_s"]["maximum"], name="composition h_max"),
            ],
            [h_min, h_max],
        )
        for duration in (h_min, h_max):
            step = transition_v4_carrier(
                QiFlowStateV3.create(base, batch_lanes=1),
            ...
            self.assertEqual(finite_float(step.receipt["duration_s"], name="carrier duration"), duration)
        for duration in (math.nextafter(h_min, -math.inf), math.nextafter(h_max, math.inf)):
            step = transition_v4_carrier(
                QiFlowStateV3.create(base, batch_lanes=1),
                geometry_profile=geometry,
                transport_profile=transport,
            ...
            self.assertFalse(step.committable)
            self.assertIn("outside the certified W3 clock interval", step.failure_reason)
