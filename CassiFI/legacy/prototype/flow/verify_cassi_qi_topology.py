...
CODEWORD_WITNESS_DOMAIN = "cassi.qi-flow-w4r-topology-codeword-witness.v1"
...
def _codeword_phases(codeword_id: str, shape: tuple[int, int]) -> list[list[float]]:
    ny, nx = shape
    require(ny >= 2 and nx >= 4, "selected topology resolution cannot represent the canonical codewords")
    phase = [[0.0 for _ in range(nx)] for _ in range(ny)]
    if codeword_id == "uniform":
        return phase
    if codeword_id in {"cycle-positive", "cycle-negative"}:
        sign = 1.0 if codeword_id == "cycle-positive" else -1.0
        rows = [[sign * 2.0 * math.pi * x / nx for x in range(nx)] for _ in range(ny)]
        return [[0.0 if value == 0.0 else value for value in row] for row in rows]
    require(codeword_id == "vortex-antivortex", f"unknown topology codeword: {codeword_id}")
    phase[0][1] = 2.0 * math.pi / 3.0
    phase[0][2] = -2.0 * math.pi / 3.0
    return phase


def _codeword_sector(phase: Sequence[Sequence[float]]) -> tuple[dict[str, Any], float, float]:
    ny = len(phase)
    nx = len(phase[0])
    ...
    dx = [[wrapped(float(phase[y][(x + 1) % nx]) - float(phase[y][x])) for x in range(nx)] for y in range(ny)]
    dy = [[wrapped(float(phase[(y + 1) % ny][x]) - float(phase[y][x])) for x in range(nx)] for y in range(ny)]
    raw_n_x = [sum(row) / (2.0 * math.pi) for row in dx]
    raw_n_y = [sum(dy[y][x] for y in range(ny)) / (2.0 * math.pi) for x in range(nx)]
    raw_p = [
        [
            (dx[y][x] + dy[y][(x + 1) % nx] - dx[(y + 1) % ny][x] - dy[y][x]) / (2.0 * math.pi)
            for x in range(nx)
        ]
        for y in range(ny)
    ]
    integer_residual = max(
        [abs(value - round(value)) for value in raw_n_x + raw_n_y]
        + [abs(value - round(value)) for row in raw_p for value in row]
    )
    sector = {
        "n_x": [int(round(value)) for value in raw_n_x],
        "n_y": [int(round(value)) for value in raw_n_y],
        "p": [[int(round(value)) for value in row] for row in raw_p],
    }
    return sector, max(abs(value) for row in dx + dy for value in row), integer_residual


def _expected_codebook(
    values: Mapping[str, Any],
    edges: Sequence[Mapping[str, Any]],
    cycles: Mapping[str, Any],
) -> dict[str, Any]:
    shape = values["shape"]
    rho_ring = values["rho_ring"]
    rho_topo = values["rho_topo"]
    delta = values["delta"]
    delta_int = values["delta_int"]
    endpoint_radius = 2.0**-45
    phase_radius = 2.0**-38
    parent_values = values["retention"]["parents"]
    require(isinstance(parent_values, Mapping), "profile lacks sealed W2 geometry identities")
    geometry_identities = {
        key: parent_values[key]
        for key in (
            "w2_profile_sha256",
            "w2_contract_root_sha256",
            "w2_geometry_contract_sha256",
            "w2_operator_semantic_sha256",
            "family",
        )
    }
    labels = {
        "uniform": "uniform-zero-sector",
        "cycle-positive": "valid-cycle-positive",
        "cycle-negative": "valid-cycle-negative",
        "vortex-antivortex": "vortex-antivortex-plaquette",
    }
    codewords: list[dict[str, Any]] = []
    for codeword_id, control_label in labels.items():
        phase = _codeword_phases(codeword_id, shape)
        sector, max_edge, integer_residual = _codeword_sector(phase)
        integer_radius = phase_radius * float(max(shape[0], shape[1], 4)) / (2.0 * math.pi)
        branch_margin = math.pi - delta - max_edge - phase_radius
        integer_margin = delta_int - integer_residual - integer_radius
        amplitude_margin = rho_ring - endpoint_radius - rho_topo
        torus_charge = sum(value for row in sector["p"] for value in row)
        require(min(branch_margin, integer_margin, amplitude_margin) > 0.0, f"canonical topology codeword has no positive guard margin: {codeword_id}")
        require(torus_charge == 0, f"canonical topology codeword violates torus neutrality: {codeword_id}")
        witness = {
            "selected_scale": values["slow"],
            "shape_yx": list(shape),
            "amplitude": _f64_tag(rho_ring),
            "phase_f64": [[_f64_tag(value) for value in row] for row in phase],
        }
        codewords.append(
            {
                ...
                "witness": witness,
                "witness_sha256": _hash(witness, CODEWORD_WITNESS_DOMAIN),
                "sector_vector": sector,
                "guard_disks": {
                    "method": "binary64-directed-operation-count.v1",
                    "endpoint_radius": _f64_tag(endpoint_radius),
                    "edge_phase_radius": _f64_tag(phase_radius),
                    "amplitude_margin": _f64_tag(amplitude_margin),
                    "branch_margin": _f64_tag(branch_margin),
                    "integer_margin": _f64_tag(integer_margin),
                    "torus_net_charge": torus_charge,
                },
            }
        )
    require(len({row["witness_sha256"] for row in codewords}) == len(codewords), "canonical topology codewords are not unique")
    metric = values["metric"]
    require(all(_close(value, metric[0], 0.0) for value in metric), "selected metric is not the sealed uniform sheet metric")
    return {
        ...
        "selected_resolution": {
            "slow_scale": values["slow"],
            "shape_yx": list(shape),
            "metric_diagonal": [_f64_tag(metric[0]) for _ in range(shape[0] * shape[1])],
            "geometry_identities": geometry_identities,
        },
        "weighted_rotation": {
            "a_topo": _f64_tag(values["a"]),
            "b_topo": _f64_tag(values["b"]),
        },
        "operator_identities": {
            "selected_sheet_projection": "slow-sheet-only.v1",
            "link_phase": "Arg(psi_j*conj(psi_i)).v1",
            "plaquette_charge": "oriented-wrapped-link-curl-over-2pi.v1",
            "cycle_winding": "registered-torus-cycle-sum-over-2pi.v1",
        },
        "edge_registry": list(edges),
        "cycle_registry": dict(cycles),
        "codewords": codewords,
...
def _verify_derived(root: Path, profile_obj: Mapping[str, Any], values: Mapping[str, Any], edges: list[dict[str, Any]], cycle: Mapping[str, Any], edge_sha: str, cycle_sha: str) -> dict[str, Any]:
    ...
    expected_codebook = _expected_codebook(values, edges, cycle)
    require(_without(codebook, "self_sha256") == expected_codebook, "topology codebook is not the canonical finite selected-sheet codebook")
    require(codebook["self_sha256"] == _hash(expected_codebook, CODEBOOK_DOMAIN), "topology codebook canonical identity mismatch")
    expected_barrier = {
    ...
    require(ret.get("topology_codebook_sha256") == codebook.get("self_sha256"), "profile topological codebook identity is not derived")
    core_barrier = {
        "schema": "cassi.qi-flow-w4r-barrier-certificate.v1",
        "law_id": "topological-v1",
        "potential": "E_topo*(lambda_ph*U_phase+lambda_core*U_core)",
...
def _verify_controls(root: Path, candidate: Mapping[str, Any], profile: Mapping[str, Any], values: Mapping[str, Any], edges: list[dict[str, Any]], cycle: Mapping[str, Any], layout: Mapping[str, Any]) -> dict[str, Any]:
    ...
    for name, expected_decision in EXPECTED_CONTROLS.items():
        ...
        output[name] = {
            "pre": predecessor_state,
            "pre_diag": diag_pre,
            "pre_raw": predecessor_raw,
            "pre_values": predecessor_values,
            "pre_meta": predecessor_meta,
            "candidate_path": candidate_path,
        }
    return output


def _verify_codebook_bindings(
    ...
    witness_ledger = extension["codebook_witnesses"]
    require(isinstance(witness_ledger, Mapping), "extension codeword witness ledger is missing")
    require(
        set(witness_ledger) == {"schema", "selected_sheet_projection", "bindings"}
        and witness_ledger["schema"] == "cassi.qi-flow-w4r-codebook-witness-bindings.v1"
        and witness_ledger["selected_sheet_projection"] == "slow-sheet-only.v1",
        "extension codeword witness ledger contract mismatch",
    )
    bindings = witness_ledger["bindings"]
    rows = codebook["codewords"]
    require(isinstance(bindings, Mapping) and isinstance(rows, list), "extension codeword witness bindings are malformed")
    by_id = {row["codeword_id"]: row for row in rows}
    require(set(bindings) == set(by_id) == {"uniform", "cycle-positive", "cycle-negative", "vortex-antivortex"}, "extension codeword witness set mismatch")
    modes = int(layout["mode_count"])
    scales = int(layout["scale_count"])
    batch = int(layout["batch_count"])
    slow = values["slow"]
    for codeword_id, codeword in by_id.items():
        label = codeword["control_label"]
        require(label in controls, f"codeword control is absent: {label}")
        control = controls[label]
        raw = control["pre_raw"]
        metadata = control["pre_meta"]
        expected = {
            "control_label": label,
            "codeword_witness_sha256": codeword["witness_sha256"],
            "selected_scale": slow,
            "state_raw_sha256": _sha(raw),
            "state_raw_identity": _raw_identity(raw),
            "state_metadata_sha256": metadata["self_sha256"],
            "state_shape": metadata["shape"],
            "raw_byte_count": metadata["raw_byte_count"],
        }
        require(bindings[codeword_id] == expected, f"extension codeword state binding mismatch: {codeword_id}")
        packed = control["pre_values"]
            ...
            for component in range(9):
                for mode in range(modes):
                    for lane in range(batch):
                        require(
                            _state_value(packed, scale, component, mode, batch, modes, lane) == 0.0,
                            f"codeword leaks outside the selected topology sheet: {codeword_id}",
                        )
        psi, _, _, _ = _psi_chi(control["pre"], values)
        phase = codeword["witness"]["phase_f64"]
        amplitude = _number(codeword["witness"]["amplitude"], f"{codeword_id} witness amplitude")
        radius = _number(codeword["guard_disks"]["endpoint_radius"], f"{codeword_id} endpoint radius")
        require(len(phase) == len(psi) and all(len(phase[y]) == len(psi[y]) for y in range(len(psi))), f"codeword witness resolution mismatch: {codeword_id}")
        for y, row in enumerate(phase):
            for x, tagged_phase in enumerate(row):
                angle = _number(tagged_phase, f"{codeword_id} phase")
                expected_psi = amplitude * complex(math.cos(angle), math.sin(angle))
                require(abs(psi[y][x] - expected_psi) <= radius, f"codeword state leaves its sealed endpoint disk: {codeword_id}")
...
def verify(root: Path) -> dict[str, Any]:
    ...
    _verify_codebook_bindings(derived["extension"], derived["codebook"], controls, values, layout)
