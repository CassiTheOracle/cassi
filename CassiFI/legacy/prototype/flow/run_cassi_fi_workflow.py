...
def _remove_path(path: Path) -> None:
    try:
        ...
        _fail("ATOMIC_WRITE_ERROR", f"cannot remove transaction path {path}: {exc}")


def _stage_bytes(payloads: Sequence[tuple[Path, bytes]], *, prefix: str) -> tuple[Path, list[tuple[Path, Path]]]:
    """Write every replacement below one temporary root before publication."""
    try:
        root = Path(tempfile.mkdtemp(prefix=prefix, dir=_REPOSITORY))
        staged: list[tuple[Path, Path]] = []
        for index, (destination, payload) in enumerate(payloads):
            stage = root / f"{index:04d}-{destination.name}"
        ...
        _fail("ATOMIC_WRITE_ERROR", f"cannot stage transaction: {exc}")
    raise AssertionError("unreachable")


def _atomic_transaction(replacements: Sequence[tuple[Path, Path]]) -> None:
    """Replace files/directories as one rollback-safe publication transaction."""
    if not replacements:
        return
    seen: set[Path] = set()
    for stage, destination in replacements:
        if destination in seen or not stage.exists():
            _fail("ATOMIC_WRITE_ERROR", "transaction contains duplicate or missing replacement")
        seen.add(destination)
    records: list[tuple[Path, Path, bool]] = []
    try:
        for stage, destination in replacements:
            destination.parent.mkdir(parents=True, exist_ok=True)
            backup_root = Path(tempfile.mkdtemp(prefix=f".{destination.name}.", dir=destination.parent))
            backup = backup_root / "previous"
            existed = destination.exists() or destination.is_symlink()
            if existed:
                os.replace(destination, backup)
            # Record immediately after moving the old destination.  If the
            # staged replacement fails, rollback must restore this item too.
            records.append((destination, backup_root, existed))
            os.replace(stage, destination)
    except Exception as exc:
        for destination, backup_root, existed in reversed(records):
            try:
                ...
                # Keep trying the remaining records; the original exception is
                # the useful deterministic failure surfaced to the caller.
                pass
        _fail("ATOMIC_WRITE_ERROR", f"cannot publish transaction: {exc}")
    finally:
        for _, backup_root, _ in records:
...
def _gate_rank(gate: str) -> int:
    order = {"schemas": 0, "profile": 1, "G1": 2}
    order.update({name: 3 + index for index, name in enumerate(_WORK_PACKAGE_ORDER)})
    return order.get(gate, 1000)


def _gate_dependencies(gate: str) -> tuple[str, ...]:
    """Return the declared prerequisite gates for dependency propagation."""
    if gate == "G1":
        return ("schemas", "profile")
    if gate == "W1":
        return ("G1",)
    try:
        index = _WORK_PACKAGE_ORDER.index(gate)
    except ValueError:
        return ()
    return ("G1",) if index == 0 else (_WORK_PACKAGE_ORDER[index - 1],)


def _gate_descendants(gate: str, candidates: Sequence[str]) -> set[str]:
    ...
    while frontier:
        next_frontier = {
            ...
            and any(parent in frontier for parent in _gate_dependencies(candidate))
        }
...
def _source_rows_changed(
    ...
    current = {str(row["path"]): dict(row) for row in new_rows}
    return set(previous) != set(current) or any(
...
def _status_update(
    *,
    ...
    if prior_current is not None and not isinstance(prior_current, Mapping):
        _fail("STATUS_INVALID", "existing status completed gate row is invalid")
    if prior_current is not None and not isinstance(prior_current.get("dependency_hashes", {}), Mapping):
        _fail("STATUS_INVALID", "existing status dependency hashes are invalid")
    gate_changed = prior_current is None or prior_current.get("receipt_sha256") != receipt_sha256
    if prior_current is not None and _gate_dependencies(gate):
        gate_changed = gate_changed or prior_current.get("dependency_hashes") != dependency_hashes
    old_source_rows = old.get("source_hashes", []) if old else []
    if prior_current is not None and isinstance(prior_current.get("source_paths"), list):
        owned_source_paths = {
            path for path in prior_current["source_paths"] if isinstance(path, str)
        }
        old_source_rows = [
            row
            for row in old_source_rows
            if isinstance(row, Mapping) and row.get("path") in owned_source_paths
        ]
    elif len(old_completed) == 1:
        # The v1 status format had no source ownership field.  A single
        # completed gate is unambiguous, so retain deletion semantics during
        # the in-place migration; multi-gate legacy status remains conservative.
        owned_source_paths = {
            str(row["path"])
            for row in old_source_rows
            if isinstance(row, Mapping) and isinstance(row.get("path"), str)
        }
        old_source_rows = list(old_source_rows)
    else:
        owned_source_paths = set()
        old_source_rows = [
            row
            for row in old_source_rows
            if isinstance(row, Mapping) and row.get("path") in {
                str(current_row["path"]) for current_row in source_rows
            }
        ]
    source_changed = _source_rows_changed(old_source_rows, source_rows)
    identity_changed = bool(old_current) and old_current != dict(identities)
    event = gate_changed or source_changed or (prior_current is None and identity_changed)
    old_gates = [
        str(row["gate"])
        for row in old_completed
        if isinstance(row, Mapping) and isinstance(row.get("gate"), str)
    ]
    descendants = _gate_descendants(gate, old_gates)
    completed: list[dict[str, Any]] = []
    invalidated = [
        _deepcopy(dict(row))
        for row in old_invalidated
        if isinstance(row, Mapping)
    ]
    for row in old_completed:
        ...
        if prior_gate == gate:
            continue
        if event and prior_gate in descendants:
            candidate: dict[str, Any] = {
                "gate": prior_gate,
                "reason": "dependency identity changed",
    ...
    current_source_paths = sorted(
        {str(row["path"]) for row in source_rows if isinstance(row.get("path"), str)},
        key=lambda item: item.encode("utf-8", "strict"),
    )
    completed.append(
        {
            ...
            "artifact_paths": sorted(
                {_repo_relative(Path(path)) for path in artifact_paths},
                key=lambda item: item.encode("utf-8", "strict"),
            ),
            "dependency_hashes": _deepcopy(dict(dependency_hashes)),
            "depends_on": list(_gate_dependencies(gate)),
    ...
    invalidated.sort(
        key=lambda row: (
            _gate_rank(str(row.get("gate", ""))),
            str(row.get("gate", "")).encode("utf-8", "strict"),
            str(row.get("prior_receipt", "")),
        )
    )
    ...
    if old:
        for row in old.get("source_hashes", []):
            if not isinstance(row, Mapping) or not isinstance(row.get("path"), str):
                continue
            path = str(row["path"])
            # Drop source rows owned by the rerun gate when they disappeared;
            # unrelated gate sources remain in the global manifest.
            if path not in owned_source_paths:
                merged_sources[path] = dict(row)
    for row in source_rows:
        if not isinstance(row, Mapping) or not isinstance(row.get("path"), str):
            _fail("STATUS_INVALID", "source hash row is invalid")
...
def _development_profile_candidate() -> tuple[QiFlowProfile, Any, str]:
    profile_id = "qi-flow-development-v1"
    if _DEVELOPMENT_PROFILE.exists():
        _, payload = _load_json(_DEVELOPMENT_PROFILE)
        value = payload.get("profile") if isinstance(payload, Mapping) else None
        if not isinstance(value, Mapping) or not isinstance(value.get("profile_id"), str) or not value["profile_id"]:
            _fail("PROFILE_INVALID", "development profile input has no valid profile_id")
        profile_id = value["profile_id"]
    try:
        profile = validate_profile(QiFlowProfile.from_defaults(profile_id=profile_id))
        root = validate_contract_root(build_contract_root(profile))
    except Exception as exc:
        _fail("PROFILE_INVALID", f"cannot materialize profile: {exc}")
    return profile, root, profile_id


def _profile_command() -> dict[str, Any]:
    profile, root, profile_id = _development_profile_candidate()
    config = {
    ...
    status_bytes = _canonical(status, context="workflow status")
    receipt_bytes = _canonical(receipt, context="profile receipt")
    stage_root, replacements = _stage_bytes(
        [
            (_DEVELOPMENT_PROFILE, config_bytes),
            (_PROFILE_OUTPUT, config_bytes),
            (receipt_path, receipt_bytes),
            (_DIAG / "cassi-fi-status.json", status_bytes),
        ],
        prefix=".cassi-fi-profile-",
    )
    try:
        _atomic_transaction(replacements)
    finally:
...
def _publish_registry(stage: Path, manifest: Mapping[str, Any], source_rows: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    profile, root, _ = _development_profile_candidate()
    identities = _identity_context(profile, root, registry_sha256=str(manifest["self_sha256"]))
    ...
    try:
        try:
            ...
            _fail("ATOMIC_WRITE_ERROR", f"cannot stage source schema registry: {exc}")
        status_bytes = _canonical(status, context="workflow status")
        receipt_bytes = _canonical(receipt, context="schema receipt")
        byte_stage_root, byte_replacements = _stage_bytes(
            [
                (receipt_path, receipt_bytes),
                (_DIAG / "cassi-fi-status.json", status_bytes),
            ],
            prefix=".cassi-fi-schema-publish-",
        )
        try:
            _atomic_transaction(
                [
                    (source_stage, _SOURCE_REGISTRY),
                    (stage, destination),
                    *byte_replacements,
                ]
            )
        finally:
            shutil.rmtree(byte_stage_root, ignore_errors=True)
    finally:
...
def _delivery_command(name: str, source: str) -> dict[str, Any]:
    ...
    receipt_bytes = _canonical(receipt, context=f"delivery/{name}/receipt")
    ...
    if target_exists or receipt_exists:
        if not target.is_file() or not receipt_path.is_file():
            _fail("DELIVERY_COLLISION", f"delivery {name} has an incomplete prior publication")
        try:
            prior_installed = target.read_bytes()
            prior_receipt = receipt_path.read_bytes()
        except OSError as exc:
            _fail("DELIVERY_COLLISION", f"delivery {name} prior publication is unreadable: {exc}")
        if prior_installed != installed or prior_receipt != receipt_bytes:
            _fail("DELIVERY_COLLISION", f"delivery {name} already names different bytes")
        return receipt
    stage_root, replacements = _stage_bytes(
        [(target, installed), (receipt_path, receipt_bytes)],
        prefix=f".cassi-fi-delivery-{name}-",
    )
    try:
        _atomic_transaction(replacements)
    finally:
...
def _g1_command() -> dict[str, Any]:
    ...
    attestation_path = candidate / "gates" / "g01-identity" / "verification.json"
    verifier_entry = (
        "verify_g1_identity"
        if attestation_path.exists() or attestation_path.is_symlink()
        else "write_g1_verification"
    )
    verification_value, call, call_error = _captured_call(
        f"verify_cassi_qi_flow.{verifier_entry}",
        lambda: _call_entry_point(
    ...
    status_bytes = _canonical(status, context="workflow status")
    receipt_bytes = _canonical(receipt, context="G1 receipt")
    stage_root, replacements = _stage_bytes(
        [
            (state_path, state_result["payload"]),
            (receipt_path, receipt_bytes),
            (_DIAG / "cassi-fi-status.json", status_bytes),
        ],
        prefix=".cassi-fi-g1-",
...
def _unavailable_work_package(reason: str) -> dict[str, Any]:
    return {
        "available": False,
        "failure_code": "WP_UNAVAILABLE",
        "failure_message": reason,
    }


# This table is deliberately explicit.  A declared package is either wired to
# importable public run/verify callables or has a stable fail-closed record; it
# is never guessed from a module name or a private helper.
_WORK_PACKAGE_DISPATCH: dict[str, dict[str, Any]] = {
    "W0": _unavailable_work_package("W0 has no focused executable run/verify entry point"),
    "W1": {
        "available": True,
    ...
    "W6": _unavailable_work_package(
        "W6 has no public continuous-circulation runner/verifier pair; the exchange runner is W5-only",
    ),
    "W6T": _unavailable_work_package(
        "W6T has no public scale-geometry/scattering runner/verifier pair",
    ),
    "W6A": _unavailable_work_package(
        "W6A has no public intrinsic-capacity runner/verifier pair",
    ),
    "W7": _unavailable_work_package(
        "W7 has no public boundary/clock/ingress runner/verifier pair",
    ),
    "W7P": _unavailable_work_package(
        "W7P has no public boundary-permeability runner/verifier pair",
    ),
    "W8": _unavailable_work_package(
        "W8 has no public body/remap runner/verifier pair",
    ),
    "W9": _unavailable_work_package(
        "W9 has no public attention/action runner/verifier pair",
    ),
    "W9O": _unavailable_work_package(
        "W9O has no public observability runner/verifier pair",
    ),
    "W10": _unavailable_work_package(
        "W10 has no public trace/recall runner/verifier pair",
    ),
    "W10R": _unavailable_work_package(
        "W10R has no public behavioral-retention runner/verifier pair",
    ),
    "W10E": _unavailable_work_package(
        "W10E exposes a plan materializer but no independent focused verifier",
    ),
    "W10A": _unavailable_work_package(
        "W10A exposes an acquisition runner but no independent focused verifier",
    ),
    "W11": _unavailable_work_package(
        "W11 has no public trajectory-text runner/verifier pair",
    ),
    "W11D": _unavailable_work_package(
        "W11D has no public dynamic-port runner/verifier pair",
    ),
    "W6B": _unavailable_work_package(
        "W6B exposes endpoint-capacity materialization but no independent focused verifier",
    ),
    "W12M": _unavailable_work_package(
        "W12M has no public transaction-model runner/verifier pair",
    ),
    "W12L": {
        "available": True,
        "run": ("run_cassi_qi_state_lineage", "run"),
        "verify": ("verify_cassi_qi_state_lineage", "verify_artifact"),
        "pass_statuses": ("PASS", "PASS_G12L"),
    },
    "W12A": _unavailable_work_package(
        "W12A has no public unified-runtime focused runner/verifier pair",
    ),
    "W12E": _unavailable_work_package(
        "W12E has no public process-evidence runner callable; its CLI is not a focused run API",
    ),
    "W13R": _unavailable_work_package(
        "W13R has no public reference-world runner/verifier pair",
    ),
    "W13C": _unavailable_work_package(
        "W13C has no independent world-adapter verifier",
    ),
    "W14A": {
        "available": True,
        "run": ("run_cassi_qi_backend_parity", "run_artifact"),
        "verify": ("verify_cassi_qi_backend_parity", "verify_artifact"),
        "pass_statuses": ("PASS",),
    },
    "W14B": {
        "available": True,
        "run": ("run_cassi_qi_backend_profile", "run"),
        "verify": ("verify_cassi_qi_backend_profile", "verify_artifact"),
        "pass_statuses": ("PASS",),
    },
    "W15A": _unavailable_work_package(
        "W15A has no public cutover runner/verifier pair",
    ),
    "W15B": _unavailable_work_package(
        "W15B is documentation-only and has no executable focused runner/verifier pair",
    ),
    "W16A": _unavailable_work_package(
        "W16A has no public release-board runner/verifier pair",
    ),
    "W16B": _unavailable_work_package(
        "W16B has no public documentation-release runner/verifier pair",
    ),
}

if tuple(_WORK_PACKAGE_DISPATCH) != _WORK_PACKAGE_ORDER:
    raise RuntimeError("work-package dispatch order is not canonical")


def _normalise_work_package(token: str) -> str:
    supported = ", ".join(_WORK_PACKAGE_ORDER)
    if not isinstance(token, str) or not token.strip():
        _fail("UNKNOWN_WORK_PACKAGE", f"supported values are {supported}")
    compact = re.sub(r"[^A-Za-z0-9]", "", token).upper()
    aliases: dict[str, str] = {}
    for package in _WORK_PACKAGE_ORDER:
        suffix = package[1:]
        aliases[package] = package
        aliases[f"WP{suffix}"] = package
        aliases[suffix] = package
        if suffix and suffix[0].isdigit():
            digit_count = 0
            while digit_count < len(suffix) and suffix[digit_count].isdigit():
                digit_count += 1
            digits = suffix[:digit_count]
            remainder = suffix[digit_count:]
            padded = digits.zfill(2)
            for prefix in ("W", "WP", "G"):
                aliases[f"{prefix}{digits}{remainder}"] = package
                aliases[f"{prefix}{padded}{remainder}"] = package
            aliases[f"{digits}{remainder}"] = package
            aliases[f"{padded}{remainder}"] = package
    aliases["G15A"] = aliases["G015A"] = "W16A"
    aliases["G15B"] = aliases["G015B"] = "W16B"
...
def _artifact_from_run_result(value: Any) -> Path | None:
    if isinstance(value, (str, os.PathLike)):
        candidate: Any = value
    elif isinstance(value, Mapping):
        candidate = value.get("artifact") or value.get("artifact_root")
    else:
        candidate = None
    if not isinstance(candidate, (str, os.PathLike)) or not str(candidate):
        return None
    path = Path(candidate)
    return (path if path.is_absolute() else _REPOSITORY / path).resolve()


def _workflow_error_from_call(
    phase: str,
    label: str,
    error: Exception,
) -> WorkflowError:
    if isinstance(error, WorkflowError):
        return error
    return WorkflowError(f"WP_{phase.upper()}_FAILED", f"{label}: {error}")
...
def _verify_work_package(token: str) -> dict[str, Any]:
    ...
    if not spec.get("available"):
        error = WorkflowError(
            str(spec.get("failure_code", "WP_UNAVAILABLE")),
            str(spec.get("failure_message", f"{package} is unavailable")),
        )
    else:
        ...
        calls.append(call)
        if call_error is not None:
            error = _workflow_error_from_call("run", run_label, call_error)
        else:
            artifact = _artifact_from_run_result(artifact_value)
            if artifact is None or not artifact.exists():
                error = WorkflowError("WP_ARTIFACT_MISSING", f"{run_label} returned no available artifact")
            else:
        ...
        if error is None:
            ...
            calls.append(call)
            if call_error is not None:
                error = _workflow_error_from_call("verification", verify_label, call_error)
