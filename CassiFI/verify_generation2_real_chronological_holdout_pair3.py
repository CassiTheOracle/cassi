"""Independently verify the third disjoint real chronological receipt.

The verifier rebuilds the pair-specific source frames, guarded field sequence,
controls, and aggregate through the independent reconstruction kernel rather
than importing any campaign runner.
"""
...
base.SCHEMA = "cassifi.generation2-real-chronological-holdout-pair3-verification.v1"
base.EXPERIMENT_ID = "generation2-real-chronological-holdout-pair3-v1"
base.RECEIPT = "CassiFI/_diag/generation2-real-chronological-holdout-pair3-v1.json"
base.OUTPUT = "CassiFI/_diag/generation2-real-chronological-holdout-pair3-verification-v1.json"
...
def run(workspace: Path, receipt_path: Path) -> dict[str, object]:
    ...
    if receipt.get("schema") != "cassifi.generation2-real-chronological-holdout-pair3.v1":
        raise base.RealHoldoutVerificationError("pair3 receipt schema differs")
    if receipt.get("experiment_id") != base.EXPERIMENT_ID or receipt.get("status") != "PASS":
        raise base.RealHoldoutVerificationError("pair3 receipt identity or status differs")
    expected_sources = base._source_manifest(workspace)
    if receipt.get("analysis_sources") != expected_sources:
        raise base.RealHoldoutVerificationError("pair3 source manifest differs")
    expected_partition = {
    ...
    if receipt.get("source_partition") != expected_partition:
        raise base.RealHoldoutVerificationError("pair3 source partition differs")
    expected_fractions = {
    ...
    if receipt.get("training_fractions") != expected_fractions:
        raise base.RealHoldoutVerificationError("pair3 training fractions differ")
    ...
    if receipt.get("configuration") != expected_configuration:
        raise base.RealHoldoutVerificationError("pair3 protocol differs")
    reproduced_slice = base._reproduce_slice(workspace, expected_sources)
    reproduced_slice["schema"] = (
        "cassifi.generation2-real-chronological-holdout-pair3-slice.v1"
    )
    base._assert_equal(receipt["slice"], reproduced_slice, "pair3.slice")
    reproduced_aggregate = _aggregate(reproduced_slice)
    base._assert_equal(receipt["aggregate"], reproduced_aggregate, "pair3.aggregate")
    return {
        "schema": base.SCHEMA,
