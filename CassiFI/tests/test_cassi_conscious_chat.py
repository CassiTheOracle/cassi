...
_PACKAGE_ROOT = _CASSI_FI_ROOT
_QI_CONFIG_PATH = CONFIG_DIR / "cassi-qi-corpus-language.json"
...
class CassiConsciousChatTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        cls.corpus_temporary = tempfile.TemporaryDirectory()
        corpus_root = Path(cls.corpus_temporary.name)
        corpus_path = corpus_root / "corpus.txt"
        corpus_path.write_text(
            "".join(
                f"Field episode {index} learns to speak through light and water.\n"
        ...
        train_corpus(
            ...
            heldout_episodes_per_source=2,
            max_episode_bytes=128,
        )
        cls.corpus_checkpoint_path = corpus_root / "trained" / "field-state.pt"

    @classmethod
    def tearDownClass(cls) -> None:
        cls.corpus_temporary.cleanup()
    def setUp(self) -> None:
    ...
    def _write_config(self, path: Path) -> None:
        source = {
            ...
            "corpus_checkpoint_path": str(self.corpus_checkpoint_path),
    ...
    def test_defaults_are_field_only_and_path_stable(self) -> None:
        self.assertEqual(DEFAULT_CONFIG_PATH, CONFIG_DIR / "conscious-chat.json")
        self.assertEqual(DEFAULT_STATE_DIR, ARTIFACT_DIR / "cassi-qi-native" / "chat")
    ...
    def test_turn_is_field_owned_with_zero_classical_counters(self) -> None:
        ...
        self.assertEqual(turn.as_dict()["architecture"]["lexical_boundary"], "none")
        self.assertEqual(turn.reply_kind, "field-symbols")
        self.assertEqual(
            turn.output_bytes_sha256,
            hashlib.sha256(turn.reply.encode("utf-8")).hexdigest(),
        )
        ...
        self.assertEqual(
            receipt["corpus_memory_sha256"],
            runtime.engine.corpus_memory_sha256,
        )
        self.assertTrue(receipt["trained_memory_preserved"])
    ...
    def test_runtime_overrides_persist_under_the_selected_session(self) -> None:
        ...
        try:
            self.assertEqual(
                second_runtime.engine.state_sha256(second_runtime.state),
    ...
    def test_one_shot_cli_emits_plain_text_and_qi_ownership_receipt(self) -> None:
        ...
        self.assertEqual(receipt["schema"], CHAT_TURN_SCHEMA)
        self.assertTrue(receipt["trained_memory_preserved"])
        self.assertRegex(receipt["corpus_memory_sha256"], r"^[0-9a-f]{64}$")
