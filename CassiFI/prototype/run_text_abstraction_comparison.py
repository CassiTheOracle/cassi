...
STATE_MAGIC = b"CASSI-TEXT-ABSTRACTION-2\0"
MAX_PROGRAM_OUTPUT_BYTES = 65_536
...
def _canonical_tokens(tokens: Sequence[TextToken]) -> tuple[TextToken, ...]:
    ...
    if (
        len(raw) < 2
        or raw[0] != TextToken.PROMPT
        or raw[-1] != TextToken.EMIT
        or any(
            token in {TextToken.PROMPT, TextToken.EMIT}
            for token in raw[1:-1]
        )
    ):
        return raw

    # ASCII case maps preserve byte count and spaces, so they commute with
    # every structural transform in this grammar. REVERSE_BYTES and
    # REVERSE_WORDS are commuting involutions within each span between
    # suffix/repeat operators. Normalize those exact algebraic equivalences
    # before programs enter the field.
    ascii_transform: TextToken | None = None
    structural: list[TextToken] = []
    reverse_bytes = False
    reverse_words = False
    for token in raw[1:-1]:
        if token == TextToken.ASCII_UPPER:
            ascii_transform = TextToken.ASCII_UPPER
        elif token == TextToken.ASCII_LOWER:
            ascii_transform = TextToken.ASCII_LOWER
        elif token == TextToken.ASCII_SWAPCASE:
            ascii_transform = {
                None: TextToken.ASCII_SWAPCASE,
                TextToken.ASCII_UPPER: TextToken.ASCII_LOWER,
                TextToken.ASCII_LOWER: TextToken.ASCII_UPPER,
                TextToken.ASCII_SWAPCASE: None,
            }[ascii_transform]
        elif token == TextToken.REVERSE_BYTES:
            reverse_bytes = not reverse_bytes
        elif token == TextToken.REVERSE_WORDS:
            reverse_words = not reverse_words
        else:
            if reverse_bytes:
                structural.append(TextToken.REVERSE_BYTES)
            if reverse_words:
                structural.append(TextToken.REVERSE_WORDS)
    ...
    if reverse_words:
        structural.append(TextToken.REVERSE_WORDS)
    body = ([] if ascii_transform is None else [ascii_transform]) + structural
    return (TextToken.PROMPT, *body, TextToken.EMIT)
...
class TextProgram:
    ...
    def __post_init__(self) -> None:
        ...
        if tokens[-1] != TextToken.EMIT:
            raise QiFieldError("text program must end in EMIT")
        if any(
            token in {TextToken.PROMPT, TextToken.EMIT}
...
def generate_text_programs() -> tuple[TextProgram, ...]:
    ...
    candidates: list[tuple[TextToken, ...]] = [
        (TextToken.PROMPT, TextToken.EMIT)
    ]
    candidates.extend(
        (TextToken.PROMPT, *stack, TextToken.EMIT)
        for depth in range(1, 4)
        for stack in product(transforms, repeat=depth)
    )
    candidates.extend(
        (TextToken.PROMPT, suffix, emitter, TextToken.EMIT)
        if emitter == TextToken.REPEAT_ONCE
        else (TextToken.PROMPT, suffix, TextToken.EMIT)
        for suffix in _SUFFIX_WIDTH
        for emitter in (TextToken.EMIT, TextToken.REPEAT_ONCE)
    )
    candidates.extend(
        (TextToken.PROMPT, left, suffix, right, TextToken.EMIT)
...
def evaluate_text_program(program: TextProgram, prompt: bytes) -> bytes:
    if not isinstance(prompt, bytes) or len(prompt) > MAX_PROGRAM_OUTPUT_BYTES // 2:
        raise QiFieldError("text program prompt is invalid or exceeds the output bound")
    value = b""
    for token in program.tokens:
        ...
        elif token == TextToken.REPEAT_ONCE:
            value = value + value
        elif token != TextToken.EMIT:
            raise QiFieldError("text program reached an unsupported token")
        if len(value) > MAX_PROGRAM_OUTPUT_BYTES:
            raise QiFieldError("text program output exceeds the fixed byte bound")
    return value
...
def evaluate_role_program(
    ...
    if not isinstance(prompt, bytes) or len(prompt) > MAX_PROGRAM_OUTPUT_BYTES:
        raise QiFieldError("role program prompt is invalid or exceeds the output bound")
...
def _role_examples_by_regime(
    ...
    *,
    ...
    for regime, program in controls.items():
        result[regime] = tuple(
            (prompt, evaluate_role_program(program, prompt))
...
class TextAbstractionController:
    def __init__(
        ...
        *,
        ...
        evaluator: Callable[[Any, bytes], bytes] = evaluate_text_program,
        state_magic: bytes = STATE_MAGIC,
        grammar_id: str = "byte-span-v2",
        breaths: int = 4,
...
def _control_programs(programs: Sequence[TextProgram]) -> dict[str, TextProgram]:
    expected = {
        "ascii_upper": (TextToken.PROMPT, TextToken.ASCII_UPPER, TextToken.EMIT),
        "suffix4": (
            TextToken.PROMPT,
            TextToken.SUFFIX_4,
            TextToken.REPEAT_ONCE,
            TextToken.EMIT,
        ),
        "reverse_words": (
            TextToken.PROMPT,
...
def _examples_by_regime(
    ...
    for regime, program in controls.items():
        result[regime] = tuple(
            (
                episode.prompt,
                evaluate_text_program(program, episode.prompt),
            )
...
def run_text_abstraction_comparison() -> dict[str, Any]:
    ...
    sources, _ = _manifest_sources(
        ROOT / "configs" / "cassi-qi-corpus-first-wave.json"
    )
    source_paths = {source.source_id: source.path for source in sources}
    ...
    natural_outputs = [
        ...
        else evaluate_text_program(natural_program, episode.prompt)
    ...
    role_natural_outputs = [
        ...
        else evaluate_role_program(role_natural_program, episode.prompt)
        for episode in heldout
    ]
    ...
    for regime in SYMBOLIC_REGIMES[1:]:
        ...
        outputs = [
            b""
            if program is None
            else evaluate_role_program(program, prompt)
    ...
    for regime in REGIMES[1:]:
        ...
        outputs = [
            b""
            if program is None
            else evaluate_text_program(program, episode.prompt)
    ...
    result = {
        ...
        "typed_program_abstraction": {
            ...
            "inference_contract": {
                ...
                "max_output_bytes": MAX_PROGRAM_OUTPUT_BYTES,
        ...
        "role_symbol_abstraction": {
            ...
            "inference_contract": {
                "target_blind": True,
                "termination": "finite typed program ending in PERIOD",
                "max_output_bytes": MAX_PROGRAM_OUTPUT_BYTES,
                "target_length_input": False,
            },
