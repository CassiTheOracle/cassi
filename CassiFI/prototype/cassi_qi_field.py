...
class QiFieldController:
    ...
    def resonance_scores(self, state: QiFieldState) -> torch.Tensor:
        """Return ungated phase-conjugate scores for field-owned decisions."""

        self._validate_state(state)
        metrics = self._metrics_raw(state)
        rho = metrics["rho"]
        epsilon = torch.finfo(state.field.dtype).eps
        scale_weights = rho / torch.clamp_min(rho.sum(dim=0), epsilon)
        return (scale_weights.unsqueeze(1) * metrics["demod_re"]).sum(dim=0).transpose(0, 1)
    ...
    def _read(
        ...
        *,
        ...
        self._validate_state(state)
        phase = _finite("phase_radians", phase_radians)
        candidates: tuple[int, ...] | None = None
        if allowed_symbols is not None:
            candidates = tuple(allowed_symbols)
            if (
                not candidates
                ...
                or any(
                    ...
                    or not 0 <= symbol < self.config.alphabet_size
                    for symbol in candidates
                )
            ):
                raise QiFieldError(
                    "allowed_symbols must be distinct in-range integer symbols"
                )
        metrics = self._metrics_raw(state)
        eps = torch.finfo(state.field.dtype).eps
        ...
        if phase != 0.0:
            ...
            shared_re, shared_im = (
                shared_re * cosine + shared_im * sine,
                shared_im * cosine - shared_re * sine,
            )
    ...
    def emit(
        ...
        *,
        ...
        """Read through a fixed phase probe, optionally over fixed symbols."""
