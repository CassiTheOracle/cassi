...
class QiNativeBuilderTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        corpus_path = cls.root / "corpus.txt"
        corpus_path.write_bytes(
            b"the field emits every symbol through qi. " * 512
        )
        config_path = _CASSI_FI_ROOT / "configs" / "cassi-qi-corpus-language.json"
        train_corpus(
            corpus_path,
            config_path,
            cls.root / "trained",
        ...
        cls.controller = QiFieldController(
            QiFieldConfig.from_dict(json.loads(config_path.read_text("utf-8")))
        )
        cls.checkpoint_path = cls.root / "trained" / "field-state.pt"
    ...
    def _result(self, max_output_symbols: int = 4) -> CassiQiTextResult:
        return generate_text(
            self.controller,
            self.engine.initial_state(device="cpu"),
