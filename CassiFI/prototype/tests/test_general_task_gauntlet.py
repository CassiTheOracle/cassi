...
def test_text_program_canonicalization_preserves_exact_semantics() -> None:
    suffix_width = {
    ...
    def raw_evaluate(tokens: tuple[TextToken, ...], value: bytes) -> bytes:
        for token in tokens:
            ...
            elif token in suffix_width:
                value = value[-min(len(value), suffix_width[token]) :]
    ...
    raw_cases: list[tuple[TextToken, ...]] = [
        (),
        *(
            stack
        ...
        *(
            (suffix,)
            for suffix in suffix_width
        ),
        *(
            (suffix, TextToken.REPEAT_ONCE, TextToken.ASCII_LOWER)
            for suffix in suffix_width
        ),
        *(
            (left, suffix, right)
    ...
    for raw_tokens in raw_cases:
        program = TextProgram((TextToken.PROMPT, *raw_tokens, TextToken.EMIT))
        assert all(
            evaluate_text_program(program, value)
            == raw_evaluate(raw_tokens, value)
            for value in values
        )

    conjugate = TextProgram(
        (
            TextToken.PROMPT,
            TextToken.REVERSE_WORDS,
            TextToken.ASCII_LOWER,
...
def test_single_regime_update_preserves_every_other_field_namespace() -> None:
    programs = (
        TextProgram((TextToken.PROMPT, TextToken.EMIT)),
        TextProgram((TextToken.PROMPT, TextToken.ASCII_UPPER, TextToken.EMIT)),
        TextProgram((TextToken.PROMPT, TextToken.REVERSE_BYTES, TextToken.EMIT)),
    )
