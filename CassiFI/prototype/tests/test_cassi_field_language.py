...
from train_cassi_field_language import (
    TrajectoryTrainingError,
    _manifest_sources,
    train_corpus,
    train_manifest,
)
...
class TrajectoryLanguageTests(unittest.TestCase):
    ...
    def test_manifest_relative_paths_are_cwd_independent_and_hash_bound(self) -> None:
        manifest_root = self.root / "relative-manifest"
        manifest_dir = manifest_root / "configs"
        source = manifest_root / "data" / "corpora" / "relative.txt"
        unrelated_cwd = self.root / "unrelated-cwd"
        source.parent.mkdir(parents=True)
        manifest_dir.mkdir(parents=True)
        unrelated_cwd.mkdir()
        source.write_text(
            "Relative manifest sources resolve from the manifest directory.\n",
            encoding="utf-8",
        )
        manifest_path = manifest_dir / "manifest.json"
        manifest_payload = {
            "holdout_fraction_denominator": 4,
            "minimum_holdout_bytes": 8,
            "schema": "cassi.qi-corpus-manifest.v1",
            "sources": [
                {
                    "id": "relative",
                    "path": "../data/corpora/relative.txt",
        ...
        self.assertEqual(sources[0].path, source.resolve())

        manifest_payload["sources"][0]["sha256"] = "0" * 64
        manifest_path.write_text(json.dumps(manifest_payload), encoding="utf-8")
        with self.assertRaisesRegex(TrajectoryTrainingError, "hash mismatch"):
            _manifest_sources(manifest_path)
