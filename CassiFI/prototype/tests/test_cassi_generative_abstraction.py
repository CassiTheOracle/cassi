...
def test_boundary_packet_and_journal_are_exact_restartable_and_bounded(
    ...
    idempotent_event_id = "a" * 64
    third_packet = _boundary_packet(
        codec_id=CODEC_JSON,
    ...
    third = journal.append(
        third_packet,
        idempotency_event_id=idempotent_event_id,
    )
    fourth_packet = _boundary_packet(
        codec_id=CODEC_JSON,
        payload=b"null",
        shape=(4,),
        dtype="uint8",
    ...
    journal.append(fourth_packet)
    retried_third = _boundary_packet(
        codec_id=CODEC_JSON,
        payload=b'{"stable":true}',
        shape=(15,),
        dtype="uint8",
    ...
    conflicting_third = _boundary_packet(
        codec_id=CODEC_JSON,
    ...
    with pytest.raises(UniversalDataError, match="idempotency event identity conflicts"):
        journal.append(
            conflicting_third,
            idempotency_event_id=idempotent_event_id,
        )
