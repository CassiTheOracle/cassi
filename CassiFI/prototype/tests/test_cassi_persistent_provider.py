...
from cassi_canonical_runtime import CanonicalActionJournal, CanonicalRuntimeError
...
def test_canonical_field_transfers_restarts_and_rejects_sidecars(
    tmp_path: Path,
) -> None:
    provider = _start(tmp_path)
    ...
    try:
        learned = provider.canonical_transition(
            _canonical_request(
                "teach",
                request_id="teach-right",
                task_scope="training",
                text="gaze right to inspect the amber target",
                action="action.gaze-right",
            )
        )
        assert learned["receipt"]["state_in_sha256"] != learned["receipt"]["state_out_sha256"]
        assert learned["receipt"]["provider_is_adaptive_owner"] is True
        assert learned["receipt"]["adaptive_sidecars"] == []
        assert learned["receipt"]["external_model_calls"] == 0
        transferred = provider.canonical_transition(
            _canonical_request(
                "plan",
                request_id="plan-unseen",
                task_scope="unseen-inspection",
                text="gaze right to inspect the new target",
                candidates=candidates,
            )
        )
        assert transferred["decision"]["selected_action"] == "action.gaze-right"
        assert transferred["receipt"]["identity_scope"] == "alice"
        assert transferred["receipt"]["task_scope"] == "unseen-inspection"
        with pytest.raises(ProviderError, match="key set"):
            provider.canonical_transition(
                {
                    **_canonical_request(
                    ...
                    "adaptive_sidecar": {"route": "action.gaze-right"},
                }
            )
    finally:
        provider.close()

    restarted = PersistentFieldProvider(_provider_config(tmp_path))
    restarted.start()
    try:
        replay = restarted.canonical_transition(
            _canonical_request(
                "recall",
                request_id="recall-after-restart",
                task_scope="unseen-inspection",
                text="gaze right to inspect another target",
                candidates=candidates,
            )
        )
        assert replay["decision"]["selected_action"] == "action.gaze-right"
        assert (
            replay["receipt"]["state_in_sha256"]
            == replay["receipt"]["state_out_sha256"]
        )
        assert restarted.store is not None
        assert restarted.initial_state is not None
        assert restarted.agency_controller is not None
        agency = restarted.agency_controller
        loaded = restarted.store.load("alice")
        assert loaded is not None
        state = loaded[0]
        mnemic = restarted.store.layout.mnemic(state)
        ...
        relevant = tuple(
            dict.fromkeys(decision.addresses["action.gaze-right"])
        )
        lesioned = agency.lesion(mnemic, relevant)
        assert agency.decide(
            lesioned,
            identity_scope="alice",
            text="gaze right to inspect another target",
            candidates=candidates,
        ).selected_action is None
        occupied = set().union(*map(set, decision.addresses.values()))
        unrelated = next(
            index for index in range(mnemic.field.numel()) if index not in occupied
        )
        control = agency.lesion(mnemic, (unrelated,))
        assert agency.decide(
            control,
            identity_scope="alice",
            text="gaze right to inspect another target",
            candidates=candidates,
        ).selected_action == "action.gaze-right"
    finally:
        restarted.close()


def _prepare_canonical_action(
    provider: PersistentFieldProvider,
) -> tuple[DeterministicQiWorld, dict[str, Any]]:
    world = DeterministicQiWorld(
        seed=17,
        world_id="world.action-lifecycle",
        episode_id="episode.action-lifecycle",
        session_id="world-session",
    )
    candidates = ["action.gaze-left", "action.gaze-right", "action.hold"]
    text = "gaze right to inspect the target"
    provider.canonical_transition(
        _canonical_request(
            "teach",
            request_id="teach-action",
            task_scope="world-task",
            text=text,
            action="action.gaze-right",
        )
    )
    ...
    action_request = {
        ...
        "authorization_path": ["test.explicit-authorization"],
        "plan_receipt_sha256": plan["receipt_sha256"],
    }
    return world, action_request


def test_action_authority_rejection_allows_authorized_retry(tmp_path: Path) -> None:
    provider = _start(tmp_path)
    try:
        world, request = _prepare_canonical_action(provider)
        with pytest.raises(ProviderError, match="required authority"):
            provider.execute_canonical_action({**request, "authority": 0.0}, world)
        assert world.logical_tick == 0
        response = provider.execute_canonical_action(request, world)
        assert response["stage"] == "observed"
        assert response["status"] == "applied"
        assert world.logical_tick == 1
    finally:
        provider.close()


@pytest.mark.parametrize("separate_provider", (False, True))
def test_concurrent_duplicate_actions_return_one_observed_result(
    tmp_path: Path, separate_provider: bool
) -> None:
    provider = _start(tmp_path)
    peer = _start(tmp_path) if separate_provider else provider
    try:
        world, request = _prepare_canonical_action(provider)
        ...
        def execute(index: int) -> dict[str, Any]:
            ...
            owner = provider if index % 2 == 0 else peer
            return owner.execute_canonical_action(request, world)

        with ThreadPoolExecutor(max_workers=8) as pool:
            results = list(pool.map(execute, range(8)))
        assert results[0]["stage"] == "observed"
        assert all(result == results[0] for result in results)
        assert world.logical_tick == 1
        assert provider.store is not None
        checkpoint = provider.store.load("alice")
        assert checkpoint is not None
        assert (
            provider.store.layout.state_sha256(checkpoint[0])
            == results[0]["state_out_sha256"]
        )
    finally:
        if peer is not provider:
            peer.close()
        provider.close()


@pytest.mark.parametrize("crash_point", ("dispatch", "outcome", "checkpoint"))
def test_action_dispatch_recovers_without_duplicate_effect_or_consolidation(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
    crash_point: str,
) -> None:
    provider = _start(tmp_path)
    world, action_request = _prepare_canonical_action(provider)
    assert provider.store is not None
    before = provider.store.load("alice")
    assert before is not None
    before_sha256 = provider.store.layout.state_sha256(before[0])
    if crash_point == "outcome":
        def fail_before_consolidation(*args: Any, **kwargs: Any) -> Any:
            raise ProviderError("simulated crash after outcome write")

        monkeypatch.setattr(provider, "_consolidate_action", fail_before_consolidation)
    elif crash_point == "checkpoint":
        save = provider.store.save

        def fail_after_checkpoint(*args: Any, **kwargs: Any) -> Any:
            save(*args, **kwargs)
            raise ProviderError("simulated crash after checkpoint write")

        monkeypatch.setattr(provider.store, "save", fail_after_checkpoint)
    with pytest.raises(ProviderError, match="simulated crash"):
        provider.execute_canonical_action(
            action_request,
            world,
            crash_after_dispatch=crash_point == "dispatch",
        )
    assert world.logical_tick == 1
    after_crash = provider.store.load("alice")
    assert after_crash is not None
    after_crash_sha256 = provider.store.layout.state_sha256(after_crash[0])
    assert (after_crash_sha256 != before_sha256) == (crash_point == "checkpoint")
    provider.close()

    restarted = PersistentFieldProvider(_provider_config(tmp_path))
    restarted.start()
    try:
        recovered = restarted.reconcile_canonical_actions(
            identity_scope="alice",
            world=world,
        )
        assert len(recovered) == 1
        assert recovered[0]["stage"] == "observed"
        assert recovered[0]["status"] == "applied"
        assert recovered[0]["consolidated_once"] is True
        assert recovered[0]["state_in_sha256"] == before_sha256
        assert recovered[0]["state_out_sha256"] != before_sha256
        if crash_point == "checkpoint":
            assert recovered[0]["state_out_sha256"] == after_crash_sha256
        assert world.logical_tick == 1
        duplicate = restarted.execute_canonical_action(action_request, world)
        assert duplicate == recovered[0]
        assert world.logical_tick == 1
        assert restarted.reconcile_canonical_actions(
            identity_scope="alice",
            world=world,
        ) == []
        assert restarted.action_journal is not None
        action_id = restarted.action_journal.action_instance_id(
            identity_scope="alice",
            task_scope="world-task",
            request_id="execute-action",
        )
        record = restarted.action_journal.load(action_id)
...
def test_action_journal_serializes_concurrent_duplicate_stage_writes(
    tmp_path: Path,
) -> None:
    journal = CanonicalActionJournal(tmp_path / "journal")
    record = journal.propose(
        identity_scope="alice",
        task_scope="task",
        request_id="concurrent",
        payload={"command_sha256": "a" * 64},
    )
    action_id = record["action_instance_id"]
    ...
    def authorize() -> dict[str, Any]:
        return journal.append(
            ...
            expected=("proposed",),
            stage="authorized",
            payload={"authority": 1.0},
        )

    with ThreadPoolExecutor(max_workers=8) as pool:
        results = tuple(pool.map(lambda _: authorize(), range(32)))
    assert all(len(result["events"]) == 2 for result in results)
    committed = journal.load(action_id)
    assert committed is not None
    assert [event["stage"] for event in committed["events"]] == [
        "proposed",
        "authorized",
    ]
    with pytest.raises(CanonicalRuntimeError, match="conflicts"):
        journal.append(
            action_id,
            expected=("proposed",),
            stage="authorized",
            payload={"authority": 0.5},
        )
