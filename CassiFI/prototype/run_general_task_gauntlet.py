...
_TEXT_FAMILY_TOKENS: Mapping[str, tuple[TextToken, ...]] = {
    "identity": (TextToken.PROMPT, TextToken.EMIT),
    "ascii_upper": (
        TextToken.PROMPT,
...
def _evaluate_program(program: Any, prompt: bytes) -> bytes:
    if isinstance(program, TextProgram):
        return evaluate_text_program(program, prompt)
    if isinstance(program, RoleProgram):
        return evaluate_role_program(program, prompt)
    ...
    for family in _CONTROL_FAMILIES:
        ...
        for observation, source in zip(observations, corpus, strict=True):
            ...
            if family == "natural":
                ...
                target = evaluate_text_program(controls["ascii_upper"], prompt)
            else:
                target = _evaluate_program(controls[family], prompt)
...
def _evaluate_family(
    ...
    if program is not None:
        exact = sum(
            _evaluate_program(program, episode.prompt)
            == episode.target
            for episode in episodes
        )
    return TaskReceipt(
    ...
    for name, families in _TRANSFER_TASKS.items():
        ...
        for episode in episodes:
            ...
            for learned_program, family in zip(learned, families, strict=True):
                ...
                predicted = _evaluate_program(learned_program, predicted)
                expected = _evaluate_program(controls[family], expected)
