"""Run the frozen L35 held-out chromatic sequence-capacity board."""
...
RUNNER = ROOT / "verification" / "run_l35_heldout_chromatic_capacity.py"
VERIFIER = ROOT / "verification" / "verify_l35_heldout_chromatic_capacity.py"
OUTPUT_DIR = ROOT / "_diag" / "l35-heldout-chromatic-capacity"
BOARD_NAME = "l35-board.json"
TRACE_NAME = "l35-traces.npz"
...
PROFILE_FACTORIES: tuple[tuple[str, Callable[[], Any]], ...] = (
    ...
    (
        "l32-quadrature",
        lambda: QuadratureChromaticFieldController(
            QuadratureChromaticFieldConfig(mode_count=MODE_COUNT)
        ),
    ),
    (
        "l34-exact",
        lambda: ExactCyclicFieldController(
...
SOURCE_PATHS = (
    ...
    ROOT / "cassi_quadrature_chromatic_field.py",
...
class L35RunnerError(RuntimeError):
    """The canonical L35 capacity board could not be completed."""
...
def main() -> int:
    ...
    if missing:
        raise L35RunnerError(f"missing bound source files: {missing!r}")
    hashes = {
