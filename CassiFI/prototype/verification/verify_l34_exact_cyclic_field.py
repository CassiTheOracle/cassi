"""Independently verify and classify the frozen L34 exact cyclic board."""
...
VERIFICATION_SCHEMA = "cassi.l34.exact-cyclic-verification.v1"
...
DEFAULT_BOARD = ROOT / "_diag" / "l34-exact-cyclic-field" / "l34-board.json"
DEFAULT_OUTPUT = ROOT / "artifacts" / "l34-exact-cyclic-field"
DEFAULT_REPORT = DEFAULT_OUTPUT / "L34-EXACT-CYCLIC-DYNAMICS-REPORT.md"
DEFAULT_JSON = DEFAULT_OUTPUT / "l34-verification.json"
EXPECTED_SOURCES = {
    "designs/L34-EXACT-CYCLIC-DYNAMICS-PREREG.md",
...
def verify_board(
    ...
    trace_path = sibling_artifact(board_path, board.get("trace"), "l34-traces.npz", "trace")
    png_path = sibling_artifact(
        board_path, board.get("projection"), "l34-projection.png", "projection"
    )

    with tempfile.TemporaryDirectory(
        prefix=".l34-verify-", dir=board_path.parent
    ) as temporary_name:
...
def report_text(verdict: str, payload: Mapping[str, Any]) -> str:
    ...
    lines = [
        "# L34 Exact Cyclic Dynamics — Verification",
        "",
        f"**Verdict: `{verdict}`**",
        "",
        explanation,
