"""Independently verify and classify the frozen L38 relational board."""
...
BOARD_SCHEMA = "cassi.l38.relational-chromatic-board.v1"
TRACE_SCHEMA = "cassi.l38.relational-chromatic-traces.v1"
VERIFICATION_SCHEMA = "cassi.l38.relational-chromatic-verification.v1"
LAYOUT_PROFILE = "cassi.qi-cyclic-chromatic-coordinate-native.v1"
OPERATOR_PROFILE = "cassi.qi-relational-chromatic-recall.v1"
PROJECTION_PROFILE = "cassi.qi-cyclic-chromatic-projection.v1"
PREREGISTRATION = ROOT / "designs" / "L38-RELATIONAL-CHROMATIC-RECALL-PREREG.md"
DEFAULT_BOARD = ROOT / "_diag" / "l38-relational-chromatic-field" / "l38-board.json"
DEFAULT_OUTPUT = ROOT / "artifacts" / "l38-relational-chromatic-field"
DEFAULT_REPORT = DEFAULT_OUTPUT / "L38-RELATIONAL-CHROMATIC-RECALL-REPORT.md"
DEFAULT_JSON = DEFAULT_OUTPUT / "l38-verification.json"
EXPECTED_SOURCES = {
    "designs/L38-RELATIONAL-CHROMATIC-RECALL-PREREG.md",
...
def relational_readout(
    differential: np.ndarray, codebook: np.ndarray
) -> dict[str, np.ndarray]:
    l30.need(bool(_COMMON_QUEUE), "relational common-coordinate queue exhausted")
    ...
    scores = (
        result["scores"] + (np.abs(coefficient) ** 2).T.astype(np.float32)
    ).astype(np.float32)
    symbols = np.argmax(scores, axis=1).astype(np.int64)
    winning = np.take_along_axis(
        result["contributions"], symbols[:, None, None], axis=2
    )[:, :, 0]
    active_count = result["active_bank_count"]
    coherence = (
        np.abs(winning.sum(axis=1)) ** 2
        / (
            active_count * np.sum(np.abs(winning) ** 2, axis=1)
            + 1.0e-12
        )
    ).astype(np.float32)
    coherence = np.where(result["available"], coherence, 0.0).astype(np.float32)
    result.update(
        {"scores": scores, "symbols": symbols, "white_coherence": coherence}
    )
    return result
...
def verify_board(
    ...
    trace_path = sibling_artifact(board_path, board.get("trace"), "l38-traces.npz", "trace")
    png_path = sibling_artifact(
        board_path, board.get("projection"), "l38-projection.png", "projection"
    )
    ...
    with tempfile.TemporaryDirectory(
        prefix=".l38-verify-", dir=board_path.parent
    ) as temporary_name:
        ...
        try:
            ...
            l30.recompute_readout = relational_readout
            verdict, payload = l30.verify_board(
                compat_board, allow_smoke_device=allow_smoke_device
            )
            l30.need(not _COMMON_QUEUE, "relational common-coordinate queue not consumed")
        finally:
            (
                l30.TRACE_SCHEMA,
                l30.coordinates_from_field,
...
def report_text(verdict: str, payload: Mapping[str, Any]) -> str:
    ...
    lines = [
        "# L38 Relational Chromatic Recall — Verification",
        "",
        f"**Verdict: `{verdict}`**",
        "",
        explanation,
