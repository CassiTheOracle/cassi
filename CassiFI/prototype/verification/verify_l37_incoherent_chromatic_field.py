"""Independently verify and classify the frozen L37 incoherent board."""
...
BOARD_SCHEMA = "cassi.l37.incoherent-chromatic-board.v1"
TRACE_SCHEMA = "cassi.l37.incoherent-chromatic-traces.v1"
VERIFICATION_SCHEMA = "cassi.l37.incoherent-chromatic-verification.v1"
LAYOUT_PROFILE = "cassi.qi-cyclic-chromatic-coordinate-native.v1"
OPERATOR_PROFILE = "cassi.qi-incoherent-chromatic-recall.v1"
PROJECTION_PROFILE = "cassi.qi-cyclic-chromatic-projection.v1"
PREREGISTRATION = ROOT / "designs" / "L37-INCOHERENT-CHROMATIC-RECALL-PREREG.md"
DEFAULT_BOARD = ROOT / "_diag" / "l37-incoherent-chromatic-field" / "l37-board.json"
DEFAULT_OUTPUT = ROOT / "artifacts" / "l37-incoherent-chromatic-field"
DEFAULT_REPORT = DEFAULT_OUTPUT / "L37-INCOHERENT-CHROMATIC-RECALL-REPORT.md"
DEFAULT_JSON = DEFAULT_OUTPUT / "l37-verification.json"
EXPECTED_SOURCES = {
    "designs/L37-INCOHERENT-CHROMATIC-RECALL-PREREG.md",
...
def verify_board(
    ...
    trace_path = sibling_artifact(board_path, board.get("trace"), "l37-traces.npz", "trace")
    png_path = sibling_artifact(
        board_path, board.get("projection"), "l37-projection.png", "projection"
    )

    with tempfile.TemporaryDirectory(
        prefix=".l37-verify-", dir=board_path.parent
    ) as temporary_name:
        ...
        try:
            ...
            l30.recompute_readout = incoherent_readout
...
def report_text(verdict: str, payload: Mapping[str, Any]) -> str:
    ...
    lines = [
        "# L37 Incoherent Chromatic Recall — Verification",
        "",
        f"**Verdict: `{verdict}`**",
        "",
        explanation,
