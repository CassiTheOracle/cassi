...
This runner deliberately has no native-Qwen or organism dependency.  It loads
the hash-pinned corpus-trained Qi field, performs the same generation twice
from the same checkpoint state, requires byte-for-byte deterministic replay,
round-trips the committed successor through ``dump_state_bytes`` /
``load_state_bytes``, binds the frozen native baseline, and compares the result
with the offline native Qwen footprint receipt.
"""
...
def _load_field_dependence(path: Path) -> tuple[bool, str, str]:
    ...
    if value.get("schema") != "cassi.qi-field-dependence.v2":
...
def build_final_evidence(args: argparse.Namespace) -> dict[str, Any]:
    ...
    engine = CassiQiTextEngine(
        controller,
        checkpoint_path=args.corpus_checkpoint,
    ...
    def run_once() -> CassiQiTextResult:
        ...
        return engine.generate(state, messages)
    ...
    reloaded_sha = engine.state_sha256(reloaded)
    ...
    expected_continuation = engine.generate(first.state, messages)
    reloaded_continuation = engine.generate(reloaded, messages)
    ...
    return {
        ...
        "model_id": "cassi-qi-corpus-language-v1",
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--baseline-receipt",
        type=Path,
        default=ARTIFACT_DIR / "qwen-displacement" / "baseline-receipt.json",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--out",
        type=Path,
        default=ARTIFACT_DIR / "qwen-displacement" / "final-qi-native-evidence.json",
    )
