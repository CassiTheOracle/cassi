...
def _receipt_path(path: Path | str, output_dir: Path) -> str:
    """Encode a receipt path relative to its artifact directory when possible."""
    try:
        relative = os.path.relpath(os.fspath(path), start=os.fspath(output_dir))
        return relative.replace(os.sep, "/")
    except ValueError:
        # Windows cannot relativize paths on different drives; retain compatibility.
...
def _train_sources(
    ...
    *,
    ...
    manifest_receipt = dict(manifest_metadata)
    manifest_path_value = manifest_receipt.get("path")
    if isinstance(manifest_path_value, str):
        manifest_receipt["path"] = _receipt_path(manifest_path_value, output_dir)
    receipt: dict[str, object] = {
        "checkpoint": {
            "memory_sha256": law.memory_sha256(state),
            "path": _receipt_path(checkpoint_path, output_dir),
            "sha256": checkpoint_sha256,
            "shape": list(state.field.shape),
            "tensor_count": 1,
        },
        "config": {
            "path": _receipt_path(config_path, output_dir),
            ...
            "sources": [
                {
                    **source.identity_dict(),
                    "path": _receipt_path(source.path, output_dir),
                }
                for source in sources
            ],
        },
