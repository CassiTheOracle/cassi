# L40 Ordered Recall Capacity — Frozen Preregistration

## Status: FROZEN — 2026-08-30

L40 is frozen after preserving the complete L39 `ADOPT` and before any L40 runner, verifier, smoke, or canonical execution. It is an evidence-only comparison of immutable L31 cyclic recall and immutable L39 ordered relational recall. It does not change, select, combine, or tune a field mechanism.

## Question

Does L39's adopted categorical current-then-predecessor readout expose at least two ordered symbols on unseen histories, and is that gain absent from its L31 dynamics/readout baseline?

Field-state difference without a readout-visible consequence is not counted as usable sequence capacity.

## Bound profiles and identities

The two arms are fixed and ordered:

1. `l31-cyclic` — `CyclicChromaticFieldController`;
2. `l39-ordered` — `OrderedRelationalChromaticFieldController`.
...
- board: `cassi.l40.ordered-recall-capacity-board.v1`;
- traces: `cassi.l40.ordered-recall-capacity-traces.v1`;
- verification: `cassi.l40.ordered-recall-capacity-verification.v1`.
...
4. paired same-tail output distance `||s_even-s_odd||_2 / max(||s_even||_2+||s_odd||_2, 1e-12)`;
...
- `SUPPORTS` if `l39-ordered` has retained and history-observable capacity at least 2, both capacities strictly exceed `l31-cyclic`, depth-2 oldest-symbol mean reciprocal rank is at least `0.05`, and depth-2 median paired distance is at least `0.01`;
- `CONTRADICTS` if `l39-ordered` has history-observable capacity below 2, retained capacity at most 1, depth-2 oldest-symbol mean reciprocal rank below `0.05`, and depth-2 median paired distance below `0.01`;
...
- `_diag/l40-ordered-recall-capacity/l40-board.json`;
- `_diag/l40-ordered-recall-capacity/l40-traces.npz`.

Verification:

- `artifacts/l40-ordered-recall-capacity/L40-ORDERED-RECALL-CAPACITY-REPORT.md`;
- `artifacts/l40-ordered-recall-capacity/l40-verification.json`.
