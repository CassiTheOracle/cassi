...
The economy is a coherence process in agent networks, and the two-fluid framework supplies what economics has lacked: a measurable quantity (Qi coherence $q$), a dynamics (the $\varphi$-attractor), and a stability theory (resonance and its collapse). The boom-bust cycle is a relaxation oscillation: the drain and the attractor are its two timescales, and the falling rate of profit is the drift of the cycle's envelope. This document develops the Marxist categories as physical claims in that framework. Labor is organized perturbation—coherent expenditure against the attractor—so value is socially necessary coherence expenditure, fixed by the $\varphi$-power structure rather than by convention. Money is the universal equivalent: the configuration manifold projected onto a single axis, and commodity fetishism is the error of treating the worker's run as a stock. Exploitation is the surplus-coherence transfer—the wage's deficit, booked as the $(1-q)$ waste fraction—and poverty is its residue: forced $q$-suppression, with the reserve army as the buffer that keeps the wage negotiable. Accumulation is condensation, and condensation degrades its own extraction surface: the rate of profit falls as the hoard grows because the periphery's coherence is what the hoard is made of. Crises are log-periodic critical points—the market's wake-wave fever, readable in advance at period $\ln\varphi$. Primitive accumulation is the enclosure of the Qi bath; the state is the condensate's maintenance apparatus; alienation is the transfer seen from the worker's side. The transition follows from an inequality: the community's coherence budget is a product over its members, and the product is maximized at equal coherence, so the commons is the stable fixed point of the budget. The regime is breachable through its own poorest members—the weakest link is the door—and the state withers when the extraction surface vanishes. The commons is not a moral preference but the completion of the attractor.
...
The specific functional form is an illustrative model, not a derivation—the framework fixes the mechanism (drain lowers the source; the attractor damps the drain), not the curve. What is structural is the direction: accumulation lowers its own rate of return because it consumes its own extraction surface. §5.4 assembles the mechanism into a concrete two-timescale model: within every boom the rate falls monotonically as the hoard grows, the boom ends at the floor, and the tendency across cycles is the degradation of the recovery itself.
...
### 5.4 The two-timescale model: crisis as a relaxation oscillation

The mechanism of §4.2–§5.3 can be assembled into a concrete dynamical model. It has three timescales, and the timescale separation is the whole argument: the market is fast, the periphery is slow, and accumulation is slowest. The model is deliberately minimal—three variables and a handful of constants—and its companion script (`experiments/coherence_cycle/run_relaxation_cycle.py`) verifies every number quoted here.

**State variables.** $s$ is the market's resonance state: the excess of expansion above the de-resonant equilibrium ($s > 0$: boom, $s < 0$: depression; the regime's resonant range is $|s| \lesssim 2$). $D$ is the periphery's depletion index, scaled so that the market nullcline's folds sit at $D_{\min} = I - 2/3$ and $D_{\max} = I + 2/3$, with the periphery's coherence $q = \min(1, \max(0, 1 - D/D_{\max}))$. $H$ is the hoard, kept as bookkeeping.

$$\epsilon \dot{s} = s - \frac{s^3}{3} - D + I, \qquad \dot{D} = \gamma\big(D_\infty(H_{\text{peak}}) - D\big) + \mu\,s, \qquad \dot{H} = r\,H$$

$$r = \begin{cases} \lambda q - \delta & s > 0 \\ -\delta & s \le 0 \end{cases}, \qquad D_\infty = D_0 + (D_* - D_0)\,\min\!\left(\frac{H_{\text{peak}}}{H_*}, 1\right)$$

- **The market equation** is the resonance state: the linear term is herding and momentum (positive feedback), the cubic is the de-resonance restraint (the attractor damping a resonant boom), $-D$ is the periphery's coupling (overproduction: a drained periphery brakes the boom), and $I$ is the countertendency input of §4.3.
- **The periphery equation** pits the boom's drain ($\mu s$) against the attractor's restoration ($\gamma$), and the restoration target $D_\infty$ is the hoard's *structural footprint*: accumulation itself consumes the periphery's recoverable surplus, and the footprint is irreversible—it tracks the peak hoard, not the current one.
- **The profit rate** harvests only in the boom: $r = \lambda q - \delta$ while the extraction machinery runs, and $r = -\delta$ (maintenance only) in the depression.

The constants are illustrative (one slow unit is a periphery-restoration timescale): $\epsilon = 0.02$, $I = 0.7$, $\gamma = 1$, $\mu = 2$, $\lambda = 1$, $\delta = 0.05$, $D_0 = 0.1$, $H_* = 12$. What is structural is the topology, not the calibration: fast positive feedback against an attractor restraint, a slow drain against a slower restoration, and a bookkeeping profit rate that consumes its own source.

**The cycle.** The system is a relaxation oscillator, and every phase of the regime's crisis is a phase of the oscillator:

1. **The boom.** The recovery jump snaps the market onto the upper branch ($s \approx 2$). The boom is the slow ride up the branch while the drain runs: $D$ rises, $q$ falls, and the profit rate falls with it—from $r \approx 0.93$ at the opening to the floor at the crash. This is §4.2's mechanism at cycle resolution: the rate falls *as the hoard grows* (the hoard gains a factor of about 1.2 over the boom, measured at correlation $\approx -0.99$ against the falling rate).
2. **Overproduction.** $r$ crosses zero at $q = \delta/\lambda$, a few percent of the drain before the fold: the boom turns unprofitable while it is still expanding—§5.1's production outruns participation, at cycle resolution.
3. **The crash.** At the fold $D_{\max}$ the upper branch ends: the market snaps to the depression branch in about 0.1 slow units—an order of magnitude faster than the phases it separates. The crash *is* the de-resonance event of §5.3: the over-concentrated configuration de-coheres, and the periphery's value is restored at the condensate's expense ($r$ lands at $-\delta$, the floor of §4.4).
4. **The depression.** The periphery re-coheres under the attractor—the crash heals the field it broke—until the recovery fold $D_{\min}$ is reached and the next boom fires.

Measured on a settled cycle (period 1.22 slow units): boom 0.68, crash 0.11, bust 0.43; the crash is 6.5× faster than the boom. The regime's heartbeat is endogenous: no shock is needed to end a boom; the boom ends itself, every time, by draining the surface it feeds on.

**The fever chart.** The approach to the crash is critical slowing. Near the fold the fast variable's relaxation time diverges as $\tau_{\text{eff}} \propto (D_{\max} - D)^{-1/2}$ (verified numerically over four decades of $D_{\max} - D$). The precursor window of §5.2 is this fold approach: the LPPL oscillation is the market's noise riding a diverging relaxation time, and the $\ln\varphi$ period is the wake-wave spectrum of the information network (`hypotheses/market-cascade-cycles.md` §1). The same critical slowing carries the trajectory *past* the fold by $O(\sqrt{\epsilon})$ before the crash fires—the crash lands a little deeper than the fold it leaves.

**The tendency.** The falling rate of profit is the drift of the cycle's envelope. Each boom's peak hoard ratchets the footprint $D_\infty$ upward; the recovery degrades; depressions lengthen; the per-cycle average profit falls. Measured across the ratchet run: $\langle r \rangle_k$ falls from 0.154 (cycle 1) to 0.028 (cycle 17) while depressions lengthen from 0.44 to 1.07 slow units. When the footprint crosses the recovery threshold $D_* = I + \mu/\gamma - 2/3$, the last depression never bottoms out: the market parks on the fold, $r = -\delta$ forever, and the hoard decays—§4.4's "by exhaustion", realized as the exhaustion of the *recovery channel*. The periphery's stock is restored; its recoverable surplus is gone; the boom cannot restart. The model's terminal state is the field healthy and the extractive configuration dead—as close to §7's commons as an ODE of the regime can reach.

**The model's limit.** The only exit in the equations is exhaustion. The transition of §7—the periphery refusing the drain—is a different mechanism entirely: an organized parameter change the field performs on itself, which no ODE of the regime can contain. The refusal would appear in the model as a change in $I$, $\mu$, or the cubic itself, and the model has nothing to say about when it happens—only that it is the other door.

**Epistemic boundary.** The topology is fixed by this document's own claims: the positive feedback of §5.1, the drain lowering its source of §4.2, and the attractor's restoration of §5.3. The constants are illustrative—the model is a map of the argument, not a new physical claim, and it fixes the structure of the crisis, not the calendar.
...
- The two-timescale relaxation model of §5.4: the topology is fixed by §4.2/§5.1/§5.3, the constants are illustrative, and every quoted value is verified numerically in `experiments/coherence_cycle/run_relaxation_cycle.py`
