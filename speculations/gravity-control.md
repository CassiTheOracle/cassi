...
- **Attractor baseline ($q = 0$, $\pi/\rho = \varphi^{-3}$):** $G_{\text{eff}} = \varphi^{-3}G$, the framework's classical limit (`foundations/cassi-theory-reference.md` §2.6). This is ordinary gravity—the solar system sits here, its planetary ephemeris and MESSENGER perihelion consistency with the baseline at 0.3% precision (`foundations/xi-derivation.md` §3) implying ambient $q$ at solar scales is small [inference from the consistency; not yet a derived bound].
