...
| $\chi_{\mathrm{circ}}$ (twist-chi scratch coupling) | Hypothesized—free coupling under test, not fitted | No fit performed: t = 4 ramp $\chi \in \{0, \pm0.25, \pm0.5, \pm1, \pm2\}$ on the TS6 helix, 2026-08-07 (`two-fluid/run_twist_chi_ramp.py`): max $\|\Delta\mathrm{Tw}\| = 3.3\times10^{-4}$ (generation null); fitted response $\Delta\mathrm{Tw} = -1.16\times10^{-4}\,\chi + 1.6\times10^{-5}\,\chi^2$; $J_{\mathrm{scale}} = \max\|g\|$ at t = 0 (6.756e-2 at N = 48, identical on all three probe arms; g = the solver-frame curl component, see `hypotheses/two-strand-five-channel-matter-organization.md` §3.2) | — (no data anchor; $J_{\mathrm{scale}}$ is an initialization convention, not a fit) | run_twist_chi_ramp.py; runs/20260806_235509_chi_ramp/ | Hypothesized |
