...
| $\Lambda$ (cosmological constant) | $\lambda\varphi^{-2}/3$—**Asserted** (postulate): the 1/3 is the 3D continuity reading; $T_{00}$ at equilibrium gives 0 or $(g/4)\varphi^2$, never $\lambda\varphi^{-2}/3$; Lagrangian derivation open; the ratified conversion→expansion term's vacuum half is this same $\lambda\varphi^{-2}/d$ (08 §A.2); identified with the frozen coherent-phase energy (16-qi-field.md): per-cell constant under the friction closure ⟹ w ≡ −1 exactly; the coherent phase carries 78% of the expansion rate | Dark energy |
...
| $r = \varphi^{-12} \approx 0.003$ | Catalog "✅ Within bound" | Exponent matched post-hoc: all three of the doc's own formulas fail ($\varphi^{-6} \neq 0.003$; $12/40^2 = 0.0075$; $(16/\pi)\xi q/\varphi^{40} = 2\times10^{-7}$); trajectory test 2026-08-06 (`computations/slow_roll_trajectory.py`) confirms the Mapped flag: the trajectory's $r$ is excluded by the BK18 bound, and $r = \varphi^{-12}$ occurs only at ~135 physical e-folds before the window end (where $n_s = 0.9883$, +5.6σ) | Planck+BICEP bound $r < 0.03$ | 03-cosmo.md:59-62, 163 | Mapped |
...
| $\sigma_8$: $\mu(k,a)$ normalization | "Slightly lower ~5%" claim; pipeline gives −43% | Plan target row labeled "target, matching observations" ($\mu = 0.980 \to -5.3\%$); free $\mu$ normalization and $q(k,a)$ machinery; reconciliation 2026-08-06 (`computations/sigma8_reconciliation.py`): the −43.5% pipeline headline is dominated by normalization + resolution (P(k) normalization 8e-5, nonlinear ICs, N=32 dissipation: δ_rms falls 32% while ΛCDM linear grows +21%); only −9.6% is mechanism-attributable ($G_{\text{eff}} = 0.9044$); "~5%" is a Mapped target | Low-z weak lensing $\sigma_8$ | 03-cosmo.md:139 | Mapped |
...
| $\chi_{\mathrm{circ}}$ (twist-chi scratch coupling) | Hypothesized—free coupling under test, not fitted | No fit performed: t = 4 ramp $\chi \in \{0, \pm0.25, \pm0.5, \pm1, \pm2\}$ on the TS6 helix, 2026-08-07 (`two-fluid/run_twist_chi_ramp.py`): max $\|\Delta\mathrm{Tw}\| = 3.3\times10^{-4}$ (generation null); fitted response $\Delta\mathrm{Tw} = -1.16\times10^{-4}\,\chi + 1.6\times10^{-5}\,\chi^2$; t = 40 lock legs ($\Omega_0 \in \{2\pi/N, 4\pi/N\}$): $\Delta\mathrm{Tw} = -3.9\times10^{-4}$ / $+5.1\times10^{-4}$ at $\chi=1$, no lock; $J_{\mathrm{scale}} = \max\|g\|$ at t = 0 (6.756e-2 at $2\pi/N$, 1.351e-1 at $4\pi/N$; g = the solver-frame curl component, see `hypotheses/two-strand-five-channel-matter-organization.md` §3.2) | — (no data anchor; $J_{\mathrm{scale}}$ is an initialization convention, not a fit) | run_twist_chi_ramp.py; runs/20260806_235509_chi_ramp/, runs/20260807_000825_chi_ramp/ | Hypothesized |
