...
| $\sigma$ | regularization scale | $\ell_{\text{Pl}}/\varphi^3$ | gravity cores, UV-finite propagator; **Derived conditional**—the exact arithmetic rung identity uses the external Planck-length anchor, with exponent $\delta=3$ selected by the noise-signal criterion under the isotropic $d=3$ choice; registry G1; no ledger row (not Calibrated/Mapped) |
