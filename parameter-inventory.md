...
|---|-----------|-------|------|--------|
| 1 | $\lambda$ (conversion) | $0.1 = 1/(2w)$ | $\varphi$-attractor timescale | Derived conditional on the doublet conversion budget + one event per cycle—$\lambda = (1/2)(1/w)$: doublet factor $\times$ per-cycle event share (`foundations/wu-xing-derivation.md` §7), $w=5$ derived (single input: coherence postulate) |
...
| $\Lambda$ (cosmological constant) | $\lambda\varphi^{-2}/3$—the 1/3 is **Derived** as the isotropic dimension factor $1/d$ at $d = 3$ ($\nabla\cdot u = dH$ steady state; `cosmology/cosmology-from-phi.md` §1, `computations/verify_h_form_one_third.py`); the $\lambda\varphi^{-2}$ rate stays **Asserted**; $T_{00}$ at equilibrium gives 0 or $(g/4)\varphi^2$, never $\lambda\varphi^{-2}/3$; the ratified conversion→expansion term's vacuum half is this same $\lambda\varphi^{-2}/d$ (08 §A.2); identified with the frozen coherent-phase energy (16-qi-field.md): per-cell constant under the friction closure ⟹ w ≡ −1 exactly; the coherent phase carries 78% of the expansion rate | Dark energy |
...
| $\alpha_0$ | equilibrium Yang fraction ($\pi/\rho$ at the $\varphi$-fixed point) | $\varphi^{-3} \approx 0.236$ | the fixed-point imbalance, not $G_{\text{eff}}/G$: at the fixed point $G_{\text{eff}}/G = \alpha_0(1+(\varphi^{6}-1)q_{\text{eq}}) \approx 3.73$ ($q_{\text{eq}} = \varphi^2/(\varphi^2+\varphi^{-2}) \approx 0.873$ at the reference density); relabel—the true fixed-point Yang fraction is $\varphi^{-1}$ (ledger §10) |
...
| $q$ | Qi coherence (flow magnitude diagnostic) | $\rho^2/(\rho^2 + \varphi^{-2} + \varepsilon^2)$ | **Derived**—defined from the densities; $q_{\text{eq}} \approx 0.873$ at the reference state ($\varepsilon = 0$); the flow's organization at $\varphi$ (`foundations/cassi-first-principles.md` §2.1; `foundations/qi-flow-double-helix.md` §6) |
| $\theta$ | density-plane angle | $\mathrm{atan2}(E_I, E_Y)$ | **Derived**—twice the amplitude-plane doublet phase; one cascade rung advances $\theta$ by $2\pi$; rate law measured by `two-fluid/run_winding_rate_probe.py` (`foundations/cassi-first-principles.md` §2.6) |
| $J$ | phase current | $\Psi_0\nabla\Psi_1 - \Psi_1\nabla\Psi_0$ | **Derived**—the doublet's phase current, the third fundamental of the framework; flow of coherence between Yang and Yin (`foundations/cassi-first-principles.md` §1, §2.2) |
| $J_z$ | axial inter-scale current | $R^2\,\partial_z\theta$ | **Derived**—phase current along the cascade axis; carries coherence between scales; measured order parameter, not a fit (`foundations/qi-flow-double-helix.md` §2, §6) |
| $\Delta\vartheta$ | relaxation winding | $\mathrm{atan}(1/\varphi) - \mathrm{atan}((\rho-\varepsilon_0)/(\rho\varphi+\varepsilon_0))$ | **Derived**—winding accumulated while a state relaxes from excess $\varepsilon_0$; $|\Delta\vartheta| \le \mathrm{atan}(\varphi) \approx 1.017$ rad (`foundations/cassi-first-principles.md` §2.6; `foundations/rung-offset-mechanism.md` §7) |
| $\delta n$ | fractional rung offset | $\Delta\vartheta/(2\pi)$ | **Derived**—relaxation winding in rung units; $|\delta n| \le \mathrm{atan}(\varphi)/(2\pi) \approx 0.162$ rungs; half-steps are parity, not relaxation (`foundations/rung-offset-mechanism.md` §7) |
...
| Proton-lifetime exponent ($n = 91.5$, $N = 4506$; boxed $\tau_p \approx 4\times10^{34}$ yr) | "Derived (from the coherence budget); not testable" (registry Q9) | Rung fixed to the repo's own mass ladder ($n = 91.46$, 2026-08-03 arithmetic sweep); per-rung survival $q_i = 1 - \varphi^{-i-\delta}$ is Hypothesized ("alternative scalings possible"); arithmetic closed 2026-08-05 (`computations/proton_budget_closure.py`): the boxed GUT formula fails its own inputs by 323×—a 4.2× slip in $M^4/m^5$ (boxed value would need $M_{\text{GUT}} \approx 4.7\times10^{15}$ GeV), corrected $\tau_p = 1.29\times10^{37}$ yr, 2.1 orders above Hyper-K reach; the coherence-budget chain ($N_{\text{max}} = \varphi^{4505.79} \to \tau_p \approx 10^{910}$ yr) is separately self-consistent | Proton mass; Super-K bound | 02-sm.md:101-106; 01-core.md:120-122 | Mapped |
