...
Measured over 300 entries of a real session's compaction segment (260,322 raw tokens): 37.4 s of observation against 571 s before, 125 ms per entry, with unchanged selection — 6 items and 16,246 tokens at the 16,384-token cap, 12 items and 49,094 tokens at 49,152, and the earliest revision still recallable exactly (23 items, 49,074 tokens). A synthetic 200-observation sequence holds per-observation latency between 36 ms and 70 ms and produces byte-identical field state, checkpoint manifest, and control ledger to the earlier runtime. The receipts are `probes/receipts/real-session-projection.json` and `probes/receipts/real-session-projection-prior-runtime.json`.
