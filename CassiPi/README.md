...
On a real 300-entry compaction segment (260,322 raw tokens) the observation phase fell from 571 s to 24–26 s across repeated runs, which is 80–86 ms per entry, with identical field state, checkpoint manifest, and control ledger bytes. Two whole-store walks per observation went with it: a capacity check reads a total maintained as evidence files are written and removed, and a head that names the checkpoint the owner already holds is not decoded a second time. What remains per observation is the commit's own state encoding and its durability writes. Receipts: `probes/receipts/real-session-projection.json` and `probes/receipts/real-session-projection-prior-runtime.json`.
