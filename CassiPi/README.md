...
On a real 300-entry compaction segment (260,322 raw tokens) the observation phase fell from 571 s to 37 s, and repeated runs of the same segment now record 37–59 s, which is 122–197 ms per entry. The remaining cost is the one full atlas decode for the new head, the commit's own state encoding, and its durability writes; it still grows with stored evidence. Field state, checkpoint manifest, and control ledger bytes are identical across those runs. Receipts: `probes/receipts/real-session-projection.json` and `probes/receipts/real-session-projection-prior-runtime.json`.
