...
where φ = (1 + √5) / 2 ≈ 1.618 is the golden ratio, and q ∈ [0, 1] is
the Qi coherence factor:

    q = 1 / (1 + (ρ / ρ_ref)²)

This is the form implemented in two-fluid/cassi_two_fluid_3d_gpu.py
(line 534, ExpandingTwoFluid3DGPU.rhs) and used consistently across
the cosmology codebase (run_galactic_rotation.py, run_baryonic_tully_fisher.py,
run_cluster_mass_profiles.py, run_dwarf_galaxy_scaling.py, etc.):

    q → 0  when  ρ >> ρ_ref   (high density → G_eff ≈ G_N, screened)
    q → 1  when  ρ << ρ_ref   (low density → G_eff ≈ φ·G_N, enhanced)
    q = 0.5 at ρ = ρ_ref       (G_eff/G_N = 1 + (φ-1)/2 ≈ 1.309)

Maximum enhancement: G_eff/G_N → φ ≈ 1.618 in the zero-density limit.
...
def geff_q_from_density(rho, rho_ref):
    """G_eff/G_N and Qi coherence q from local density.

    Exact form from two-fluid/cassi_two_fluid_3d_gpu.py line 534:
        q = 1 / (1 + (ρ / ρ_ref)²)
...
def make_figure(r_grid, results_newt, results_soft, results_best,
    ...
    ax.text(0.03, 0.03,
            rf'$\varphi = {PHI:.6f}$' + '\n'
            rf'$G_{{\mathrm{{eff}}}} = G_N\,[1 + (\varphi-1)\,q]$' + '\n'
    ...
    line_g, = ax2.plot(r_grid, results_best['g_ratio'], 'm-', lw=2.2,
                       label='$G_{\\mathrm{eff}}/G_N$')
    ax2.axhline(1.0, color='purple', ls=':', lw=1, alpha=0.5)
    ax2.axhline(PHI, color='red', ls=':', lw=1, alpha=0.5,
                label=f'$\\varphi = {PHI:.3f}$')
    ax2.set_ylabel('$G_{\\mathrm{eff}}/G_N$', fontsize=13, color='purple')
    ax2.tick_params(axis='y', labelcolor='purple')
    ax2.set_ylim(0.98, PHI * 1.05)
    ...
    fig.suptitle('Path 8: $\\varphi$-Enhanced Gravity and Galactic Rotation Curves\n'
                 r'$G_{\mathrm{eff}}(q)/G_N = 1 + (\varphi-1)\cdot q$,  '
                 r'$q = [1 + (\rho/\rho_{\mathrm{ref}})^2]^{-1}$',
                 fontsize=14, y=1.04)
...
def main():
    ...
    print(f"  φ = {PHI:.10f}")
    print(f"  Max G_eff enhancement: φ·G_N = {PHI:.4f}·G_N")
    print()
    ...
    print(f"  G_eff/G_N at 30 kpc = {results_best['g_ratio'][i30]:.4f}")
    print(f"  G_eff/G_N at ∞     → {PHI:.4f} (asymptotic)")
  ...
  1. Does the φ-enhanced model produce flat rotation curves with a
     ...
     NO—not for the standard Milky Way mass budget.

     At best-fit ρ_ref = {rho_ref_best:.2e} M_sun/kpc³, the rotation curve
     still declines from {results_best['v_phi'][i5]:.0f} km/s at 5 kpc to
     {v_enhanced_30:.0f} km/s at 30 kpc—a flattening ratio v(30)/v(5) =
     {flat_ratio:.3f}.  A truly flat curve requires v(30)/v(5) ≈ 1.

     The χ² (fit to 200 km/s flat over 5-30 kpc) improves from {chi2_newt:.0f}
     (Newtonian) to {chi2_best:.0f} (φ-enhanced), but this is still very high
    —a good fit would be χ² ≈ N_data ≈ 25.

     WHY IT'S STILL INSUFFICIENT:
     The maximum boost is sqrt(φ) ≈ {np.sqrt(PHI):.2f}× in v_circ.  At 30 kpc,
     v_newt ≈ {v_newt_30:.0f} km/s, so even with FULL enhancement (q=1,
     G_eff → φ·G_N), v_max = {v_newt_30:.0f} × {np.sqrt(PHI):.2f} ≈
     {np.floor(v_newt_30 * np.sqrt(PHI)):.0f} km/s.  This is set by the
     baryonic mass—even φ×G_N cannot sustain 200 km/s with only
     {M_DISK + M_BULGE:.0e} M_sun at 30 kpc.

     IMPROVEMENT OVER NEWTONIAN: The φ-enhancement DOES make the curve
     flatter—the RMS deviation drops from {rms_newt:.0f} to {rms_best:.0f}
     km/s.  The dark matter mass required would be reduced by a factor of
     about (v_obs² - v_phi²) / (v_obs² - v_newt²) ≈
     {(V_FLAT_OBS**2 - v_enhanced_30**2) / max(V_FLAT_OBS**2 - v_newt_30**2, 1):.0%}
     at 30 kpc.  But it does not eliminate the need for DM.
""")
...
""")
  ...
  3. Does this approach work for the Milky Way? For other galaxies?
     ...
     At 8 kpc, ρ/ρ_ref ≈ {results_best['rho_midplane'][np.argmin(np.abs(r_grid-8))]/rho_ref_best:.3f},
     giving q ≈ {results_best['q'][np.argmin(np.abs(r_grid-8))]:.3f}—the enhancement
     is {'fully active' if results_best['q'][np.argmin(np.abs(r_grid-8))] > 0.8 else 'partial'} at the Solar circle.

     However, the curve{' DOES' if milky_way_ok else ' does NOT'} match
     the 220 km/s at 8 kpc, and more importantly the curve still{' flattens' if flat_ratio > 0.8 else ' declines'}
     from 8 to 30 kpc (v(30)/v(8) = {v_enhanced_30 / v_phi_8:.3f}).

     The key limitation: sqrt(φ) ≈ {np.sqrt(PHI):.2f}× maximum boost is
     fundamentally bounded by φ.  For the Milky Way's baryonic mass, even
     the fully enhanced v(30) = {v_enhanced_30:.0f} km/s falls well short
     of 200 km/s.  The mass deficit at 30 kpc is a factor of
     (200/{v_enhanced_30:.0f})² ≈ {(200/v_enhanced_30)**2:.1f}×.
     ...
     observed.  This is because at the characteristic radius R_ch:
         v_flat² ≈ φ·G_N·M_baryon / R_ch
         and R_ch ∝ R_d ∝ M_baryon^{{1/2}} (disk scaling relation),
         so v_flat⁴ ∝ G_N·φ·M_baryon.
     ...
     CONCLUSION: φ-enhanced gravity reduces the dark matter requirement
     by ≈ {(V_FLAT_OBS**2 - v_enhanced_30**2) / max(V_FLAT_OBS**2 - v_newt_30**2, 1):.0%}
     at 30 kpc, but cannot eliminate it entirely.  The Cassi framework
     needs either more baryonic mass, or an additional mechanism (e.g.,
     the two-fluid's π∇Φ buoyancy force) to fully replace DM.
""")
  ...
  4. Enhancement ratio v_enhanced(30 kpc) / v_newton(30 kpc):
     ...
     This equals sqrt(G_eff(30)/G_N) = sqrt({results_best['g_ratio'][i30]:.4f})
     = {np.sqrt(results_best['g_ratio'][i30]):.4f}

     The maximum possible enhancement is sqrt(φ) = {np.sqrt(PHI):.4f}
     (when q → 1, G_eff → φ·G_N).

     Interpretation: The φ-enhancement boosts v_circ by at most
     {np.sqrt(PHI):.2f}×.  For the Milky Way's baryonic mass, this
     gives v_max(30) = {np.floor(v_newt_30 * np.sqrt(PHI)):.0f} km/s,
     which is {'still well below' if v_newt_30 * np.sqrt(PHI) < 180 else 'closer to'}
     200 km/s but not sufficient.

     Achieving 200 km/s would require either:
     (a) ~{(200/v_newt_30)**2:.0f}× more enclosed baryonic mass at 30 kpc
         (i.e., M_baryon ≈ {M_DISK + M_BULGE:.0e} → {((200/v_newt_30)**2 * (M_DISK + M_BULGE)):.0e} M_sun), or
     (b) A stronger enhancement (G_eff/G_N ≈ {(200/v_newt_30)**2:.1f} at 30 kpc,
         requiring φ ≈ {(200/v_newt_30)**2:.2f}—not the golden ratio), or
     (c) Additional physics from the two-fluid (e.g., the π∇φ buoyancy force
         in the momentum equation acts as an extra effective gravity).
""")
