...
#### 24. The IBEX Ribbon
The origin of the enhanced energetic-neutral-atom ribbon at the heliopause is open. The standard status: secondary ENA production at the boundary is favored, with details unresolved. **[No framework claim]**—the IBEX ribbon is not addressed. A Speculative candidate is only a conditional comparison: if an independently identified heliopause boundary and direction are supplied, its structure could be compared with the directional proxy $R(\theta)=\frac{\sqrt{1+\varphi^2}}{2}\sqrt{\frac{1+\theta}{\theta}}$, equal to $1.7072\times$ only at the selected $\theta_{\mathrm{cond}}=0.45$ and variable with $\theta$. No $C=0.45$ edge survives the fixed-step PDE endpoint and the cosmological boundary receipt is null, so this comparison provides no observational support and is not a universal, zero-parameter, canonical, or PDE output. `speculations/cascade-infrastructure.md` §3.2.
