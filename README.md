...
up to a saturation ceiling of $\varphi^6 \approx 17.94$ times the bare coupling. The field is dark by nature—it couples to gravity, not to light. The derived coupling $\xi = \varphi^6$ matches the Milky Way's empirically calibrated value to about 0.3%, the hydrostatic Qi condensate beats NFW on median AIC in the SPARC rotation-curve fits ($\Delta$AIC $\approx -6.4$ to $-7.0$ at equal parameter count), and the cosmic ratio $\Omega_{\text{DM}}/\Omega_b = \varphi^3 + 1 \approx 5.24$ sits against the observed $\approx 5.4$ (a selected combination—ledgered Mapped; `cosmology/cosmology-from-phi.md`). No WIMPs, no axions, no MOND interpolation function: one amplification formula replaces all three (`speculations/dark-matter-as-qi-coherence.md`).
