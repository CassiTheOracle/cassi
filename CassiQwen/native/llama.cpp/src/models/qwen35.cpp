...
llama_model_qwen35::graph::graph(const llama_model & model, const llm_graph_params & params) :
    ...
    if (params.cassi_field != nullptr && params.cassi_field->enabled) {
        const uint32_t mode_count = params.cassi_field->mode_count;
        const uint32_t n_tokens = ubatch.n_tokens;
        const uint32_t n_seqs = std::max(1u, ubatch.n_seqs_unq);
        const uint32_t field_layer = params.cassi_field->layer_index;
        GGML_ASSERT(field_layer < (uint32_t) n_layer);
        GGML_ASSERT(res->t_layer_inp[field_layer] != nullptr);

        auto inp_field = std::make_unique<llm_graph_input_cassi_modal>(
            static_cast<const llm_cassi_modal_config *>(params.cassi_field), n_seqs);
        inp_field->state = ggml_new_tensor_4d(ctx0, GGML_TYPE_F32,
                params.cassi_field->state_stride, n_seqs, 1, 1);
        inp_field->mode_params = ggml_new_tensor_4d(ctx0, GGML_TYPE_F32,
                mode_count, 1, 1, 1);
        inp_field->seq_ids = ggml_new_tensor_1d(ctx0, GGML_TYPE_I32, n_tokens);
        ggml_set_input(inp_field->state);
        ggml_set_input(inp_field->mode_params);
        ggml_set_input(inp_field->seq_ids);

        auto * field_inp = static_cast<llm_graph_input_cassi_modal *>(
            res->add_input(std::move(inp_field)));

        // The selected layer input is the direct [2*M,T] field sense.
        ggml_tensor * sense = ggml_reshape_2d(
            ctx0, res->t_layer_inp[field_layer], hparams.n_embd, n_tokens);
        cb(sense, "cassi_field_sense", field_layer);

        ggml_tensor * t_field = ggml_cassi_field_step(
            ctx0, sense, field_inp->state, field_inp->mode_params, field_inp->seq_ids,
            params.cassi_field->retained_weight,
            params.cassi_field->phi,
            params.cassi_field->dt,
            params.cassi_field->omega2,
            params.cassi_field->coupling,
            params.cassi_field->steps_per_layer);
        cb(t_field, "cassi_field_step", field_layer);
        res->t_cassi_field = t_field;

        ggml_tensor * correction = ggml_view_2d(
            ctx0, t_field, hparams.n_embd, n_tokens,
            (size_t) hparams.n_embd * sizeof(float), 0);
    ...
    if (params.cassi != nullptr && params.cassi->enabled) {
        const uint32_t mode_count = params.cassi->mode_count;
        const uint32_t n_tokens = ubatch.n_tokens;
        const uint32_t n_seqs = std::max(1u, ubatch.n_seqs_unq);

        auto inp_cassi = std::make_unique<llm_graph_input_cassi_modal>(params.cassi, n_seqs);
        inp_cassi->state = ggml_new_tensor_4d(ctx0, GGML_TYPE_F32,
                params.cassi->state_stride, n_seqs, 1, 1);
        inp_cassi->mode_params = ggml_new_tensor_4d(ctx0, GGML_TYPE_F32,
                mode_count, 1, 1, 1);
        inp_cassi->seq_ids = ggml_new_tensor_1d(ctx0, GGML_TYPE_I32, n_tokens);
        ggml_set_input(inp_cassi->state);
        ggml_set_input(inp_cassi->mode_params);
        ggml_set_input(inp_cassi->seq_ids);

        auto * cassi_inp = static_cast<llm_graph_input_cassi_modal *>(res->add_input(std::move(inp_cassi)));

        // Layer inputs are direct signed hidden pairs.  Reshape each
        // [n_embd, T] tensor to [2*M, 1, T], then concatenate layer-major.
        ggml_tensor * layer_modes = nullptr;
        for (int il = 0; il < n_layer; ++il) {
            ggml_tensor * layer = ggml_reshape_3d(ctx0, res->t_layer_inp[il], n_embd, 1, n_tokens);
            layer_modes = layer_modes == nullptr ? layer : ggml_concat(ctx0, layer_modes, layer, 1);
        }
        cb(layer_modes, "cassi_layer_modes", -1);

        ggml_tensor * t_cassi = ggml_cassi_modal(
                ctx0, layer_modes, cassi_inp->state, cassi_inp->mode_params, cassi_inp->seq_ids,
                n_layer,
                params.cassi->retained_weight,
                params.cassi->phi,
                params.cassi->dt,
                params.cassi->omega2,
                params.cassi->coupling,
                params.cassi->steps_per_layer);
        cb(t_cassi, "cassi_modal", -1);
        res->t_cassi = t_cassi;

        // The operation's first [2*M,T] region is laid out exactly like the
        // hidden matrix, while its tail holds one final state per sequence.
        ggml_tensor * correction = ggml_view_2d(
                ctx0, t_cassi, n_embd, n_tokens, (size_t) n_embd * sizeof(float), 0);
