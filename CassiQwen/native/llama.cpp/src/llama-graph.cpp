...
llm_graph_input_cassi_modal::llm_graph_input_cassi_modal(
    ...
    config(config),
    n_seqs(n_seqs),
    host_state((size_t) n_seqs * (config ? config->state_stride : 0), 0.0f),
    host_mode_params(config ? config->mode_count : 0, 0.0f),
    host_seq_ids() {
}

void llm_graph_input_cassi_modal::set_input(const llama_ubatch * ubatch) {
    GGML_ASSERT(config != nullptr && config->enabled);
    GGML_ASSERT(state != nullptr && mode_params != nullptr && seq_ids != nullptr);
    GGML_ASSERT(ggml_element_size(state) == sizeof(float));
    GGML_ASSERT(ggml_element_size(mode_params) == sizeof(float));
    GGML_ASSERT(ggml_element_size(seq_ids) == sizeof(int32_t));
    ...
    host_seq_ids.assign(n_tokens, 0);

    if (ubatch != nullptr && ubatch->seq_id_unq != nullptr && config->state != nullptr) {
        const uint32_t n = std::min(n_seqs, ubatch->n_seqs_unq);
        for (uint32_t s = 0; s < n; ++s) {
            const llama_seq_id seq_id = ubatch->seq_id_unq[s];
            if (seq_id < 0 || (uint32_t) seq_id >= config->n_seq_max) {
                continue; // fail closed: unknown sequence state stays zero
            }

            std::memcpy(
                host_state.data() + (size_t) s * config->state_stride,
                config->state + (size_t) seq_id * config->state_stride,
                (size_t) config->state_stride * sizeof(float));
        }
    }

    for (uint32_t t = 0; t < n_tokens; ++t) {
        int32_t slot = -1;
        if (ubatch->seq_id != nullptr && ubatch->n_seq_id != nullptr &&
                ubatch->seq_id[t] != nullptr && ubatch->n_seq_id[t] > 0 &&
                ubatch->seq_idx != nullptr) {
            const llama_seq_id seq_id = ubatch->seq_id[t][0];
            if (seq_id >= 0 && seq_id < LLAMA_MAX_SEQ) {
                slot = ubatch->seq_idx[seq_id];
            }
        }

        if (slot < 0 && n_seqs > 0) {
            if (ubatch->n_seq_tokens > 0) {
                slot = (int32_t) std::min<uint32_t>(t / ubatch->n_seq_tokens, n_seqs - 1);
            } else {
                slot = (int32_t) std::min<uint32_t>(t, n_seqs - 1);
            }
        }
        host_seq_ids[t] = slot >= 0 && (uint32_t) slot < n_seqs ? slot : 0;
    }

    ggml_backend_tensor_set(state, host_state.data(), 0, host_state.size() * sizeof(float));
    ggml_backend_tensor_set(mode_params, host_mode_params.data(), 0, host_mode_params.size() * sizeof(float));
    if (!host_seq_ids.empty()) {
        ggml_backend_tensor_set(seq_ids, host_seq_ids.data(), 0, host_seq_ids.size() * sizeof(int32_t));
    }
}

bool llm_graph_input_cassi_modal::can_reuse(const llm_graph_params & params) {
    const bool matches_field = params.cassi_field != nullptr &&
        static_cast<const llm_cassi_modal_config *>(params.cassi_field) == config;
    const bool matches_qi = params.cassi_qi != nullptr &&
        static_cast<const llm_cassi_modal_config *>(params.cassi_qi) == config;
    const bool matches_modal = params.cassi == config;
    if ((!matches_modal && !matches_field && !matches_qi) || config == nullptr || !config->enabled) {
        return false;
    }

    return state != nullptr && mode_params != nullptr && seq_ids != nullptr &&
        state->ne[0] == (int64_t) config->state_stride &&
        state->ne[1] == (int64_t) n_seqs &&
        state->ne[2] == 1 && state->ne[3] == 1 &&
        mode_params->ne[0] == (int64_t) config->mode_count &&
        mode_params->ne[1] == 1 && mode_params->ne[2] == 1 && mode_params->ne[3] == 1 &&
        seq_ids->ne[0] == (int64_t) params.ubatch.n_tokens &&
        seq_ids->ne[1] == 1 && seq_ids->ne[2] == 1 && seq_ids->ne[3] == 1 &&
        std::max(1u, params.ubatch.n_seqs_unq) == n_seqs;
}
...
void llm_graph_result::set_outputs(const llm_graph_params & params) {
    ...
    if (t_cassi != nullptr) {
        ggml_set_output(t_cassi);
    }
    if (t_cassi_field != nullptr) {
        ggml_set_output(t_cassi_field);
    }
