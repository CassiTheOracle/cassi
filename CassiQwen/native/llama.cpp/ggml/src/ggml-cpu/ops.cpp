...
// ggml_compute_forward_cassi_modal

static void ggml_compute_forward_cassi_modal_f32(
        const ggml_compute_params * params,
        ggml_tensor * dst) {
    const ggml_tensor * src0 = dst->src[0]; // [2*M, L, T, 1]
    const ggml_tensor * src1 = dst->src[1]; // [8*M, S, 1, 1]
    const ggml_tensor * src2 = dst->src[2]; // [M, 1, 1, 1]
    const ggml_tensor * src3 = dst->src[3]; // [T, 1, 1, 1]
    ...
    GGML_ASSERT(src1->ne[0] % 8 == 0);
    const int64_t mode_count = src0->ne[0] / 2;
    const int64_t layer_count = ggml_get_op_params_i32(dst, 0);
    const int64_t steps_per_layer = ggml_get_op_params_i32(dst, 6);
    const int64_t token_count = src0->ne[2];
    const int64_t sequence_count = src1->ne[1];

    GGML_ASSERT(mode_count > 0);
    GGML_ASSERT(layer_count >= 1);
    GGML_ASSERT(steps_per_layer >= 1);
    GGML_ASSERT(src0->ne[1] == layer_count);
    GGML_ASSERT(src0->ne[3] == 1);
    GGML_ASSERT(src1->ne[0] / 8 == mode_count);
    GGML_ASSERT(src1->ne[2] == 1);
    GGML_ASSERT(src1->ne[3] == 1);
    GGML_ASSERT(src2->ne[0] == mode_count);
    GGML_ASSERT(src2->ne[1] == 1);
    GGML_ASSERT(src2->ne[2] == 1);
    GGML_ASSERT(src2->ne[3] == 1);
    GGML_ASSERT(src3->ne[0] == token_count);
    GGML_ASSERT(src3->ne[1] == 1);
    GGML_ASSERT(src3->ne[2] == 1);
    GGML_ASSERT(src3->ne[3] == 1);

    const int64_t correction_count = 2 * mode_count * token_count;
    const int64_t state_count = 8 * mode_count * sequence_count;
    GGML_ASSERT(dst->ne[0] == correction_count + state_count);
    GGML_ASSERT(dst->ne[1] == 1);
    GGML_ASSERT(dst->ne[2] == 1);
    GGML_ASSERT(dst->ne[3] == 1);

    const float retained_weight = ggml_get_op_params_f32(dst, 1);
    const float phi             = ggml_get_op_params_f32(dst, 2);
    const float dt              = ggml_get_op_params_f32(dst, 3);
    const float omega2          = ggml_get_op_params_f32(dst, 4);
    const float coupling        = ggml_get_op_params_f32(dst, 5);
    const float incoming_weight = 1.0f - retained_weight;

    const float * layer_modes = (const float *) src0->data;
    const float * state_in    = (const float *) src1->data;
    ...
    const int64_t seq_end = MIN(seq_start + dr, sequence_count);
    const int64_t layer_stride = 2 * mode_count;
    const int64_t state_stride = 8 * mode_count;
    const int64_t token_stride = layer_stride * layer_count;
    const int64_t correction_stride = 2 * mode_count;

    for (int64_t sequence = seq_start; sequence < seq_end; ++sequence) {
        float * output_state = output + correction_count + sequence * state_stride;

        for (int64_t mode = 0; mode < mode_count; ++mode) {
            float ey_re = state_in[sequence * state_stride + 8 * mode + 0];
            float ey_im = state_in[sequence * state_stride + 8 * mode + 1];
            float ei_re = state_in[sequence * state_stride + 8 * mode + 2];
            float ei_im = state_in[sequence * state_stride + 8 * mode + 3];
            float vy_re = state_in[sequence * state_stride + 8 * mode + 4];
            float vy_im = state_in[sequence * state_stride + 8 * mode + 5];
            float vi_re = state_in[sequence * state_stride + 8 * mode + 6];
            float vi_im = state_in[sequence * state_stride + 8 * mode + 7];
            const float symbol = mode_params[mode];

            for (int64_t token = 0; token < token_count; ++token) {
                const int32_t seq_id = seq_ids[token];
                GGML_ASSERT(seq_id >= 0 && seq_id < sequence_count);
                if (seq_id != sequence) {
                    continue;
                }

                const float * layer_modes_token = layer_modes + token * token_stride;
                for (int64_t layer = 0; layer < layer_count; ++layer) {
                    const float signal_re = layer_modes_token[layer * layer_stride + 2 * mode + 0];
                    const float signal_im = layer_modes_token[layer * layer_stride + 2 * mode + 1];

                    ey_re = retained_weight * ey_re + incoming_weight * 0.5f * signal_re;
                    ey_im = retained_weight * ey_im + incoming_weight * 0.5f * signal_im;
                    ei_re = retained_weight * ei_re - incoming_weight * 0.5f / phi * signal_re;
                    ei_im = retained_weight * ei_im - incoming_weight * 0.5f / phi * signal_im;

                    for (int64_t step = 0; step < steps_per_layer; ++step) {
                        const float diff_re = ey_re - phi * ei_re;
                        const float diff_im = ey_im - phi * ei_im;
                        const float acc_y_re = (symbol - omega2) * ey_re + omega2 * phi * ei_re;
                        const float acc_y_im = (symbol - omega2) * ey_im + omega2 * phi * ei_im;
                        const float acc_i_re = symbol * ei_re + omega2 * diff_re;
                        const float acc_i_im = symbol * ei_im + omega2 * diff_im;
                ...
                const int64_t correction_offset = token * correction_stride + 2 * mode;
                output[correction_offset + 0] = coupling * (ey_re - phi * ei_re);
                output[correction_offset + 1] = -coupling * (ey_im - phi * ei_im);
            }

            output_state[8 * mode + 0] = ey_re;
            output_state[8 * mode + 1] = ey_im;
            output_state[8 * mode + 2] = ei_re;
            output_state[8 * mode + 3] = ei_im;
            output_state[8 * mode + 4] = vy_re;
            output_state[8 * mode + 5] = vy_im;
            output_state[8 * mode + 6] = vi_re;
            output_state[8 * mode + 7] = vi_im;
        }
    }
}

void ggml_compute_forward_cassi_modal(
        const ggml_compute_params * params,
        ggml_tensor * dst) {
    const ggml_tensor * src0 = dst->src[0];
    const ggml_tensor * src1 = dst->src[1];
    ...
    if (src0->type == GGML_TYPE_F32 &&
        ...
        src3->type == GGML_TYPE_I32 &&
        dst->type == GGML_TYPE_F32) {
        ggml_compute_forward_cassi_modal_f32(params, dst);
    } else {
        GGML_ABORT("fatal error");
    }
}

// ggml_compute_forward_cassi_field_step

static void ggml_compute_forward_cassi_field_step_f32(
        const ggml_compute_params * params,
        ggml_tensor * dst) {
    const ggml_tensor * src0 = dst->src[0]; // [2*M, T, 1, 1]
    const ggml_tensor * src1 = dst->src[1]; // [8*M, B, 1, 1]
    const ggml_tensor * src2 = dst->src[2]; // [M, 1, 1, 1]
    const ggml_tensor * src3 = dst->src[3]; // [T, 1, 1, 1]
    ...
    const float retained_weight = ggml_get_op_params_f32(dst, 0);
    const float phi             = ggml_get_op_params_f32(dst, 1);
    const float dt              = ggml_get_op_params_f32(dst, 2);
    const float omega2          = ggml_get_op_params_f32(dst, 3);
    const float coupling        = ggml_get_op_params_f32(dst, 4);
    const int64_t steps         = ggml_get_op_params_i32(dst, 5);
    const float incoming_weight = 1.0f - retained_weight;

    GGML_ASSERT(retained_weight >= 0.0f && retained_weight <= 1.0f);
    GGML_ASSERT(phi > 0.0f && dt > 0.0f && omega2 > 0.0f && coupling > 0.0f);
    GGML_ASSERT(steps >= 1);
...
void ggml_compute_forward_cassi_field_step(
        const ggml_compute_params * params,
        ggml_tensor * dst) {
    const ggml_tensor * src0 = dst->src[0];
    const ggml_tensor * src1 = dst->src[1];
    ...
    if (src0->type == GGML_TYPE_F32 &&
        ...
        src3->type == GGML_TYPE_I32 &&
        dst->type == GGML_TYPE_F32) {
        ggml_compute_forward_cassi_field_step_f32(params, dst);
    } else {
        GGML_ABORT("fatal error");
    }
}
...
static inline float cassi_qi_clamp(const float value, const float lo, const float hi) {
    return std::min(std::max(value, lo), hi);
}

static inline float cassi_qi_state_value(const float value) {
    return std::isfinite(value) ? cassi_qi_clamp(value, -64.0f, 64.0f) : 0.0f;
}

static inline void cassi_qi_chirp(const uint32_t scale, const uint32_t mode, float & re, float & im) {
    static const uint32_t primes[4] = {4093u, 4099u, 4127u, 4133u};
    ...
    const uint32_t index = (mode * mode + mode * primes[scale & 3u] + 17u * scale) & 15u;
    re = c[index];
    im = s[index];
}

static inline void cassi_qi_metrics(
        const float ey_re, const float ey_im, const float ei_re, const float ei_im,
        const float epsilon2_ema, const float phi, const float energy_floor,
        float & rho, float & q, float & chi, bool & available) {
    const float e_y = ey_re * ey_re + ey_im * ey_im;
    const float e_i = ei_re * ei_re + ei_im * ei_im;
    rho = cassi_qi_clamp(e_y + e_i, 0.0f, 64.0f);
    const float inv_phi2 = 1.0f / (phi * phi);
    const float rho2 = rho * rho;
    const float denominator = rho2 + inv_phi2 + std::max(epsilon2_ema, 0.0f);
    q = denominator > 0.0f ? cassi_qi_clamp(rho2 / denominator, 0.0f, 1.0f) : 0.0f;
    const float q_max = rho2 / (rho2 + inv_phi2);
    available = rho > energy_floor;
    chi = available && q_max > 0.0f ? cassi_qi_clamp(q / q_max, 0.0f, 1.0f) : 0.0f;
}

// ggml_compute_forward_cassi_qi_field_step

static void ggml_compute_forward_cassi_qi_field_step_f32(
        const ggml_compute_params * params,
        ggml_tensor * dst) {
    const ggml_tensor * src0 = dst->src[0]; // [2*M, T, 1, 1]
    const ggml_tensor * src1 = dst->src[1]; // [9*M*S, B, 1, 1]
    const ggml_tensor * src2 = dst->src[2]; // [M, 1, 1, 1]
    const ggml_tensor * src3 = dst->src[3]; // [T, 1, 1, 1]
    GGML_ASSERT(src0->type == GGML_TYPE_F32 && src1->type == GGML_TYPE_F32 &&
                src2->type == GGML_TYPE_F32 && src3->type == GGML_TYPE_I32 &&
                dst->type == GGML_TYPE_F32);
    ...
    const float dt = ggml_get_op_params_f32(dst, 3);
    const float coupling = ggml_get_op_params_f32(dst, 4);
    const float damping_min = ggml_get_op_params_f32(dst, 5);
    const float damping_max = ggml_get_op_params_f32(dst, 6);
    const float epsilon_tau = ggml_get_op_params_f32(dst, 7);
    ...
    const float dt_base = cassi_qi_clamp(dt, 1.0e-6f, 0.25f);
    const float alpha = cassi_qi_clamp(epsilon_tau, 0.0f, 1.0f);
    ...
    for (int64_t sequence = seq_start; sequence < seq_end; ++sequence) {
        ...
        for (int64_t mode = 0; mode < mode_count; ++mode) {
            ...
            for (int64_t token = 0; token < token_count; ++token) {
                ...
                bool avail_local[4] = {};
                for (int64_t scale = 0; scale < scale_count; ++scale) {
                    cassi_qi_metrics(work[scale * 9 + 0], work[scale * 9 + 1],
                                     work[scale * 9 + 2], work[scale * 9 + 3],
                                     std::max(work[scale * 9 + 8], 0.0f), phi,
                                     energy_floor, rho_local[scale], q_local[scale],
                                     chi_local[scale], avail_local[scale]);
                    ...
                    if (scale > 0 && avail_local[scale - 1] && avail_local[scale]) {
                        const float d0_re = work[(scale - 1) * 9 + 0] - phi * work[(scale - 1) * 9 + 2];
                        const float d0_im = work[(scale - 1) * 9 + 1] - phi * work[(scale - 1) * 9 + 3];
                        const float d1_re = work[scale * 9 + 0] - phi * work[scale * 9 + 2];
                        const float d1_im = work[scale * 9 + 1] - phi * work[scale * 9 + 3];
                        const float denom = std::sqrt(std::max(rho_local[scale - 1] * rho_local[scale], 1.0e-12f));
                        phase_sum += cassi_qi_clamp(0.5f + 0.5f * (d0_re * d1_re + d0_im * d1_im) / denom, 0.0f, 1.0f);
                    }
                }
                ...
                for (int64_t step = 0; step < steps; ++step) {
                    ...
                    work[7] = cassi_qi_state_value(work[7] + ai_im * dt_base);
                    work[0] = cassi_qi_state_value(work[0] + work[4] * dt_base);
                    work[1] = cassi_qi_state_value(work[1] + work[5] * dt_base);
                    work[2] = cassi_qi_state_value(work[2] + work[6] * dt_base);
                    work[3] = cassi_qi_state_value(work[3] + work[7] * dt_base);
                }
                const float e_y = work[0] * work[0] + work[1] * work[1];
                const float e_i = work[2] * work[2] + work[3] * work[3];
                const float epsilon = e_y - phi * e_i;
                work[8] = cassi_qi_state_value((1.0f - alpha) * std::max(work[8], 0.0f) +
                    alpha * cassi_qi_clamp(epsilon * epsilon, 0.0f, 64.0f));

                float scale_dt = dt_base;
                for (int64_t scale = 1; scale < scale_count; ++scale) {
                    scale_dt = cassi_qi_clamp(scale_dt / std::max(scale_ratio, 1.0e-6f), 1.0e-6f, 0.25f);
                    float source_rho = 0.0f, source_q = 0.0f, source_chi = 0.0f;
                    bool source_available = false;
                    cassi_qi_metrics(work[(scale - 1) * 9 + 0], work[(scale - 1) * 9 + 1],
                                     work[(scale - 1) * 9 + 2], work[(scale - 1) * 9 + 3],
                                     std::max(work[(scale - 1) * 9 + 8], 0.0f), phi,
                                     energy_floor, source_rho, source_q, source_chi, source_available);
                    float target_rho = 0.0f, target_q = 0.0f, target_chi = 0.0f;
                    bool target_available = false;
                    cassi_qi_metrics(work[scale * 9 + 0], work[scale * 9 + 1],
                                     work[scale * 9 + 2], work[scale * 9 + 3],
                                     std::max(work[scale * 9 + 8], 0.0f), phi,
                                     energy_floor, target_rho, target_q, target_chi, target_available);
                    const float sd_re = work[(scale - 1) * 9 + 0] - phi * work[(scale - 1) * 9 + 2];
                    const float sd_im = work[(scale - 1) * 9 + 1] - phi * work[(scale - 1) * 9 + 3];
                    const float td_re = work[scale * 9 + 0] - phi * work[scale * 9 + 2];
                    const float td_im = work[scale * 9 + 1] - phi * work[scale * 9 + 3];
                    const float j_scale = sd_re * td_im - sd_im * td_re;
                    const float denom = std::sqrt(std::max(source_rho * target_rho, 1.0e-12f));
                    const float phase = cassi_qi_clamp(0.5f + 0.5f * (sd_re * td_re + sd_im * td_im) / denom, 0.0f, 1.0f);
                    float gain = 0.0f;
                    if (source_available) {
                        const float target_open = 1.0f - target_q;
                        if (!target_available) {
                            gain = scale_dt * source_chi;
                        } else if (phase >= 0.5f && j_scale >= 0.0f) {
                            gain = scale_dt * source_chi * phase * cassi_qi_clamp(j_scale / denom, 0.0f, 1.0f) * target_open;
                        }
                    }
                    gain = cassi_qi_clamp(gain, 0.0f, 0.5f);
                    last_consolidation[scale] = gain;
                    for (int component = 0; component < 9; ++component) {
                        work[scale * 9 + component] = cassi_qi_state_value(
                            (1.0f - gain) * work[scale * 9 + component] +
                            gain * work[(scale - 1) * 9 + component]);
                    }
                }
                ...
                for (int64_t final_scale = 0; final_scale < scale_count; ++final_scale) {
                    ...
                    cassi_qi_metrics(work[final_scale * 9 + 0], work[final_scale * 9 + 1],
                                     work[final_scale * 9 + 2], work[final_scale * 9 + 3],
                                     std::max(work[final_scale * 9 + 8], 0.0f), phi,
                                     energy_floor, final_rho, final_q, final_chi, final_avail);
                    if (final_avail) {
            ...
            float rho_final[4] = {};
            bool available_final[4] = {};
            for (int64_t scale = 0; scale < scale_count; ++scale) {
                float q = 0.0f, chi = 0.0f;
                cassi_qi_metrics(work[scale * 9 + 0], work[scale * 9 + 1],
                                 work[scale * 9 + 2], work[scale * 9 + 3],
                                 std::max(work[scale * 9 + 8], 0.0f), phi,
                                 energy_floor, rho_final[scale], q, chi,
                                 available_final[scale]);
            }
            float final_phase_sum = 0.0f;
            int final_phase_count = 0;
            for (int64_t scale = 1; scale < scale_count; ++scale) {
                if (available_final[scale - 1] && available_final[scale]) {
                    const float d0_re = work[(scale - 1) * 9 + 0] - phi * work[(scale - 1) * 9 + 2];
                    const float d0_im = work[(scale - 1) * 9 + 1] - phi * work[(scale - 1) * 9 + 3];
                    const float d1_re = work[scale * 9 + 0] - phi * work[scale * 9 + 2];
                    const float d1_im = work[scale * 9 + 1] - phi * work[scale * 9 + 3];
                    const float denom = std::sqrt(std::max(rho_final[scale - 1] * rho_final[scale], 1.0e-12f));
                    final_phase_sum += cassi_qi_clamp(0.5f + 0.5f * (d0_re * d1_re + d0_im * d1_im) / denom, 0.0f, 1.0f);
                    ++final_phase_count;
                }
            }
            const float final_cross = final_phase_count > 0 ? final_phase_sum / float(final_phase_count) : (available_final[0] ? 1.0f : 0.0f);
            for (int64_t scale = 0; scale < scale_count; ++scale) {
                float rho = 0.0f, q = 0.0f, chi = 0.0f;
                bool available = false;
                cassi_qi_metrics(work[scale * 9 + 0], work[scale * 9 + 1],
                                 work[scale * 9 + 2], work[scale * 9 + 3],
                                 std::max(work[scale * 9 + 8], 0.0f), phi,
                                 energy_floor, rho, q, chi, available);
                const float d_re = work[scale * 9 + 0] - phi * work[scale * 9 + 2];
                const float d_im = work[scale * 9 + 1] - phi * work[scale * 9 + 3];
                const float dd_re = work[scale * 9 + 4] - phi * work[scale * 9 + 6];
                const float dd_im = work[scale * 9 + 5] - phi * work[scale * 9 + 7];
                const float j_temporal = cassi_qi_clamp(d_re * dd_im - d_im * dd_re, -64.0f, 64.0f);
                float j_scale = 0.0f;
                if (scale + 1 < scale_count) {
                    const float nd_re = work[(scale + 1) * 9 + 0] - phi * work[(scale + 1) * 9 + 2];
                    const float nd_im = work[(scale + 1) * 9 + 1] - phi * work[(scale + 1) * 9 + 3];
                    j_scale = cassi_qi_clamp(d_re * nd_im - d_im * nd_re, -64.0f, 64.0f);
                }
                const float read_gate = available ? cassi_qi_clamp(chi * final_cross, 0.0f, 1.0f) : 0.0f;
                diag_sum[scale][0] += rho;
                diag_sum[scale][1] += q;
                diag_sum[scale][2] += chi;
                diag_sum[scale][3] += j_temporal;
                diag_sum[scale][4] += j_scale;
                diag_sum[scale][5] += read_gate;
                diag_sum[scale][6] += available ? final_cross : 0.0f;
                diag_sum[scale][7] += available ? cassi_qi_clamp(last_write[scale], 0.0f, 1.0f) : 0.0f;
...
void ggml_compute_forward_cassi_qi_field_step(
        const ggml_compute_params * params,
        ggml_tensor * dst) {
    const ggml_tensor * src0 = dst->src[0];
    const ggml_tensor * src1 = dst->src[1];
...
// ggml_compute_forward_cassi_field_resonance

static void ggml_compute_forward_cassi_field_resonance_f32(
        const ggml_compute_params * params,
        ggml_tensor * dst) {
    const ggml_tensor * src0 = dst->src[0];
    const ggml_tensor * src1 = dst->src[1];
    ...
    GGML_ASSERT(src0->ne[0] % 8 == 0);
    GGML_ASSERT(src0->ne[1] > 0);
    GGML_ASSERT(src0->ne[2] == 1);
    GGML_ASSERT(src0->ne[3] == 1);
    GGML_ASSERT(src1->ne[0] % 2 == 0);
    GGML_ASSERT(src1->ne[1] > 0);
    GGML_ASSERT(src1->ne[2] == 1);
    GGML_ASSERT(src1->ne[3] == 1);

    const int64_t mode_count = src0->ne[0] / 8;
    const int64_t batch_count = src0->ne[1];
    const int64_t candidate_count = src1->ne[1];

    GGML_ASSERT(mode_count > 0);
    GGML_ASSERT(src1->ne[0] / 2 == mode_count);
    GGML_ASSERT(dst->ne[0] == candidate_count);
    GGML_ASSERT(dst->ne[1] == batch_count);
    GGML_ASSERT(dst->ne[2] == 1);
    GGML_ASSERT(dst->ne[3] == 1);

    const float phi = ggml_get_op_params_f32(dst, 0);
    const float velocity_scale = ggml_get_op_params_f32(dst, 1);

    const float * field = (const float *) src0->data;
    const float * probes = (const float *) src1->data;
    float * output = (float *) dst->data;

    const int64_t field_stride = 8 * mode_count;
    const int64_t probe_stride = 2 * mode_count;
    const int64_t dr = (batch_count + params->nth - 1) / params->nth;
    const int64_t batch_start = dr * params->ith;
    const int64_t batch_end = MIN(batch_start + dr, batch_count);

    for (int64_t batch = batch_start; batch < batch_end; ++batch) {
        const float * field_batch = field + batch * field_stride;
        float * output_batch = output + batch * candidate_count;

        for (int64_t candidate = 0; candidate < candidate_count; ++candidate) {
            const float * probe = probes + candidate * probe_stride;
            float correlation_re = 0.0f;
            float correlation_im = 0.0f;

            for (int64_t mode = 0; mode < mode_count; ++mode) {
                const float ey_re = field_batch[8 * mode + 0];
                const float ey_im = field_batch[8 * mode + 1];
                const float ei_re = field_batch[8 * mode + 2];
                const float ei_im = field_batch[8 * mode + 3];
                const float vy_re = field_batch[8 * mode + 4];
                const float vy_im = field_batch[8 * mode + 5];
                const float vi_re = field_batch[8 * mode + 6];
                const float vi_im = field_batch[8 * mode + 7];

                const float diff_re = ey_re - phi * ei_re;
                const float diff_im = ey_im - phi * ei_im;
                const float flux_re = diff_re + velocity_scale * (vy_re - phi * vi_re);
                const float flux_im = diff_im + velocity_scale * (vy_im - phi * vi_im);
                const float probe_re = probe[2 * mode + 0];
                const float probe_im = probe[2 * mode + 1];

                correlation_re += probe_re * flux_re + probe_im * flux_im;
                correlation_im += probe_re * flux_im - probe_im * flux_re;
            }

            output_batch[candidate] =
                correlation_re * correlation_re + correlation_im * correlation_im;
        }
    }
}

void ggml_compute_forward_cassi_field_resonance(
        const ggml_compute_params * params,
        ggml_tensor * dst) {
    const ggml_tensor * src0 = dst->src[0];
    const ggml_tensor * src1 = dst->src[1];
    ...
    if (src0->type == GGML_TYPE_F32 &&
        src1->type == GGML_TYPE_F32 &&
        dst->type == GGML_TYPE_F32) {
        ggml_compute_forward_cassi_field_resonance_f32(params, dst);
    } else {
        GGML_ABORT("fatal error");
    }
}
