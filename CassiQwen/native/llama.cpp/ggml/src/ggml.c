...
struct ggml_tensor * ggml_cassi_modal(
        struct ggml_context * ctx,
        struct ggml_tensor  * layer_modes,
        struct ggml_tensor  * state,
        struct ggml_tensor  * mode_params,
        ...
        float                 omega2,
        float                 coupling,
        int64_t               steps_per_layer) {
    GGML_ASSERT(layer_modes->type == GGML_TYPE_F32);
    GGML_ASSERT(state->type       == GGML_TYPE_F32);
    GGML_ASSERT(mode_params->type == GGML_TYPE_F32);
    GGML_ASSERT(seq_ids->type     == GGML_TYPE_I32);
    GGML_ASSERT(ggml_is_contiguous(layer_modes));
    GGML_ASSERT(ggml_is_contiguous(state));
    GGML_ASSERT(ggml_is_contiguous(mode_params));
    GGML_ASSERT(ggml_is_contiguous(seq_ids));

    GGML_ASSERT(layer_modes->ne[0] % 2 == 0);
    GGML_ASSERT(layer_count >= 1 && layer_count <= INT32_MAX);
    GGML_ASSERT(steps_per_layer >= 1 && steps_per_layer <= INT32_MAX);
    GGML_ASSERT(retained_weight >= 0.0f && retained_weight <= 1.0f);
    GGML_ASSERT(phi > 0.0f && dt > 0.0f && omega2 > 0.0f && coupling > 0.0f);

    const int64_t mode_count = layer_modes->ne[0] / 2;
    const int64_t token_count = layer_modes->ne[2];
    const int64_t sequence_count = state->ne[1];
    GGML_ASSERT(mode_count > 0);
    GGML_ASSERT(layer_modes->ne[1] == layer_count);
    GGML_ASSERT(seq_ids->ne[0] == token_count);
    GGML_ASSERT(layer_modes->ne[3] == 1);

    GGML_ASSERT(state->ne[0] % 8 == 0);
    GGML_ASSERT(state->ne[0] / 8 == mode_count);
    GGML_ASSERT(state->ne[2] == 1);
    GGML_ASSERT(state->ne[3] == 1);

    GGML_ASSERT(seq_ids->ne[1] == 1);
    GGML_ASSERT(seq_ids->ne[2] == 1);
    GGML_ASSERT(seq_ids->ne[3] == 1);

    GGML_ASSERT(mode_params->ne[0] == mode_count);
    GGML_ASSERT(mode_params->ne[1] == 1);
    GGML_ASSERT(mode_params->ne[2] == 1);
    GGML_ASSERT(mode_params->ne[3] == 1);

    GGML_ASSERT(token_count <= INT64_MAX / (2 * mode_count));
    GGML_ASSERT(sequence_count <= INT64_MAX / (8 * mode_count));
    const int64_t correction_count = 2 * mode_count * token_count;
    const int64_t state_count = 8 * mode_count * sequence_count;
    GGML_ASSERT(correction_count <= INT64_MAX - state_count);

    struct ggml_tensor * result = ggml_new_tensor_4d(ctx, GGML_TYPE_F32,
            correction_count + state_count, 1, 1, 1);

    ggml_set_op_params_i32(result, 0, (int32_t) layer_count);
    ggml_set_op_params_f32(result, 1, retained_weight);
    ggml_set_op_params_f32(result, 2, phi);
    ggml_set_op_params_f32(result, 3, dt);
    ggml_set_op_params_f32(result, 4, omega2);
    ggml_set_op_params_f32(result, 5, coupling);
    ggml_set_op_params_i32(result, 6, (int32_t) steps_per_layer);

    result->op     = GGML_OP_CASSI_MODAL;
    result->src[0] = layer_modes;
    result->src[1] = state;
...
struct ggml_tensor * ggml_cassi_field_step(
        struct ggml_context * ctx,
    ...
    GGML_ASSERT(mode_params->type == GGML_TYPE_F32);
    GGML_ASSERT(seq_ids->type    == GGML_TYPE_I32);
    GGML_ASSERT(ggml_is_contiguous(sense));
    GGML_ASSERT(ggml_is_contiguous(state));
    GGML_ASSERT(ggml_is_contiguous(mode_params));
    GGML_ASSERT(ggml_is_contiguous(seq_ids));

    GGML_ASSERT(sense->ne[0] % 2 == 0);
    GGML_ASSERT(sense->ne[1] > 0);
    GGML_ASSERT(sense->ne[2] == 1);
    GGML_ASSERT(sense->ne[3] == 1);
    GGML_ASSERT(state->ne[0] % 8 == 0);
    GGML_ASSERT(state->ne[1] > 0);
    GGML_ASSERT(state->ne[2] == 1);
    GGML_ASSERT(state->ne[3] == 1);
    GGML_ASSERT(mode_params->ne[1] == 1);
    GGML_ASSERT(mode_params->ne[2] == 1);
    ...
    ggml_set_op_params_f32(result, 0, retained_weight);
    ggml_set_op_params_f32(result, 1, phi);
    ggml_set_op_params_f32(result, 2, dt);
    ggml_set_op_params_f32(result, 3, omega2);
    ggml_set_op_params_f32(result, 4, coupling);
    ggml_set_op_params_i32(result, 5, (int32_t) steps);

    result->op     = GGML_OP_CASSI_FIELD_STEP;
    result->src[0] = sense;
    result->src[1] = state;
    result->src[2] = mode_params;
    result->src[3] = seq_ids;
...
struct ggml_tensor * ggml_cassi_qi_field_step(
        struct ggml_context * ctx,
        struct ggml_tensor  * sense,
        struct ggml_tensor  * state,
        struct ggml_tensor  * mode_params,
    ...
    GGML_ASSERT(ggml_is_contiguous(mode_params));
    GGML_ASSERT(ggml_is_contiguous(seq_ids));
    GGML_ASSERT(sense->ne[0] % 2 == 0 && sense->ne[1] > 0);
    GGML_ASSERT(sense->ne[2] == 1 && sense->ne[3] == 1);
    GGML_ASSERT(state->ne[0] % 9 == 0 && state->ne[1] > 0);
    GGML_ASSERT(state->ne[2] == 1 && state->ne[3] == 1);
    GGML_ASSERT(mode_params->ne[1] == 1 && mode_params->ne[2] == 1 && mode_params->ne[3] == 1);
    GGML_ASSERT(seq_ids->ne[1] == 1 && seq_ids->ne[2] == 1 && seq_ids->ne[3] == 1);
    GGML_ASSERT(scale_count >= 1 && scale_count <= 4);
    GGML_ASSERT(phi > 0.0f && dt > 0.0f && coupling >= 0.0f);
    ...
    struct ggml_tensor * result = ggml_new_tensor_4d(ctx, GGML_TYPE_F32,
            flux_count + state_count + diag_count, 1, 1, 1);

    ggml_set_op_params_i32(result, 0, (int32_t) scale_count);
    ggml_set_op_params_i32(result, 1, (int32_t) steps);
    ggml_set_op_params_f32(result, 2, phi);
    ggml_set_op_params_f32(result, 3, dt);
    ggml_set_op_params_f32(result, 4, coupling);
    ggml_set_op_params_f32(result, 5, damping_min);
    ggml_set_op_params_f32(result, 6, damping_max);
    ggml_set_op_params_f32(result, 7, epsilon_tau);
...
struct ggml_tensor * ggml_cassi_field_resonance(
        struct ggml_context * ctx,
        struct ggml_tensor  * field,
        struct ggml_tensor  * probes,
        float                 phi,
        float                 velocity_scale) {
    GGML_ASSERT(field->type  == GGML_TYPE_F32);
    GGML_ASSERT(probes->type == GGML_TYPE_F32);
    GGML_ASSERT(ggml_is_contiguous(field));
    GGML_ASSERT(ggml_is_contiguous(probes));

    GGML_ASSERT(field->ne[0] % 8 == 0);
    GGML_ASSERT(field->ne[1] > 0);
    GGML_ASSERT(field->ne[2] == 1);
    GGML_ASSERT(field->ne[3] == 1);
    GGML_ASSERT(probes->ne[0] % 2 == 0);
    GGML_ASSERT(probes->ne[1] > 0);
    GGML_ASSERT(probes->ne[2] == 1);
    GGML_ASSERT(probes->ne[3] == 1);

    const int64_t mode_count = field->ne[0] / 8;
    const int64_t batch_count = field->ne[1];
    const int64_t candidate_count = probes->ne[1];

    GGML_ASSERT(mode_count > 0);
    GGML_ASSERT(probes->ne[0] / 2 == mode_count);
    GGML_ASSERT(candidate_count <= INT64_MAX / batch_count);

    struct ggml_tensor * result = ggml_new_tensor_4d(ctx, GGML_TYPE_F32,
            candidate_count, batch_count, 1, 1);

    ggml_set_op_params_f32(result, 0, phi);
    ggml_set_op_params_f32(result, 1, velocity_scale);

    result->op     = GGML_OP_CASSI_FIELD_RESONANCE;
    result->src[0] = field;
    result->src[1] = probes;
