...
static struct ggml_backend_meta_split_state ggml_backend_meta_get_split_state(
    ...
    auto handle_cassi_modal = [&](const std::vector<ggml_backend_meta_split_state> & src_ss) -> ggml_backend_meta_split_state {
        // The packed output combines token corrections and all sequence tails,
        // so modal execution is mirrored rather than split across devices.
    ...
    auto handle_cassi_field_step = [&](const std::vector<ggml_backend_meta_split_state> & src_ss) -> ggml_backend_meta_split_state {
        for (size_t i = 0; i < 4; i++) {
            GGML_ASSERT(src_ss[i].axis == GGML_BACKEND_SPLIT_AXIS_MIRRORED);
        }
        return {GGML_BACKEND_SPLIT_AXIS_MIRRORED, {0}, {1}, 1};
    };
    auto handle_cassi_qi_field_step = [&](const std::vector<ggml_backend_meta_split_state> & src_ss) -> ggml_backend_meta_split_state {
        for (size_t i = 0; i < 4; i++) {
            GGML_ASSERT(src_ss[i].axis == GGML_BACKEND_SPLIT_AXIS_MIRRORED);
        }
        return {GGML_BACKEND_SPLIT_AXIS_MIRRORED, {0}, {1}, 1};
    };
    auto handle_cassi_field_resonance = [&](const std::vector<ggml_backend_meta_split_state> & src_ss) -> ggml_backend_meta_split_state {
    ...
    auto calculate_split_state = [&]() -> ggml_backend_meta_split_state {
        ...
        switch (tensor->op) {
            ...
            case GGML_OP_CASSI_MODAL: {
                split_state = handle_cassi_modal(src_ss);
            } break;
            case GGML_OP_CASSI_FIELD_STEP: {
                split_state = handle_cassi_field_step(src_ss);
            } break;
