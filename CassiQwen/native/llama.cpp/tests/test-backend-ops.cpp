...
// GGML_OP_CASSI_MODAL
struct test_cassi_modal : public test_case {
    const int64_t mode_count;
    const int64_t layer_count;
    const int64_t sequence_count;
    const int64_t token_count;

    std::string vars() override {
        return "mode_count=" + std::to_string(mode_count) +
            ",layer_count=" + std::to_string(layer_count) +
            ",sequence_count=" + std::to_string(sequence_count) +
    ...
    test_cassi_modal(int64_t mode_count = 4, int64_t layer_count = 3, int64_t sequence_count = 2) :
        mode_count(mode_count),
        layer_count(layer_count),
        sequence_count(sequence_count),
        token_count(sequence_count * 2 + 1) {}

    ggml_tensor * build_graph(ggml_context * ctx) override {
        ggml_tensor * layer_modes = ggml_new_tensor_3d(ctx, GGML_TYPE_F32,
                2 * mode_count, layer_count, token_count);
        ggml_set_param(layer_modes);
        ggml_set_name(layer_modes, "layer_modes");

        ggml_tensor * state = ggml_new_tensor_2d(ctx, GGML_TYPE_F32,
                8 * mode_count, sequence_count);
        ggml_set_param(state);
        ggml_set_name(state, "state");

        ggml_tensor * mode_params = ggml_new_tensor_1d(ctx, GGML_TYPE_F32, mode_count);
        ggml_set_param(mode_params);
        ggml_set_name(mode_params, "mode_params");

        ggml_tensor * seq_ids = ggml_new_tensor_1d(ctx, GGML_TYPE_I32, token_count);
        ggml_set_param(seq_ids);
        ggml_set_name(seq_ids, "seq_ids");

        ggml_tensor * out = ggml_cassi_modal(ctx, layer_modes, state, mode_params, seq_ids,
                layer_count, 0.9f, 1.618033988749895f, 0.005f, 20.0f, 1.0f, 4);
        ggml_set_name(out, "out");
        return out;
    }
    ...
    void initialize_tensors(ggml_context * ctx) override {
        for (ggml_tensor * t = ggml_get_first_tensor(ctx); t != nullptr; t = ggml_get_next_tensor(ctx, t)) {
            if (strcmp(t->name, "seq_ids") == 0) {
                std::vector<int32_t> data(token_count);
                for (int64_t token = 0; token < token_count; ++token) {
                    data[token] = (int32_t) (token % sequence_count);
                }
                ggml_backend_tensor_set(t, data.data(), 0, data.size() * sizeof(int32_t));
            } else if (strcmp(t->name, "mode_params") == 0) {
                init_tensor_uniform(t, -0.25f * (float) mode_count, 0.0f);
            } else {
...
// Conv_2D test cases
 ...
 // A deep layer of a ConvNet, several images in the batch
    ...
    test_cases.emplace_back(new test_cassi_modal());
