...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    if (ex != LLAMA_EXAMPLE_SERVER) {
        add_opt(common_arg(
            {"--cassi-modal"},
            {"--no-cassi-modal"},
            string_format("enable Cassi modal controls (default: %s)", params.cassi_modal ? "enabled" : "disabled"),
            [](common_params & params, bool value) {
                params.cassi_modal = value;
                if (value) {
                    params.cassi_field_step = false;
                    params.cassi_qi_field = false;
                }
            }
        ).set_env("LLAMA_ARG_CASSI_MODAL").set_examples({LLAMA_EXAMPLE_CLI, LLAMA_EXAMPLE_PERPLEXITY}));
    }
    add_opt(common_arg(
        {"--cassi-field-step"},
        {"--no-cassi-field-step"},
        string_format("enable Qwen35 Cassi field-step injection (default: %s)", params.cassi_field_step ? "enabled" : "disabled"),
        [](common_params & params, bool value) {
            params.cassi_field_step = value;
            if (value) {
                params.cassi_modal = false;
                params.cassi_qi_field = false;
            }
        }
    ).set_env("LLAMA_ARG_CASSI_FIELD_STEP").set_examples({LLAMA_EXAMPLE_CLI, LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_PERPLEXITY}));
    add_opt(common_arg(
        {"--cassi-field-layer"}, "N",
        string_format("Qwen35 captured layer input used as field sense (default: %d)", params.cassi_field_layer),
        [](common_params & params, int value) {
            if (value < 0) {
                throw std::invalid_argument("error: --cassi-field-layer must be >= 0\n");
            }
            params.cassi_field_layer = value;
        }
    ).set_env("LLAMA_ARG_CASSI_FIELD_LAYER").set_examples({LLAMA_EXAMPLE_CLI, LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_PERPLEXITY}));
    add_opt(common_arg(
        {"--cassi-qi-field"},
        {"--no-cassi-qi-field"},
        string_format("enable Qwen35 Qi multi-scale field injection (default: %s)", params.cassi_qi_field ? "enabled" : "disabled"),
        [](common_params & params, bool value) {
            params.cassi_qi_field = value;
            if (value) {
                params.cassi_modal = false;
                params.cassi_field_step = false;
            }
    ...
    add_opt(common_arg(
        ...
        [](common_params & params, int value) {
            if (value < 0) {
                throw std::invalid_argument("error: --cassi-qi-field-layer must be >= 0\n");
            }
            params.cassi_qi_field_layer = value;
        }
    ).set_env("LLAMA_ARG_CASSI_QI_FIELD_LAYER").set_examples({LLAMA_EXAMPLE_CLI, LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_PERPLEXITY, LLAMA_EXAMPLE_COMMON}));
    add_opt(common_arg(
        {"--cassi-qi-field-scales"}, "N",
        string_format("Qwen35 Qi fixed scale count, 1..4 (default: %d)", params.cassi_qi_field_scales),
        [](common_params & params, int value) {
            if (value < 1 || value > 4) {
                throw std::invalid_argument("error: --cassi-qi-field-scales must be in [1,4]\n");
            }
    ...
    if (ex == LLAMA_EXAMPLE_SERVER) {
        ...
        ).set_env("LLAMA_ARG_CASSI_MODAL").set_examples({LLAMA_EXAMPLE_SERVER}));
        add_opt(common_arg(
            {"--cassi-modal-retained"}, "N",
            "Cassi retained modal weight (range [0,1])",
            [](common_params & params, const std::string & value) {
                const float v = std::stof(value);
                if (!std::isfinite(v) || v < 0.0f || v > 1.0f) throw std::invalid_argument("error: --cassi-modal-retained must be finite and in [0,1]\n");
                params.cassi_modal_retained_weight = v;
            }
        ).set_env("LLAMA_ARG_CASSI_MODAL_RETAINED").set_examples({LLAMA_EXAMPLE_SERVER}));
        add_opt(common_arg(
            {"--cassi-modal-phi"}, "N", "Cassi modal phi (finite and positive)",
            [](common_params & params, const std::string & value) {
                const float v = std::stof(value);
                if (!std::isfinite(v) || v <= 0.0f) throw std::invalid_argument("error: --cassi-modal-phi must be finite and positive\n");
                params.cassi_modal_phi = v;
            }
        ).set_env("LLAMA_ARG_CASSI_MODAL_PHI").set_examples({LLAMA_EXAMPLE_SERVER}));
        add_opt(common_arg(
            {"--cassi-modal-dt"}, "N", "Cassi modal timestep (finite and positive)",
            [](common_params & params, const std::string & value) {
                const float v = std::stof(value);
                if (!std::isfinite(v) || v <= 0.0f) throw std::invalid_argument("error: --cassi-modal-dt must be finite and positive\n");
                params.cassi_modal_dt = v;
            }
        ).set_env("LLAMA_ARG_CASSI_MODAL_DT").set_examples({LLAMA_EXAMPLE_SERVER}));
        add_opt(common_arg(
            {"--cassi-modal-omega2"}, "N", "Cassi modal omega squared (finite and positive)",
            [](common_params & params, const std::string & value) {
                const float v = std::stof(value);
                if (!std::isfinite(v) || v <= 0.0f) throw std::invalid_argument("error: --cassi-modal-omega2 must be finite and positive\n");
                params.cassi_modal_omega2 = v;
            }
        ).set_env("LLAMA_ARG_CASSI_MODAL_OMEGA2").set_examples({LLAMA_EXAMPLE_SERVER}));
        add_opt(common_arg(
            {"--cassi-modal-coupling"}, "N", "Cassi modal coupling (finite and positive)",
            [](common_params & params, const std::string & value) {
                const float v = std::stof(value);
                if (!std::isfinite(v) || v <= 0.0f) throw std::invalid_argument("error: --cassi-modal-coupling must be finite and positive\n");
                params.cassi_modal_coupling = v;
            }
        ).set_env("LLAMA_ARG_CASSI_MODAL_COUPLING").set_examples({LLAMA_EXAMPLE_SERVER}));
        add_opt(common_arg(
            {"--cassi-modal-steps-per-layer"}, "N", "Cassi modal integration steps per layer (>= 1)",
            [](common_params & params, int value) {
                if (value < 1) throw std::invalid_argument("error: --cassi-modal-steps-per-layer must be >= 1\n");
                params.cassi_modal_steps_per_layer = value;
            }
        ).set_env("LLAMA_ARG_CASSI_MODAL_STEPS_PER_LAYER").set_examples({LLAMA_EXAMPLE_SERVER}));
        add_opt(common_arg(
            {"-np", "--parallel"}, "N",
