...
extern "C" {
    ...
    struct llama_context_params {
        ...
        // Qwen35 hidden-state Cassi modal injection (enabled by default; opt out with --no-cassi-modal).
        bool   cassi_modal;
        // Explicit opt-in Qwen35 field-step injection (default off).
        ...
        float    cassi_modal_retained_weight; // default 0.9
        float    cassi_modal_phi;             // default 1.618033988749895
        float    cassi_modal_dt;              // default 0.005
        float    cassi_modal_omega2;          // default 20.0
        float    cassi_modal_coupling;        // default 1.0
        uint32_t cassi_modal_steps_per_layer; // default 4
    };
    ...
    // For hybrid models (e.g. Qwen35), the number of full-attention layers
    // (llama_model_n_layer minus recurrent) and recurrent layers.  For
    // non-hybrid models these equal n_layer and 0 respectively.
    LLAMA_API int32_t llama_model_n_layer_attn   (const struct llama_model * model);
    LLAMA_API int32_t llama_model_n_layer_recurrent(const struct llama_model * model);
    ...
    // Cassi Qwen35 ownership footprint (baseline measurement only).
    // Populated by llama_qwen35_footprint(); all counters are zero if the
    // corresponding handle is NULL.  The layer counts are the distinct
    // full-attention / recurrent / non-main-MTP counts (16/48/1 for the
    // pinned Qwen3.8-27B), never the summed main-layer total.
    struct llama_qwen35_footprint {
        uint64_t model_size_bytes;
        uint64_t model_params;
        int32_t  n_layer_full_attn;
        int32_t  n_layer_recurrent;
        int32_t  n_layer_mtp;
        int32_t  n_vocab;
        uint64_t kv_bytes;
        uint64_t recurrent_bytes;
        uint64_t serialized_state_bytes;
        uint32_t gguf_open_count;
    };

    // Populates a footprint from the given model and context.  Either
    // handle may be NULL; the corresponding counters stay zero.
    LLAMA_API void llama_qwen35_footprint(
            const struct llama_model * model,
            struct llama_context * ctx,
