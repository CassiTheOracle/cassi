...
def run_arm(
    *,
    ...
    command = [
        ...
        "-File", str(ROOT / "start-llama-server.ps1"),
        ...
        "-ServerPath", str(server_path.relative_to(ROOT)),
        "-Port", str(port),
    ]
    if not field:
    ...
    with log_path.open("wb") as log:
        proc = subprocess.Popen(command, cwd=str(ROOT), env=environment, stdout=log, stderr=subprocess.STDOUT)
        try:
...
def main() -> int:
    ...
    server_path = (ROOT / args.binary) if not pathlib.Path(args.binary).is_absolute() else pathlib.Path(args.binary)
    model_path = (ROOT / args.model) if not pathlib.Path(args.model).is_absolute() else pathlib.Path(args.model)
    for path in (server_path, model_path):
        if not path.is_file():
            raise SystemExit(f"missing required artifact: {path}")

    out_dir = ROOT / "_diag" / "cassi-field-attestation"
