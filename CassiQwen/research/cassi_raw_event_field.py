...
_CASSIFI_ROOT = Path(__file__).resolve().parents[2] / "CassiFI"
...
_ATOM_ALPHABET = 256
_CHECKPOINT_MAGIC = b"CASSI-RAW-EVENT-FIELD\x03"
_CHECKPOINT_SCHEMA = "cassi.raw-event-field-checkpoint.v3"
_PROFILE_SCHEMA = "cassi.raw-event-acquisition-profile.v3"
...
class AcquisitionProfile:
    ...
    def __post_init__(self) -> None:
        ...
        component_limit = _finite_float("component_limit", self.component_limit, minimum=1.0e-9)
        if component_limit > 4.0:
            raise ValueError("component_limit exceeds the fixed capacity bound")
        ...
        object.__setattr__(self, "component_limit", float(component_limit))
...
def _controller_for(profile: AcquisitionProfile) -> QiFieldController:
    physics = QiFieldPhysicsConfig(
        ...
        max_mode_amplitude=profile.component_limit,
    ...
    config = QiFieldConfig(
        ...
        epsilon_clip=profile.component_limit,
        sense_gain=1.0,
        correction_gain=0.5,
...
class RawEventLearner:
    ...
    def _validate_field(self, field: torch.Tensor) -> None:
        ...
        if max_abs > self.profile.component_limit + 1.0e-10:
            raise CapacityError(
                f"field candidate exceeds the {self.profile.component_limit:g} component bound"
            )
    ...
    def _consolidate_state(
        ...
        """Reproject bounded memory through the fixed multiscale Qi ladder."""

        before = _sha_tensor(state.field)
        candidate = state
        steps = int(self._qi_config.consolidation_steps)
        try:
            for _ in range(steps):
                candidate = self._controller.consolidate(candidate)
                self._validate_field(candidate.field)
        except Exception as exc:
            raise CapacityError(f"fixed Qi consolidation rejected the candidate: {exc}") from exc
        return candidate, {
            "mode": "controller-multiscale-reprojection",
            "steps": steps,
            "before_sha256": before,
            "after_sha256": _sha_tensor(candidate.field),
        }

    def consolidate(self) -> dict[str, Any]:
        """Consolidate the owned field and return its identity transition."""

        candidate, receipt = self._consolidate_state(self.state)
        self.state = candidate
        return receipt

    def _write_relation_with_consolidation(
        self,
        state: QiFieldState,
        *,
        observation: bytes,
        ...
        scale: int,
        gain: float,
    ) -> tuple[QiFieldState, dict[str, Any] | None]:
        try:
            return (
        ...
        except CapacityError as initial:
            consolidated, receipt = self._consolidate_state(state)
            try:
            ...
            except CapacityError as retry:
                raise retry from initial
    ...
    def _record_consolidation(
        ...
        *,
        ...
        if details is None:
            return
        receipt["consolidation_attempted"] = True
        receipt["consolidation_steps"] += int(details["steps"])
        receipt["consolidations"].append({"stage": stage, **dict(details)})
            ...
            if event.kind == "reset":
                ...
                if self._pending is None:
                    ...
                    if learn:
                        ...
                        candidate, consolidation = self._write_relation_with_consolidation(
                            self.state,
                            observation=pending.observation,
                            action=pending.action,
                            history=pending.history,
