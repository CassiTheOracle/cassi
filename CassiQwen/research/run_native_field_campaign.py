...
SERVER = _CASSIQWEN_ROOT / "native/llama.cpp/b8/bin/Release/llama-server.exe"
MODEL = _CASSIQWEN_ROOT / "Qwen3.8-27B-Q4_K_M.gguf"
RUNNER = _CASSIQWEN_ROOT / "benchmarks/run_cassi_field_qwen_workcases.py"
VERIFIER = _CASSIQWEN_ROOT / "benchmarks/verify_cassi_field_qwen_workcases.py"
...
def run(cmd, timeout):
    ...
    try:
        r = subprocess.run([str(c) for c in cmd], cwd=str(_CASSIQWEN_ROOT), capture_output=True,
...
def start_own_server(port):
    ...
    proc = subprocess.Popen([str(c) for c in cmd], cwd=str(SERVER.parent),
...
def main():
    results = {}
    for cfg in CONFIGS:
        run_dir = _CASSIQWEN_ROOT / "_diag" / f"native-field-{cfg['name']}"
    ...
    summary = _CASSIQWEN_ROOT / "_diag" / "native-field-campaign-summary.json"
