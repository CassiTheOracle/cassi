    ...
    with localcontext() as context:
        ...
        for stage in stages:
            ...
            stage_records.append({
                ...
                "branch": "exact-analytic-damped-oscillator" if "propagation" in name or "spectral" in name else "identity-or-force-kick",
        ...
        for scale, row in enumerate(rows):
            ...
            damping_by_scale.append(weight * gamma_values[scale] * mass * velocity_cap * velocity_cap * h)
        ...
        return {
            ...
            "derivation_formulae": {
                ...
                "damped_branches": "exact underdamped/critical/overdamped analytic 2x2 matrix, evaluated once per spectral half-stage",
                "half_amplification": "metric-normalized Frobenius enclosure of exp(A_D(k)*tau), tau=stage duration",
                "force_kick": "metric-normalized Frobenius enclosure with cubic-force Jacobian norm <=3*abs(kappa_D,s)*D_domain^2",
...
def build_numerical_certificate(*, geometry: Any, transport: Any, accepted_w3_artifact_identity: Mapping[str, Any] | None = None) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    section["self_sha256"] = canonical_hash(section, NUMERICAL_SECTION_SCHEMA)
    extension = {
        "schema": CERTIFICATE_EXTENSION_SCHEMA,
        "certificate_chain_id": chain_id,
        "chain_ordinal": 1,
        "parent_certificate_sha256": root["self_sha256"],
        "parent_section_inventory": [],
        "owning_package": "W3N",
        "gate": "G3N",
        "consumed_semantic_subhashes": consumed,
        "accepted_w3_artifact_identity": artifact_identity,
        "added_section": section,
        "complete_section_inventory": [section],
        "chain_status": "final",
    }
    extension["final_certificate_identity_sha256"] = canonical_hash(extension, NUMERICAL_EXTENSION_DOMAIN)
    extension["self_sha256"] = canonical_hash(extension, NUMERICAL_EXTENSION_DOMAIN)
...
__all__ = [
    ...
    "NUMERICAL_DERIVATION_SCHEMA", "NUMERICAL_GUARD_RECEIPT_SCHEMA",
    "NUMERICAL_RAW_STATE_DOMAIN", "NumericalCertificateError",
    "G3N_EXECUTED_CONTROL_IDS", "build_numerical_certificate",
    "build_registry_extension", "derive_intrinsic_w3_enclosures",
    "evaluate_online_guard", "raw_state_bytes_from_field",
