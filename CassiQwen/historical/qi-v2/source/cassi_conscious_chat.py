...
CHAT_CONFIG_SCHEMA = "cassi.qi-native-chat-config.v1"
CHAT_TURN_SCHEMA = "cassi.qi-native-chat-turn.v1"
_PACKAGE_ROOT = Path(__file__).resolve().parent
DEFAULT_CONFIG_PATH = _PACKAGE_ROOT / "conscious-chat.json"
DEFAULT_STATE_DIR = _PACKAGE_ROOT / "_diag" / "cassi-qi-native" / "chat"
...
class CassiConsciousChatConfig:
    """Validated Qi-native terminal configuration (schema ``cassi.qi-native-chat-config.v1``)."""
...
class CassiConsciousChatRuntime:
    ...
    def open(
        ...
        *,
        ...
        try:
            ...
            engine = CassiQiTextEngine(controller, max_output_symbols=max_output_symbols)
            store = CassiQiSessionStore(
                state_root,
                controller,
    ...
    def _trim_transcript(self) -> None:
        if len(self.messages) > _MAX_TRANSCRIPT_MESSAGES:
            self.messages = self.messages[-_MAX_TRANSCRIPT_MESSAGES:]
    ...
    def chat(self, user_text: object) -> CassiChatTurn:
        with self._mutex:
            ...
            # The field carries the conversational memory in its slow scales; only
            # the new user message is sensed on top of the persisted state.
