"""Zero-import and runtime ownership gate for adopted Cassi-native text paths."""
...
def verify_runtime() -> dict[str, object]:
    ...
    config_bytes = (_ROOT / "cassi-qi-language.json").read_bytes()
    config = QiFieldConfig.from_dict(json.loads(config_bytes.decode("utf-8")))
    controller = QiFieldController(config)
    engine = CassiQiTextEngine(controller, max_output_symbols=4)
    result = engine.generate(
        engine.initial_state(),
        ({"role": "user", "content": "Cassi-native ownership gate"},),
    )
...
def main() -> int:
    receipt = {
        ...
        "schema": "cassi.qi-native-runtime-gate.v1",
