...
def measure_baseline(args: argparse.Namespace) -> dict[str, Any]:
    ...
    omp_candidates = (
        ...
        Path(__file__).resolve().parent / "libomp140.x86_64.dll",
    )
    ...
    native_root = Path(__file__).resolve().parent / "native/llama.cpp"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--dll-dir", type=Path, default=root / "native/llama.cpp/build-cassi/bin/Release")
    parser.add_argument("--out", type=Path, default=root / "_diag/qwen-displacement/baseline-receipt.json")
