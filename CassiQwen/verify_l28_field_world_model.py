...
def _expected_code_digests() -> dict[str, str]:
    source = Path(__file__).with_name("field_world_model.py")
    return {
        "field_world_model.py": _sha256(source),
        "cassi_modal_torch.py": _sha256(source.with_name("cassi_modal_torch.py")),
        "L28-FIELD-WORLD-MODEL-PREREG.md": _sha256(source.with_name("L28-FIELD-WORLD-MODEL-PREREG.md")),
    }
...
def main() -> int:
    ...
    parser.add_argument("--board", type=Path, default=Path("_diag/l28-field-world-model/l28-board.json"))
    parser.add_argument("--report", type=Path, default=Path("L28-FIELD-WORLD-MODEL-REPORT.md"))
    parser.add_argument(
        "--require-supports",
        action="store_true",
