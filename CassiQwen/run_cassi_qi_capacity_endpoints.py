...
def _intervention(
    ...
    *,
    ...
    return QiEndpointIntervention(
        ...
        path_hashes={
            "spatial": identity(f"spatial-{source}-{target}"),
            "passive": identity(f"passive-{source}-{target}"),
        },
...
def demo_interventions(profile: QiEndpointCapacityProfile | None = None) -> tuple[QiEndpointIntervention, ...]:
    """Return treatment interventions covering every source/target coordinate."""
    ...
    target_response = {"boundary": 1.0, "text": 1.2, "action": 1.4, "effect": 1.6}
    treatment: list[QiEndpointIntervention] = []
    for source_index, source in enumerate(profile.source_coordinate_ids):
        for target_index, target in enumerate(profile.target_coordinate_ids):
            consequence = None if target == "boundary" else _sha({"source": source, "target": target, "committed": True})
...
def build_demo_receipt():
    ...
    profile = QiEndpointCapacityProfile.from_dependencies(
        **(dict(profile.payload()) | {"control_specs": demo_control_specs(profile)})
    )
