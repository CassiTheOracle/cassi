...
def _tuple_document(object_schema: str) -> dict[str, object]:
    return {
        ...
        "nullable_keys": [],
        "properties": {
            "schema": {"type": "string", "enum": [object_schema]},
            "shape": {
                "type": "array",
                "min_items": 2,
                "max_items": 2,
...
def test_tuple_descriptors_are_reserved_for_structural_root_profile_documents() -> None:
    with pytest.raises(verifier.VerificationError):
        verifier._validate_schema_document(
