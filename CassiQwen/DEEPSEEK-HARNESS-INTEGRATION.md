...
At this revision, the first host/client slice exists in [`dsh-cassi-entity/`](dsh-cassi-entity/) with the side-by-side [`dsh-cassi-profile/`](dsh-cassi-profile/) launch manifest. It covers the existing authenticated entity routes, the `/cassi` RPC channel, the `cassi-entity` adapter, explicit portfolio/guidance/evidence tools, and the core typed-turn lifecycle: durable turn admission, idempotent replay, committed/failure events, bounded event delivery, terminal cancellation reconciliation, and actual usage forwarding. The live Harness execution bridge is exercised through `cassi_list_programs`: the real `ToolRuntime` result is submitted to the entity, the turn continues, and the adapter returns the committed response. The dedicated portfolio UI below remains a specified follow-on surface; no installed Harness package, profile, credential, or default model was changed while producing this integration.
...
The current researcher executes its own scoped capabilities. It has no remote worker lease API. Connecting Harness tools therefore requires an explicit work-order seam, not an HTTP alias for arbitrary shell commands.

Proposed host-broker operations:

- Advertise the tool schemas actually visible to the owning Harness agent, their effect classes, allowed roots, network destinations, cancellation behavior, resource limits, and expiry. Advertising availability grants no permission.
- Lease one entity-authored work order to one named executor with an expiry and idempotent operation identity.
- Return progress and an exact terminal outcome with source versions, artifact digests, stdout/stderr or bounded projections, exit disposition, and actual resource use where measured.
- Acknowledge cancellation only after the worker is quiescent; retain unresolved external effects explicitly.
- On executor disconnect, expire the lease. Replay safe observations only under their declared policy; preserve an unknown-effect state for non-idempotent actions.

Each work order binds entity, program, operation, field predecessor, capability/schema version, immutable argument digest, authority reference, budget, and expected result form. Admission checks current source and authority versions before the field interprets the result.

Use `ctx.tools.execute` through the normal policy pipeline, never call a tool's implementation directly. Respect the installed native/code tool-presentation mode; do not invent a parent token to bypass Code Mode. The bridge advertises only what that scoped executor can really dispatch.

A background program needs a real Harness-owned execution session and an open audited turn. The approval service explicitly rejects idle asks. Build a dedicated execution-turn lifecycle around the installed agent/session facilities; do not manufacture an `Agent`-shaped object or launch a second autonomous reasoning loop. If no such authorized execution turn or human answerer is available, the work order remains waiting.

The initial registered subset should cover scoped reads, searches, artifact construction, and already-authorized computations. Arbitrary generated code remains unavailable until an enforceable containment backend actually restricts filesystem, network, credentials, and child processes. A timeout, Windows process job, or a package named sandbox is not evidence of those restrictions.
...
| Source changed | Keep both source identities; reassess dependent work before acceptance |
| Cycle allowance exhausted | Pause with the current frontier intact; resumption does not silently reset accounting |

The current director holds its cycle lock across planning and synthesis, so controls can wait behind inference. The complete conversation/broker integration needs an admitted control queue, explicit in-flight disposition, and a separately scheduled model lane. Do not advertise instantaneous cancellation using the present API.
...
**Extend Harness execution.** The first real `ToolRuntime` result path is present for the registered Cassi tools. Add broader work orders, leases, execution-turn ownership, outcome admission, approval handling, and full cancellation reconciliation while keeping the entity's locally safe methods available when a Harness executor is offline.
