...
def _close(actual: Any, expected: Any, *, name: str, rtol: float = 2e-11, atol: float = 2e-12) -> None:
    if isinstance(actual, str) and actual.startswith("f64:"):
        actual = _f64(actual, name=name)
    if isinstance(expected, str) and expected.startswith("f64:"):
        expected = _f64(expected, name=name)
    try:
...
def _verify_energy_and_ledger(
    ...
    *,
    ...
    composition_keys = {
        ...
        "wave_energy_delta", "total_coupled_closure", "force_D_sum_re", "force_C_sum_re",
...
def _verify_receipt_hashes(node: Any, path: str = "") -> None:
    if isinstance(node, Mapping):
        if "self_sha256" in node:
            domains = [str(node.get("schema", "")), RECEIPT_DOMAIN, DERIVATION_DOMAIN, SECTION_DOMAIN, EXTENSION_DOMAIN, "cassi.qi-flow-w3n-certificate.v1", "cassi.qi-flow-w3n-extension.v1"]
...
def verify(root: Path, *, allow_staging_root: bool = False) -> dict[str, Any]:
    ...
    for key, expected in (("w3_profile_sha256", transport.profile_sha256), ("w2_geometry_profile_sha256", geometry.profile_sha256), ("carrier_profile_sha256", profile["profile_sha256"]), ("carrier_root_sha256", carrier_root["self_sha256"])):
    ...
    curvature = max(curvature_by_scale, default=0.0); work_bound = (curvature + 1.0) * duration * duration * 128.0
    ...
    if not isinstance(source_composition, Mapping) or set(source_composition) != {"base_energy_pre", "base_energy_post", "U_pre", "U_D_path", "U_center", "U_C_path", "U_post", "Delta_U", "W_D", "W_center", "W_C", "coordinate_work_closure", "wave_energy_delta", "total_coupled_closure", "force_D_sum_re", "force_C_sum_re", "slow_carrier_bias_re", "per_scale_U_pre", "per_scale_U_post"}:
    ...
    return {"status": "PASS", "root": str(root), "run_id": index["run_id"], "parent_w3n_run_id": parent_index["run_id"], "control_ids": list(CONTROL_IDS), "scale_count": params["scale_count"], "mode_count": params["mode_count"], "batch_count": params["batch_limit"]}
