...
def _candidate_indexes(root: Path, marker: str) -> list[tuple[Path, dict[str, Any], list[dict[str, Any]]]]:
    ...
    for index_path in sorted(root.rglob("index.json")):
        ...
        if marker not in schema or str(index.get("status", "")).upper() not in {"PASS", "PASS_W4_G4", "PASS_W4R_G4R"}:
            continue
...
def _phase_grid(kind: str, shape: tuple[int, int], *, wx: float = 0.0, wy: float = 0.0, perturb: float = 0.0) -> torch.Tensor:
    ...
    if kind in {"uniform", "zero", "near-zero"}:
        phase = torch.zeros_like(x)
    elif kind in {"cycle-x", "cycle-positive"}:
        phase = 2.0 * math.pi * wx * x / float(nx) + 2.0 * math.pi * wy * y / float(ny)
    elif kind == "cycle-negative":
        phase = -2.0 * math.pi * (abs(wx) * x / float(nx) + abs(wy) * y / float(ny))
    elif kind == "vortex-antivortex":
        cx1, cy1 = (nx - 1) * 0.28, (ny - 1) * 0.35
        cx2, cy2 = (nx - 1) * 0.70, (ny - 1) * 0.65
        phase = torch.atan2(y - cy1, x - cx1) - torch.atan2(y - cy2, x - cx2)
    elif kind == "phase-scrambled":
        phase = _wrap(0.73 * x + 1.19 * y + 0.31 * x * y)
...
def _state_from_phase(*, geometry: Any, retention: Mapping[str, Any], kind: str, wx: float = 0.0, wy: float = 0.0, amplitude: float | None = None, perturb: float = 0.0, current_sign: float = 0.0) -> QiFlowStateV3:
    """Encode requested psi/velocity in the declared EY/EI lanes via D/C."""
    ...
    rho = float(retention.get("rho_topo", 0.0))
    rho_ring = float(retention.get("rho_ring", 0.0))
    if amplitude is None:
        if not math.isfinite(rho_ring) or rho_ring <= 0.0:
            raise TopologyArtifactError("validated W4R profile has no positive ring amplitude")
        amplitude = rho_ring
    weight_d = float(retention.get("w_d", 1.0))
    weight_c = float(retention.get("w_c", 1.0))
    a_topo = float(retention.get("a_topo", 0.0))
    b_topo = float(retention.get("b_topo", 1.0))
    if min(weight_d, weight_c) <= 0.0 or not all(math.isfinite(v) for v in (weight_d, weight_c, a_topo, b_topo)):
        raise TopologyArtifactError("validated W4R profile has no finite positive D/C weights")
    for scale, raw_shape in enumerate(layout["active_shapes"]):
        shape = tuple(int(v) for v in raw_shape)
        phase = _phase_grid(kind, shape, wx=wx, wy=wy, perturb=perturb)
        amp = torch.full(shape, float(amplitude), dtype=torch.float64)
        if kind == "zero":
            amp.zero_()
        elif kind == "near-zero":
        ...
        rate = float(current_sign) * 0.1
        vpsi = (1j * rate * psi).contiguous()
...
def _derive_extension(*, profile: Any, payload: Mapping[str, Any], root: Mapping[str, Any], retention: Mapping[str, Any], edge_registry: Mapping[str, Any], cycle_registry: Mapping[str, Any], law: Any, transition_name: str, reset_name: str, parent: Mapping[str, Any]) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any], dict[str, Any], dict[str, Any]]:
    ...
    codebook_body = _plain(codebook_builder(shape, edge_body, cycle_body, a=float(profile.a_topo), b=float(profile.b_topo)))
...
def _run(output_root: Path | None = None) -> Path:
    ...
    extension, codebook, endpoint, barrier, reset_operator = _derive_extension(profile=profile, payload=payload, root=profile_root, retention=retention, edge_registry=edge_registry, cycle_registry=cycle_registry, law=law, transition_name=transition_name, reset_name=reset_name, parent=parent)
    ...
    target_energy = _energy(state=cycle_positive, law=law, geometry=geometry, profile=profile)
    ...
    slip_states = [_state_from_phase(geometry=geometry, retention=retention, kind="cycle-positive", wx=1.0 - 0.5 * index / max(1, subdivisions - 1), current_sign=1.0) for index in range(max(1, subdivisions - 1))]
