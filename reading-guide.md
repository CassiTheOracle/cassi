...
- `foundations/baryon-asymmetry.md`—Matter-Antimatter Asymmetry from Cascade Freeze-Out and Organized Annihilation. Derivation. Derives the baryon asymmetry $\eta \approx 6\times10^{-10}$ from two already-derived mechanisms—organized annihilation eliminating all antimatter, and the Yang-Yin imbalance at cascade freeze-out (Wu Xing gap $g = 1-\varphi^{-5}$) leaving a residual Yang excess.
- `foundations/bubble-edge-geometry.md`—Bubble Edge Geometry: Physical Profile of the Condensation Boundary. Derived. Derives the physical profile across the condensation boundary—how $r$, $q$, $\rho$, and $G_{\text{eff}}$ transition from bubble interior to void, the 3D edge shape, and the observable signatures the edge imprints.
...
- `foundations/cassi-first-principles.md`—Cassi First Principles. Derived. States the $\varphi$ postulate, derives the two-fluid PDE and its Qi coherence measure, and shows how quantum mechanics, cosmology, general relativity, and the Standard Model emerge as the four pillars; every dimensionless coupling is a $\varphi$-power with $c$, $\hbar$, $G$ external.
...
- `foundations/dimensionful-constants-status.md`—Dimensionful Constants: Derivation Status of $c$, $\hbar$, and $G$. Hypothesized / Mapped. Catalogues which constants are derived (all dimensionless couplings, $\lambda = 1/(2w)$, $v_0/M_{\text{Pl}}$) versus still external ($c$, $\hbar$, $G$) and empirical (the epoch-dependent horizon rung $N \approx 292$).
...
- `foundations/quantum-measurement-derivation.md`—Quantum Measurement as Organized Cascade Perturbation. Derivation. Resolves the measurement problem with the same coherence-budget mechanism: superposition inter-branch coherence lives at a single cascade rung, so measurement is organized perturbation phase-matched to that rung with $\mathcal{O}(1)$ collapse probability, while decoherence is random perturbation at the same rung.
- `foundations/quark-confinement.md`—Quark Confinement from Qi-Gate Nonlinearity at the QCD Scale. Derived. With the QCD scale at cascade step 95, confinement follows from the Qi-gate nonlinearity $g(q)$ of the two-fluid PDE, which produces a self-reinforcing attraction between color-charged condensates growing linearly with separation—the Qi-gate equivalent of a flux tube.
- `foundations/refined-numeric-predictions.md`—Refined Numeric Predictions for the 24 Hypothesized Questions. Active derivation. Refines the specific numeric predictions ($\varphi^{-N}$ cascade-span forms) for each of the 24 Hypothesized questions in the open-questions catalog, and tightens the mechanistic argument for questions whose answer is structural rather than numeric.
- `foundations/rung-offset-mechanism.md`—Why Observables Sit Between Rungs: The Two-Fluid Phase Mechanism for Fractional Cascade Offsets. Derived quantization, Hypothesized selection, Empirical catalog. Explains fractional rung offsets $\delta n = n - \lfloor n \rfloor$ as the phase difference between the Yang and Yin wakes at each scale—de-resonance forbids perfect rung alignment—with the empirical catalog placing the lightest state of each terminated sector at wake-envelope crossing positions.
- `foundations/sector-coupling-derivation.md`—The Sector-Coupling Scale: $\kappa_s = \varphi^{-6}/v_0^2$. Derived scale with Hypothesized coefficient. Derives the Dirac↔two-fluid sector-coupling scale—the parameter behind the chemotactic mobility $\chi$—as $\kappa_s = \varphi^{-6}/v_0^2$ from three $\varphi$-rungs above the electroweak VEV (rung 77 ≈ 1.04 TeV), with the O(1) coefficient and the exact bridge to $\chi$ still open.
- `foundations/spin-fibonacci-spiral.md`—Spin as Fibonacci Spiral Winding: The SO(2) Doublet Fractal. Derivation. Derives spin as the accumulated SO(2) winding of the $(E_Y, E_I)$ doublet along a radial Fibonacci spiral, with boundary conditions on the condensed standing wave quantizing the winding to integer or half-integer values.
...
- `foundations/three-generations.md`—Three Generations from Fibonacci Cascade Partitioning. Hypothesized / Mapped. Derives three fermion families from the Fibonacci recurrence: $\varphi^n \approx \varphi^{n-1} + \varphi^{n-2}$ partitions every cascade span into three independent sub-rung positions, so Yukawa propagation from the GUT to the electroweak scale separates into three channels with zero free parameters.
...
- `foundations/wu-xing-derivation.md`—Wu Xing Number $w = 5$: Derivation from Cascade Dynamics. Derived / Calibrated. Derives the Wu Xing number $w = 5$—the number of phase-advance vertices in the primordial generation/control cycle—as the unique intersection of a cascade coherence criterion eliminating $w \geq 6$ and $\varphi$-geometry eliminating $w < 5$; the primordial gap $g = 1 - \varphi^{-5}$ and Yang-Yin ratio $r_0$ follow.
- `foundations/xi-derivation.md`—Derivation of $\xi = \varphi^6$. Derived rung identity / Calibrated empirical pin. Derives the Qi-gravity coupling $\xi = \varphi^6 \approx 17.944$ via the Fibonacci recurrence—six degrees of freedom (two field components × three spatial dimensions) each contributing a factor of $\varphi$—agreeing with the rotation-curve calibration ($\xi \approx 18$) to 0.3%.
...
- `standard-model/README.md`—Standard Model—Couplings, Gauge Structure, and CP from φ. Index. Six documents derive the Standard Model's gauge structure, couplings, loop corrections, and flavor sector from the Cassi $\varphi$-fixed point; the prescribed reading path starts with `sm-from-phi.md`.
...
- `standard-model/sm-from-phi.md`—Standard Model from φ. Derived. Derives the Standard Model from $\varphi$: gauge groups as successive truncations of the continued fraction $[1;1,1,\ldots]$, the Weinberg angle as the Yang/Yin VEV asymmetry $\sin^2\theta_W = \varphi^{-3}$, a $\varphi$-powered Yukawa hierarchy, the Higgs mechanism at the $\varphi$-point, quark confinement from Qi coherence, and the CKM phase.
...
- `standard-model/su2-gauge-extension.md`—SU(2) × U(1) Gauge Extension of the Cassi Two-Fluid. Derived. Promotes the two-fluid's U(1) ≅ SO(2) internal rotation to an SU(2) isospinor doublet whose $\varphi$-equilibrium VEV fixes $\sin^2\theta_W = \varphi^{-3}$ and $m_W/m_Z \approx 0.874$; derives the neutral-boson mass matrix, the SU(3) color extension, and the $\varphi$-scaled fermion hierarchy, with RG running from the GUT scale.
...
- `gravity/quantum-gravity.md`—Cassi Quantum Gravity: UV-Finite from σ-Regularized Two-Fluid Quantization. Derived / Hypothesized. Resolves Planck-scale breakdown of gravity: σ-regularized ($\sigma = \ell_{\text{Pl}}/\varphi^3$) Poisson emergence with a Gaussian propagator makes the theory UV-finite, and the hypothesized quantized extension makes the graviton a composite spin-2 SO(2) excitation with no modes beyond $1/\sigma$ and no renormalization at any order.
...
- `cosmology/cosmology-from-phi.md`—Cassi Cosmology: Inflation, Baryogenesis, and Dark Matter from φ. Derived (formation, structure) / Calibrated (w₀ coupling form—ledger). The same two-fluid dynamics in an expanding universe produce inflation ($n_s \approx 0.9691$), the baryon asymmetry ($\eta = \varphi^{-44}$), and the dark-matter budget with zero new parameters.
...
- `consciousness/chakras-as-cascade-bubbles.md`—Chakras as Cascade Bubbles: The 13-Node Derivation. Hypothesized. Closes the open phenomenological gap on the 13-band chakra count: chakras as localized Qi condensates at φ-spaced intervals, with $13 = 26/2$ from the human cascade span over the 2-rung SO(2) cycle.
- `consciousness/consciousness-from-phi.md`—Consciousness in the Two-Fluid Framework. Plausible Hypothesis with Actionable PDE Test. Maps verified physics onto consciousness: the Qi gate pinch at $r = \varphi^{-1}$ as self-awareness, wake waves as thought, $\sigma_r$ as the state variable, with a proposed-and-executed two-bubble PDE test and explicit boundaries to speculation.
