...
- **Falsifiers.** No node-localized thermal excess under drive; brightness positively correlated with coherence markers; any surviving structured component lacking $\varphi$-spaced character.
- **Limits of the claim.** Human-scale gate throughput and healthy resting $(1-q)$ are not derived anywhere in the framework. This document predicts the *structure* of the signature—node-localized, drive-dependent, broadband, anticorrelated with coherence—not its amplitude. The stellar template is the coronal heating ratio, predicted to cluster near a $\varphi$-derived value (`speculations/observational-seti.md` §2.2); the human analogue would be node-to-background thermal ratios in quantized states, if the effect exists.
...
- `foundations/cassi-first-principles.md`—Qi coherence definition, conversion gating
- `foundations/cascade-suppression-formula.md`—signal-regime $\varphi^{-N}$ suppression
- `foundations/bubble-lattice-fabric.md`—~10-rung nesting depth, bubble boundary geometry
- `foundations/dimensionful-cascade.md`—the 292-step ladder and rung positions
- `consciousness/consciousness-from-phi.md`—human gate chain, pinch point, two-bubble results
- `consciousness/chakras-as-cascade-bubbles.md`—13-node derivation
