...
def _registries(files):
    tests = _discovered_tests(files)
    scenarios = {name: list(paths) for name, paths in SCENARIOS.items()}
    runners = dict(SCENARIO_RUNNERS)
    commands = {name: list(command) for name, command in SCENARIO_COMMANDS.items()}
    if tests:
        scenarios["discovered-tests"] = [SCENARIOS["core-behavior"][0], RUNTIME_PATH, *tests]
    ...
    return tests, scenarios, runners, commands
...
def build_index(files):
    ...
    tests, scenarios, runners, commands = _registries(files)
    return {
        "schema": SCHEMA,
        "files": entries,
        ...
        "scenario_runners": dict(sorted(runners.items())),
        "file_count": len(entries),
        "symbol_count": sum(len(item["symbols"]) for item in entries.values()),
        "scenario_commands": {name: list(command) for name, command in sorted(commands.items())},
        "symbol_operations": [dict(operation) for operation in SYMBOL_OPERATIONS],
        "discovered_tests": list(tests),
        "test_commands": {path: ["$PYTHON", path] for path in tests},
    }