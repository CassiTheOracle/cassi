...
def _rename_candidate(files):
    ...
    imported = next(
        (
            ...
            if alias.name == "advance_field" and alias.asname is None
        ),
        None,
    )
...
def propose_candidates(files):
    candidates = []
    rename = _rename_candidate(files)
    if rename:
...
def apply_patch_set(files, patch_set):
    ...
    for change in changes:
        ...
        if path not in updated:
            raise ValueError("patch set path is outside workspace")
        if change.get("schema") != PATCH_SCHEMA or change.get("operation") != "replace_once":
            raise ValueError("patch set change is invalid")
        if change.get("base_source_sha256") != sha256_text(updated[path]):
            raise ValueError("patch set base hash mismatch")
        ...
        if not isinstance(find, str) or not find or not isinstance(replacement, str):
            raise ValueError("patch set text is invalid")
        if updated[path].count(find) != 1:
            raise ValueError("patch set change must match exactly once")
