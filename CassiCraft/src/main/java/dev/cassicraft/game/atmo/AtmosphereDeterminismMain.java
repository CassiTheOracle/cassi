 ...
 *       published snapshot (no mutation, never perturbs) — so the run-2 arm
 *       re-measures the same frozen snapshot and must equal run-1. Settle
 *       determinism is not re-proved here; it is hard-pinned byte-identically by
 *       the domainHarness gate (and by every mutating gate that still boots fresh).</li>
...
public final class AtmosphereDeterminismMain {
	...
	public static void main(String[] args) throws Exception {
		...
		// (identical result — the same frozen field), so the report prints once.
