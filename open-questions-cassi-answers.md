...
| Q2 | **Strong CP problem** | Why $\bar{\theta}_{\text{QCD}} < 10^{-10}$? | $\varphi$ as CP-conserving fixed point at GUT scale; $\bar{\theta} \approx 0$ is the attractor value | De-resonance principle: $\varphi$ is maximally CP-symmetric; deviations are Yukawa-mixing corrections | **Hypothesized** — mechanism identified, computation pending | `cp-violation.md` |
...
|------|-------|-----------|
| **Derived** ($\varphi$ + PDE consequence, zero freedom) | 10 | Dark energy $w_0$ (C1), dark matter $\xi$ (C2), flatness (C5), Big Bang singularity (C8), hierarchy (Q1), gauge unification (Q4), proton lifetime (Q9), quantum gravity (G1), BH singularity (G3), galaxy rotation (G4), DESI $w_0$ (T1), arrow of time (F2) |
...
**Total: 28 open questions mapped to Cassi answers.** Of those, 10 are derived — mathematical consequences of $\varphi$ and the PDE requiring no fitting. The remaining 18 have proposed mechanisms and testable predictions, with several (C1, C2, C4, C5, Q4, Q9, G4, T1, W1) already confirmed at the level of observational match or PDE verification.
