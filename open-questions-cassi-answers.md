...
| **Mechanism** | Qi gate $(1-q)$ modulates $H$ during ratio evolution; wake-wave mechanism imprints $\varphi$-scaled perturbations. Gate closure replaces fine-tuned inflaton potential. The closed formula subset has no additional free inputs after its named conditions; $r$ and the $N_e$ window remain Mapped, while the Weinberg boundary is separately asserted. $r \approx \varphi^{-12} = \xi^{-1} \cdot \varphi^{-6}$. |
