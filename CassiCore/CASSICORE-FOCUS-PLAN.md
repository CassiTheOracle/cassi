...
| **P3 — the spine + focused runtime, in parallel** | Build the spine extension (`pi.registerTool` mind tools, lifecycle mirroring, `appendEntry` snapshots, `mind_complete`, mnemonic backend adapter) against the retained mind runtime; stand up the thin mind host internal channel. | New spine/runtime suite (contract tests: registered tools, event bridge, backend store/search/status). Mind suites untouched. |
| **P4 — model-access cutover** | Replace retained `ModelHandle.stream` loops: run cognitive loops as ohmypi task agents (primary); route corpus/brainstem pure completions through `mind_complete` (fallback). Port `providers`/`ai` consumers out; delete the two packages. | `providers` 120 + `ai` 36 tests die with deletion (clean commit, history preserved). Mind suites updated for new model seam. |
