...
**Date:** 2026-08-14
**Status:** RATIFIED (owner 2026-08-14) — MIGRATION **COMPLETE**. P6 surface removal DONE (4 UI apps + 3 external bridges deleted); P7 DONE — `host` empty shell deleted (22 retained packages), the zero-import focus gate (`npm run verify:focus`) is the acceptance test and PASSES, docs finalized to the focused shape.
**Companion inputs:**
...
> **P5 shipped (2026-08-14):** `pipeline`, `commands`, `workers`, `plugins`, `jobs`, `mcp`, `admin-api`, `mcp-gateway` deleted; `host` reduced to an empty-shell placeholder (its retained brain folded into `@cassicore/mind-runtime` + the health read slice). **Final retained package count: 23** (22 functional + `host` empty-shell placeholder for P7 deletion): `mind-runtime`, `spine`, `helix`, `constellation`, `aurora`, `mnemic-field`, `flux-team`, `dreamer-reverie-subconscious`, `cognitive-feed`, `cortex-pineal-dialectic`, `mini-helix`, `workspace`, `embeddings`, `training-trust-ledger`, `lamina-locus-bridge`, `foundation`, `events`, `utils`, `workflow`, `model-pool`, `tools`, `thalamus`, `host`. Deleting host at P7 leaves 22. Full retained suite **2336 tests green**; typecheck 0 errors; zero-import guard clean.
> **P7 shipped (2026-08-14):** `host` empty-shell placeholder deleted (`git rm -r packages/host`); it only exported `HOST_RETIRED=true` and nothing imported it. **Final retained package count: 22** — `mind-runtime`, `spine`, `helix`, `constellation`, `aurora`, `mnemic-field`, `flux-team`, `dreamer-reverie-subconscious`, `cognitive-feed`, `cortex-pineal-dialectic`, `mini-helix`, `workspace`, `embeddings`, `training-trust-ledger`, `lamina-locus-bridge`, `foundation`, `events`, `utils`, `workflow`, `model-pool`, `tools`, `thalamus`. Full retained suite **2336 tests green** (host was passWithNoTests — no test loss). The zero-import **focus gate** (`npm run verify:focus`) is the migration's acceptance test and PASSES. See `MIGRATION-STATUS.md` for the final picture.
...
| **P5 — sessions cutover + memory backend** | **DONE (2026-08-14).** Sessions cutover + memory backend executed: moved the retained brain (`createIntelligence` + vendored `core/intelligence/**` + `orchestration-bus` + the turn/session/testing sibling modules) OUT of `@cassicore/host` into `@cassicore/mind-runtime` (its `src/vendor/` — the sole retained composition root, host-agnostic). Folded the retained admin mind-health read slice into `mind-runtime/src/health/` (cortex/pineal/thalamus/memory/replay/observability, read-only over the field). Extracted mcp-gateway's consolidated mind-tool schemas into `@cassicore/tools/schemas/mcp-consolidated` (helix consumer repointed); deleted the mcp server + admin-api surface + host standalone surface (daemon/entry/cli/bridge) + deleted `pipeline`/`commands`/`workers`/`plugins`/`jobs`/`mcp` + deleted `_reflect`/`_remember`/`remember`/`memory_search` (merged into ohmypi memory built-ins). Re-homed the `@cassicore/pipeline` overflow/classification helpers (`hasQuestionResult`/`buildToolUseMapFromMessages`/`contentLength`/…) into `@cassicore/utils`. | Mind suites: mind-runtime 22 → 25 (+ health, − redundant memory-tool body), spine 16, tools 39, helix 75, constellation 568, etc. stay green. Surface suites removed: pipeline 2, commands/plugins/jobs/mcp (passWithNoTests), host 17, admin-api 9 (surface), mcp-gateway 40 (server). Overflow helpers re-homed. |
...
| **P7 — verification + docs** | **DONE (2026-08-14).** Zero-import guard: assert no retained package imports any deleted/standalone-surface `@cassicore/*` package or the pre-migration bare `core/intelligence`/`core/daemon` layout. Wrote `scripts/verify-focus-gate.mjs` (node, no deps) + root `verify:focus` script — **PASSES (exit 0)**, the deliverable's acceptance test. Deleted the `host` empty-shell placeholder (22 retained packages). Updated `MIGRATION-STATUS`/`CASSICORE-FOCUS-PLAN` to the focused shape; `recon-*` + `P3-implementation-brief` left untracked as reference artifacts. | Full retained-suite run (2336 tests green; host was passWithNoTests); "focus gate" PASSES. |
