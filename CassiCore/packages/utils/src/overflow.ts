/**
 * Context overflow detection + message-classification utilities.
 *
 * Re-homed from `@cassicore/pipeline`'s turn/overflow.ts into @cassicore/utils
 * (P5) so the retained mind brain + @cassicore/thalamus share the helpers after
 * the standalone pipeline surface is deleted (CASSICORE-FOCUS §5 #21). Pure,
 ...
 *
 * When adding new patterns, keep in sync with the AI package's
 * `src/utils/overflow.ts` for consistency.
 */
