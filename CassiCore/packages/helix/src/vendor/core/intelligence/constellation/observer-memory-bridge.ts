/**
 * Observer Memory Bridge (vendor stub)
 *
 * Faithful port of CassiCore `core/intelligence/constellation/observer-memory-bridge.ts`.
 * Consumed by helix-synapse.ts as runtime (`ObserverMemoryBridge`,
 * `extractConceptHints`, `priorityToConfidence`) and type (`ObserverMemorySource`).
 * Depends only on `@cassicore/foundation`'s ILogger.
 */
 ...
 *
 * Pulls capitalised multi-word phrases plus quoted strings. Returned list is
 * deduped, lower-cased, and capped at 8 hints.
 */
...
export class ObserverMemoryBridge {
  ...
  /** Allow the sink to be wired up after construction. */
  ...
  /**
   * Emit a typed observer insight. Fans out to the Mnemic store and the
   * Claustrum sink. Idempotent within the dedup window.
   */
