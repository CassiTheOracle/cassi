...
export class RadianceLoop {
   ...
   */
  setHandleFactory(
    factory: (config: { tier: string; purpose: string; sessionId: string }) => Promise<import('@cassicore/model-pool').ModelHandle>,
    toolExecutor: import('@cassicore/tools').ToolExecutor,
    toolRegistry: import('@cassicore/tools').ToolRegistry,
    eventBus: IEventBus,
  ): void {
    this.observerRunner = async (prompt, toolSchemas, handlers, maxIterations) => {
      const { runSoloExplorer } = await import('@cassicore/constellation')
