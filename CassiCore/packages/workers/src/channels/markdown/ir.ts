 ...
 *
 * Ported from OpenClaw's src/markdown/ir.ts.
 * Uses markdown-it to parse GFM markdown into a structured IR with
 * text + style spans + link spans, which can then be rendered to any
 * format (HTML, Telegram HTML, etc.) via render.ts.
 */
...
function appendCellTextOnly(state: RenderState, cell: TableCell) {
  ...
  state.text += cell.text
  // Do not append styles — used for code blocks where inner styles would overlap
}
 ...
 * at newlines or whitespace boundaries.
 */
