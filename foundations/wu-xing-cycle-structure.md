...
- The control-release rule as the gate's ke coupling: `gate_model='five_ke'` reproduces the derived ring algebra to ≤ 6×10⁻⁴; excess channels restrain ke targets and release ke partners; the ring is sub-critical (no driverless self-sustenance); the φ-phased drive still dissolves the displaced site at the held configuration (§4 WX3, `two-fluid/run_trauma_ke_ring.py`)
