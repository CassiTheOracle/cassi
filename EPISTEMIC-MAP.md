...
| `foundations/proton-coherence-budget.md` | Derivation (rung exponent Mapped—ledger; per-rung $q_i$ profile Hypothesized) | Conditional coherence-budget proton lifetime $\sim \varphi^{4506}$ cycles; organized anti-phase annihilation pathway is Hypothesized, with the canonical density PDE leaving that pathway open |
...
| `foundations/bubble-edge-geometry.md` | Derived geometry and threshold relation conditional on asserted gate | The directional edge proxy is $R(\theta)=\frac{\sqrt{1+\varphi^2}}{2}\sqrt{\frac{1+\theta}{\theta}}$, equal to $1.7072\times$ only at selected $\theta_{\mathrm{cond}}=0.45$ and variable with $\theta$; no $C=0.45$ edge survives the fixed-step PDE endpoint and the cosmological boundary receipt is null; threshold and any biological/cosmological mapping remain conditional on independently identified boundaries and $g(q)$, never universal, zero-parameter, canonical, or PDE output; CMB axis $12.2°$ (magnitude $2\pi/\varphi^7 = 12.40°$ Derived; direction Calibrated; boundary mechanism Hypothesized, orientation fitted to measured axis); radial ring ladder §3 remains Derived conditional with the radial-reading inference flagged |
...
| `principles/de-resonance-principle.md` | Derived (number-theory identity) / Hypothesized (physical attractor/de-resonance mapping) | Why $\varphi$ is maximally irrational; the physical attractor and de-resonance mapping remain Hypothesized |
...
| `foundations/unified-lagrangian.md` | Hypothesized—August 2026 | Optional extended action assembly around the canonical real-density pair and rank-one conversion; Dirac/particle, GR, SM, and cross-coupling sectors remain conditional; the named C-class/framework convention $\lambda=0.1$ is Asserted while the implementation class default is $\lambda=0.02$; $\lambda=1/(2w)$ is a Hypothesized Wu Xing linkage, and $c,\hbar,G$ remain external |
...
| `foundations/quantum-measurement-derivation.md` | Derived (coherent-field statistics) / Hypothesized (Born/branch mapping); outcome basis open—August 2026 | Single-rung coherence-budget mechanism and phase-matching factor $\mathcal{M}$; Born probability and branch-selection mapping remain Hypothesized, with the outcome basis open |
...
| `gravity/quantum-gravity.md` | Derived conditional on the noise–signal identification ($\sigma = \ell_{\text{Pl}}/\varphi^3$, G1) / Hypothesized (two-fluid quantization, G2; geometric $d=3$ identification) | $\sigma$-regularized S-matrix; $\delta = 3$ from the Planck-core noise–signal crossover; the geometric reading $\delta = d = 3$ is conditional and Hypothesized; black-hole information (G2) |
...
| `hypotheses/exoplanet-phi-spacing.md` | Hypothesized | Disk mechanism = the bubble-shell ring ladder ($r_k = R\varphi^{-k}$, Derived conditional); DSHARP gap-ratio design is registered but has no observation-level verdict because input and run receipts are absent; period-ratio prediction $\varphi^{3/2}$ remains registered without a current catalog result; channel reading: disk gas is the coherence-coupled channel, while the orbital/matter channel is not expected to carry the signal |
...
| `parameter-inventory.md` | Parameter registry (48 parameters: 1 F + 7 D + 5 C + 11 M + 9 E + 7 I + 8 N) |
