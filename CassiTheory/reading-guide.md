...
Section 78 closes the continuum question left by the incomplete QCD-anchored
chiral quench. A generic winding-changing zero of its linear $O(4)$ field has
normalized-Skyrme energy diverging as $3\pi^2/(4|\tau|)$ on approach. On the
zero-time slice every rank-three spatial Jacobian carries an inverse-radius
coefficient at least $2\pi$. The simulation's max-normalization instead gives
$4\pi/\epsilon$, identifying a regulator-dependent modified action. Two
independent calculations pass all four numerical gates and return
`CONTRADICTS` for continuum topology-changing use of the selected action.
Prepared textures and regular microscopic quark-bearing completions remain
outside that restriction. Read
...
results. The empirical action, interacting vacuum, nonradial persistence,
thermal formation rate, observable nucleon map and baryogenesis remain outside
that calculation. Read `foundations/matter-completion-boundary.md` §24 for the
compact boundary. Its finite-cutoff confining completion is in Section 81.
...
Section 81 adds color dynamics to the regular carrier. Renormalized
two-flavour QCD is the empirical target; a finite-cutoff chromodielectric
quark–meson action is the tractable bridge. The action defines the
two-flavour fermion sea, remains regular at a chiral zero and preserves local
$SU(3)_C$ and exact baryon number. Every nonzero total Cartan charge has
energy growing without bound with volume, while a color-neutral three-arm
...
- `foundations/loop-rate-selection-candidates.md`—A Search for a $\varphi$-Free Selection Principle for the Loop's Conversion Rate Ratio. Analysis—September 2026. Tests thirteen candidate functionals on the loop's own data: four families are closed by structural reasons that hold for every member, the three routes that return $\varphi$ each import an input, the fivefold selector's integer $5$ is the same unforced input as the ratio, tied by $2\cos(\pi/5)=\varphi$, $\operatorname{disc}(x^2-x-1)=5$, $\sqrt5=2\varphi-1$ and $Q(\zeta_5)^+=Q(\sqrt5)$, and a cross-check finds four documents that read the ratio, or evidence for it, from a rate.
- `foundations/loop-to-bubble-projection-theorem.md`—Loop-to-Bubble Projection Theorem: Shared-Support Counterflow, Coherence, and Scale Separation. Derived conditional projection and population spectrum; Derived regulated pure-gauge identities, exact fixed-boundary gauge-fibre support, exact isolated-square radial Feshbach transfer and weak-coupling character-cutoff theorem, exact character-cutoff form theorem on every fixed finite graph, volume-uniform local character-cutoff density theorem with a global-norm obstruction, conditional fixed-regulator thermodynamic ground-state and Euclidean reflection-positive Gibbs subsequences, exact fixed-graph anisotropic Wilson transfer-to-Hamiltonian limit, exact fixed-link three-sphere geometry, two-scale conditional-score recurrence, $H^{-1}$ transport-score margin transfer, residual-recovery Gramian and score-penalty separation, conditional full-holonomy block theorem, exact bare-cylindrical refinement obstruction, finite positive-transfer reducing-subspace criterion, conditional local Wilson-word/GNS vacuum-orthogonal density, and the uniform lower-form/recovery criterion; Hypothesized microscopic physical identification—September 2026. Four carrier populations project to the canonical PDE and their coherence fills the affine bubble volume. In the separate source-free $SU(2)$ comparison, exact finite gaps are true-vacuum gauge-invariant Poincaré rates. Haar path blocking is Haar-isometric with exact path-length-weighted electric compression, while magnetic characters exclude the bare cylindrical full-Hamiltonian block. The isolated-square radial problem has an exact Feshbach pencil and sharp bare-cutoff requirement $gN\to\infty$. Fixed-graph character cutoffs converge in form and low spectrum, while local cutoff errors are uniform in periodic volume and global wavefunction norm is not. Conditional compactness gives fixed-regulator Hamiltonian and Euclidean subsequential states. For the separate anisotropic trajectory, the normalized symmetric Wilson transfer has difference and logarithmic generators converging in strong-resolvent sense to (YM3) on every fixed finite graph, and its products converge to the heat semigroup. The exact bare map is $g_W=2^{1/4}g$, $a_\tau=\epsilon/\sqrt2$. The universal two-loop Wilson scale requires $N F_W\to\infty$ and $\delta_W/F_W$ bounded above and below. The conditional RG endpoint theorem derives that gap scaling from an $O(1)$ cumulative scale defect, exact correlation-preserving blocking, complete retained physical channels and a fixed positive interacting endpoint gap; its 139/139 primary and 32/32 independent arithmetic checks construct none of those hypotheses. The finite-transfer and local Wilson/GNS results are conditional finite-algebra boundaries. The uniform lower-form bound, strong recovery/form transport, physical scale matching, uniform interacting-vacuum recovery, transport, fibre and coarse-margin control, identification of the fixed-$\beta$ Euclidean state with the anisotropic family, spatial-volume uniformity, the cumulative RG bound, the correlation-preserving block map, the interacting endpoint gap, continuum existence, a regulator-independent mass gap and Cassi identification remain open. See `field-experience/probe-outcome-ledger.md` §§42–44 and `foundations/loop-to-bubble-projection-theorem.md` §§9.31–9.34.
...
- `foundations/interscale-current-soliton.md`—Interscale Yang/Yin Current and the Conditional Soliton Pinch. Action and Wilson extension Hypothesized; conditional current, endpoint, transport, support and phase-current identities Derived; carrier coupling Mapped; finite-grid spectrum, rotational and phase-coercivity controls, and conditional smooth scalar binding Tested—September 2026. Separates the scale-current mechanism from the ultraviolet Cartesian branch and prepared scalar binding. Empty-sector invariance, INCONCLUSIVE smooth constrained spatial stability and physical matching delimit the matter claim.
  Its §12 derives positive collective phase inertia, the constrained
  longitudinal branch, causal exterior memory and a Mermin–Ho rotational
  projection. Surrounding population and correlations enter local initial
  data; boundary-induced frequency gaps remain conditional. The 84-check
  helical follow-up excludes static first-order phase energy as an enstrophy
  or critical-residual bound while leaving whole-field dynamical control open.
  The formation calculation and its physical limits are in
...
Conditional turbulence spectra, exact Navier–Stokes transfer identities,
geometric regularity questions, adaptive positive-metric,
forward-deformation covariance and vorticity-seeded active-occupation
reductions, scalar-Beltrami and time–curl residual reductions, a bounded Cassi
fluid-feasibility study, an LTE radiative-material branch, and a compressible
species-resolved multi-angle extension with accounted stellar energy sources.
The initial-layer rate analysis then quantifies the order mismatch between
covariance-volume recovery and seeded stretching.
...
The longer-horizon extension uses the same four controls at $T=1/2$ and
$N=2,4,8,16$ under
`computations/navier-stokes-galerkin-long-trajectory-prereg.md` and
`computations/verify_navier_stokes_galerkin_long_trajectory.py`; its 48-run
...
- `turbulence/navier-stokes-strain-departure.md`—Strain Departure and Critical Spectral Concentration. Derived conditional estimates, helical reduction and cumulative mixing obstruction / Open arbitrary-data critical work—September 2026. Couples energy and enstrophy to sharpen departure; 71 checks. Its exact radial spectral-spread dissipation and centered production yield a cumulative positive-production continuation criterion; the cyclic datum generates production from zero spread and the scalar-budget obstruction remains; 134 checks. The forced extension gives exact source budgets and the compactness boundary of vanishing-source magnification; 215 checks in each separate evidence record. An exact globally smooth mixing family excludes amplitude-linear cumulative bounds for critical transfer, spread dissipation and positive spread production, with a continuum comparison proof and a finite nonlinear critical-transfer bound within the family; 601 checks. The signed curl spectrum yields the critical residual $\omega-Hu/(2K)$: finite squared $L^3$ time integral controls enstrophy and radial positive production. Its 84-check follow-up constructs a positive one-band doublet with bounded first-order phase energy but unbounded enstrophy and critical residual. Initial-$H^3$ control of either critical time integral, whole-field dynamical exclusion, global regularity and unforced blow-up remain open.
- `turbulence/navier-stokes-second-order-field-energy.md`—Second-Order Field Energy and a Time–Curl Continuation Criterion. Derived field and Navier–Stokes identities / Derived conditional connection-sector map / Open arbitrary-data residual bound—September 2026. Derives a nonnegative weighted energy for the continuous-time continuum Yang/Yin family associated with the live shader's source-free coupling; the extent-dependent unit-coefficient finite-difference runtime carries no exact time-step conservation claim. It also derives a dimensionally converted connection-sector functional on an Abelian one-color, temporal-gauge, charge-free, scale-flat slice and an exact unforced Navier–Stokes balance. Finite $L_t^2L_x^3$ time–curl residual work implies continuation; the 110-check audit-qualified receipt passes. A uniform fixed-$c$ estimate on bounded mean-zero $H^3$ data balls remains open.
- `turbulence/navier-stokes-adaptive-metric.md`—Adaptive Positive Metrics for Navier–Stokes Vortex Stretching. Derived exact weighted balances and conditional continuation reduction / Open active-distortion bound—September 2026. Derives scalar, material, terminal-adjoint and forward-parabolic SPD metrics that cancel weighted vortex-stretching production. Positive propagation and the unscaled determinant floor are analytic smooth-interval results; 48 fixed-control checks pass across two schedules. Local algebraic controls exclude coercivity inferred from positivity or determinant alone. A uniform initial-$H^3$-controlled active-distortion bound would imply continuation and remains open.
- `turbulence/navier-stokes-deformation-covariance.md`—Deformation Covariance and Covariant Enstrophy in Navier–Stokes. Derived exact covariance, inverse-metric and projection identities / Open active Rayleigh-quotient bound—September 2026. Under stated compact-interval stochastic assumptions, the Constantin–Iyer forward deformation generates a closed covariance and inverse-covariance metric whose weighted enstrophy law has one nonnegative covariant-square dissipation. The same law is an orthogonal-regression identity for sampled initial vorticity. The audit-rechecked source-bound schedule passes 40 exact symbolic and fixed-control checks, including the citation-derived Stratonovich–Itô conversion; stochastic-flow and Navier–Stokes trajectory integration remain outside its executable scope. The finite-ensemble projection and inverse-Jensen group is **SUPPORTS**, while two scalar checks are only prototypes for the analytic endpoint bridge. Homogeneous extension excludes algebraic coercivity, periodic shear isolates inactive covariance, and a uniform active-quotient bound over every mean-zero, divergence-free initial-$H^3$ data ball remains open.
- `turbulence/navier-stokes-active-deformation-occupation.md`—Vorticity-Seeded Deformation Occupation and Active Cascade Dose. Derived exact seeded-covariance, occupation and conditional continuation identities / Open uniform active-dose bound—September 2026. The common-noise second moment seeded by $\omega_0\omega_0^{\mathsf T}$ obeys a closed equation, dominates enstrophy and equals an exponential directional-strain occupation selected by initial vorticity. A reachable-direction Khasminskii condition and summable positive active dose imply continuation. Forty source-bound component checks cover polynomial identities, finite fixtures, local controls and exponents; the stochastic-flow and continuation arguments remain analytical. Generic energy-class-strain-only occupation control and geometric-spacing-only dose decay are contradicted; the uniform all-data dose bound remains unresolved.
- `turbulence/navier-stokes-replica-coherence.md`—Replica Coherence and Viscous Compensation in Navier–Stokes. Derived exact replica, covariance, temporal-rank and conditional continuation identities / Open recovered-envelope and cross-scale bounds—September 2026. Two independent stochastic Cauchy replicas recover enstrophy through overlap and covariance through disagreement. The covariance source is twice viscous palinstrophy and its Duhamel law retains every source time and subsequent directional deformation. Applying the determinant root after source histories accumulate gives a temporal-rank functional stronger than the instantaneous determinant integral and a sharpened enstrophy envelope. An exact rank-two periodic control develops full-rank covariance on an open set while the instantaneous determinant functional remains zero; periodic shear retains a common missing direction. The continuation bridge is explicit: a bounded enstrophy envelope gives $u\in L^4_tL^6_x$ by the periodic kinetic-energy identity and Sobolev embedding, so Prodi–Serrin supplies the finite-endpoint step. An indexed common-noise kernel diffuses only in the center coordinate; an absolute diagonal bound would control the active quotient, while increment bounds leave spatially constant extension unbounded. Sixty source-bound symbolic and exact-control checks pass; a separate source-bound Galerkin endpoint-control schedule passes 8 of 8 finite-mode checks. Uniform production-relative recovery, signed shell-stretching, recovered-envelope, cutoff-uniform Galerkin time-integral, and arbitrary-data regularity bounds remain open.
- `turbulence/navier-stokes-covariance-recovery-rate.md`—Initial-Layer Rate Boundary for Covariance Recovery. Derived—September 2026. The active rank-deficient control has $\mathcal K(t)=6\nu^{4/3}\langle f^{1/3}\rangle t^{4/3}+o(t^{4/3})$ while its seeded occupation grows by $\frac12t+O(t^2)$. The ratio diverges like $t^{-1/3}$, so a finite recovery-only coefficient cannot close the initial layer; the full data-uniform continuation estimate remains open.
- `turbulence/navier-stokes-near-rank-recovery-obstruction.md`—Near-Rank Full-3D Obstruction to Uniform Covariance Recovery. Derived—September 2026. A uniformly bounded smooth family has full-rank initial vorticity-gradient source almost everywhere for every positive perturbation, but its determinant-root scale is $A_\varepsilon\sim A_*\varepsilon^{2/3}$ while production stays $1/4$. The recovery-only coefficient diverges; the independent schedule passes 7 of 7 checks, and production-relative recovery plus arbitrary-data regularity remain unresolved.
- `turbulence/navier-stokes-helical-dynamic-depletion.md`—Helical Geometry and the Dynamical Depletion of Vortex Stretching. Tested finite helical families / Universal sign depletion contradicted / Open scale-uniform direction-strain estimate—September 2026. A 24-run Fourier–Galerkin matrix holds the exact Beltrami control at $P=0$ while the five helical-tube families, including opposite-handed tubes with normalized helicity near $10^{-18}$, are positive at every reported checkpoint from $t=1/8$ onward. The universal sign statement is contradicted for the declared families; the scale-uniform direction-strain estimate and global regularity remain open.
- `turbulence/cassi-fluid-feasibility.md`—Cassi Fluid Mechanics: Conservative Reduction, Thermal Closure, and Phase Currents. Derived conditional mechanical, thermal, and phase-current identities / Tested solver, rotational and phase-coercivity boundaries / Open physical-fluid completion—September 2026. Derives first-order pressure, counterflow and quantum stress with supplied mass. The native solver passes 246 checks across 28 trajectories while its mean self-acceleration rejects a closed internal-force interpretation. A selected capillary/thermal model closes momentum, energy and entropy and passes 395 checks across 27 model trajectories plus an independent matrix reference. The phase-current reduction adds an explicit rotational class; its separate helical follow-up excludes static first-order phase energy as an enstrophy or critical-residual control. Physical identification, microscopic viscosity, whole-field dynamical concentration control, arbitrary-flow closure and global regularity remain open.
- `turbulence/cassi-fluid-phase-current-hydrodynamics.md`—Cassi Fluid Phase Currents: Rotation, Helicity, and the Viscosity Boundary. Derived conditional current and topology identities / Tested rotational, memory and first-order coercivity boundaries / Open microscopic viscosity and arbitrary-flow closure—September 2026. Derives Mermin–Ho vorticity, the one-chart helicity obstruction, full-doublet Hopf helicity and a two-scale-band periodic Beltrami field. Its 227 checks support exact scalar-diffusion/viscosity equivalence for that fixture, contradict the general identification, and verify the recurrent closed-exterior memory boundary. A separate 84-check follow-up proves that bounded first-order positive-doublet energy does not control enstrophy or the critical scalar-Beltrami residual.
...
- `turbulence/compressible-radiative-plasma-closure.md`—Compressible Radiative Plasma and Stellar-Light Closure. Derived conditional / Tested reference controls and integrity qualification—September 2026. Closes mass, momentum and total energy with pressure work and shocks; constrains species mass to level populations; provides line and continuum transfer, contraction, accretion, nuclear power and neutrino loss; and preserves crossing beams with discrete ordinates. Its source-snapshotted schedule passes 70 of 70 checks, and a separate 36-check integrity qualification covers defensive state construction, thermodynamic identities, physical-frequency Doppler normalization, rejection boundaries, exchange cancellation, complete stellar and nuclear ledgers, and missing-prerequisite classification. Physical Cassi material identification, evaluated-data qualification and a production CassiCosmos solver remain open.
