...
- `foundations/geometric-manifold-completion.md`—Cassi Geometric Manifold Completion Ansatz. Hypothesized completion ansatz / Derived canonical reduction and conditional fibre geometry—September 2026. Places the canonical density diameter, loop-derived positive coherence ball, rank-one projective shell, affine bubble image, and cross-glued two-rail scale circuit in one stratified bundle. Its minimal completely positive conversion lift reproduces canonical populations exactly and conditionally gives $\gamma_c=\gamma_\varepsilon/2$. Charged coherent and one-way open endpoint realizations are available conditionally. Point-core Chern flux supplies an exterior support coefficient, while the magnetic core, finite-energy stationary solution, physical endpoint normalization, scale metric, reservoir, observation map, and particle quantum numbers remain open.
- `foundations/endpoint-link-and-localization-boundary.md`—Gauge-Covariant Endpoint Closure and the Localization Boundary. Derived conditional endpoint closure / Derived minimal-sector localization no-go—September 2026. A charge-$-g_Q$ vertex section supplies coherent gauge-covariant Yang/Yin turning with a finite current capacity; a one-way Markov alternative gives $\gamma_-/\gamma_+=\varphi$ in the uniform circuit while damping endpoint coherence. The full coherence fibre is contractible, the smooth object base has $H^2=0$, and the endpoint terms supply no positive $1/R$ core support. The point-core branch has a derived exterior coefficient and a current-action finite-energy obstruction.
- `foundations/point-core-flux-sector.md`—Quantized Point-Core Flux and the Persistent-Defect Boundary. Derived conditional exterior support / Derived current-action completion no-go—September 2026. Derives the sharp fixed-flux coefficient $\mathcal B_G=2\pi N_G^2\int d\mathfrak s/e_x^2$, the support condition $\mathcal B_G>\mathcal D$, the reduced radius, and its positive scaling curvature. The Abelian Bianchi identity and charged-condensate angular spectrum exclude a smooth isolated finite-energy point defect in the current action, so the stationary solver and full fluctuation spectrum await a magnetic completion.
...
- `foundations/interscale-current-soliton.md`—Interscale Yang/Yin Current and the Conditional Soliton Pinch. Hypothesized action / Derived conditional endpoint and localization boundaries—September 2026. Introduces a complex doublet on a separately normalized continuous scale coordinate, derives exact species and total continuity plus co-flow/counterflow identities, and identifies a conditional mixed-curvature force channel. On the Mapped Planck-to-proton interval, equal-and-opposite Yang/Yin currents form a closed relative-current circuit. Coherent charged and one-way open endpoint realizations are explicit; the smooth zero-Chern sector has no finite radius, and point-core flux supplies a conditional exterior coefficient without completing a finite-energy particle.
...
- `foundations/proton-coherence-budget.md`—Proton Coherence Budget and Planck-to-Proton Scale Circuit. Mapped proton coordinate / Derived conditional arithmetic / Hypothesized physical mechanisms. Records the independent-step coherence-product model and, in §10, a distinct Planck-to-proton two-rail current with zero total scale-number flow and nonzero relative current. Conditional endpoint equations and the point-core exterior coefficient are explicit; physical normalization, scale tension, magnetic core support, proton quantum numbers, and decay rate remain open.
