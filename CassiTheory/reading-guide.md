...
- `foundations/loop-to-bubble-projection-theorem.md`—Loop-to-Bubble Projection Theorem: Shared-Support Counterflow, Coherence, and Scale Separation. Derived conditional projection and population spectrum; Derived regulated pure-gauge identities, exact fixed-boundary gauge-fibre support, exact isolated-square radial Feshbach transfer and weak-coupling character-cutoff theorem, exact fixed-link three-sphere geometry and two-scale conditional-score recurrence, conditional full-holonomy block theorem, and exact bare-cylindrical refinement obstruction; Hypothesized microscopic physical identification—September 2026. Four carrier populations project to the canonical PDE and their coherence fills the affine bubble volume. In the separate source-free $SU(2)$ comparison, exact finite gaps are true-vacuum gauge-invariant Poincaré rates. Haar path blocking is Haar-isometric, preserves endpoint covariance and has exact path-length-weighted electric compression, while magnetic characters exclude the bare cylindrical full-Hamiltonian block. The isolated-square radial problem has an exact Feshbach pencil and sharp bare-cutoff requirement $gN\to\infty$. A round spatial $S^3_R$ has free coexact frequency $2/R$, which vanishes as $R\to\infty$ and is not the interacting Hamiltonian gap. Each fixed-metric $SU(2)$ link has exact character spectrum; the auxiliary one-link Wilson-weight Bakry–Émery bound holds only for concentration $\beta<2$, with no identification to the Hamiltonian coupling or exact vacuum. A factor-two Haar-preserving horizontal/fibre split gives an exact all-function conditional-score recurrence; its zero-score form is $\lambda_f\geq\min\{2\lambda_c,\lambda_{\mathrm{fib}}/2\}$ and it lower-bounds the gauge-invariant physical rate. The primary verifier passes 118 checks; a 90-check implementation-independent receipt audit re-evaluates the fixed primary schedule. Uniform weak-coupling fibre, score and coarse-margin control, continuum existence, a regulator-independent mass gap and Cassi identification remain open.
