...
### 9.33 Local gauge-invariant observable completeness

The completeness premise can be stated without selecting a finite list of
Wilson loops. Let $\Gamma=(V,E)$ be a finite connected graph and let

$$
\mathcal A_\Gamma=C(SU(2)^E)^{SU(2)^V}
$$

be its continuous gauge-invariant cylinder algebra. Let
$\mathcal W_\Gamma$ be the unital *-algebra generated by the fundamental
Wilson traces

$$
\tau_\ell(U):=\operatorname{tr}(U_\ell)
$$

over all closed oriented graph paths $\ell$, with products and inverse
oriented paths included. The finite-graph density statement is

$$
\boxed{
\overline{\mathcal W_\Gamma}^{\|\cdot\|_\infty}
=\mathcal A_\Gamma.
}
\tag{YM259}
$$

To see the structure, gauge-fix a rooted spanning tree. The tree links become
the identity and the chord variables are a tuple
$(h_1,\ldots,h_r)\in SU(2)^r$ modulo simultaneous conjugation. Fundamental
matrix coefficients are dense by Peter–Weyl. Every finite-dimensional
$SU(2)$ representation is generated by the fundamental doublet, and its
invariant tensor contractions are built from $\delta$ and $\epsilon$. The
pseudoreality identity
...
reduce a closed fundamental contraction to a finite linear combination of
products of traces of words in the chord variables. These word traces are
invariant under simultaneous conjugation. Their uniform closure is therefore a
unital, *-closed algebra containing the invariant Peter–Weyl span and equals
$\mathcal A_\Gamma$. Equivalently, the word traces separate the compact
gauge-orbit space and Stone–Weierstrass gives (YM259).

For the infinite cubic lattice, define the local gauge-invariant algebra by
...
For every positive state $\omega$ on this algebra, its GNS construction
satisfies
...
=\bigl(\pi_\omega(A)-\omega(A)I\bigr)\Omega_\omega
+\omega(A)\Omega_\omega
$$

shows that centered local gauge-invariant vectors are dense in the
...
This removes one ambiguity in (YM252). If the retained family is declared to
be the full centered local gauge-invariant algebra, completeness is an
algebraic consequence of the GNS construction. A finite Wilson list, a fixed
block ansatz, or an RG family whose closure has not been identified with
$\mathcal A_{\mathrm{loc}}^G$ does not inherit this conclusion.

The remaining spectral target can now be written directly on a dense core. If
$H_{L,a}$ is the finite-volume physical Hamiltonian and $v$ is any centered
local vector, a uniform lower bound of the form

$$
\boxed{
\langle v,aH_{L,a}v\rangle
\geq c_-F_W(g_0)\|v\|^2,
\qquad c_->0,
}
\tag{YM261}
...
along a weak-coupling, growing-volume trajectory extends by closure to the
full vacuum-orthogonal form domain. A nonzero local vector with the matching
upper scale supplies the finite upper witness in (YM252). Thus the unresolved
dynamical problem is a uniform lower form bound together with the exact
correlation transport that identifies the retained family with the full local
algebra; completeness itself is no longer an unspecified choice of finite
observables.

The frozen protocol
`computations/yang-mills-local-observable-completeness-prereg.md` checks the
finite algebraic controls behind (YM259)–(YM260) on a four-cycle, a
three-edge theta graph and a three-loop bouquet. Tree reduction, normalized
word traces, character recursion, the $SU(2)$ contraction identities and the
orientation-sensitive three-word witness pass **36/36** primary checks. The
independent reconstruction passes **41/41** decisions, comprising the same
36 scientific checks, four mutation firings and the primary-receipt binding
audit. The receipts classify the finite algebra/GNS statement as
**DERIVED CONDITIONAL** and retain the RG retained-family identification,
...
The nested-cutoff screen extends this test over all ten pairs
$(c_P,c_Q)$ with $1\le c_P<c_Q\le5$. The forty-row primary calculation
passes $403/403$ controls, and the independent arithmetic and source-binding
audit passes $370/370$ checks. Its classification is
`NO_POSITIVE_FAMILY_CERTIFICATE`: positive Schur roots occur in $36/40$ rows.
The four rows without a positive root are all at $x=16$, with pairs
$(1,4)$, $(1,5)$, $(2,5)$ and $(3,5)$; their
$\beta^2/(\alpha\delta_Q)$ ratios are $1.08018$, $1.21046$, $1.27583$ and
$1.02498$. At the same coupling, the pairs $(2,4)$, $(3,4)$ and $(4,5)$
recover positive roots, so the finite result shows retained-cutoff dependence
and motivates testing a growth schedule for the interacting fibre. The screen
reaches outer cutoff $5$ on this fixed graph; its constants remain
finite-family data.
...
| Reserved-plaquette volume-adapted Feshbach family (§9.37) | **INCONCLUSIVE** finite-volume measurement (`SUPPORTS_FINITE_VOLUME_RESERVED_PLAQUETTE_FAMILY`) | $84/84$ primary and $85/85$ independent checks pass with positive roots on both graphs; retained rank and coupling control remain finite-volume evidence |
