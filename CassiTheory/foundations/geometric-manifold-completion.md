...
## Status: Hypothesized completion ansatz / Derived canonical reduction and conditional fibre geometry—August 2026
...
positive coherence fibre, and a declared open-system lift. These additions are
not consequences of the canonical PDE. The physical scale metric, endpoint
microphysics, localized solution, particle quantum numbers, observation map,
and decay rate remain unselected.
...
| Endpoint closure | Gauge-covariant cross-rail intertwiners represented by flux-unitary boundary phases $\delta_-$ and $\delta_+$ | Hypothesized boundary condition |
...
flux, impose a boundary condition, or supply another invariant defined on the
full fibre. The pure-$CP^1$ obstruction remains valid inside its declared
rank-one sector.
...
finite spatial radius additionally requires flux/core support. The completion
...
| Endpoint phase gluing and Kirchhoff current conditions | Hypothesized boundary completion |
...
| Scale tension and endpoint selection | Open |
...
| Stable spatially localized solution | Open |
...
3. endpoint fields or vertex channels that realize (GM11)–(GM12);
...
5. a relative-connection action and boundary state that produce the required
   mixed curvature;
6. a short-distance core or flux sector that prevents collapse;
7. a finite-energy solution localized in $M_3$ and compact through scale;
...
without identifying it with a spatial torus. Its endpoint phases and scale
length remain boundary data. Mixed curvature supplies a conditional
localization route, while a stable finite-radius solution and all particle
