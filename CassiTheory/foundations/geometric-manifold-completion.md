...
## Status: Hypothesized completion ansatz / Derived canonical reduction and conditional fibre geometry / Mapped selected carrier coupling / Tested finite-grid spectrum and conditional smooth scalar binding—September 2026
...
positivity-preserving two-jump lift reproduces the canonical population
...
endpoint section, its separately declared Wilson-dressed inter-vertex
extension, and a one-way Markov vertex channel supply conditional realizations
of the endpoint closure in
`foundations/endpoint-link-and-localization-boundary.md`. Their physical
normalization, scale metric, endpoint microphysics, Wilson-link coefficient,
observation map, and decay rate remain unselected. The smooth zero-Chern
endpoint sector has no
...
(`foundations/particle-stationary-action-closure.md`). The stored Cartesian
branch is a finite-grid observable across the $N=17,21,25,29$ sequence and
one larger-domain comparison. It retains a localized, nodeless carrier field;
the six-mode constrained $C_4$ PA42 spectrum contains one numerically
near-zero carrier-phase mode, no negative mode, and five positive modes. Its
finest field nevertheless places
$99.98631608\%$ of the carrier norm on one parity sublattice, while the edge
gradient energy grows like $\Delta x^{-2}$. The frozen verdict is
**CONTRADICTS—smooth-carrier interpretation on the measured sequence**. The
finite-grid observables and six-mode spectrum remain valid for that
discretization; they do not establish continuum stability or full Hessian
positivity.

Separately, the continuum-consistent scalar calculation at the same Mapped
$h_C=2.9598260763447164$ finds prepared static binding for $Q_C=16$ and
$256$, independently reproduced by collocation. The tested $Q_C=4$ profiles
spread with the domain, while $Q_C=64$ is inconclusive because its residual
fails qualification. The constrained smooth $Q_C=16$ spatial study resolves
no negative mode on four tested grids, but its combined verdict remains
**INCONCLUSIVE** because the coarse translation mode and same-spacing domain
comparison fail the frozen criteria. Under closed boundaries the supplied
first-order carrier action obeys $dQ_C/dt=0$ and
$Q_C(0)=0\Rightarrow\chi_C(t)=0$; it therefore supplies no creation
mechanism from the empty carrier sector. Continuum existence, formation,
normalization, quantum numbers, statistics, nonlinear stability, and physical
particle identification remain open. Source measurements and decisions are
recorded in `computations/matter-formation-continuum-report.md`.
...
| Endpoint closure | Gauge-covariant cross-rail intertwiners represented by flux-unitary boundary phases $\delta_-$ and $\delta_+$; charged coherent sections, their Wilson-dressed inter-vertex extension, or trace-preserving vertex channels provide conditional realizations | Hypothesized physical endpoint and Wilson coefficient / Derived conditional covariance, source, transport, and conservation algebra |
| Conversion off the diagonal subcone | Minimal positivity-preserving two-jump lift; fixed-$q$ generator is linear GKSL | Hypothesized lift / exact canonical diagonal reduction |
...
scale tension, gauge normalization, Wilson hopping $t_\Upsilon$, bath
normalization, and physical scale measure retain their open status.
...
second-order particle action remains a separate temporal sector. Its stored
Cartesian finite-grid branch retains the original six-mode $C_4$ spectrum, but
the finest field is parity-concentrated: $99.98631608\%$ of its carrier norm
lies on one parity sublattice and the edge energy grows like $\Delta x^{-2}$.
The frozen verdict is **CONTRADICTS—smooth-carrier interpretation on the
measured sequence**. A separate continuum-consistent scalar calculation at
the same $h_C$ binds prepared $Q_C=16$ and $256$; the smooth $Q_C=16$
constrained spatial study remains **INCONCLUSIVE**, with no resolved negative
mode but failed coarse-translation and same-spacing-domain criteria. The
first-order carrier action conserves $Q_C$ under closed boundaries, so
$Q_C(0)=0\Rightarrow\chi_C(t)=0$ and no creation mechanism is supplied.
Continuum formation, full Hessian and dynamical stability, normalization,
quantum numbers, statistics, and physical particle identification remain
open.
Source measurements are in `computations/matter-formation-continuum-report.md`.
...
or inter-vertex endpoint transport. The separately declared Wilson coupling
$-t_\Upsilon(\Upsilon_+^*\mathcal W_{+\leftarrow-}\Upsilon_-+\mathrm{c.c.})$
supplies one such conservative channel. It closes the homogeneous circuit when
$I_{-\to+}=\mathcal J_Q$, carries the added scale-edge relative-charge current,
and has capacity $I_c=2t_\Upsilon u_-u_+/\hbar$. Its coefficient and local
scale-bulk completion remain open. Open or driven channels, non-harmonic
...
### 4.4 Minimal positivity-preserving fibre lift
...
For frozen $q$, this is the minimal time-local linear GKSL generator built only
from the two canonical directed conversion channels. With state-dependent
$q$, factoring $L_a=\sqrt{\gamma_{\rm conv}}J_a$ gives
$\mathcal L_{\rm conv}[\Gamma]
=\gamma_{\rm conv}(\Gamma)\mathcal L_0[\Gamma]$, where $\mathcal L_0$ is a
fixed GKSL generator and $\gamma_{\rm conv}\ge0$. Each trajectory is a
nondecreasing time reparametrization of the $\mathcal L_0$ flow, so the
positive cone and trace are preserved. The complete vector field is nonlinear
and does not define a linear completely positive map or semigroup. The exact
...
The fixed $\mathcal L_0$ in the factorization of (GM51) is a reduced
semigroup generator. The canonical state-dependent prefactor makes the full
conversion vector field nonlinear. A microscopic theory must identify the
traced-out reservoir or auxiliary fields. At the mesoscopic trajectory level,
the compatible density noise, stochastic-calculus convention, positivity
boundary rule, and finite-volume normalization remain those recorded in
`foundations/physical-becoming-hierarchy.md` §4.4.
...
| GM-3 | Undriven transverse coherence | $\gamma_c=\gamma_\varepsilon/2$; at the gated reference state, $\gamma_c=\lambda/6$; at every finite density with $\lambda>0$ and $c\ne0$, $\mathrm d|c|^2/\mathrm dt<0$ | Derived within the minimal two-jump lift; stationary $c$ requires an identified support term |
...
| Minimal positivity-preserving two-jump conversion lift; fixed-$q$ generator linear GKSL | Hypothesized off-diagonal lift / exact canonical reduction |
| Coherence half-rate and strict finite-density undriven-decay boundary | Derived within the minimal lift; stationary $c$ requires identified support |
...
| Stable spatially localized solution | Absent in the minimal smooth zero-Chern and registered confined-pair sectors. Point-core flux supplies a conditional exterior coefficient, an auxiliary adjoint $SU(2)_Q$ branch supplies a smooth local core, and a neutral fixed-$Q_C$ carrier supplies one stable reduced separation under support, retention, and matching inequalities. The stored Cartesian branch remains a finite-grid result whose finest field is parity-concentrated ($99.98631608\%$ of carrier norm on one sublattice) and therefore has the frozen verdict **CONTRADICTS—smooth-carrier interpretation on the measured sequence**; its six-mode $C_4$ spectrum is retained only for that discretization. Separately, continuum-consistent scalar prepared binding is reproduced for $Q_C=16$ and $256$, while smooth $Q_C=16$ spatial stability is **INCONCLUSIVE**. Empty-sector invariance blocks creation from $Q_C=0$; continuum formation, full stability, normalization, quantum numbers, statistics, and particle identification remain open. |
...
coexist through separate dynamical blocks. The minimal positivity-preserving
two-jump lift proves exact canonical reduction and exposes a specific
boundary: the canonical conversion environment damps undriven transverse
coherence. At every finite density with $\lambda>0$, nonzero $c$ decays in the
minimal lift. A persistent projective shell therefore requires physical
support beyond the canonical conversion law.
...
a coherent realization with finite current capacity. Its Wilson-dressed
inter-vertex extension closes the homogeneous circuit conditionally and
preserves the endpoint and relative-charge ledgers; a one-way open channel
...
- `foundations/yin-yang-qi-dynamical-geometry.md`—integrated open-system
  matrix balance, positivity-preserving conversion flow, and coherence-support
  boundary
...
- `foundations/endpoint-link-and-localization-boundary.md`—charged endpoint,
  Wilson-link transport, open vertex alternative, invariant classification,
  and minimal-sector localization boundary
