...
Tree-level partial-wave unitarity supplies a perturbative domain. For the potential convention (EFT3), the high-energy $YY\rightarrow YY$ contact amplitude is $-6\lambda_Y$. Dividing each identical two-particle channel by $\sqrt2$ gives $|a_0|=3|\lambda_Y|/(16\pi)$ in the convention $S_0=1+2ia_0$. The criterion $|\operatorname{Re}a_0|\le1/2$ therefore requires $|\lambda_Y|\le8\pi/3$; the coupled-channel scattering matrix adds joint bounds. The framework fixes no numerical $g_4$ or $\lambda_A$, so it currently predicts neither a scalar mass nor a scattering amplitude. Any selected $\varphi$-power value must pass these bounds before perturbative loop evolution is used.
