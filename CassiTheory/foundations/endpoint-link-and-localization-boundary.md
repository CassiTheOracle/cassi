...
| Reduced spatial radius | $\mathcal A>0$, $\mathcal C\geq0$, and $\mathcal B_G>\mathcal D$ in the point-core branch | Collapse when support does not exceed $\mathcal D$; large-radius instability when $\mathcal C<0$ | Reduced curvature Derived; the coupled functional has a higher-precision Q1–Q4 finite-grid background and a nonnegative matched low PA42 spectrum, while localization and domain/resolution qualification fail |
| Auxiliary magnetic core | Adjoint-only $SU(2)_Q$ sector has the exact BPS core; the registered nonzero fundamental condensate confines flux | Condensate coupling removes the isolated magnetic sector; a finite pair can shrink and annihilate | Static adjoint Hessian nonnegative conditionally; the one-point strict-shell $C_4$ PA42 low spectrum is nonnegative within the frozen uncertainty, while its global phase mode remains grid-scale |
| Core-trapped carrier pair | $A_C>C_Q$, $\hbar\omega_C<\varepsilon_{C,\rm out}$, and separation beyond core overlap | Carrier leakage, profile delocalization, core merger, or transition to another fixed-$Q_C$ configuration | Reduced length and line-density curvature Derived conditionally; the fixed-charge background and one-point PA42 sign classification are tested, while the field fails localization and retention and lacks domain/resolution convergence |
| Fixed-$Q_C$ stationary particle | Selected static dimensionless groups, charge, domain, boundary data, gauge condition, and qualified coupled solution | Basin change, charge leakage, boundary failure, or instability in the second variation | Action and Gauss constraint Derived conditionally; one coefficient point has a higher-precision Q1–Q4 background and `PASS—NONNEGATIVE C4 FINITE-GRID PA42 HESSIAN`. The phase mode remains grid-scale; localization, domain/resolution convergence, unrestricted ordering, PA43, and continuum qualification remain open |
