...
| Reduced spatial radius | $\mathcal A>0$, $\mathcal C\geq0$, and $\mathcal B_G>\mathcal D$ in the point-core branch | Collapse when support does not exceed $\mathcal D$; large-radius instability when $\mathcal C<0$ | Reduced curvature Derived; the coupled stationary functional has a Q2-qualified finite-grid primary background that fails localization and domain/resolution qualification |
| Auxiliary magnetic core | Adjoint-only $SU(2)_Q$ sector has the exact BPS core; the registered nonzero fundamental condensate confines flux | Condensate coupling removes the isolated magnetic sector; a finite pair can shrink and annihilate | Static adjoint Hessian nonnegative conditionally; temporal action and finite-grid PA42 quotient defined, with augmented-gradient preflight stopping before eigenvalue evaluation |
| Core-trapped carrier pair | $A_C>C_Q$, $\hbar\omega_C<\varepsilon_{C,\rm out}$, and separation beyond core overlap | Carrier leakage, profile delocalization, core merger, or transition to another fixed-$Q_C$ configuration | Reduced length and line-density curvature Derived conditionally; the fixed-charge action has a Q2-qualified primary background and explicit physical quotient, while that field fails localization and retention and the augmented-gradient preflight leaves its spectrum open |
| Fixed-$Q_C$ stationary particle | Selected static dimensionless groups, charge, domain, boundary data, gauge condition, and qualified coupled solution | Basin change, charge leakage, boundary failure, or instability in the second variation | Action and Gauss constraint Derived conditionally; one coefficient point has a Q2-qualified finite-grid primary background and explicit physical quotient. The augmented-gradient preflight stops before eigenvalue evaluation; localization, domain/resolution convergence, unrestricted ordering, and the spectrum remain open |
