...
$S^3_R$ first transverse frequency is $2/R$ and vanishes in the infrared
limit. In the fixed Casimir normalization, one link has character spectrum
$n(n+2)/4$ and Wilson-weight Bakry–Émery rate at least
$(2-\beta)/4$ only for $\beta<2$. A factor-two path block has an exact
Haar-preserving horizontal/fibre splitting. Disintegration of the true
interacting vacuum gives a closed conditional Poincaré recurrence for the
all-function fine rate $\lambda_f$, with coarse rate $\lambda_c$, fibre rate
$\lambda_{\mathrm{fib}}$ and transported score covariance $\kappa$. The
physical rate satisfies $\lambda_{\mathrm{gi}}\geq\lambda_f$. At zero score,
the recurrence reduces to
$\lambda_f\geq\min\{2\lambda_c,\lambda_{\mathrm{fib}}/2\}$; nonzero score
...
primary verifier and 84-check independent reconstruction pass. Uniform
weak-coupling fibre, score and coarse-margin estimates remain open.
