...
The open routes are each a new model degree, none a re-weighting of the existing fields: the Yin-excess branch's t > 40 continuation (does the contraction merge or stall, §3.5); a steady-Yin-excess representation change with a derived negative attractor (e.g. sign-opposite conversion on the $\varepsilon<0$ half-space); and a wake sourced between the ridges ($S_W\propto|\nabla\varepsilon|^2$, the one source geometry in which the mass-like flux pulls the pair inward).
...
E1 status: the Yin-excess branch changes the lock-timescale outcome from escape to persistence—the first finite-separation survival under the existing PDE—but it is a transient-driven contraction, not an equilibrium with derived coefficients: $d(t)$ shows no turnaround and the Yin excess itself is erased by conversion at t $\approx$ 21. E1 (a finite-$d_0$ bound state with projected $K_d$, $T_d$) remains open; the natural next probes are the t > 40 continuation (does the contraction merge or stall?) and the open routes of §3.4—a steady-Yin-excess representation change with a derived negative attractor, or a wake sourced between the ridges—each a new model degree.
...
| E3 | Wake coupling $g$, damping length $\ell$ | Open; the wake aggregation candidate was instantiated with $g=\chi_w$ (protocol parameter), $\ell=\mathrm{SIG}$, $\tau_W=1/\lambda$ and tested null (§3.4); the recorded iterations ($\rho$-weighted source, sub-critical bound) are rejected (§3.4); coefficients remain unprojected |
