...
| Interscale stress and attenuation bridge | Exact scale-window momentum ledger; reciprocal positive-stress ladder; canonical quadratic boundary flux; unitary Hermitian-Robin scattering family; gauge-covariant frozen-background rail Hessian $\Lambda_{\mathrm{link},v}=2\kappa_vu_vM(\alpha_v)$; selected-point golden matching; conditional current-capacity and positive fixed-amplitude phase-stiffness bound $k_\star>0.0964640362$ for the unbiased proton branch; closed homogeneous endpoint-current boundary; positive symmetric zero-background quartic boundary; routed forward/return conservation receipt; source-action Nambu response with analytic endpoint poles | Self-adjointness leaves a generic endpoint matrix free; the charged endpoint action supplies the frozen family and the active Nambu response conditionally. The AR1–AR6 receipt passes with zero $\mathcal K/\mathcal D$ residual and $2.220\times10^{-16}$ covariance residual. The DR receipt remains `FAIL` because its DR5 endpoint block has the opposite source-action sign. Every closed homogeneous conservative time-harmonic extremum has zero coherent conversion current, so the nonzero-current branch requires spatial endpoint flux, an open or driven channel, a non-harmonic state, or a larger coupled background. Generic active backgrounds generate anomalous Nambu blocks, while the ordinary Cayley matrix covers the frozen or anomalous-cancelling branch. Coherent returns interfere unless separate routing removes them from later forward inputs | A physical stress carrier, Noether tensor for the time-completed action, density/current-to-momentum map, physical species-port identification, selected endpoint potential and nonzero-current background, microscopic damping channel, doubled port-flux law, full coupled fluctuation spectrum, carrier velocity or impedance, signed return routing, and an identified observable (`foundations/endpoint-link-and-localization-boundary.md` §§3.6–3.9; `foundations/interscale-stress-attenuation-boundary.md`; `computations/endpoint_dynamical_response_report.md`; `computations/endpoint_action_response_prereg.md`; `computations/endpoint_action_response_check.py`; `computations/endpoint_action_response_report.md`) |
