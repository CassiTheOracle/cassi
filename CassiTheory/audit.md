...
| Regulated pure Yang–Mills comparison | Gauss invariance gives the girth-four electric threshold; Yarotsky's theorem supplies a uniform strong-bare-coupling gap; the ground-state transform identifies each finite regulated physical gap with the exact-vacuum gauge-invariant Poincaré rate; a conditional full-holonomy block theorem gives $\lambda_{\mathrm{gi}}\ge\lambda_{\mathrm{loc}}/(A_{\mathrm{AT}}\rho)$. An exact normalized-Haar pullback on edge-simple, pairwise edge-disjoint paths preserves endpoint gauge covariance and compresses the electric Casimir. With all eight boundary links fundamental, Clebsch–Gordan constraints give an exact 14-state internal Gauss fibre for $j_{\max}\geq1$ and conditional electric spacing $2g^2/a$. The exact-vacuum primary/independent schedules pass 305/120 checks, and the path-map schedules pass 53/169 checks | The equal-weight one-plaquette state is excluded as the exact interacting vacuum on the stated lattice families, conditional gaps alone do not imply a volume-uniform rate, and static pure configuration marginalization does not reproduce the exact quantum block. On the fixed genuine $2\times2$ refinement, four unresolved fine plaquette characters give $\|Qh_fJ1\|_2=2x_f$, contradicting exact full-Hamiltonian intertwining by that bare cylindrical map. Pure graph subdivision has no leakage, so path length alone does not determine the block dynamics. Fundamental plaquette multiplication exits the fixed 14-state boundary sector | Coupled boundary-sector Clebsch–Gordan and $6j$ amplitudes and the induced connection, scalar, boundary and multi-loop terms; a uniform discarded-sector Feshbach-resolvent estimate; cutoff- and volume-uniform weak-coupling bounds; a four-dimensional continuum construction with the required axioms; a positive continuum mass independent of the regulator; any Cassi microscopic identification (`foundations/loop-to-bubble-projection-theorem.md` §§9.4–9.19) |
