...
| Interscale stress and attenuation bridge | Exact scale-window momentum ledger; reciprocal positive-stress ladder; canonical quadratic boundary flux; unitary Hermitian-Robin scattering family; inverse endpoint matching for the declared golden matrix; routed forward/return conservation receipt | Self-adjointness leaves the endpoint coupling free; the registered two-rail gluing is perfect transfer; the golden split occurs at a selected design wave number for the local coupling; coherent returns interfere unless separate routing removes them from later forward inputs | A physical stress carrier, Noether tensor for the time-completed action, density/current-to-momentum map, gauge-covariant endpoint field selecting $\Lambda_v(k)$, carrier velocity or impedance, signed return routing, and an identified observable (`foundations/interscale-stress-attenuation-boundary.md`) |
