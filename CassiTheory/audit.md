...
| 10 | Proton stability mechanisms | The stated GUT inputs give $\tau_p=1.29\times10^{37}$ yr, 2.1 orders above Hyper-K reach; the boxed $3.99\times10^{34}$ yr value corresponds to $M_{\text{GUT}}\approx4.7\times10^{15}$ GeV rather than the registered input. The separate coherence-budget chain uses the registered two-decimal coordinate $N_p^{\mathrm{budget}}=91.46$, giving $N_{\max}=\varphi^{4505.5758}$; it reaches $\tau_p\sim10^{910}$ yr only after the Hypothesized per-step profile and Compton-cycle trial map are supplied. The interscale candidate has zero total scale-number flow, normalized relative current $0.0162173$, and normalized energy $0.0509481$ on the precise Mapped proton interval; endpoint conversion, scale tension, localization, and a winding-changing rate remain open (`computations/proton_budget_closure.py`; `computations/planck_proton_scale_current_check.py`) | ⚠️ **Conditional mechanisms; physical rate open** |
