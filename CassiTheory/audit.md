...
| Yin/Yang/Qi dynamical geometry and matter-completion boundary | The canonical $q$-gated populations, positive Hermitian coherence fibre, relative-$U(1)_Q$ currents, endpoint/Wilson channels, and bounded two-domain interface form one explicit effective matrix balance. The nine-part boundary adds a rank-two minimal exterior dilation, the lowest-order reciprocal bilinear interface, the one-forward-carrier $\varphi^{-N/2}$ amplitude law, canonical single-mode stress-flux power, a fresh-return support equation, the closed-action stress Ward identity, the constant-$G$ Einstein–Hilbert branch, the Gram/Cartan particle map, and fixed-$Q_C$ energetic and dynamical qualification operators; DG1–DG7, EC1–EC7, and MCC1–MCC9 pass. The higher-precision `P:separated_core` continuation passes Q1–Q4 at physical-gradient RMS $5.47125\times10^{-5}$. Its strict-shell $C_4$ tangent has dimension $13622$ after removal of the rank-$1677$ coupled gauge image. Independent PA42 eigensolvers identify one near-zero global-$U(1)_C$ phase mode and five positive matched modes with no verified negative mode, giving `PASS—NONNEGATIVE C4 FINITE-GRID PA42 HESSIAN` | The receipts establish conditional mathematics and one finite-matrix sign classification. Physical exterior selection, microscopic coefficients, the golden-port interpretation, a multimode coherence-to-power map, a physical reservoir and its local stress, state-dependent gravity, and the coherence-fibre particle identification remain unresolved. The selected field fails localization and carrier retention, every domain plus high-resolution arm fails Q2, and the global phase mode exceeds the registered high-frequency cutoff. Domain and resolution convergence, perturbations outside $C_4$, the PA43 mixed pencil, continuum stability, and lifetime remain open | Select the physical carrier, topology, preparation, boundary conditions, interface coefficients, port normalization, reservoir source and bath, local reservoir action, and gravity branch; derive a nonduplicating coherence-to-particle map; establish localization, retention, and Q2-qualified domain and resolution backgrounds; repeat the PA42 quotient spectrum on those backgrounds; then select temporal groups and compute the PA43 mixed pencil (`foundations/yin-yang-qi-dynamical-geometry.md`; `foundations/matter-completion-boundary.md`; `foundations/particle-stationary-action-closure.md`; `computations/matter_completion_boundary_report.md`; `computations/particle-stationary-precision-v5-report.md`; `computations/particle-physical-hessian-precision-v2-report.md`) |
