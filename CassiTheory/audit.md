...
| Interscale stress and attenuation bridge | Exact scale-window momentum ledger; reciprocal positive-stress ladder; canonical quadratic boundary flux; unitary Hermitian-Robin scattering family; gauge-covariant frozen-background rail Hessian $\Lambda_{\mathrm{link},v}=2\kappa_vu_vM(\alpha_v)$; selected-point golden matching; conditional current-capacity and positive fixed-amplitude phase-stiffness bound $k_\star>0.0964640362$ for the unbiased proton branch; routed forward/return conservation receipt | Self-adjointness leaves a generic endpoint matrix free; the charged endpoint action supplies the allowed frozen-background matrix family while leaving its amplitude, dressed phase, trace normalization, and $k_\star$ unselected; the registered two-rail gluing is perfect transfer; the golden split occurs at one selected design wave number for the frozen local coupling; full endpoint fluctuation stability remains open; coherent returns interfere unless separate routing removes them from later forward inputs | A physical stress carrier, Noether tensor for the time-completed action, density/current-to-momentum map, physical species-port identification, full endpoint potential and fluctuation spectrum, retarded/advanced active-response convention, carrier velocity or impedance, signed return routing, and an identified observable (`foundations/endpoint-link-and-localization-boundary.md` §§3.6–3.8; `foundations/interscale-stress-attenuation-boundary.md`) |
