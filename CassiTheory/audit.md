...
| Open Yin/Yang/Qi dynamical geometry closure | The canonical $q$-gated populations, positive Hermitian coherence fibre, relative-$U(1)_Q$ currents, endpoint/Wilson channels, and bounded two-domain source interface form one explicit effective matrix balance. The nonlinear two-jump flow reproduces the canonical diagonal PDE and preserves the positive cone by fixed-GKSL time reparametrization. A positive enlarged interior/exterior block with reciprocal coupling derives $S_c^{\rm ext}$, closes enlarged number and relative charge, and obeys two-sided attenuation, capacity, and phase controls; DG1–DG7 and EC1–EC7 pass | The effective closure has no derived microscopic action. For every finite density with $\lambda>0$, its undriven minimal lift strictly damps nonzero transverse coherence. The two-domain source remains conditional on an unselected physical exterior, interface coupling, transfer factors, boundary or reservoir dynamics, and carrier normalization. Total stress, geometry backreaction, the particle-branch interface, and a stationary matter solution remain open | Physical identification of the exterior and $S_c$, independently fixed $V,L_a,R_a$, explicit reservoir reduction and noise, full field-plus-reservoir Noether stress, geometric response equation, interface to the separate local-$SU(2)_Q$ particle action, finite-energy boundary data, and the coupled stability spectrum (`foundations/yin-yang-qi-dynamical-geometry.md`; `computations/dynamical_geometry_closure_check.py`; `computations/dynamical_geometry_closure_report.md`; `computations/cross_scale_coherence_interface_check.py`; `computations/cross_scale_coherence_interface_report.md`) |
| Interscale stress and attenuation bridge | Exact scale-window momentum ledger; reciprocal positive-stress ladder; canonical boundary flux; Hermitian-Robin scattering; gauge-covariant frozen endpoint response; selected-point golden matching; closed-current and stationary spatial-flux boundaries; first-order source-action Nambu response; and a separately declared Wilson-link transport channel with exact endpoint and relative-charge conservation | The AR1–AR6 response receipt passes; the DR receipt remains `FAIL` at DR5. The registered EL9 action alone has zero coherent current on closed homogeneous time-harmonic backgrounds, and closed spatial domains have zero integrated source at each endpoint. The Wilson extension closes the homogeneous circuit conditionally, while $t_\Upsilon$ remains free and its finite scale edge has no local bulk derivation. Physical energy, stress, inertial mass, amplitude stability, and the full coupled spectrum remain open | A physical stress carrier, Noether tensor for the time-completed action, density/current-to-momentum map, physical species-port identification, selected endpoint potentials and nonzero-current background, microscopic damping, temporal relative-gauge connection, microscopic $t_\Upsilon$ and local scale-bulk completion, doubled port-flux law, coupled fluctuation spectrum, carrier velocity or impedance, signed return routing, and an identified observable (`foundations/endpoint-link-and-localization-boundary.md` §§3.6–3.11; `foundations/interscale-stress-attenuation-boundary.md`; `computations/endpoint_action_response_report.md`; `computations/endpoint_spatial_flux_prereg.md`; `computations/endpoint_spatial_flux_check.py`; `computations/endpoint_spatial_flux_report.md`; `computations/endpoint_intervertex_transport_prereg.md`; `computations/endpoint_intervertex_transport_check.py`; `computations/endpoint_intervertex_transport_report.md`) |
