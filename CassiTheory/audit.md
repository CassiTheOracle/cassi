...
| Regulated pure Yang–Mills comparison | Gauss invariance gives the girth-four electric threshold; Yarotsky's theorem supplies a uniform strong-bare-coupling gap; the ground-state transform identifies each finite regulated physical gap with the exact-vacuum gauge-invariant Poincaré rate. Exact Haar path blocking, radial Feshbach transfer, the sharp $gN\to\infty$ bare-cutoff condition and the 14-state fixed-boundary fibre are derived. A factor-two Haar-preserving horizontal connection gives the exact all-function conditional-score recurrence $\lambda_f\ge C_*^{-1}$, which lower-bounds the gauge-invariant rate and reduces at zero score to $\min\{2\lambda_c,\lambda_{\mathrm{fib}}/2\}$; its 118 primary checks and 90 implementation-independent receipt checks pass | The equal-weight one-plaquette state is excluded as the exact vacuum; conditional gaps alone do not imply a volume-uniform rate; bare configuration marginalization does not reproduce the quantum block; the fixed 14-state boundary fibre is not dynamically closed; finite or fixed character cutoffs fail at weak coupling. The auxiliary one-link Bakry–Émery pointwise bound reaches zero at concentration $\beta=2$, and no map from that $\beta$ to the Hamiltonian coupling or exact vacuum is assumed. Exact marginal interactions survive through the transported score $\kappa$, and no uniform weak-coupling estimate is established | Uniform conditional-fibre rate, transported-score bound and coarse-rate margin across exact generated interactions; boundary-sector recoupling and $6j$ amplitudes; thermodynamic and four-dimensional continuum construction with the required axioms; positive regulator-independent mass; Cassi microscopic gauge-field identification (`foundations/loop-to-bubble-projection-theorem.md` §§9.4–9.20) |
