...
| 11 | σ₈ magnitude | Pipeline −45.1% headline (re-run 2026-08-07, $r_0 = 1/23$) dominated by normalization + resolution (P(k) normalization 8e-5, nonlinear ICs, N=32 dissipation: δ_rms falls 32% while ΛCDM linear grows +24%); ≈ −9.6% mechanism-attributable on the 2026-08-06 state (G_eff 0.9044); "~5%" is a Mapped target (μ = 0.98 → −5.3%) (`computations/sigma8_reconciliation.py`) — settled 2026-08-07: P-A operative, IC r₀ = 0.0472; the row's value −16.6% (R = 0.834) | ⚠️ **Reconciled** (normalization-dominated) |
