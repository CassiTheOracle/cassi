...
| Open Yin/Yang/Qi dynamical geometry closure | The canonical $q$-gated populations, positive Hermitian coherence fibre, relative-$U(1)_Q$ currents, and endpoint/Wilson channels form one explicit effective matrix balance. The nonlinear two-jump flow reproduces the canonical diagonal PDE, preserves the positive cone by fixed-GKSL time reparametrization, and closes the registered number and relative-charge ledgers; DG1–DG7 pass | The effective closure has no derived microscopic action. For every finite density with $\lambda>0$, its undriven minimal lift strictly damps nonzero transverse coherence. The receipt does not supply physical $c$, a reservoir, support source, total stress, geometry backreaction, a particle-branch interface, or a stationary matter solution | An identified $S_c$ satisfying the stationary support budget, explicit reservoir reduction and noise, full field-plus-reservoir Noether stress, geometric response equation, interface to the separate local-$SU(2)_Q$ particle action, finite-energy boundary data, and the coupled stability spectrum (`foundations/yin-yang-qi-dynamical-geometry.md`; `computations/dynamical_geometry_closure_report.md`) |
