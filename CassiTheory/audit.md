...
| Yin/Yang/Qi dynamical geometry and matter-completion boundary | The canonical $q$-gated populations, positive Hermitian coherence fibre, relative-$U(1)_Q$ currents, endpoint and Wilson channels, and bounded two-domain interface form one explicit effective matrix balance. The nine-part boundary adds a rank-two minimal exterior dilation, the lowest-order reciprocal bilinear interface, the one-forward-carrier $\varphi^{-N/2}$ amplitude law, canonical single-mode stress-flux power, a fresh-return support equation, the closed-action stress Ward identity, the constant-$G$ Einstein–Hilbert branch, the Gram/Cartan particle map, and fixed-$Q_C$ energetic and dynamical qualification operators; the finite algebraic and reduced analytic receipts pass. At the Mapped coupling $h_C=2.9598260763447164$, the fixed-charge action has an independently verified localized, nodeless, carrier-retaining branch across four same-domain finite grids and one larger-domain comparison. Adjacent principal observables agree and its absolute energy differences decrease twice. The finest localized artifact has a $77000$-dimensional fixed-charge $C_4$ physical quotient after removal of the rank-$11775$ complete allowed gauge image. Independent eigensolvers identify six matched lowest PA42 eigenpairs containing one numerically near-zero carrier-phase symmetry mode, no negative mode, and five positive modes; the positive gap is $0.01527618220595$ against uncertainty $6.092903959\times10^{-4}$. The independently matched lowest energetic eigenpairs of a distinct diffuse $h_C=1.50$ field also contain no negative mode | The receipts establish conditional mathematics, one resolution-consistent localized retained finite-grid stationary branch, and the absence of a negative mode among the independently matched lowest energetic eigenpairs on one localized and one diffuse finite-grid $C_4$ physical quotient. The localized phase-symmetry mode has high-frequency fraction $0.8744032081$, and no localized Hessian-resolution sequence exists. Physical exterior selection, microscopic coefficients, the golden-port interpretation, a multimode coherence-to-power map, a physical reservoir and its local stress, state-dependent gravity, coherence-fibre particle identification, perturbations outside $C_4$, continuum stability, PA43 dynamics, formation, and lifetime remain unresolved | Select the physical carrier, topology, preparation, boundary conditions, interface coefficients, port normalization, reservoir source and bath, local reservoir action, and gravity branch; derive a nonduplicating coherence-to-particle map; extend the localized branch and its constrained Hessian through additional domain growth and continuum refinement while tracking phase-mode spatial resolution and the positive gap; expand beyond $C_4$; then select temporal groups, compute the PA43 mixed dynamical pencil, and test real-time formation and persistence (`foundations/yin-yang-qi-dynamical-geometry.md`; `foundations/matter-completion-boundary.md`; `foundations/particle-stationary-action-closure.md`; `computations/matter_completion_boundary_report.md`; `computations/particle-carrier-resolution-recovery-report.md`; `computations/particle-physical-hessian-precision-v2-report.md`; `computations/particle-localized-physical-hessian-report.md`) |
...
| 10 | Proton stability mechanisms | The stated GUT inputs give $\tau_p=1.29\times10^{37}$ yr, 2.1 orders above Hyper-K reach; the boxed $3.99\times10^{34}$ yr value corresponds to $M_{\text{GUT}}\approx4.7\times10^{15}$ GeV rather than the registered input. The separate coherence-budget chain uses $N_p^{\mathrm{budget}}=91.46$, giving $N_{\max}=\varphi^{4505.5758}$; it reaches $\tau_p\sim10^{910}$ yr only after the Hypothesized per-step profile and Compton-cycle trial map are supplied. The interscale candidate has zero total scale-number flow, normalized relative current $0.0162173$, and normalized energy $0.0509481$ on the precise Mapped proton interval. Point flux fixes the exterior coefficient. A Hypothesized auxiliary adjoint $SU(2)_Q$ action has an exact decoupled core, while the registered fundamental condensate confines flux and supplies no persistent pair by itself. A Hypothesized neutral core carrier gives a conditional reduced root under explicit support, retention, and matching inequalities. The separate source-free charged-field action defines the fixed-$Q_C$ stationary problem. At the Mapped coupling $h_C=2.9598260763447164$, that action has an independently verified localized, nodeless, carrier-retaining branch across four same-domain finite grids and one larger-domain comparison; adjacent observables agree and absolute energy drift decreases twice. Its finest artifact's six independently matched lowest constrained $C_4$ finite-grid PA42 eigenpairs contain one numerically near-zero carrier-phase symmetry mode, no negative mode, and five positive modes. The symmetry mode retains unresolved odd-even structure. Continuum existence, localized Hessian resolution, perturbations beyond $C_4$, PA43 dynamics, physical normalization, proton quantum numbers, formation, and decay rate remain open (`foundations/core-trapped-charge-support.md`; `foundations/particle-stationary-action-closure.md`; `computations/particle-carrier-resolution-recovery-report.md`; `computations/particle-physical-hessian-precision-v2-report.md`; `computations/particle-localized-physical-hessian-report.md`) | ⚠️ **Conditional mechanisms; Tested resolution-consistent localized finite-grid branch and six-mode constrained $C_4$ finite-grid PA42 low-spectrum sign; physical rate open** |
