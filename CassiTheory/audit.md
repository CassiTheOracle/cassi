...
| Yin/Yang/Qi dynamical geometry and matter-completion boundary | The canonical $q$-gated populations, positive Hermitian coherence fibre, relative-$U(1)_Q$ currents, endpoint and Wilson channels, and bounded two-domain interface form one explicit effective matrix balance. The nine-part boundary adds a rank-two minimal exterior dilation, the lowest-order reciprocal bilinear interface, the one-forward-carrier $\varphi^{-N/2}$ amplitude law, canonical single-mode stress-flux power, a fresh-return support equation, the closed-action stress Ward identity, the constant-$G$ Einstein–Hilbert branch, the Gram/Cartan particle map, and fixed-$Q_C$ energetic and dynamical qualification operators; the finite algebraic and reduced analytic receipts pass. At the Mapped coupling $h_C=2.9598260763447164$, the fixed-charge action has an independently verified localized, nodeless, carrier-retaining branch across four same-domain finite grids and one larger-domain comparison. Adjacent principal observables agree and its absolute energy differences decrease twice. A distinct diffuse $h_C=1.50$ field has a $13622$-dimensional strict-shell $C_4$ physical quotient after removal of the rank-$1677$ coupled gauge image. Independent eigensolvers identify one near-zero global carrier-phase mode and five positive matched low energetic modes with no verified negative mode | The receipts establish conditional mathematics, one resolution-consistent localized retained finite-grid stationary branch, and one finite-matrix sign classification on a different diffuse field. Physical exterior selection, microscopic coefficients, the golden-port interpretation, a multimode coherence-to-power map, a physical reservoir and its local stress, state-dependent gravity, and the coherence-fibre particle identification remain unresolved. The localized branch has no constrained energetic or mixed dynamical spectrum and no continuum limit. The diffuse field's global phase direction exceeds the registered high-frequency cutoff. Perturbations outside $C_4$, continuum stability, formation, and lifetime remain open | Select the physical carrier, topology, preparation, boundary conditions, interface coefficients, port normalization, reservoir source and bath, local reservoir action, and gravity branch; derive a nonduplicating coherence-to-particle map; extend the localized branch through additional domain growth and continuum refinement; construct and verify its fixed-charge physical quotient and constrained energetic Hessian; then select temporal groups, compute the mixed dynamical pencil, and test real-time formation and persistence (`foundations/yin-yang-qi-dynamical-geometry.md`; `foundations/matter-completion-boundary.md`; `foundations/particle-stationary-action-closure.md`; `computations/matter_completion_boundary_report.md`; `computations/particle-carrier-resolution-recovery-report.md`; `computations/particle-physical-hessian-precision-v2-report.md`) |
