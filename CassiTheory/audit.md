...
| Regulated pure Yang–Mills comparison | Gauss invariance gives the girth-four electric threshold; Yarotsky's theorem supplies a uniform strong-bare-coupling gap; the ground-state transform identifies each finite regulated physical gap with the exact-vacuum gauge-invariant Poincaré rate. Exact Haar path blocking, radial Feshbach transfer, the sharp $gN\to\infty$ bare-cutoff condition and the 14-state fixed-boundary fibre are derived. A factor-two Haar connection gives the exact all-function covariance recurrence $\lambda_f\ge C_*^{-1}$. The conditional Poisson inverse sharpens it to $\lambda_f\ge C_{-1}^{-1}$ using minimum transport cost $\vartheta$ and gives the exact coarse/vertical/output margin budget. The residual Gramian identifies $A_{\mathrm{AT}}^{\mathrm{opt}}=\gamma_{\mathrm{rec}}^{-1}$ and yields $\lambda_{\mathrm{gi}}\ge\gamma_{\mathrm{rec}}\lambda_{\mathrm{loc}}/\rho$. The 118/90 geometry, 86/32 transport and 58/30 recovery controls pass. The seven-link block study adds tree-exterior boundary independence and a nodal Ritz obstruction whose twelve-row path and two-parameter torus families confine that zero-gap region to the strong-coupling half of the schedule, and its 180 frozen boundary rows classify INCONCLUSIVE. The eight-link bowtie study supplies finite loop-holonomy boundary sensitivity, while all 108 retained-rate rows classify INCONCLUSIVE under the residual or nested-rank qualification rules. The recovered open $3\times2\times2$ Hamiltonian has complete 868- and 955835-state bases, passes 226/226 finite checks and 256/256 independent checks, and has nonnegative scheduled ground energies; every aggregate separation-based character-tail qualification is INCONCLUSIVE. The fixed-graph form theorem supplies exact tree-gauge Haar coordinates, an explicit electric-form constant, form-core convergence, a separation-free discarded-mass bound and a noncommuting Ritz-error bound; its 22/22 primary and 18/18 independent controls pass with `clay_verdict=NULL`. The periodic-cubic local theorem gives discarded mass at most $2x|S|/\kappa_C$ on every fixed link support, uniformly in volume; its 16/16 primary and 17/17 independent controls pass. Conditional on the finite-volume ground densities and this local estimate, the trace-norm compactness argument extracts a compatible, symmetry-invariant locally normal ground-state subsequence at each fixed regulator and coupling; its finite-identity controls pass 18/18 primary and 19/19 independently | The equal-weight one-plaquette state is excluded as the exact vacuum; conditional gaps alone do not imply a volume-uniform rate; bare configuration marginalization does not reproduce the quantum block; the fixed 14-state boundary fibre is not dynamically closed; finite or fixed character cutoffs fail as weak-coupling schedules. The auxiliary one-link Bakry–Émery bound has no map to the Hamiltonian coupling or exact vacuum. The score Gramian is an upper tangent-space penalty rather than a physical-function recovery operator. Product, near-parallel and massless Gaussian controls show that score-kernel and finite-kernel rigidity statements supply no uniform recovery floor. Edge-only larger-volume receipts omit spectator-channel deltas and are excluded from Hamiltonian and tail evidence. The fixed-graph form constants are not uniform in graph size, coupling or lattice spacing. The local theorem cuts only a fixed support; an exact bounded-energy-density product family has discarded whole-wavefunction norm tending to one at fixed cutoff. The thermodynamic receipts verify finite identities and implication controls; they construct no interacting $SU(2)$ ground state and prove no uniform tail estimate. The subsequence theorem supplies no full-sequence convergence, phase uniqueness, clustering, uniform positive gap or continuum estimate | A scale-uniform residual-recovery floor, vertical transport field or $H^{-1}$ score upper bound, conditional-fibre rate and coarse-rate margin across exact generated interactions; boundary-sector recoupling and $6j$ amplitudes; full-sequence phase control, clustering and weak-coupling, global-norm and lattice-spacing estimates sufficient for a four-dimensional continuum construction with the required axioms; positive regulator-independent mass; Cassi microscopic gauge-field identification (`foundations/loop-to-bubble-projection-theorem.md` §§9.4–9.27) |
...
obstruction. A generic nondegenerate winding-changing zero gives
$E_4(\tau,R)\sim3\pi^2/(4|\tau|)$ and a punctured zero-time energy
$2\pi(1/a-1/R)$. The rank-three anisotropic coefficient is bounded below by
$2\pi$. The code's max-normalization instead gives
$4\pi/\epsilon-2\pi/R$, so its fixed cutoff defines a modified action with
no regulator-independent limit. Primary and independent calculations pass
QZO1–QZO4 and agree exactly; QZO5 returns `CONTRADICTS` and QZO6 returns
`FAIL`. This rules out continuum topology change through a generic zero for
the selected action. It does not rule out QCD, prepared textures,
exceptional degenerate events or a regular quark-bearing microscopic action
...
two-flavour sea and supplies action-level Gauss-sector confinement. Every
nonzero total Cartan charge has radial energy density bounded below by
$4\sqrt6|Q|\sqrt B/19$, hence infinite energy in infinite volume. A
color-neutral three-arm flux network instead has finite minimized tension. At
the dimensionless verification point, the isolated-color asymptotic slope is
reproduced to $8.44\times10^{-5}$ relative error and the independent
...
## 12. Zap-and-Zip Campaign (2026-09-25)

A local pulse's evoked complexity, read the way Massimini's zap-and-zip
protocol reads it from a real brain, differs sharply between the two
pinch regimes; the gate's self-feedback contributes nothing
measurable to that difference. The frozen protocols and decision trees are
`two-fluid/run_zap_and_zip.py`, `two-fluid/run_zap_expansion.py`, and
`two-fluid/run_zap_chaos.py`; run records live under `runs/zap_and_zip/`
(gitignored, not retained in this checkout—regenerate with the named
scripts). All three share one canonical pinch configuration
(`two-fluid/cassi_two_fluid_3d_gpu.py` `ExpandingTwoFluid3DGPU`,
$N=48$, $\lambda=0.02$, conversion Hubble mode, single gate, seed 42)
prepared to $t=6$ into a below-pinch state ($r_0=0.047$, $r=0.20$ at
$t=6$, stays below $\varphi^{-1}$ through the window) and an
above-pinch state ($r_0=1.0$, $r=1.03$ at $t=6$), then probed with a
local balance-shift pulse ($\delta E_Y=+0.1g$, $\delta E_I=-0.1g$,
Gaussian width 2 cells) after 2 time units of noise burn-in. Complexity
...
pinch the echo is widespread, differentiated, and late-peaking; below
it the echo is local and passively carried. At $\sigma=0.005$ the two
states sit on either side of the human PCI cutoff, though that
...
and the rest by perturbation growth roughly $2.5\times$ faster above
it. The gate-mechanism prediction is **NULL**.
