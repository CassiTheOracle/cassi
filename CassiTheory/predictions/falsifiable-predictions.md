...
| **EL-9** | Source-free temporal particle action, fixed-$Q_C$ stationary closure, and localized PA42 low spectrum | Direct local gauging of the first-order Yang/Yin time term gives a nonzero fundamental-condensate Gauss source with $S^aS^a=\rho^2$. The separate second-order charged-field branch is covariant under time-dependent local $SU(2)_Q$, has positive temporal curvature terms, and makes $\mathcal A_0=0$ with static charged fields Gauss-compatible. At the Mapped $h_C=2.9598260763447164$, one nodeless branch is localized and carrier-retaining across four same-domain finite grids and one larger-domain comparison. Its finest artifact's six independently matched lowest constrained $C_4$ finite-grid PA42 eigenpairs contain one numerically near-zero carrier-phase symmetry mode, no negative mode, and five positive modes; the symmetry mode has high-frequency fraction $0.8744032081$ and remains spatially unresolved | **Derived conditional action algebra / Mapped selected carrier coupling / Tested resolution-consistent localized finite-grid branch and six-mode constrained PA42 low-spectrum sign.** The independently matched lowest energetic eigenpairs of a distinct diffuse $h_C=1.50$ field also contain no negative mode, while that field fails localization and retention. Localized Hessian resolution, perturbations beyond $C_4$, continuum existence, PA43 dynamics, physical coefficient calibration, and the particle map remain open | The Pauli identity fails, the first-order source vanishes for a nonzero fundamental condensate without added charge, the selected action fails local temporal covariance or Gauss variation, the static ansatz violates Gauss's law, the localized branch loses its frozen qualifications, or an independently matched negative $C_4$ PA42 mode appears |
...
| **MB-9** | Reduced stationary curvature and low-spectrum qualification | The reduced fixed-charge branch has one bounded positive-curvature separation and positive nonconstant line modes. At the Mapped coupling $h_C=2.9598260763447164$, the full stationary action has a localized, nodeless, carrier-retaining branch across four same-domain finite grids and one larger-domain comparison; adjacent observables agree and absolute energy drift decreases twice. Its finest artifact's six independently matched lowest constrained $C_4$ finite-grid PA42 eigenpairs contain one numerically near-zero carrier-phase symmetry mode, no negative mode, and five positive modes. The positive gap is $0.01527618220595$. The symmetry mode has high-frequency fraction $0.8744032081$ and remains spatially unresolved | **Reduced analytic support / MCC9 tested / Mapped $h_C$ / independently verified resolution-consistent localized retained finite-grid branch / Tested six-mode constrained $C_4$ finite-grid PA42 low-spectrum sign.** The independently matched lowest energetic eigenpairs of a distinct diffuse background also contain no negative mode, while that field fails localization and retention. Localized Hessian resolution, perturbations beyond $C_4$, continuum existence, PA43 dynamics, formation, and physical particle identity remain open | The reduced equations lack their registered root or positive modes; the stationary claim fails if the localized field loses qualification or adjacent convergence; the finite-grid PA42 claim fails if an independent low spectrum finds a negative mode or an unassigned near-zero mode; stronger claims fail under unresolved refinement, a growing PA43 mode, failed continuum convergence, or absent physical identification |
