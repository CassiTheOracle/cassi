...
| 42 | Sector-coupling scale (formal $C=1$ cascade candidate) | Conditional microscopic and transport matching | **Formal $\kappa_{s,\mathrm{scale}}^{-1/2}=\varphi^3v_0\approx1.04$ TeV**, conditional on $\delta=3$; $\kappa_{s,\mathrm{scale}}=\varphi^{-6}/v_0^2=0.92$ TeV$^{-2}$; exact cascade-anchor counterpart $E_{77}$ | The chiral-scalar projection has dimensional and Hermiticity obstructions. The separate positive chiral-current map fails closed Dirac conversion; the specified massive minimal lift changes its stationary ratio and permits positive-energy leakage. Physical energy-density normalization, reservoir matching, $\kappa_s$, equilibration, $\chi$ and an FCC-ee test remain unselected | **Physical interaction and controlled density reduction required** |
...
| **GM-3** | Undriven transverse coherence and composition relaxation in the minimal two-jump lift | $\gamma_c=\gamma_\varepsilon/2$; at the gated reference state, $(\gamma_\varepsilon,\gamma_c)=(\lambda/3,\lambda/6)$. For every finite density with $\lambda>0$ and $c\ne0$, $\mathrm d|c|^2/\mathrm dt=-\varphi^2\lambda(1-q)|c|^2<0$, so stationary transverse coherence requires an identified support term. A positive two-domain realization gives $S_c^{\rm ext}=(\mathcal S_{\rm in})_{IY}$ and the necessary capacity bound $2\|V\|_{\rm op}\|\mathsf C_{\rm io}\|_F/\hbar\ge\gamma_{c,\min}\|c\|_2$, together with the phase-sensitive stationary ledger | **Derived within the minimal lift and declared interface; DG1–DG7 and EC1–EC7 PASS; physically untested.** The arbitrary-state sign, positive-block factorization, exchange ledgers, and source bound follow analytically; the frozen checkers supply deterministic implementation witnesses. A carrier-to-$c$ observable, independently identified undriven regime, physical exterior, interface coupling, transfer factors, and sustaining dynamics remain open | Under the declared two-jump lift and controlled undriven conditions, measured coherence violates the half-rate relation or remains nonzero and stationary at finite density without any coherent source, protected sector, boundary or transport influx, modified reservoir, or different lift |
...
`computations/dynamical_geometry_closure_check.py`;
`computations/dynamical_geometry_closure_report.md`;
`computations/cross_scale_coherence_interface_prereg.md`;
`computations/cross_scale_coherence_interface_check.py`;
...
| **EL-7** | Auxiliary smooth magnetic core and confined-defect boundary | In the adjoint-only Prasad-Sommerfield branch, $K=X/\sinh X$, $H=\coth X-1/X$, $\Phi_G=4\pi N_G/g_Q$, and $M_{\mathrm{BPS}}=4\pi L_{\mathfrak s}v_Q\lvert N_G\rvert/(\mu_xg_Q)$. With the registered nonzero doublet, $H_{\mathrm{full}}=\{1\}$, $\pi_2(SU(2)_Q)=0$, and the homogeneous transverse response has $\kappa_L^2=e_x^2K_x\rho_0/4>0$. The declared attractive residual-vector-exchange string ansatz has $E'(L)>0$ | **Derived conditional on a Hypothesized internal $SU(2)_Q$ completion.** The exact BPS branch is adjoint-only. The full tube profile and tail matching remain unselected; the condensate-coupled branch has no isolated magnetic sector or registered persistent pair. No Standard Model identification or numerical particle prediction is assigned | The profile fails the first-order equations or unit energy integral; the residual flux or exterior coefficient mismatches EL-6; the displayed full-vacuum stabilizer or topology is wrong for the declared fields; the constrained transverse kernel fails its component-energy calculation; or the stated asymptotic pair slope is nonpositive with positive tension and attraction |
...
| **EL-9** | Source-free temporal action and fixed-charge particle evidence | Direct local gauging of the first-order Yang/Yin term gives the nonzero fundamental-condensate Gauss source $S^aS^a=\rho^2$. The separate second-order charged-field branch has time-dependent local $SU(2)_Q$ covariance and a Gauss-compatible static sector. Its invariant $\mathcal O_N=\Psi^T i\sigma^2(\Phi^a\sigma^a)\Psi$ carries a physical common phase with $\omega^2=(J_x/J_t)k^2+O(k^4)$; the component projection, constrained spectra and linear wave qualify with `PASS` (`computations/matter-formation-continuum-report.md` §40). Under the smooth finite-energy $\mathbb R^3$, positive-coefficient, unit-scale-measure, zero-boundary-work and no-external-source assumptions of §41, the relative-equilibrium class has $\omega_N=0$ and its homogeneous Gauss identity gives $\mathcal A_0=0$, including a bounded core where both charged fields vanish. Thus this isolated stationary sector has no gauge-electric support. Separately, at $\chi_C=0$, fixed nonzero common-number momentum $P_N$ has $E_R=P_N^2/[2J_t(2\pi)^{3/2}R^3]\to0$ along a Gaussian constrained initial-data sequence with no zero-energy minimizer; this is variational spreading, not a dispersal trajectory or universal metastability theorem. $P_N/\hbar$ is a number normalization, distinct from the first-order density excess $\delta Q_N$ and neutral-carrier number $Q_C$. The neutral first-order carrier preserves an exactly empty closed sector. At the Mapped coupling, the stored Cartesian branch has a six-mode constrained finite-grid spectrum but fails its smooth-carrier diagnostic. Separate continuum-consistent scalar binding at prepared $Q_C\in\{16,256\}$ is independently reproduced | **Derived conditional action, observable, stationary electric obstruction and fixed-$P_N$ variational sequence / Mapped coupling / Tested linear wave, finite-grid spectrum, ultraviolet obstruction, prepared smooth binding and electric-support reconciliation.** The model-unit $J_t=0.8971293123871708$ is a supplied numerical witness, not calibration or a physical parameter prediction. Smooth $Q_C=16$ constrained spatial stability is INCONCLUSIVE. The stationary obstruction excludes only the declared isolated relative-equilibrium class; general time-dependent or multifrequency solutions, other vacua, external sourcing and quantum bound states remain open. The fixed-$P_N$ sequence is not a dynamics result. Microscopic production, quantum state, normalization, full stability and particle identity remain open; the charge-two classical composite selects no quantum paired condensate or exchange statistics | The Pauli identity, temporal covariance, Gauss variation, common-phase projection, empty-sector conservation, or the §41 finite-energy equality argument fails under the stated action and boundary assumptions; independent reconstruction fails the reported phase, finite-grid, scalar, Gaussian, electric-support or control measurements |
| **EL-10** | Stationary charged-endpoint spatial flux and closed-domain zero mode | The action gives $\partial_t|\Upsilon_v|^2+\nabla\cdot\mathbf J_{\Upsilon,v}=\Gamma_v=-\mathcal I_{\mathrm{link},v}/2$. Every periodic, no-flux, or sufficiently localized stationary endpoint domain has $\int\Gamma_vd^3x=0$. On a constant-amplitude periodic cube with $K_v\ne0$ and $u_v>0$, each zero-mean source has $\alpha_{v,\mathbf k}=-\hbar\Gamma_{v,\mathbf k}/(K_vu_v^2|\mathbf k|^2)$; for $K_v>0$ its cost $H_{\nabla\Upsilon,v}=(\hbar^2V/2K_vu_v^2)\sum_{\mathbf k\ne0}|\Gamma_{v,\mathbf k}|^2/|\mathbf k|^2$ is positive | **Derived conditional and SF1–SF6 tested.** The frozen receipt passes at one normalized point with $K_v=2.3$ and $u_v=0.9$. The coupled rail background, physical boundary flux, selected Wilson-link coefficient, physical coefficients, and particle interpretation remain open | The registered action violates the local source identity or time-independent gauge covariance; a periodic divergence supports a nonzero source mean; or the direct and Fourier gradient costs disagree under the frozen conventions |
| **EL-11** | Gauge-covariant inter-vertex endpoint transport | The separately declared Wilson coupling $\mathcal H_{\mathrm{tr}}=-t_\Upsilon(\Upsilon_+^*\mathcal W_{+\leftarrow-}\Upsilon_-+\mathrm{c.c.})$ is invariant under the registered charge-$-g_Q$ endpoint law. It carries $I_{-\to+}=-(2t_\Upsilon/\hbar)\operatorname{Im}(\Upsilon_+^*\mathcal W_{+\leftarrow-}\Upsilon_-)$, gives endpoint balances $\dot n_-+\nabla\cdot\mathbf J_-=\Gamma_--I_{-\to+}$ and $\dot n_++\nabla\cdot\mathbf J_+=\Gamma_++I_{-\to+}$, and closes the registered homogeneous circuit when $I_{-\to+}=\mathcal J_Q$. Its fixed-amplitude capacity is $I_c=2t_\Upsilon u_-u_+/\hbar$; the principal subcritical branch has positive phase curvature | **Hypothesized action extension / Derived conditional and IT1–IT6 tested.** The frozen first execution passes at one normalized point. The microscopic origin and physical value of $t_\Upsilon$, local scale-bulk completion, self-consistent rail solution, amplitude spectrum, and particle interpretation remain open | The dressed bilinear fails the declared local relative-gauge law, endpoint transport fails to cancel in the summed number balance, the scale-edge current fails the local relative-charge ledger, or a target with $|\mathcal J_Q|>I_c$ has a stationary fixed-amplitude phase under the declared Hamiltonian |
...
`computations/endpoint_intervertex_transport_report.md`;
...
| **MB-9** | Reduced stationary curvature and particle-spectrum boundary | The reduced carrier branch has one bounded positive-curvature separation and positive nonconstant line modes. The Cartesian localized field has six matched lowest constrained $C_4$ eigenpairs, a positive gap $0.01527618220595$ and a grid-scale phase direction. Its finest carrier norm is $99.98631608\%$ on one parity sublattice; the scoped verdict is `CONTRADICTS—smooth-carrier interpretation on the measured sequence`. Separate continuum-consistent scalar binding at prepared $Q_C\in\{16,256\}$ is reproduced independently; smooth $Q_C=16$ constrained spatial stability is INCONCLUSIVE | **Derived conditional reduced support / Tested algebraic receipts and finite-grid measurements / Mapped $h_C$ / Tested prepared smooth scalar binding.** The first-order carrier preserves empty-sector data. The qualified scalar endpoints grow with prepared population while their energy per carrier falls; no preferred particle size is established. Production, full continuum and temporal stability, physical normalization and particle identity remain open | The reduced root or curvature fails its assumptions; independent reconstruction fails the stated measurements. Positive prepared binding and a finite-grid low-spectrum sign cannot establish creation or physical particle identity |

The carrier-formation calculation preserves these conditional scope limits.
The first-order action conserves a nonnegative population, so exactly empty
carrier data remain empty under closed boundaries. The stored Cartesian
localized sequence fails the frozen smooth-carrier diagnostic through
parity-concentrated ultraviolet structure. A continuum-consistent scalar
branch at the selected coupling binds prepared $Q_C=16$ and $256$ with
independent collocation agreement; $Q_C=4$ spreads in the tested basins,
$Q_C=64$ is numerically inconclusive, and the smooth $Q_C=16$ combined
spatial-stability verdict is `INCONCLUSIVE`. These measurements introduce
...
- **Conditional test rows (CT-n, QGI-n, SB-n, LP-n, DR-n, GM-n, and EL-n):**
  ...
  topology, smooth and auxiliary localization boundaries, stationary action,
  spatial-flux closure, and Wilson-link transport.
