...
**Prediction 42:** Conditional formal scale test: with $\delta=3$, the cascade supplies the coefficient-free $C=1$ candidate $\kappa_{s,\mathrm{scale}}^{-1/2}=\varphi^3 v_0\approx1.04$ TeV at rung 77 (formal exponent arithmetic); equivalently $\kappa_{s,\mathrm{scale}}=\varphi^{-6}/v_0^2=0.92$ TeV$^{-2}$. The optional Dirac$\leftrightarrow$two-fluid construction is dimensionally incomplete and generically non-Hermitian as written, so a physical $\kappa_s$, equilibration timescale, or $\chi$ value is unresolved; the measured conditional scalar normalization does not repair that map. A valid prediction requires a distinct dimensionally homogeneous Hermitian microscopic interaction and an independently specified positive-density map from microscopic fields to canonical energy densities. Until those are sourced, no FCC-ee or $\chi$ test is defined.

**Source:** `foundations/sector-coupling-derivation.md` §§2–4; `foundations/unified-lagrangian.md` §§2.2–2.4, 5.2, 6; `foundations/particle-stationary-action-closure.md` §8.12; `computations/matter-formation-continuum-report.md` §12. The coefficient-free scale form and rung identity are conditional on $\delta=3$; the formal $C=1$ values retain the electroweak anchor's discretization residual. The source documents a $[M]^3$ spinor-density versus $[M]^2$ condensate-square mismatch, with ordinary-square and linear-projection terms generically non-Hermitian, and the scalar campaign records conditional normalization nonuniqueness plus a rejected core-cell assignment. No sourced microscopic interaction or positive-density map exists, so the physical sector coupling, equilibration scale, $\chi$ bridge and FCC-ee test remain unresolved; $\mathcal{N}_{\mathrm{pde}}$ has no established value.
...
| 42 | Sector-coupling scale (formal $C=1$ cascade candidate) | Conditional field-theory repair / FCC-ee | **Formal $\kappa_{s,\mathrm{scale}}^{-1/2} = \varphi^3 v_0 \approx 1.04$ TeV** at rung 77, conditional on $\delta=3$; $\kappa_{s,\mathrm{scale}}=\varphi^{-6}/v_0^2=0.92$ TeV$^{-2}$ | Formal arithmetic only; current Dirac projection has a $[M]^3$/$[M]^2$ mismatch and generically non-Hermitian terms. A valid test requires a distinct dimensionally homogeneous Hermitian microscopic interaction plus an independently specified positive-density map; no physical $\kappa_s$, equilibration scale, $\chi$ value, or FCC-ee test is defined | **After distinct Hermitian interaction and positive-density map** |
...
| **EL-9** | Source-free temporal action and fixed-charge particle evidence | Direct local gauging of the first-order Yang/Yin term gives the nonzero fundamental-condensate Gauss source $S^aS^a=\rho^2$. The separate second-order charged-field branch has time-dependent local $SU(2)_Q$ covariance and a Gauss-compatible static sector. Its neutral first-order carrier preserves an exactly empty closed sector. At the Mapped coupling, the stored Cartesian branch has a six-mode constrained finite-grid spectrum but fails its smooth-carrier diagnostic. Separate continuum-consistent scalar binding at prepared $Q_C\in\{16,256\}$ is independently reproduced | **Derived conditional action and empty-sector algebra / Mapped coupling / Tested finite-grid spectrum, ultraviolet obstruction and prepared smooth binding.** Smooth $Q_C=16$ constrained spatial stability is INCONCLUSIVE. Microscopic production, normalization, full stability and particle identity remain open | The Pauli identity, temporal covariance, Gauss variation or empty-sector conservation fails under the stated action and boundary assumptions; independent reconstruction fails the reported finite-grid or scalar measurements |
