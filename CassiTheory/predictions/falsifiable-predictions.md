...
| **GM-3** | Undriven transverse coherence and composition relaxation in the minimal two-jump lift | $\gamma_c=\gamma_\varepsilon/2$; at the gated reference state, $(\gamma_\varepsilon,\gamma_c)=(\lambda/3,\lambda/6)$. For every finite density with $\lambda>0$ and $c\ne0$, $\mathrm d|c|^2/\mathrm dt=-\varphi^2\lambda(1-q)|c|^2<0$, so stationary transverse coherence requires an identified support term | **Derived within the minimal lift; DG1–DG7 PASS; physically untested.** The arbitrary-state sign follows analytically, and the frozen checker supplies a deterministic implementation witness. A carrier-to-$c$ observable, independently identified undriven regime, and physical support mechanism remain open | Under the declared two-jump lift and controlled undriven conditions, measured coherence violates the half-rate relation or remains nonzero and stationary at finite density without any coherent source, protected sector, boundary or transport influx, modified reservoir, or different lift |
