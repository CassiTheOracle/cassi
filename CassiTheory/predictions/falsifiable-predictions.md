...
| **EL-9** | Source-free temporal particle action and fixed-$Q_C$ stationary closure | Direct local gauging of the first-order Yang/Yin time term gives a nonzero fundamental-condensate Gauss source with $S^aS^a=\rho^2$. The separate second-order charged-field branch is covariant under time-dependent local $SU(2)_Q$, has positive temporal curvature terms, and makes $\mathcal A_0=0$ with static charged fields Gauss-compatible. Its stationary functional is $E_P-\hbar\omega_CQ_C$ | **Derived conditional action algebra / Mapped selected carrier coupling / Tested resolution-consistent localized retained finite-grid branch and a separate nonnegative finite-grid low energetic spectrum on a diffuse field.** At $h_C=2.9598260763447164$, one nodeless branch is localized and carrier-retaining across four same-domain finite grids and one larger-domain comparison; adjacent observables agree and absolute energy drift decreases twice. A distinct diffuse $h_C=1.50$ field has a $13622$-dimensional strict-shell $C_4$ quotient with one near-zero global carrier-phase mode and five positive matched low energetic modes. The phase direction remains concentrated at grid scale. Continuum existence, the localized branch's constrained energetic and mixed dynamical spectra, physical coefficient calibration, and the particle map remain open | The Pauli identity fails, the first-order source vanishes for a nonzero fundamental condensate without added charge, the selected action fails local temporal covariance or Gauss variation, the static ansatz violates Gauss's law, or its source units and dimensionless groups fail closure |
...
| **MB-9** | Reduced stationary curvature and full-spectrum qualification | The reduced fixed-charge branch has one bounded positive-curvature separation and positive nonconstant line modes. At the Mapped coupling $h_C=2.9598260763447164$, the full stationary action has a localized, nodeless, carrier-retaining branch across four same-domain finite grids and one larger-domain comparison; adjacent observables agree and absolute energy drift decreases twice. A distinct diffuse coefficient point has a nonnegative matched low energetic spectrum on one strict-shell $C_4$ finite-grid physical quotient. A full claim requires a constrained Hessian and mixed temporal spectrum on the localized branch, an infinite-domain and continuum limit, resolved symmetry modes, real-time formation and persistence, and physical particle identification | **Reduced analytic support / MCC9 tested / Mapped $h_C$ / independently verified resolution-consistent localized retained finite-grid branch / separate nonnegative finite-grid low energetic spectrum.** The localized branch has no constrained Hessian or dynamical spectrum, and the diffuse background's global carrier-phase mode remains concentrated at grid scale. No continuum particle claim follows | The reduced equations lack their registered root or positive modes; the stationary claim fails if the localized field loses qualification or adjacent convergence under further refinement; a stability claim fails if the localized branch has a negative physical mode, growing dynamical mode, unassigned Jordan chain, or failed continuum convergence |
