...
| **MB-9** | Reduced stationary curvature and full-spectrum qualification | The frozen CC29 branch has one bounded positive-curvature root and positive nonconstant CC47 line modes. The selected finite-grid PA32 primary background passes Q1–Q4 at higher precision. Its strict-shell $C_4$ physical quotient has a nonnegative matched low PA42 spectrum within the frozen uncertainty. A full claim requires carrier localization and retention, domain and resolution convergence, spatial resolution of the global phase mode, the mixed PA43 temporal pencil, and a qualified continuum | **Reduced analytic support / MCC9 tested / one-point `PASS—HIGHER-PRECISION BACKGROUND` and `PASS—NONNEGATIVE C4 FINITE-GRID PA42 HESSIAN`.** The near-zero global phase mode remains grid-scale, the selected field fails localization and retention, and no Q2-qualified D/H background exists. No full particle or continuum-spectrum claim follows | The frozen reduced equations lack their registered root or positive modes; a full claim is also falsified by any negative physical mode, growing dynamical mode, unassigned Jordan chain, or failed continuum convergence |
