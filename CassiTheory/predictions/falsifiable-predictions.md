...
| **EL-2** | Frozen-background endpoint response at one matching wave number and at least one off-match wave number, together with the coherent endpoint current | For a declared species-port identification, common trace normalization, and frozen background $\Upsilon_{v,0}=u_ve^{i\alpha_v}$, the rail-rail Hessian is $\Lambda_{\mathrm{link},v}=2\kappa_vu_vM(\alpha_v)$. A dressed quarter-turn phase and $2\kappa_vu_v/(K_{\mathfrak s}k_\star)=\tau_\varphi:=r_\varphi/(1+t_\varphi)$ give $S_{\mathrm{link}}(k_\star)=S_{\varphi,\epsilon}$; the same frozen link has $a(k)=(k_\star/k)\tau_\varphi$. Unbiased proton-current capacity with positive fixed-amplitude phase stiffness requires $k_\star>0.0964640362$, with $k_\star$ free above the bound | **Derived conditional boundary algebra under the stated premises.** The frozen receipt passes ER1–ER5 with covariance residual $1.110\times10^{-16}$, unitarity residual $0$, selected-match residual $1.173\times10^{-16}$, $k_{\min,1}=0.096464036203895$, and off-match difference $0.227151634836$. Full fluctuation stability, physical port identification, trace normalization, endpoint background, dressed-phase selection, $k_\star$, and active response remain open | No consistent species-port trace map exists; the inferred $\Lambda$ is non-Hermitian in a declared closed elastic sector; the dressed-phase or coupling condition fails at the selected point; the current-capacity/phase-stiffness bound fails; or the measured off-match response contradicts the frozen-link Cayley law |
