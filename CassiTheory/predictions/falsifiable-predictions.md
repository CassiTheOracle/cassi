...
| **MB-9** | Reduced stationary curvature and full-spectrum qualification | The frozen CC29 branch has one bounded positive-curvature root and positive nonconstant CC47 line modes. The selected finite-grid PA32 primary background passes Q1–Q4. A full claim requires carrier localization, domain and resolution convergence, the PA42 eigenspectrum, the mixed temporal pencil, and a qualified continuum | **Reduced analytic support / MCC9 tested / PA32 one-point primary background and PA42 finite-grid quotient tested.** The stationary verdict is `PASS—Q2-QUALIFIED PRIMARY BACKGROUND`; the Hessian campaign returns `INCONCLUSIVE—HESSIAN PREFLIGHT` before eigenvalue evaluation, so no full particle or spectrum claim follows | The frozen reduced equations lack their registered root or positive modes; a full claim is also falsified by any negative physical mode, growing dynamical mode, unassigned Jordan chain, or failed continuum convergence |
