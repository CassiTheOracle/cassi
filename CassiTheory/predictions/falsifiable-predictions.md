...
| **EL-9** | Source-free temporal action and fixed-charge particle evidence | Direct local gauging of the first-order Yang/Yin term gives the nonzero fundamental-condensate Gauss source $S^aS^a=\rho^2$. The separate second-order charged-field branch has time-dependent local $SU(2)_Q$ covariance and a Gauss-compatible static sector. Its invariant $\mathcal O_N=\Psi^T i\sigma^2(\Phi^a\sigma^a)\Psi$ carries a physical common phase with $\omega^2=(J_x/J_t)k^2+O(k^4)$; the component projection, constrained spectra and linear wave qualify with `PASS` (`computations/matter-formation-continuum-report.md` §40). Its neutral first-order carrier preserves an exactly empty closed sector. At the Mapped coupling, the stored Cartesian branch has a six-mode constrained finite-grid spectrum but fails its smooth-carrier diagnostic. Separate continuum-consistent scalar binding at prepared $Q_C\in\{16,256\}$ is independently reproduced | **Derived conditional action, observable and empty-sector algebra / Mapped coupling / Tested linear wave, finite-grid spectrum, ultraviolet obstruction and prepared smooth binding.** Smooth $Q_C=16$ constrained spatial stability is INCONCLUSIVE. Microscopic production, quantum state, normalization, full stability and particle identity remain open; the charge-two classical composite selects no quantum paired condensate or exchange statistics | The Pauli identity, temporal covariance, Gauss variation, common-phase projection or empty-sector conservation fails under the stated action and boundary assumptions; independent reconstruction fails the reported phase, finite-grid or scalar measurements |
