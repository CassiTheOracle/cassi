...
- **Standalone-readable paragraphs**: write each paragraph so its physical or mathematical claim is understandable without decoding a test code, gate label, equation number, filename, or campaign identifier. State what was measured or derived, what it means, and what remains open in ordinary language. Identifiers such as `Q2`, `H7`, `PA42`, script names, and receipt paths may follow as traceability, but they must never serve as the subject, explanation, or shorthand for the result. A cross-reference may carry derivation detail; the surrounding paragraph must still stand on its own.
