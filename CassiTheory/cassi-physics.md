...
Full catalog: `predictions/falsifiable-predictions.md` (38 entries). The physics-specific predictions (1–3, 8–10) are listed here; the full set including the biological and psychological predictions is in the catalog and in `cassi-psychology.md` §22.
...
- **Hypothesized:** structurally consistent with Derived machinery, testable predictions supplied, but not yet experimentally confirmed or PDE-verified at the relevant scale. Examples: dark energy's $w_0, w_a$; dark matter as high-$q$ condensate; the identification of cascade rungs with specific physics scales; the chakra count and spacing; the pinch-point model of self-awareness; the trauma gate-lock model (PDE-tested 2026-07-31: pinning null as implemented, $\varphi$-phased drive effect supported and $\varphi$-specific).
