...
- **Hypothesized:** structurally consistent with Derived machinery, testable predictions supplied, but not yet experimentally confirmed or PDE-verified at the relevant scale. Examples: dark energy's $w_0, w_a$; dark matter as high-$q$ condensate; the identification of cascade rungs with specific physics scales; the chakra count and spacing; the pinch-point model of self-awareness; the trauma gate-lock model (PDE-tested 2026-07-31: pinning null as implemented, $\varphi$-phased drive effect supported and $\varphi$-specific at the held configuration).
