...
| $\mu_{a,L},\bar\mu,\nu_V,\lambda_{\mathrm{gi}},\lambda_{\mathrm{loc}},\lambda_f,\lambda_c,\lambda_{\mathrm{fib}},\kappa,C_*,m_*,r_f,r_c,M,A_{\mathrm{AT}},\rho,C_B$ in the pure-gauge comparison | exact finite-vacuum, coarse-marginal and conditional measures; gauge-invariant, local, fine, coarse and fibre Poincaré rates; transported score covariance; recurrence coefficient; target physical mass and fine/coarse dimensionless gap thresholds; mixed-Hessian control; tensorization, cover and boundary-density constants | $\Delta_{\mathrm{phys}}=(g^2/2a)\lambda_{\mathrm{gi}}$; $\lambda_{\mathrm{gi}}\ge\lambda_{\mathrm{loc}}/(A_{\mathrm{AT}}\rho)$ under (YM29)–(YM31); $\lambda_f\ge C_*^{-1}$ under (YM105)–(YM114), with $\lambda_f\ge\min\{2\lambda_c,\lambda_{\mathrm{fib}}/2\}$ at $\kappa=0$; $r_f=2a_fm_*/g_f^2$, $r_c=r_f/2$ for the factor-two matching, and $\kappa^2\le M^2/\lambda_{\mathrm{fib}}$ under the mixed-Hessian hypothesis | **Derived regulated identities / Derived conditional sufficient theorems**, `foundations/loop-to-bubble-projection-theorem.md` §§9.13–9.14, 9.20. These target-theory mathematical quantities introduce no fitted Cassi parameter. Their cutoff-, volume- and weak-coupling-uniform bounds remain open, and they do not change the §7 parameter count |
