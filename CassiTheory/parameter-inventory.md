...
| $N_G,\Sigma$ | first Chern number of the compact relative connection and its closed supporting two-cycle | $N_G=(g_Q/4\pi)\int_\Sigma G\in\mathbb Z$; $H^2(\mathbb R^3\times S^1_{\mathfrak s};\mathbb Z)=0$ on the smooth object base | **Derived conditional bundle topology.** A nonzero sector requires an excised core or line, boundary flux, different spatial topology, and a compact connection. It is independent of mixedness in $\Gamma$ and adds no numerical parameter or §7 count (`foundations/endpoint-link-and-localization-boundary.md` §5) |
