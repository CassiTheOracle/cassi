...
**Row count: 45.** A row here records the quantity's epistemic status. Each
...
that cite the quantity. The three scalar-parent normalization rows and the
conditional baryon calibration row above are selection records for supplied
comparison models, not additional independent parameters in the §7 total.
...
The baryon calibration fixes both coefficients from two measured masses and is
therefore **Mapped**. Its stationary profile, collective-coordinate formulas
and relaxation receipt do not promote the calibration to a derivation. The
conditional particle rules and physical-completion boundary remain separate
from the fit record.
