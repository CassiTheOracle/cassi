...
| $\mathbb W_P,T^{\rm closed}_{\mu\nu},J_\nu^{\rm io},\mathbb K_Q^{(2)},\mathbb M_Q,\mathbb G_Q,\mathbb P_Q(\omega)$ | multimode flux operator, total Hilbert stress, subsystem exchange density, physical fixed-charge Hessian, temporal and gyroscopic fluctuation operators, and mixed spectral pencil | $P=\operatorname{tr}(\mathbb W_P\rho_{\rm car})$; $\nabla^\mu T^{\rm closed}_{\mu\nu}=0$; $\nabla^\mu T^{\rm in}_{\mu\nu}=-J_\nu^{\rm io}$; $\mathbb P_Q=\mathbb K_Q^{(2)}-i\omega\mathbb G_Q-\omega^2\mathbb M_Q$ | **Derived conditional definitions and conservation/stability boundary / Tested low energetic Hessian sign on one diffuse finite-grid field.** $\mathbb W_P$ and explicit stress components require a selected metric-dependent carrier and reservoir action. The diffuse $h_C=1.50$ background has a $13622$-dimensional strict-shell $C_4$ representation of $\mathbb K_Q^{(2)}$ with a nonnegative matched low spectrum; its global phase direction remains concentrated at grid scale. The localized $h_C=2.9598260763447164$ branch has no constrained Hessian or mixed dynamical spectrum. Continuum qualification, $\mathbb M_Q$, and $\mathbb G_Q$ remain open. These operators introduce no universal parameter or §7 count (`foundations/matter-completion-boundary.md` §§5, 7–10; `foundations/particle-stationary-action-closure.md` §8.6; `computations/particle-carrier-resolution-recovery-report.md`; `computations/particle-physical-hessian-precision-v2-report.md`) |
...
| Finite stationary-solver boundary data | numerical domains, grids, seeds, tolerances, localization shells, and stopping rules | $\rho=1$, $c=0$, and a fixed gauge representative on $\partial\Omega_R$; no-flux scale interval; fixed $q_C$; diffuse point $(R,N)=(4,17)$ plus $(5,21)$ comparison; localized branch at $R=4$ on $N=17,21,25,29$ grids and at $R=5,N=21$ | **Numerical boundary data / Tested finite-grid localized branch.** These implementation choices carry no physical parameter status and remain outside the §7 count (`foundations/particle-stationary-action-closure.md` §§6–8; `computations/particle-carrier-resolution-recovery-report.md`) |
