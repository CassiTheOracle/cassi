...
| $m_W,m_H,\beta_Q,M_{\mathrm{BPS}},M_i,\kappa_L$ | conditional auxiliary vector and adjoint inverse lengths, mass ratio, BPS core mass, condensate London mass, and penetration inverse length | $m_W=g_Qv_Q$, $m_H^2=2\mu_x\lambda_Hv_Q^2$, $\beta_Q^2=2\mu_x\lambda_H/g_Q^2$, $M_{\mathrm{BPS}}=4\pi L_{\mathfrak s}v_Q\lvert N_G\rvert/(\mu_xg_Q)$, $M_i^2=g_Q^2K_x\rho_0/\varphi^3$, and $\kappa_L^2=\mu_xM_i^2$ | **Derived conditional identities.** The core formulas are adjoint-only, while the London formulas apply to the registered nonzero asymptotic condensate. Their free inputs have no selected values, so these combinations add no §7 numerical parameter (`foundations/nonabelian-magnetic-core-boundary.md`) |
| $M_{\mathrm{core}},\sigma_Q,C_Q$ | conditional monopole-core energy, flux-tube tension, and screened-tail strength | $E_{M\bar M}(L)=2M_{\mathrm{core}}+\sigma_QL-C_Qe^{-\kappa_LL}/L+\cdots$ with $\sigma_Q,C_Q>0$ in the tested long-distance branch | **Hypothesized coupled-composite inputs / Derived conditional positive-slope no-go.** No value or full tube profile is selected, and the registered energy has no finite-separation minimum; not counted in §7 (`foundations/nonabelian-magnetic-core-boundary.md`) |
