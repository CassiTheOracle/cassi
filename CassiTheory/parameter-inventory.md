...
| $\kappa_s$ (sector coupling) | $\varphi^{-6}/v_0^2$ | $0.92$ TeV$^{-2}$ | **M** | Optional selected Dirac↔two-fluid sector-coupling candidate; $\varphi^{-6}/v_0^2$ is a formal $C=1$ arithmetic scale conditional on the sector construction, not a physical $\kappa_s$ or equilibration timescale. The optional projection bracket is dimensionally incomplete ($[M]^3$ spinor density versus $[M]^2$ condensate square), and its ordinary-square and linear-projection interactions are generically non-Hermitian; a distinct dimensionally homogeneous Hermitian microscopic interaction and independently specified positive-density map are required before $\kappa_s$, $\chi$ or an FCC-ee test can be physical (`foundations/sector-coupling-derivation.md` §§2–4; `foundations/unified-lagrangian.md` §§2.2–2.4, 5.2, 6; `computations/matter-formation-continuum-report.md` §12) |
...
The proposed arithmetic bridge

$$\chi=\frac{\kappa_s\varphi^{-1}}{m_e(1+\varphi)}$$

has dimension $[M]^{-3}$ under the proposed $[\kappa_s]=[M]^{-2}$.
A factor $\mathcal N_{\rm pde}$ would need physical dimension $[M]^3$
before this expression could represent a dimensionless solver mobility.
Its formal substitution gives $4.25\times10^{-4}$ in mixed units.
The numerical value $\mathcal N_{\rm pde}\approx2.35\times10^3$ is
back-solved to a solver target and is **Mapped** in §10; the apparent
closure depends on the chosen grid convention
(`computations/n_pde_bridge_check.py`).

A physical mobility requires a distinct dimensionally homogeneous
Hermitian microscopic interaction, suitable positive-density or current
...
the specified local one-loop sea energy. It is a conditional benchmark restriction and
adds no fitted or derived physical parameter
(`computations/matter-formation-continuum-report.md` §§15.6–15.7).

The corresponding local static energy with these same inputs has no
global lower bound. The exact sign witness $m=64$ ($f=252$) is a
selected algebraic evaluation point, with no fitted physical mass,
preferred amplitude or new parameter. The bulk and interface energy
of a full spatial quantum completion remains unselected
...
conservative and angular properties under those inputs. The separate 36-check
integrity qualification establishes the species-level mass constraint,
defensive state construction, thermodynamic identities, physical-frequency
Doppler normalization, input rejection boundaries, exchange cancellation,
complete stellar and nuclear accounting, and missing-prerequisite
classification. The
...
| $\chi \approx 1.0$ | **Calibrated solver mobility; physical bridge unresolved.** The formal sector scale multiplies no selected admissible microscopic interaction, and its density map has dimensional, positivity and Hermiticity obstructions | The proposed $\chi=\mathcal N_{\rm pde}\kappa_s\varphi^{-1}/[m_e(1+\varphi)]$ requires a dimension-three normalization and independent transport matching; its numerical closure is Mapped (§10) |
...
current fit-status ledger contains **31 Calibrated or Mapped quantities**. The six
...
Symbols used framework-wide that are not counted parameter rows in §§1–6 (or names for rows that are). This section also records genuine free inputs of conditional action extensions descriptively; they remain outside the §7 count of 47 because those extensions are not part of the counted canonical classes. Bare $\alpha$ is prohibited repo-wide; always subscript.
...
| $B,\chi_v,\epsilon$ in the chromodielectric quark–meson bridge | dielectric vacuum-energy scale, dielectric field scale and positive gauge regulator | $B>0$, $\chi_v>0$, $\epsilon\to0^+$; dimensionless verification uses $B=\chi_v=1$ and $\epsilon\in\{10^{-2},10^{-4},10^{-6},10^{-8}\}$ | **External target-theory inputs / fixed regulator schedule**, `computations/qcd-confining-carrier-prereg.md`. No hadron observable fixes $B$ or $\chi_v$ in the calculation. The action is a finite-cutoff empirical QCD bridge, so these inputs are outside the canonical §§1–6 classes and do not change the §7 count |
| $N_c,f_\pi,m_\pi,m_\sigma,g,M_\chi,\gamma,\eta,\delta$ in the interacting chromodielectric baryon test | colour occupation, chiral vacuum and masses, quark coupling, dielectric mass, dielectric-potential coordinates and positive coupling regulator | $3,\ 93.0,\ 139.6,\ 1200,\ 23.0,\ 1700\ {\rm MeV},\ 0.2,\ 0.12,\ \delta\to0^+$ | **External empirical QCD inputs / fixed regulator schedule**, `computations/qcd-interacting-baryon-prereg.md`. These published comparison values define a supplied four-coordinate mean-field calculation; they are outside the canonical §§1–6 classes, do not change the §7 count, and do not follow from $\varphi$ |
...
| $\sigma$ | regularization scale | $\ell_{\text{Pl}}/\varphi^3$ | Conditional softened-kernel scale and Gaussian free-propagator form factor; no physical gravity core or interacting UV-finite theory is established. The nonzero-$\sigma$ Gaussian is excluded as a standard unsubtracted positive physical scalar covariance (`gravity/quantum-gravity.md` §3.1); an auxiliary/regulator interpretation needs a separately qualified physical sector. The length assignment is **Derived conditional** on the noise–signal identification, Hypothesized cascade-dephasing family $d_i=\varphi^{-i-\delta}$ and selected $d=3$ domain; only the $\varphi^{-3}$ arithmetic follows once $\delta=3$ is selected by that criterion. Registry G1; no ledger row or added §7 parameter |
...
| $g_f,a_f,b$ and induced $x_f,g_c,x_c$ in the pure-gauge comparison | supplied bare $SU(2)$ coupling, fine spacing and integer path length, with block-map coefficients | $g_f,a_f>0$; $x_f=2/g_f^4$; $g_c^2=b^2g_f^2$ and $x_c=x_f/b^4$ when $a_c=ba_f$; $\Delta_E=3g_f^2/(2a_f)$ on the girth-four graph; $\Delta E_{\mathrm{el},\partial}=2g_f^2/a_f$ inside the fixed fundamental-boundary fibre | **External target-theory inputs / Derived regulated spectrum, representation-support and unit identities**, `foundations/loop-to-bubble-projection-theorem.md` §§9.4–9.27. Numerical couplings, path lengths, support sizes and representation cutoffs are fixed controls. No Cassi parameter, fitted value, physical mass prediction or change to the §7 count is introduced |
| $d_C,\rho_{\infty,S},\omega_x,\mathcal G_x$ in the thermodynamic pure-gauge comparison | one-link Peter–Weyl cutoff rank, compatible subsequential local densities, fixed-regulator algebraic state and stabilized local commutator | $d_C=(C+1)(C+2)(2C+3)/6$; $\omega_x(A)=\operatorname{Tr}(\rho_{\infty,S}A)$ and $\omega_x(A^*\mathcal G_x(A))\geq0$ on the local gauge-invariant $*$-algebra whose operators have finite Peter–Weyl matrix support | **Derived conditional mathematical objects**, `foundations/loop-to-bubble-projection-theorem.md` §9.27. The finite-volume ground densities and volume-uniform local tail estimate are analytic inputs. These symbols introduce no fitted value, physical mass prediction or change to the §7 parameter count; full-sequence convergence, clustering, a uniform gap and the continuum limit remain open |
| $\beta,C_n(\beta),\mu_{L,\beta},\mu_{\infty,\beta},\gamma_{\Delta,\beta},(\cdot,\cdot)_{\mathrm{OS}}$ in the Euclidean pure-gauge comparison | supplied Wilson inverse coupling, positive $SU(2)$ character coefficients, finite-torus and subsequential infinite-lattice Gibbs measures, local specification and fixed-lattice Osterwalder–Schrader form | $C_n(\beta)=I_{n-1}(\beta)-I_{n+1}(\beta)=2nI_n(\beta)/\beta>0$; $\mu_{L_k,\beta}^{(m)}\Rightarrow\nu_\beta^{(m)}$ on every fixed link box; $\mu_{\infty,\beta}$ is translation invariant, gauge invariant, reflection positive and DLR | **External target-theory input / Derived conditional mathematical objects**, `foundations/loop-to-bubble-projection-theorem.md` §9.28. The standard finite-lattice Wilson reflection and transfer theorems are analytic inputs. These symbols add no fitted value, physical mass prediction or §7 parameter; full-sequence convergence, uniqueness, clustering, identification with the anisotropic Hamiltonian family, $a\to0$, continuum reconstruction and a uniform gap remain open |
| $\epsilon,B_\tau,B_\sigma,q_B,T_\epsilon,A_\epsilon,H_\epsilon,g_W^{\mathrm{YM}},a_\tau$ in the anisotropic pure-gauge comparison | target Hamiltonian step, temporal and spatial Wilson character weights, normalized one-link density, symmetric transfer, difference and logarithmic generators, Wilson bare coupling and Euclidean temporal spacing | $B_\tau=4a/(g^2\epsilon)$; $B_\sigma=2\epsilon/(g^2a)$; $q_B(W)=\exp[(B/2)\operatorname{Tr}W]/C_1(B)$; $T_\epsilon=e^{-\epsilon V/2}\mathcal R_{B_\tau}e^{-\epsilon V/2}$; $A_\epsilon=(I-T_\epsilon)/\epsilon$; $H_\epsilon=-\epsilon^{-1}\log T_\epsilon$; $g_W^{\mathrm{YM}}=2^{1/4}g$; $a_\tau=\epsilon/\sqrt2$ | **External target-theory inputs / Derived fixed-graph mathematical objects**, `foundations/loop-to-bubble-projection-theorem.md` §9.29. These quantities introduce no fitted value, physical mass prediction or change to the §7 count. The theorem's Wilson $g_W^{\mathrm{YM}}$ is distinct from the CassiFI Kähler metric $g_W$ below. Spatial-volume uniformity, the lattice-spacing limit, continuum reconstruction and a regulator-independent gap remain open |
| $b_0,b_1,p,F_W,F_H,N(g_0),\delta_W,c_-,c_+$ in the continuum pure-gauge comparison | universal two-loop beta-function coefficients and exponent, Wilson/Hamiltonian lattice scale factors, simultaneous volume schedule, dimensionless Wilson gap and finite renormalized-gap bounds | $b_0=11/(24\pi^2)$; $b_1=17/(96\pi^4)$; $p=b_1/(2b_0^2)=51/121$; $F_W=\exp[-1/(2b_0g_0^2)](b_0g_0^2)^{-p}$; $F_H(g_H)=F_W(2^{1/4}g_H)$; $g_0\to0$ and $N(g_0)F_W(g_0)\to\infty$; $0<c_-\leq\liminf\delta_W/F_W\leq\limsup\delta_W/F_W\leq c_+<\infty$ | **External target-theory inputs / Derived two-loop asymptotic relation and necessary conditional criterion**, `foundations/loop-to-bubble-projection-theorem.md` §9.30. The universal beta-function coefficients introduce no Cassi parameter; $c_\pm$ denote bounds to be proved, not selected values. These symbols add no fitted value, mass prediction or change to the §7 count. An interacting gap, coupled continuum/thermodynamic trajectory, nontrivial Osterwalder–Schrader limit and regulator-independent mass remain open |
| $B,r_k,R_n,C_{\mathrm{RG}},f_\pm,d_\pm,\mathcal R_k,\gamma_k,\delta_k$ in the conditional pure-gauge RG comparison | isotropic block factor, one-step and cumulative two-loop scale defects, cumulative bound, endpoint scale and spectral-rate windows, retained observable families, channel rates and family infimum rates | $r_k=\log F_W(g_{k+1})-\log F_W(g_k)-\log B$; $R_n=\sum_{k<n}r_k$; $|R_n|\leq C_{\mathrm{RG}}$; $0<f_-\leq F_W(g_n)\leq f_+$; $0<d_-\leq\delta_n\leq d_+$; exact retained correlations give $\delta_0=B^{-n}\delta_n$ | **Derived conditional mathematical objects**, `foundations/loop-to-bubble-projection-theorem.md` §9.31. The executable values $B=2$, $f_-=1/64$, $C_{\mathrm{RG}}=0.025$ and $d_\pm=(0.5,2)$ are frozen synthetic controls, not physical Yang–Mills parameters or eigenvalues. The RG trajectory, cumulative bound, block map, observable completeness and endpoint gap remain analytic inputs; this row does not change the §7 parameter count |
| $\beta_{\mathrm Y},\gamma_{\mathrm Y},\delta_{\mathrm Y},C_D,\alpha_{\mathrm{rel}}$ in the pure-gauge comparison | stability-theorem constants, finite-depth local remainder bound and relative interaction coefficient | $64/g^4\le\beta_{\mathrm Y}$ suffices for $\Delta H\ge3g^2\gamma_{\mathrm Y}/(32a)$; $\alpha_{\mathrm{rel}}=32|x|/3$; $\|r_B\|\le C_Dx^2$ | **Derived regulated theorem application**, `foundations/loop-to-bubble-projection-theorem.md` §§9.10–9.12. The positive theorem constants and circuit remainder constant are unevaluated; no numerical critical coupling is asserted. The isolated-square bound is separately $(2+\sqrt2)x^2/3$. These mathematical bounds and fixed method choices add no physical parameter to §7 |
| $\mu_{a,L},\bar\mu,\nu_V,\mathcal L_V,\lambda_{\mathrm{gi}},\lambda_{\mathrm{loc}},\lambda_f,\lambda_c,\lambda_{\mathrm{fib}},\kappa,\vartheta,C_*,C_{-1},m_*,r_f,r_c,t_f,\delta_c,\delta_v,\delta_f,R_{\mathrm{cmp}},M,A_{\mathrm{AT}},\rho,C_B,P_B,R_B,w_B,\mathscr R,\gamma_{\mathrm{rec}},\gamma_*,T_{j\to n},\mathcal N_n$ in the pure-gauge comparison | exact finite-vacuum, coarse-marginal and conditional measures; conditional Poisson operator; gauge-invariant, local, fine, coarse and fibre Poincaré rates; $L^2$ score covariance and $H^{-1}$ minimum transport cost; recurrence coefficients; target physical mass, normalized rate and margin coordinates; Gaussian score-comparison factor; mixed-Hessian, tensorization, cover and boundary-density constants; conditional projections, residual weights and Gramian; finite and uniform recovery floors; scale isometries and declared nonphysical null sector | $\Delta_{\mathrm{phys}}=(g^2/2a)\lambda_{\mathrm{gi}}$; $\lambda_{\mathrm{gi}}\ge\lambda_{\mathrm{loc}}/(A_{\mathrm{AT}}\rho)$ under (YM29)–(YM31); $\lambda_f\ge C_*^{-1}$ under (YM105)–(YM114); $\lambda_f\ge C_{-1}^{-1}$ under (YM126)–(YM132), with $\vartheta^2\le\kappa^2/\lambda_{\mathrm{fib}}$; the exact margin budget is (YM139); $\mathscr R=\sum_Bw_BR_B$, $A_{\mathrm{AT}}^{\mathrm{opt}}=\gamma_{\mathrm{rec}}^{-1}$ and $\lambda_{\mathrm{gi}}\ge\gamma_{\mathrm{rec}}\lambda_{\mathrm{loc}}/\rho$ under (YM152)–(YM158); multiscale recovery requires $\mathscr R_n\succeq\gamma_*(I-\Pi_{\mathcal N_n})$; $r_f=2a_fm_*/g_f^2$, $r_c=r_f/2$, and $\kappa^2\le M^2/\lambda_{\mathrm{fib}}$ under the mixed-Hessian hypothesis | **Derived regulated identities / Derived conditional sufficient theorems**, `foundations/loop-to-bubble-projection-theorem.md` §§9.13–9.14, 9.20–9.23. These target-theory mathematical quantities introduce no fitted Cassi parameter. Their cutoff-, volume- and weak-coupling-uniform interacting-vacuum bounds remain open, and they do not change the §7 parameter count |
| $\delta_\square(x):=\lambda_1(h_x)-\lambda_0(h_x),\Delta_{\square,1},\|Q_N\psi_j^{(x)}\|^2,\lambda_j(h_x),N(x)$ in the isolated-square radial comparison | dimensionless class-function gap, physical isolated-square gap, discarded probability, fixed-level eigenvalues and character cutoff schedule | $\delta_\square(1)\approx3.42628856573$; with $x=2/g^4$, $a\Delta_{\square,1}=\delta_\square(x)/\sqrt{2x}$, its $x=65536$ control is approximately $2.8249709905$, and $a\Delta_{\square,1}\to2\sqrt2$ as $x\to\infty$; for each fixed $j$, $\lambda_j(h_x)/\sqrt{x}\to4j+3$, while bare finite-section convergence and $\|Q_N\psi_j^{(x)}\|^2\to0$ hold iff $N(x)/x^{1/4}\to\infty$, equivalently $gN\to\infty$ | **Derived regulated spectral quantities and fixed controls**, `foundations/loop-to-bubble-projection-theorem.md` §§9.18.1–9.18.7. The version-3 finite campaign uses $q(N)=\min\{3,N+1\}$; its 30 schedule rows contain 88 requested low eigenvalues, including two $N=1$ rows with $j=0,1$ only. The exact energy-dependent Feshbach reduction is retained at finite scaled cutoff, while bare cutoff convergence fails for fixed or finite $N/x^{1/4}$. These mathematical controls add no fitted Cassi parameter or change to the §7 count |
...
| $\gamma_{a,n},r^{YI}_n,r^{IY}_n$ | carrier jump, conversion, and hopping rates | $r^{YI}_n=\lambda(1-q_n)k_{Y,n}$; $r^{IY}_n=\varphi\lambda(1-q_n)k_{I,n}$; nearest-neighbour hopping uses the declared finite-volume rates | **Derived conditional** generator once the canonical $\lambda$, $q_n$, transport coefficients, lattice, and carrier projection are supplied. Positivity, conservation, and the projected first-moment law are certified by QC1–QC9. These are the specified unsaturated occupation-shift rates; bilinear Bose/Fermi jumps carry additional final-state factors (`computations/matter-formation-continuum-report.md` §38). No new fitted drift coefficient is introduced |
...
| $\alpha,k_Y,k_I,\mu_Y,\mu_I,J_0,\kappa,K$ | conditional compact-counterflow ratio, phase-gradient magnitudes, mobilities, imposed current, adjustment rate, and accumulated exposure | $\alpha=k_I/k_Y=(\mu_Y/\mu_I)(E_Y/E_I)-J_0/(\mu_IE_Ik_Y)$; $K(t)=\int_0^t\kappa(s)\,ds$ | **Hypothesized compact-current closure / Derived conditional dynamics.** Equal mobilities, zero net current, and adiabatic adjustment give $\alpha=E_Y/E_I$; adding the declared local lift $\dot\alpha=-\kappa(1+\alpha)(\alpha-\varphi)$ gives the verified transient. The compact phases, constitutive current, phase-slip dynamics, and nonnegative rate $\kappa$ remain physical inputs; no value is fixed or counted in §7 (`principles/de-resonance-principle.md` §1) |
| $g_{M_0},\eta_{M_0},a_{M_0},N,\Delta t,T$ | passive compact-field candidate stiffness, alignment, initial notch, lattice, step, and exposure | $(1,0.05,0.05)$; $(64,0.02,600)$ and $(96,0.01,600)$ | **Tested numerical candidate—$\mathrm{REJECT}\ M_0$.** The frozen passive model passes sampled descent and produces zero $\varphi$-band counteroriented sectors in both arms. These are protocol settings, introduce no canonical parameter, and are not counted in §7 (`computations/phase-slip-selection-report.md`) |
| $\operatorname{Var}(E^{\rm car}_{I,n}),\epsilon^{\rm var}_{a,n}$ | exact finite-carrier binomial fluctuation and declared transport-noise variance | $\eta^2N_c\,\varphi^{-2}(1-\varphi^{-2})=\eta^2N_c\,\varphi^{-3}$ at fixed positive conversion rate; $\epsilon^{\rm var}_{a,n}$ sets the finite transport-noise power | **Derived conditional** fluctuation law for the specified unsaturated carrier process, whose equilibrium Yin probability is $\varphi^{-2}$ (`foundations/quantum-measurement-derivation.md` §8.4.6); $\epsilon^{\rm var}_{a,n}$ is a declared regulator/noise input. Neither quantity is a universal constant or included in the §7 numerical totals |
...
| $\Gamma,c,\eta_c,\mathbf n,\mathbf X,\mathcal C_2^+$ | positive Yang/Yin coherence matrix, cross moment, normalized coherence, Bloch-ball coordinate, affine bubble point, and positive Hermitian fibre | $\Gamma=\begin{psmallmatrix}E_Y&c^*\\c&E_I\end{psmallmatrix}\in\mathcal C_2^+=\operatorname{Herm}_2^+$; $\eta_c=|c|/\sqrt{E_YE_I}$; $\det\Gamma=\rho^2(1-\|\mathbf n\|^2)/4$; $\mathbf X=D\mathbf n$ | **Derived conditional coordinate geometry.** The nonzero fibre is $\mathbb R_{>0}\times B^3$ and is contractible; rank one gives the $\mathbb{CP}^1$ shell, and the canonical real-density state is its diagonal diameter. State-only projective charge can unwind through the interior. The coherence metric obeys $2\operatorname{tr}(d\widehat\Gamma^2)=d\mathbf n^2=d\mathbf X^{\mathsf T}D^{-2}d\mathbf X$. The positive axes in $D$ remain physically unset. These are state diagnostics with no new fixed parameter or §7 count (`foundations/geometric-manifold-completion.md`; `foundations/yin-yang-qi-dynamical-geometry.md`) |
| $\Gamma_{\rm in},\Gamma_{\rm out},\Gamma_{\rm io},\mathsf C_{\rm io},K_{\rm io}$ | interior and exterior Yang/Yin coherence fibres, enlarged state, cross-domain coherence, and normalized cross-coherence | $\Gamma_{\rm io}=\begin{psmallmatrix}\Gamma_{\rm in}&\mathsf C_{\rm io}\\\mathsf C_{\rm io}^\dagger&\Gamma_{\rm out}\end{psmallmatrix}\succeq0$; for positive-definite local fibres, $\mathsf C_{\rm io}=\Gamma_{\rm in}^{1/2}K_{\rm io}\Gamma_{\rm out}^{1/2}$ with $\|K_{\rm io}\|_{\rm op}\le1$ | **Derived conditional interface geometry and channel boundary.** The singular case requires the pseudoinverse support condition. For the selected reduced channel, the mathematical exterior is its complementary Stinespring output, unique up to an environment unitary in a minimal dilation; the registered single-excitation golden witness has environment dimension two. The physical exterior fibre, carrier, topology, and boundary dynamics remain Hypothesized/open. These variables add no fitted numerical parameter or §7 count (`foundations/yin-yang-qi-dynamical-geometry.md` §7.4; `foundations/matter-completion-boundary.md` §2; `computations/matter_completion_boundary_report.md`) |
| $V,L_a,R_a,\mathcal S_{\rm in},\mathcal S_{\rm out}$ | reciprocal two-domain coupling, left/right scale-transfer maps, and induced interface sources | $\mathcal H_{\rm int}=\Psi_{\rm in}^\dagger V\Psi_{\rm out}+\Psi_{\rm out}^\dagger V^\dagger\Psi_{\rm in}$; $V\mapsto U_{\rm in}VU_{\rm out}^\dagger$; $K_{a+1}=L_aK_aR_a^\dagger$; $\mathcal S_{\rm in}=-(i/\hbar)(V\mathsf C_{\rm io}^\dagger-\mathsf C_{\rm io}V^\dagger)$ with equal-and-opposite enlarged number exchange | **Derived conditional lowest-order interface form, exchange algebra, and transfer covariance / Hypothesized physical coefficients.** Locality, bilinearity, Hermiticity, enlarged-number conservation, and independent-frame covariance fix the displayed zero-derivative form. A discrete map leaves matrix-logarithm branches unresolved. For one routed forward carrier, $L_a=t_\varphi U_a$ and unitary $R_a$ give $\|K_N\|_F/\|K_0\|_F=\varphi^{-N/2}$; symmetric attenuation describes two routed legs. The values and microscopic carrier remain unselected and add no §7 count (`foundations/matter-completion-boundary.md` §§3–4; `computations/matter_completion_boundary_report.md`) |
| $\mathcal E_T,E_0,E_1,\Delta t,\gamma_{\rm route},\Omega_{\rm io},F_{\rm io}$ | reduced golden channel, its Kraus operators, collision interval, derived decay rate, cross-mode frequency, and maintained-coherence source | $E_0=\operatorname{diag}(1,\sqrt T)$, $E_1=\sqrt{1-T}\,|0\rangle\langle1|$, $\gamma_{\rm route}=-\ln T/\Delta t$, and $C_*=F_{\rm io}/(\gamma_{\rm route}/2+i\Omega_{\rm io})$ | **Derived conditional channel and response law / Hypothesized physical timing and reservoir inputs.** $T=\varphi^{-1}$ only under the declared golden port-power identification. $\Delta t$, $\Omega_{\rm io}$, $F_{\rm io}$, the bath spectrum, and the physical reservoir are unselected. $\gamma_{\rm route}$ is derived after $T$ and $\Delta t$ are supplied and is distinct from $\gamma_{\rm conv}$; no quantity in this row enters the §7 count (`foundations/matter-completion-boundary.md` §§2, 6) |
| $\mathbb W_P,T^{\rm closed}_{\mu\nu},J_\nu^{\rm io},\mathbb K_Q^{(2)},\mathbb M_Q,\mathbb G_Q,\mathbb P_Q(\omega)$ | multimode flux operator, total Hilbert stress, subsystem exchange density, physical fixed-charge Hessian, temporal and gyroscopic fluctuation operators, and mixed spectral pencil | $P=\operatorname{tr}(\mathbb W_P\rho_{\rm car})$; $\nabla^\mu T^{\rm closed}_{\mu\nu}=0$; $\nabla^\mu T^{\rm in}_{\mu\nu}=-J_\nu^{\rm io}$; $\mathbb P_Q=\mathbb K_Q^{(2)}-i\omega\mathbb G_Q-\omega^2\mathbb M_Q$ | **Derived conditional definitions and conservation/stability boundary / Tested finite-grid low spectra and INCONCLUSIVE smooth constrained spatial stability.** The diffuse $h_C=1.50$ and localized $h_C=2.9598260763447164$ Cartesian fields have independently matched low energetic spectra in their finite-grid $C_4$ quotients. The localized carrier is parity-concentrated and fails the smooth-carrier diagnostic. The separate smooth $Q_C=16$ calculation resolves no negative constrained spatial mode but fails the coarse symmetry and same-spacing domain criteria. $\mathbb W_P$, explicit stress components, full continuum and nonlinear stability, $\mathbb M_Q$ and $\mathbb G_Q$ remain open. These operators introduce no universal parameter or §7 count (`foundations/matter-completion-boundary.md` §§5, 7–10; `foundations/particle-stationary-action-closure.md` §§8.6–8.7; `computations/matter-formation-continuum-report.md`) |
...
| $\Gamma_0$ | reference-state gated composition-relaxation rate | $\lambda/3$ at $(E_Y,E_I)=(1,\varphi^{-1})$; ungated $\varphi^2\lambda$ | **Derived conditional**—from $\Gamma_0=(1+\varphi)\lambda(1-q_{\mathrm{eq}})$, $q_{\mathrm{eq}}=\varphi^2/3$, and $1+\varphi=\varphi^2$; not an independent parameter and not counted in §7. The fixed-$\rho=\varphi$ nonlinear rate is physically restricted to $-\varphi^2\le\varepsilon\le\varphi$; see `computations/verify_physical_becoming_reduction.py`. **Measured** at $N=32$, $\lambda=0.1$, $\Delta t=0.002$: the ungated volume-mean rate is $0.2618034$ against $\varphi^2\lambda=0.2618034$ across four off-ray compositions at $T=30$, and the gated volume-mean rate at the reference state is $0.0296$ to $0.0298$ against $\Gamma_0=\lambda/3=0.0333$, reproduced by $(1+\varphi)\lambda\Xi$ with the measured openness weighting $\Xi=\langle w\varepsilon\rangle/\langle\varepsilon\rangle=0.11254$. The measured rate is the trace $\kappa(1+\varphi)$ of the declared conversion block: a rank-one block whose entries carry the ratio has $\operatorname{tr}M^k=(-1)^k(a+b)^k$, so its spectrum, characteristic polynomial and relaxation rate are functions of $a+b$ alone while the ratio lives in the null vector (`foundations/loop-rate-selection-candidates.md` §2.2; `foundations/phi-input-or-selection.md:177-181`); the agreement measured here is therefore between two slots of the declared pair, not a determination of the ratio. (`computations/verify_two_fluid_phi_ray_relaxation.py`; `runs/two_fluid_phi_ray_relaxation/verification.json`) |
| $\gamma_{\rm conv},\mathcal L_0,\gamma_\varepsilon,\gamma_c$ | state-dependent conversion prefactor, fixed GKSL generator, composition-imbalance rate, and transverse-coherence rate in the minimal two-jump lift | $\gamma_{\rm conv}=\lambda(1-q)$; $\mathcal L_{\rm conv}[\Gamma]=\gamma_{\rm conv}(\Gamma)\mathcal L_0[\Gamma]$; $\gamma_\varepsilon=\varphi^2\gamma_{\rm conv}$; $\gamma_c=\gamma_\varepsilon/2$; at the gated reference state $(\gamma_\varepsilon,\gamma_c)=(\lambda/3,\lambda/6)$ | **Derived within the minimal lift.** The population equations reduce exactly to canonical conversion. The state-dependent flow is nonlinear and preserves the positive cone by nondecreasing time reparametrization of the fixed $\mathcal L_0$ flow; only the fixed-$q$ generator defines a linear completely positive semigroup. For finite density, $\lambda>0$, and $c\ne0$, $\mathrm d|c|^2/\mathrm dt=-\varphi^2\lambda(1-q)|c|^2<0$. These are derived combinations, not independent parameters or §7 counts (`foundations/yin-yang-qi-dynamical-geometry.md` §§5–7) |
...
| $\mathfrak s,\ell_\star,\Psi(\mathbf x,\mathfrak s,t)$ | continuous logarithmic scale coordinate, reference length, and complex Yang/Yin doublet | $\mathfrak s=\log_\varphi(\ell/\ell_\star)$; $\ell_\star$ external; $[\Psi]=L^{-3/2}$ under the flat-$d\mathfrak s$ number-density convention | **Hypothesized interscale extension** in `foundations/interscale-current-soliton.md`; changing $\ell_\star$ translates the coordinate, and the current framework selects neither the flat-density nor invariant-measure normalization branch; not counted in §7 |
| $K_x,K_{\mathfrak s},\lambda_\rho,\lambda_\varphi,\rho_0$ | spatial and scale kinetic stiffnesses, density and composition quartics, and reference density of the interscale action | positive free coefficients; $[K_x]=\hbar L^2/T$, $[K_{\mathfrak s}]=\hbar/T$, $[\lambda_{\rho,\varphi}]=\hbar L^3/T$, $[\rho_0]=L^{-3}$ under the declared flat-density convention | **Hypothesized interscale inputs**; $K_x=\hbar^2/m_c$ only under a supplied nonrelativistic carrier interpretation, and the action does not derive $m_c$ or the scale metric; not counted in §7 |
| $g_Q$ | relative Yang/Yin gauge normalization shared by the Abelian endpoint action and the auxiliary internal $SU(2)_Q$ completion | positive dimensionless free input; minimum fundamental charges are $\pm g_Q/2$; under connection rescaling $g_Q'=g_Q/a$, while $e_x^2=g_Q^2\mu_x$ and $e_m^2=g_Q^2\mu_m$ remain invariant | **Hypothesized interscale and auxiliary-completion input.** No value is selected or derived from $\varphi$; it is a normalization coordinate rather than an additional §7 numerical parameter (`foundations/interscale-current-soliton.md`; `foundations/nonabelian-magnetic-core-boundary.md`) |
| $B_i,B_{\mathfrak s},\mu_x,\mu_m$ | relative Yang/Yin connection components and spatial/mixed curvature stiffnesses | $[B_i]=L^{-1}$, $[B_{\mathfrak s}]=1$, $[\mu_x]=T/(\hbar L)$, $[\mu_m]=TL/\hbar$; only $e_x^2=g_Q^2\mu_x$ and $e_m^2=g_Q^2\mu_m$ are internally identifiable | **Hypothesized interscale inputs.** The first-order interscale branch is source-free Gauss-obstructed for a nonzero fundamental condensate. A separate conditional second-order particle branch supplies temporal curvatures and Gauss's law in `foundations/particle-stationary-action-closure.md`; not counted in §7 |
| $W_\sigma,C_\sigma,\widetilde C_\sigma,\sigma,p,\Gamma_0$ | collective density and phase Hessians, constrained longitudinal phase Hessian, combined spatial/scale symbol, scale wave number and initial covariance | $W_\sigma=(\lambda_\rho/2)aa^T+\lambda_\varphi bb^T+(\sigma/4)D^{-1}$; $\sigma=K_x\lvert\mathbf k\rvert^2+K_{\mathfrak s}p^2$; phase inertia $\hbar^2W_\sigma^{-1}>0$; $[W_\sigma]=\hbar L^3/T$, $[C_\sigma]=[\widetilde C_\sigma]=\hbar/(TL^3)$ and $[p]=1$ | **Derived conditional collective response.** Admissible longitudinal connection variation gives one propagating bulk branch. Its boundary-dependent gap selects no particle mass. The covariance and exterior population are initial-state inputs. The dimensionless check uses $\hbar=K_x=K_{\mathfrak s}=\rho_0=1$, $\lambda_\rho=\lambda_\varphi=4$, scale interval length $8$ and the two-level witness $H_L=1,H_E=4,V=1$; these are unfitted calculation data and add no §7 parameter (`foundations/interscale-current-soliton.md` §12; `computations/matter-formation-continuum-report.md` §37) |
| $u_{a,i},M_a,c_{T,a},\kappa_a,p_{a,i},T_{a,ij},\Pi_{a+1/2,i}$ | effective reciprocal scale-ladder displacement, mass density, transverse speed, interface stiffness, momentum density, spatial stress, and discrete mixed stress | $[u]=L$, $[M]=ML^{-3}$, $[c_T]=LT^{-1}$, $[\kappa]=ML^{-3}T^{-2}$, $[p]=ML^{-2}T^{-1}$, $[T_{ij}]=ML^{-1}T^{-2}$, and $[\Pi]=ML^{-2}T^{-2}$ for dimensionless cascade-step separation | **Hypothesized stress carrier / Derived conditional conservation ledger.** Positive $M_a$ and $\kappa_a$ give pairwise momentum cancellation, positive energy, real reciprocal normal modes, and series compliance. The physical carrier, coefficient values, and map to the canonical Yang/Yin action remain unselected; not counted in §7 (`foundations/interscale-stress-attenuation-boundary.md`) |
| $T_\varphi,R_\varphi,t_\varphi,r_\varphi,S_\varphi,\sigma_a;\ \Lambda_v,\Lambda_{\mathrm{link},v},k_\star,\tau_\varphi,S_\Lambda(k)$ | forward and return power fractions, representative amplitudes, golden two-port matrix, spatial-momentum routing label; Hermitian endpoint coupling, charged-link realization, design wave number, inverse-Cayley coefficient, and two-lead scattering family | $T_\varphi=\varphi^{-1}$, $R_\varphi=\varphi^{-2}$, $t_\varphi=\varphi^{-1/2}$, $r_\varphi=\varphi^{-1}$, $S_\varphi=\left(\begin{smallmatrix}t_\varphi&r_\varphi\\-r_\varphi&t_\varphi\end{smallmatrix}\right)$, $\sigma_a\in\{+1,-1\}$; for dimensionless $\mathfrak s$, $\Lambda_v$ and $\Lambda_{\mathrm{link},v}$ are Hermitian $2\times2$ auxiliary couplings with $[\Lambda]=[K_{\mathfrak s}]=\hbar/T$, while $k_\star$, $\tau_\varphi$, and $S_\Lambda$ are dimensionless; $S_\Lambda(k)=(ikK_{\mathfrak s}I-\Lambda_v)^{-1}(ikK_{\mathfrak s}I+\Lambda_v)$, $\tau_\varphi=r_\varphi/(1+t_\varphi)$, and matching the Hypothesized target at $k_\star$ requires $\Lambda_\varphi=iK_{\mathfrak s}k_\star\tau_\varphi\left(\begin{smallmatrix}0&1\\-1&0\end{smallmatrix}\right)$ | **Hypothesized golden target and species-port map / Derived conditional frozen and first-order source-action endpoint-response algebra / Hypothesized physical endpoint selection.** The quadratic action fixes the boundary flux; Hermitian endpoint data give unitary $S_\Lambda(k)$; the registered two-rail gluing is perfect transfer; the charged endpoint action gives $\Lambda_{\mathrm{link},v}=2\kappa_vu_vM(\alpha_v)$; a dressed quarter-turn phase and $2\kappa_vu_v/(K_{\mathfrak s}k_\star)=\tau_\varphi$ realize the target at one $k_\star$; a fixed frozen link gives a wave-number-dependent split. First-order source-action elimination of EL9 gives the active Nambu response with covariance under constant relative-frame rotations. The frozen AR1–AR6 receipt passes; the DR receipt remains `FAIL` because its DR5 endpoint block has the opposite source-action sign. The symmetric zero-background eliminated-source-action term begins at quartic rail order with a positive coefficient when $\mu_{v,0}:=W_v'(0)>0$; physical energy, stress, inertial mass, and stability signs remain open. Routed non-reentry gives forward quadratic flux $\varphi^{-N}$ with complementary scalar return flux $1-\varphi^{-N}$. The physical port map, trace normalization, potential, nonzero-current background, microscopic damping channel, temporal relative-gauge connection, doubled port-flux law, dressed-phase selection, $k_\star$, routing geometry, impedance, and momentum-current identification remain open. These auxiliary boundary quantities introduce no §7 count (`foundations/particle-stationary-action-closure.md`; `foundations/endpoint-link-and-localization-boundary.md`; `foundations/interscale-stress-attenuation-boundary.md`; `computations/endpoint_robin_link_prereg.md`; `computations/endpoint_dynamical_response_report.md`; `computations/endpoint_action_response_report.md`) |
| $\mathcal A,\mathcal B,\mathcal C,\mathcal D,\mathcal T_\star,K_\parallel,W;\ A,B,\mathcal E_{\mathcal Q}$ | reduced Derrick gradient, core, bulk, attraction, loop tension, phase stiffness, and winding data; illustrative Fibonacci-sector costs and dimensionless profile energy | free profile-dependent quantities with $\mathcal B>\mathcal D$ required for a small-radius barrier and $W\in\mathbb Z$ only under compact finite-energy boundary conditions; $[\mathcal A]=\hbar/(TL)$, $[\mathcal B]=[\mathcal D]=\hbar L/T$, $[\mathcal C]=\hbar/(TL^3)$; $A,B,\mathcal E_{\mathcal Q}$ have no selected values | **Conditional reduced-model inputs.** The minimal smooth endpoint sector supplies $\mathcal B=0$. In the imposed point-core branch, $\mathcal B$ is replaced by the Derived exterior coefficient $\mathcal B_G$ and support requires $\mathcal B_G>\mathcal D$; the current action supplies no smooth finite-energy core. The loop geometry, compact winding, Fibonacci sector cost, and mass-profile normalization remain unselected; no §7 count (`foundations/interscale-current-soliton.md`; `foundations/endpoint-link-and-localization-boundary.md`; `foundations/point-core-flux-sector.md`) |
| $\mathfrak s_p,m,\delta_{\mathrm{end}},\kappa_{0,p},\alpha_{0,p},\mathcal T_{\mathfrak s}$ | Planck-to-proton scale endpoint, circuit winding, endpoint phase bias, frozen-amplitude endpoint strengths/phases, and scale tension | $\mathfrak s_p=91.4616$ is Mapped from $m_p$; $m\in\mathbb Z$ only for a compact closed circuit; the remaining quantities are free. In the unbiased $m=1$ closure, selecting $\mathfrak s_p$ requires $\mathcal T_{\mathfrak s}/(K_{\mathfrak s}\rho)=5.57043\times10^{-4}$ | **Hypothesized proton scale-circuit inputs.** The frozen-amplitude cosine strength is the combination $\kappa_v^{\mathrm{eff}}=\kappa_v|\Upsilon_v|$. Current, energy, endpoint-capacity, and open-rate identities are Derived conditionally; scale selection, physical endpoint normalization, proton quantum numbers, and decay rate remain open. No §7 count (`foundations/interscale-current-soliton.md`; `foundations/endpoint-link-and-localization-boundary.md`) |
| $\widetilde I_{\mathfrak s},S_\pm,\delta_\pm$ | two-rail scale metric graph, endpoint gluing maps, and endpoint phases | two vertices and two edges; $b_1=1$; circumference $2L_{\mathfrak s}$; $S_\pm$ are gauge-covariant intertwiners represented by flux-unitary Yang/Yin boundary phases $\delta_\pm$ and accompanied by Kirchhoff current conservation | **Hypothesized object-sector topology and physical boundary / Derived graph and endpoint covariance algebra.** The quotient gives one compact internal scale cycle. The registered coherent phase gluing is perfect transfer; partial splitting requires the additional endpoint interaction in `foundations/interscale-stress-attenuation-boundary.md` §4.3. A charged coherent section and a one-way trace-preserving channel realize the vertex conversion conditionally. The physical scale metric, endpoint potential or bath, coupling normalization, and phase biases remain open; no new numerical value or §7 count (`foundations/geometric-manifold-completion.md`; `foundations/endpoint-link-and-localization-boundary.md`) |
| $\nu_v,\ k_{\min,m}$ | Derived auxiliary endpoint-response quantities: frozen-link strength and conditional positive-phase-stiffness current-capacity wave-number boundary | $\nu_v:=2\kappa_vu_v$; $k_{\min,m}:=\lvert\Delta_m\rvert/(\varphi^{3/2}\mathfrak s_p\tau_\varphi)$; for dimensionless $\mathfrak s$, $[\nu_v]=[K_{\mathfrak s}]=\hbar/T$ and $[k_{\min,m}]=1$ | **Derived conditional.** $\nu_v/(K_{\mathfrak s}k_\star)=\tau_\varphi$ under the selected golden match; $\mathfrak s_p$ is Mapped and ledgered; $k_{\min,1}=0.096464036203895$ uses $\Delta_1=2\pi$ and the Mapped proton endpoint. The condition establishes current capacity and positive fixed-amplitude phase stiffness. The first-order active endpoint Hessian from EL9 is explicit; full coupled fluctuation stability depends on the unselected endpoint potential and background. Neither quantity enters the §7 canonical count (`foundations/endpoint-link-and-localization-boundary.md` §§3.6–3.9; `computations/endpoint_robin_link_prereg.md`; `computations/endpoint_dynamical_response_prereg.md`) |
| $W_v',U_v'',\mu_{v,0}$ ($m_{v,0}$ in the frozen AR protocol), $\mathcal Z_v,A_v(\mathbf q),B_v,\mathcal C_v,\mathcal D_v^{R/A},\mathcal K_v^{R/A},\mathbb\Lambda_{\mathrm{eff},v}^{R/A},\omega_{\mathrm{end}},\gamma_v$ | rotating-frame endpoint derivatives, static zero-background source-action curvature, temporal weight, normal and anomalous endpoint curvatures, mixed rail-endpoint Hessian, endpoint energy and source-action kernels, first-order active Nambu Robin response, endpoint pole frequency, and phenomenological retarded-continuation rate | $\mu_{v,0}:=W_v'(0)$ and $\mathcal K_{v,0}(0)=-\mu_{v,0}I_2$; the frozen AR protocol denotes the same curvature by $m_{v,0}$ and sets $m_{v,0}=1.1$; $\mathcal Z_v=\hbar u_v^2$; $A_v(\mathbf q)=u_v^2[W_v'(u_v^2)+u_v^2U_v''(u_v^2)+K_v|\mathbf q|^2/2]$; $B_v=u_v^4U_v''(u_v^2)$ in the fractional fluctuation basis; $\mathcal D_v^R=\mathcal H_v-\mathcal Z_v(\omega+i\gamma_v)\sigma_3$; $\mathcal K_v^R=-\mathcal D_v^R=\mathcal Z_v(\omega+i\gamma_v)\sigma_3-\mathcal H_v$; $\mathcal K_v^A=(\mathcal K_v^R)^\dagger$ for real $\omega$; $\mathbb\Lambda_{\mathrm{eff}}^R=\mathbb\Lambda_0-\mathcal C^\dagger(\mathcal K^R)^{-1}\mathcal C=\mathbb\Lambda_0+\mathcal C^\dagger(\mathcal D^R)^{-1}\mathcal C$; $\omega_{\mathrm{end}}=\sqrt{A_v^2-|B_v|^2}/\mathcal Z_v$ when $A_v>|B_v|$; in the declared normalized boundary basis $[\mu_{v,0}]=[\mathcal K_v]=[\mathcal H_v]=\hbar/T$ and $\gamma_v\ge0$ has $[\gamma_v]=T^{-1}$ | **Hypothesized first-order endpoint inputs / Derived conditional response.** $W_v'$, $U_v''$, $K_v$, $u_v$, $\mu_{v,0}$, the nonzero-current background, and normalization remain unselected. $\gamma_v$ is a nonnegative angular-frequency input implementing a phenomenological retarded continuation of the first-order Schrödinger/Berry action; it supplies no second-order inertial coefficient and selects no microscopic bath. The advanced response follows by conjugate/adjoint continuation. Conditional on those inputs, $\mathcal Z_v$, $A_v$, $B_v$, $\mathcal C_v$, $\mathcal D_v^{R/A}$, $\mathcal K_v^{R/A}$, $\mathbb\Lambda_{\mathrm{eff},v}^{R/A}$, and $\omega_{\mathrm{end}}$ are derived from the first-order action. Every closed homogeneous conservative time-harmonic endpoint extremum has zero coherent conversion current. At the symmetric zero background, the eliminated source action begins at quartic rail order with a positive coefficient when $\mu_{v,0}>0$; physical energy, stress, inertial mass, and stability signs remain open. The frozen AR1–AR6 receipt passes; the DR receipt remains `FAIL` because its DR5 endpoint block has the opposite source-action sign. The derived quantities add no §7 count (`foundations/endpoint-link-and-localization-boundary.md` §3.9; `computations/endpoint_dynamical_response_prereg.md`; `computations/endpoint_dynamical_response_report.md`; `computations/endpoint_action_response_prereg.md`; `computations/endpoint_action_response_check.py`; `computations/endpoint_action_response_report.md`) |
| $\Gamma_v,\mathcal I_{\mathrm{link},v},J_{\Upsilon,v}^i,H_{\nabla\Upsilon,v}$ | Yang source coefficient, negative rail-difference rate, charged-endpoint number current, and stationary spatial-flux gradient cost | $\mathcal I_{\mathrm{link},v}=-2\Gamma_v$; $J_{\Upsilon,v}^i=(K_v/\hbar)\operatorname{Im}[\Upsilon_v^*D_i^{(-g_Q)}\Upsilon_v]$; $[\Gamma_v]=[n_v]T^{-1}$ and $[J_{\Upsilon,v}]=[n_v]LT^{-1}$ for $n_v=|\Upsilon_v|^2$; on a constant-amplitude periodic domain, $H_{\nabla\Upsilon,v}=(\hbar^2V/2K_vu_v^2)\sum_{\mathbf k\ne0}|\Gamma_{v,\mathbf k}|^2/|\mathbf k|^2$, positive for a nonzero source when $K_v>0$ and $u_v>0$ | **Derived conditional endpoint identities.** Stationarity gives $\nabla\cdot\mathbf J_{\Upsilon,v}=\Gamma_v$ and closed domains require zero integrated source. The quantities add no selected coefficient or §7 numerical parameter; the endpoint normalization, $K_v$, $u_v$, source profile, boundary transport, and coupled rail background remain Hypothesized (`foundations/endpoint-link-and-localization-boundary.md` §3.10) |
| $K_v,\kappa_v,U_v$ and the $\Upsilon_v$ normalization | primitive endpoint-section stiffness, rail-endpoint coupling, self-potential, and field-normalization convention in EL9 | Under the conditional raw-field density normalization $[\Upsilon_v]=[\psi_Y]=[\psi_I]=L^{-3/2}$ and a dimensionless scale coordinate, $[K_v]=\hbar L^2/T$, $[\kappa_v]=\hbar L^{3/2}/T$, $[U_v]=\hbar/(TL^3)$, and $[U_v']=\hbar/T$. The interaction closes at the action level as $[dt\,d^3x\,\kappa_v\Upsilon_v^*\psi_Y^*\psi_I]=T L^3(\hbar L^{3/2}/T)L^{-9/2}=\hbar$. A different endpoint or boundary-trace normalization rescales these coefficients together | **Hypothesized endpoint-action inputs.** EL9 establishes their allowed gauge-covariant roles and selects no values or normalization. These are genuine free optional-extension inputs recorded descriptively in §9, outside the counted §§1–6 classes; they therefore do not change the §7 primary count of 47 while the endpoint action remains conditional (`foundations/endpoint-link-and-localization-boundary.md` §3.1) |
| $t_\Upsilon$ | conservative inter-vertex endpoint hopping coefficient | positive free coefficient with $[t_\Upsilon]=\hbar/T$ under the EL9 normalization | **Hypothesized Wilson-link action input.** The normalized checker value $t_\Upsilon=0.8$ is protocol data without physical calibration. The microscopic origin, physical value, and local scale-bulk completion remain open. This genuine free optional-extension input is recorded descriptively in §9, outside the counted §§1–6 classes; it therefore does not change the §7 primary count of 47 while the extension remains conditional (`foundations/endpoint-link-and-localization-boundary.md` §3.11) |
| $\mathcal B,\mathcal W_{+\leftarrow-},I_{-\to+},I_c$ | oriented relative-connection integral, existing-connection Wilson transporter, endpoint number current, and fixed-amplitude transport capacity | $\mathcal B=\int_{v_-}^{v_+}B_{\mathfrak s}\,d\mathfrak s$; $\mathcal W_{+\leftarrow-}=e^{-ig_Q\mathcal B}$; $I_{-\to+}=-(2t_\Upsilon/\hbar)\operatorname{Im}(\Upsilon_+^*\mathcal W_{+\leftarrow-}\Upsilon_-)$; $I_c=2t_\Upsilon u_-u_+/\hbar$; $[\mathcal B]=[\mathcal W]=1$ and $[I]=[n_v]T^{-1}$ | **Derived conditional composites.** $\mathcal B$ and $\mathcal W$ use the registered relative connection and introduce no new connection parameter. The IT1–IT6 receipt verifies covariance, conservation, capacity, and phase curvature at one normalized point. These composites add no §7 parameter count (`computations/endpoint_intervertex_transport_report.md`) |
| $N_G,\Sigma$ | first Chern number of the compact relative connection and its closed supporting two-cycle | $N_G=(g_Q/4\pi)\int_\Sigma G\in\mathbb Z$; $H^2(\mathbb R^3\times S^1_{\mathfrak s};\mathbb Z)=0$ on the smooth object base | **Derived conditional bundle topology.** A nonzero point sector removes a spatial core for every scale position, giving an $S^2$ link around a scale-worldtube. It remains an imposed defect or boundary sector because the current Abelian Bianchi identity cannot smooth the core. The integer adds no numerical parameter or §7 count (`foundations/endpoint-link-and-localization-boundary.md` §5; `foundations/point-core-flux-sector.md`) |
| $\Phi_G,\mathcal B_G,L_G$ | point-core flux, fixed-flux exterior coefficient, and source-action scale-support length | $\Phi_G=4\pi N_G/g_Q$; $\mathcal B_G=(\Phi_G^2/8\pi)\int_{I_{\mathfrak s}}d\mathfrak s/\mu_x=2\pi N_G^2\int d\mathfrak s/e_x^2$; for constant coefficients $L_G=L_{\mathfrak s}$ in the source interval and $\mathcal B_G=2\pi L_GN_G^2/e_x^2$ | **Derived conditional exterior gauge energy / Derived current-action completion no-go.** It is gauge-normalization invariant and gives a supported reduced radius only when $\mathcal B_G>\mathcal D$. A separate two-edge gauge-energy lift would double the coefficient. The auxiliary adjoint $SU(2)_Q$ branch smooths the local core and matches this exterior; the registered nonzero fundamental condensate confines flux and supplies no persistent finite-separation object. No physical particle value or §7 count (`foundations/point-core-flux-sector.md`; `foundations/nonabelian-magnetic-core-boundary.md`) |
| $\mathcal A_A^a,\Phi^a,\Delta_\varphi$ | auxiliary internal $SU(2)_Q$ connection, adjoint core field, and gauge-invariant composition scalar | $[\Phi]=L^{-1}$; $\Delta_\varphi=\tfrac12[(1-\varphi)\rho+(1+\varphi)\Phi^aS^a/v_Q]$ reduces to $E_Y-\varphi E_I$ in the asymptotic unitary gauge | **Hypothesized auxiliary field content / Derived conditional composition identity.** $SU(2)_Q$ is distinct from electroweak $SU(2)_L$; these fields and the derived scalar add no §7 numerical parameter (`foundations/nonabelian-magnetic-core-boundary.md`) |
| $v_Q$ | auxiliary adjoint symmetry-breaking scale | positive free input with $[v_Q]=L^{-1}$ and normalization transformation $v_Q'=av_Q$ | **Hypothesized auxiliary-completion input.** No value is selected or derived from $\varphi$, and it is not assigned the electroweak vacuum value; not counted in §7 (`foundations/nonabelian-magnetic-core-boundary.md`) |
| $\lambda_H$ | auxiliary adjoint quartic coefficient | nonnegative free input with $[\lambda_H]=\hbar L/T$ and normalization transformation $\lambda_H'=\lambda_H/a^4$; the exact Prasad-Sommerfield branch sets $\lambda_H=0$ | **Hypothesized auxiliary-completion input.** No value is selected or derived from $\varphi$; not counted in §7 (`foundations/nonabelian-magnetic-core-boundary.md`) |
| $m_W,m_H,\beta_Q,M_{\mathrm{BPS}},M_i,\kappa_L$ | conditional auxiliary vector and adjoint inverse lengths, mass ratio, BPS core mass, transverse condensate London mass, and penetration inverse length | $m_W=g_Qv_Q$, $m_H^2=2\mu_x\lambda_Hv_Q^2$, $\beta_Q^2=2\mu_x\lambda_H/g_Q^2$, $M_{\mathrm{BPS}}=4\pi L_{\mathfrak s}v_Q\lvert N_G\rvert/(\mu_xg_Q)$, $M_i^2=g_Q^2K_x\rho_0/4$, and $\kappa_L^2=\mu_xM_i^2$ | **Derived conditional identities.** The core formulas are adjoint-only. The London formulas describe the homogeneous transverse response with constrained scalar phases; the integrable counterflow stiffness is $K_x\rho_0/\varphi^3$. Their free inputs have no selected values, so these combinations add no §7 numerical parameter (`foundations/nonabelian-magnetic-core-boundary.md` §5.3; `foundations/interscale-current-soliton.md` §5.3) |
| $\mathcal O_N,\vartheta,J_t,J_x,p_N,P_N,E_R$ | gauge-invariant common-phase observable, temporal/spatial stiffnesses, common-number momentum density and integrated generator, and fixed-$P_N$ Gaussian temporal energy | $\mathcal O_N=\Psi^T i\sigma^2(\Phi^a\sigma^a)\Psi$, $\vartheta=\arg\mathcal O_N$; $[\mathcal O_N]=L^{-4}$. The stiffnesses are $J(a,d)=4a\rho_0d v_Q^2\sin^2\beta/(a\rho_0+4d v_Q^2)$, $J_t=J(C_\Psi,C_\Phi)$, $J_x=J(K_x,1/\mu_x)$, with $[J_t]=\hbar T/L^3$ and $[J_x]=\hbar/(TL)$. For the Gaussian initial data with $\mathcal A_0=0$, $p_N=C_\Psi\operatorname{Im}(\Psi^\dagger\dot\Psi)$, $[p_N]=\hbar L^{-3}$, $P_N=\int_{\mathbb R^3\times I_{\mathfrak s}}p_N\,d^3x\,d\mathfrak s$, $[P_N]=\hbar$, and $P_N/\hbar$ is its dimensionless number normalization. At unit scale measure and $\chi_C=0$, $E_R=P_N^2/[2J_t(2\pi)^{3/2}R^3]\to0$ at fixed nonzero $P_N$ as $R\to\infty$ | **Derived conditional composites / Tested linear-wave dynamics and fixed-$P_N$ variational sequence.** The supplied action has $\omega^2=(J_x/J_t)k^2+O(k^4)$, with $J_x/J_t=0.8649109511178478$ at the declared phase model-unit inputs; the electric-support reconciliation supplies $J_t=0.8971293123871708$ as a model-unit numerical witness with no physical calibration. The fixed-$P_N$ sequence has zero energy infimum and no finite-energy minimizer under the stated nonnegative $\chi_C=0$ Hamiltonian. It supplies admissible initial states alone; real-time dispersal and local metastability elsewhere require separate analysis. $P_N$ is distinct from the first-order density excess $\delta Q_N$ and the neutral-carrier number $Q_C$. The common-phase identification is $\Theta\sim\Theta+\pi$; the local gauge stabilizer stays trivial. No physical normalization, quantum condensate or statistics is selected; no fit or §7 numerical parameter is added (`foundations/particle-stationary-action-closure.md` §§4.3–4.4; `computations/matter-formation-continuum-report.md` §§40–41) |
| $M_{\mathrm{core}},\sigma_Q,C_Q$ | conditional monopole-core energy, flux-tube tension, and screened-tail strength | $E_{M\bar M}(L)=2M_{\mathrm{core}}+\sigma_QL-C_Qe^{-\kappa_LL}/L+\cdots$ with $\sigma_Q,C_Q>0$ in the residual-vector-exchange ansatz | **Hypothesized coupled-composite inputs / Derived conditional positive-slope no-go.** The ansatz uses the transverse penetration scale as its tail decay rate; the full endpoint/tube profile and asymptotic matching remain unselected. Its energy has no finite-separation minimum; not counted in §7 (`foundations/nonabelian-magnetic-core-boundary.md` §5.4) |
| $\chi_C,\ Q_C$ | neutral auxiliary core-carrier field and its global $U(1)_C$ Noether number | $[\chi_C]=L^{-3/2}$ and $Q_C=\int d^3x\,d\mathfrak s\,|\chi_C|^2$ is dimensionless; $Q_C$ is a classical sector label with no electric, baryonic, leptonic, or observed-particle identification | **Hypothesized auxiliary field / Derived conditional conserved number.** No field normalization or charge sector is selected; not counted in §7 (`foundations/core-trapped-charge-support.md`) |
| $K_{Cx}$ | spatial carrier stiffness | positive free input with $[K_{Cx}]=\hbar L^2/T$ | **Hypothesized auxiliary-carrier input.** No value is selected or derived from $\varphi$; not counted in §7 (`foundations/core-trapped-charge-support.md`) |
| $K_{C\mathfrak s}$ | scale-direction carrier stiffness | positive free input with $[K_{C\mathfrak s}]=\hbar/T$ | **Hypothesized auxiliary-carrier input.** No value or scale profile is selected; not counted in §7 (`foundations/core-trapped-charge-support.md`) |
| $\varepsilon_{C,\rm out}$ | carrier bulk-threshold energy | positive free input with $[\varepsilon_{C,\rm out}]=\hbar/T$; localization requires $\hbar\omega_C<\varepsilon_{C,\rm out}$ | **Hypothesized auxiliary-carrier input.** No value or particle-energy identification is selected; not counted in §7 (`foundations/core-trapped-charge-support.md`) |
...
| $\kappa a_n$, bending/transverse/axial rate split | vorticity-direction coherence margin of a tube-like region and the three terms of its rate | $\kappa=\lvert(\xi\cdot\nabla)\xi\rvert$ with $\xi=\omega/\lvert\omega\rvert$; $a_n=\lvert\omega\rvert^{1/2}\,(n\cdot\nabla^2\lvert\omega\rvert\,n)^{-1/2}$ along the geometric normal; $D_\tau\log(\kappa a_n)=n\cdot(\partial_s\nabla u)t/\kappa+2n\cdot Sn-2\ell$ | **Derived exact kinematic identity / Measured on declared tube families.** Controls reach the pole in finite time ($t_\ast=5.714286$, margin $1.000000000000$), the exponential regime ($4\alpha=1.2$) and exact conservation ($3.5\times10^{-11}$); the retained helical families run $0.2394\to0.3836$, $0.1203\to0.3221$ and $0.3160\to0.6928$, the tight-pitch family arresting at $0.6935$ and reversing while under-resolved; arbitrary-data regularity untouched (`turbulence/navier-stokes-curvature-clock.md`) |
| $\kappa a_n$, bending/transverse/axial rate split | vorticity-direction coherence margin of a tube-like region and the three terms of its rate | $\kappa=\lvert(\xi\cdot\nabla)\xi\rvert$ with $\xi=\omega/\lvert\omega\rvert$; $a_n=\lvert\omega\rvert^{1/2}\,(n\cdot\nabla^2\lvert\omega\rvert\,n)^{-1/2}$ along the geometric normal; $D_\tau\log(\kappa a_n)=n\cdot(\partial_s\nabla u)t/\kappa+2n\cdot Sn-2\ell$ | **Derived exact kinematic identity / Measured on declared tube families.** Controls reach the pole in finite time ($t_\ast=5.714286$, margin $1.000000000000$), the exponential regime ($4\alpha=1.2$) and exact conservation ($3.5\times10^{-11}$); the retained helical families run $0.2394\to0.3836$, $0.1203\to0.3221$ and $0.3160\to0.6928$, the tight-pitch family arresting at $0.6935$ and reversing while under-resolved; arbitrary-data regularity untouched (`turbulence/navier-stokes-curvature-clock.md`) |
| $(\mathrm{KF})$–$(\mathrm{KC4})$, enstrophy budget of the widening channel | vorticity-direction coherence margin charged to the flow's own enstrophy | frame closure $\ell+n\!\cdot\!Sn+b\!\cdot\!Sb=0$; $D_\tau\log(\kappa a_n)=\text{bend}/\kappa-2D_\tau\log e+4\nu\,\omega\!\cdot\!\Delta\omega/|\omega|^2-2b\!\cdot\!Sb$, capped at a local $|\omega|$ maximum by $\Delta|\omega|$ | **Derived exact identities / Measured on the declared tube families.** Frozen-field reading: cap deficit exactly $0$ at all $3075$ evaluated points, the rate integrating to the advective increment within $7.1\times10^{-5}$ over $1024$ intervals, the residual a measured finite-difference floor falling $1.11\times10^{-5}\to5.55\times10^{-6}\to2.78\times10^{-6}$ at spacings $4,2,1$. Material reading: the enstrophy identity holds along the carried trajectory to $4.5\times10^{-6}$ worst, while the margin moves $+0.44094$ against a frozen-field integral of $+0.00507$ and the width channel $+0.46684$ against $+0.00941$, so the geometric channels are carried by the field's own evolution. A finite-time bound on cumulative enstrophy from general smooth data and the material transport of the frame are **UNRESOLVED** |
...
| $C_C,\ a_C,\ \mathcal N_Q$ | optional carrier temporal stiffness, dimensionless parent coefficient and action normalization | $[C_C]=\hbar T$, $a_C=C_CK_x/(\hbar^2\ell_Q^2)>0$, $\mathcal N_Q=\rho_0\ell_Q^3$; matching the carrier and Yang/Yin principal speeds requires $a_C=k_{Cx}c_\Psi/2$ | **Hypothesized temporal-parent input / Derived conditional dimensionless and common-cone relations.** The parent has signed conserved charge and a conditional quadratic quantum creation channel. The normalization campaign selects $\mathcal N_Q\mathcal Q=1$ as one unit of the internal scalar $U(1)$ generator for a demonstrator, while $a_C$ remains free and the physical action normalization, electric-charge map and particle identity remain open. These optional-extension quantities remain outside the counted §7 parameters; canonical interaction strength $u_C/(\mathcal N_Qa_C^2)$ remains unselected (`foundations/particle-stationary-action-closure.md` §8.8, §8.12; `computations/matter-formation-continuum-report.md` §12) |
| $a_{\rm dep},\ a_{\rm vac}$ | conditional parent depleted-mediator and global-vacuum boundaries | $a_{\rm dep}=1/[4(h_C-e_C)]$ when $h_C>e_C$; $a_{\rm vac}=1/[4(h_C-e_C-\sqrt{u_\rho u_C/2})]$ when its denominator is positive. The canonical scalar potential is nonnegative exactly when $h_C-e_C-1/(4a_C)\le\sqrt{u_\rho u_C/2}$ | **Derived conditional classical boundaries.** These are functions of existing coefficients, introduce no fit and add no §7 parameter. At the selected coefficients $a_{\rm vac}=0.3142233129944425$ gives the allowed minimum $\ell_Q=6.7893919382\times10^{-13}$ m at cascade coordinate $n=108.25197355$, above the mapped cell upper endpoint $6.0141121609\times10^{-13}$ m; the exact $\ell_Q=\lambda_*$ root has denominator $2(k_{Cx}-2e_C)=-1$ and no positive solution. The scalar electron-core assignment is therefore **CONTRADICTS** at this fixed coefficient point (`foundations/particle-stationary-action-closure.md` §8.12; `computations/matter-formation-continuum-report.md` §12) |
| $\delta,\sigma_{\rm w},s,a_{\rm wall}$ | signed-wall width, tension, normal-bound spectral index and carrier surface-growth boundary | $\delta=\sqrt{2/u_\rho}$, $\sigma_{\rm w}=2\sqrt{2u_\rho}/3$, $K=k_{Cx}u_\rho/4$, $s(s+1)=h_C/K$ with $s>0$, and $a_{\rm wall}=1/[4(Ks^2-e_C)]$ when $Ks^2>e_C$; the retained coefficients give $s=1.2915987487$ and $a_{\rm wall}=0.2722637330<a_{\rm vac}$ | **Derived conditional interface quantities / Tested exact and finite-difference spectra.** The signed-real boundary is an added assumption outside the canonical positive-root domain. The complex-direction eigenvalue is $-u_\rho/2$, so this wall is energetically unstable. These composites reuse the Mapped $h_C$, introduce no fit and add no §7 parameter (`computations/matter-formation-continuum-report.md` §33) |
| $\Omega_0,n_0,q_0,\sigma$ | scalar charged-coexistence frequency, carrier population density, signed charge-density magnitude and surface grand potential | $S=\sqrt{u_\rho u_C/2}$, $D=h_C-S$, $B=e_C+1/(4a_C)$; $\Omega_0^2=(B-D)/a_C$, $n_0=\sqrt{u_\rho/(2u_C)}$, $q_0=2a_C\Omega_0n_0$. At the retained coefficients and $a_C=1/16$, $\Omega_0=7.1603212062$, $q_0=1.2657779201$ and the planar surface cost is $\sigma=0.8738238914$ in the supplied dimensionless units | **Derived conditional coexistence, thin-interface and fixed-charge energy identities / Tested independent stationary fronts, finite-grid scalar spectra and finite-charge radial condensation.** These composites reuse the existing Mapped $h_C$, introduce no fit and add no §7 parameter. The energy bound is $E\ge\Omega_0\lvert\mathcal Q_a\rvert$. Physical charge, all-sector stability and microscopic identity remain unselected (`computations/matter-formation-continuum-report.md` §§34–35) |
| Finite-charge radial-condensation witnesses | supplied signed charge, initial widths and numerical schedule | $\mathcal Q_a=256$, Gaussian widths $w=4,8$, $a_C=1/16$, $c_\Psi=1/8$, initially uniform mediator $f=1$; core $r<8$, late window $32\le t\le48$; four primary schedules and independent RK4 | **Numerical preparation data / Tested finite-time radial condensation.** Independent mean core fractions are $0.7476513029$ and $0.5637353289$, compared with $0.0545181040$ and $0.1537575301$ under disabled mediator coupling. All sampled late times meet the frozen retention/depletion criteria; spatial, domain, time-step and independent-integrator comparisons pass. Signed charge here is distinct from prepared population $Q_C=256$. The inputs use the existing Mapped $h_C$, introduce no fit and add no §7 parameter. They select no physical charge, quantum state or particle identity (`computations/matter-formation-continuum-report.md` §35) |
| $N_*,G_*,P_*,Q_{\rm tr},R_{256}$ | fixed continuum comparison profile and sufficient binding threshold | The piecewise affine shell has $\delta=1/4$ and $n_0=\sqrt{u_\rho/(2u_C)}$. Its sufficient threshold is $Q_{\rm tr}\approx149.36022508149227$; at signed charge $Q=256$, $R_{256}\approx3.332799624728784$ gives $E_{256}/256\approx8.283930463343918<\Omega_\infty=\sqrt{76}$. | **Derived conditional binding and continuum minimizer-set orbital stability / Tested independent algebra and quadrature.** The theorem covers the real-mediator/full-complex-carrier energy space, including nonradial and nearby-charge perturbations. The threshold is sufficient; the comparison radius is not a measured or minimizing radius. No fit, physical unit or §7 parameter is added. The inherited $h_C$ remains Mapped; capture or stability of the formed radiating clouds and additional field sectors remain open (`computations/matter-formation-continuum-report.md` §36; `foundations/matter-completion-boundary.md` §18) |
| $\mathcal Q,\ G(N),\ \gamma,\ S$ | signed parent charge, minimized temporal energy, radial population penalty and stationary response | $N=Q_C>0$, $G=(N-\mathcal Q)^2/(4a_CN)$, $\mathcal Q=\sqrt{1+4a_C\omega_C}\,N$, $\gamma=(1+4a_C\omega_C)/(2a_CN)$, $S=(\nabla N)^TH_0^{-1}\nabla N$ | **Derived conditional constrained-energy and response identities.** With one negative base direction and no zero, the parent radial Hessian is positive exactly when $1+\gamma S<0$. These are functions of the existing action and prepared charge, introduce no fit and add no §7 parameter. The demonstrator's $\mathcal N_Q\mathcal Q=1$ is one internal scalar-generator unit only; it is not an electric-charge assignment or a particle identity. Under the stated phase convention, the scalar rotation is $+1$ under $2\pi$ while the Dirac rotation is $-1$; this is a topological-quantization caveat, not an identity proof (`foundations/particle-stationary-action-closure.md` §8.10, §8.12; `computations/matter-formation-continuum-report.md` §12) |
| $H_\ell,\ L_\ell,\ \Omega,\ M_a^2(1)-\Omega^2$ | scalar-parent angular and phase operators, canonical stationary frequency and exterior frequency gap | Nonradial and phase curvatures contain $\omega_C=\omega+a_C\omega^2$; carrier stiffness is $k_{Cx}D_\ell$. $\Omega=\sqrt{1+4a_C\omega_C}/(2a_C)$ and $M_a^2(1)-\Omega^2=(e_C-\omega_C)/a_C$ | **Derived conditional scalar spatial identities.** Exact nodeless stationary profiles have a nonnegative phase form. Exact positive monotone radial profiles have a cooperative nonnegative angular-amplitude form with translation kernel, subject to the stated form-domain and boundary assumptions. Finite sampled monotonicity establishes no continuum theorem. These are functions of existing coefficients, add no fitted value or §7 parameter, and supply no nonlinear or physical stability conclusion (`foundations/particle-stationary-action-closure.md` §8.11) |
| Parent fixed-signed-charge numerical witnesses | radial fields, temporal coefficients, mass-weighted operators and spectral comparisons | Eight immutable population-16 and population-256 fields at $(R,n)=(12,192),(12,384),(12,768),(24,768)$, each with $a_C=1/64,1/32,1/16$; six base and parent eigenpairs per row; independent complete banded base spectra and rank-one inertia brackets | **Numerical boundary data / Tested finite-grid radial fixed-charge energetic support / INCONCLUSIVE aggregate radial domain/resolution qualification.** All 24 radial parent minima are positive and independently verified. Nine of twelve comparisons pass; the three population-16 domain comparisons fail. Population 256 passes its radial comparisons. No field relaxation, physical coefficient selection, all-sector stability or new counted parameter follows (`computations/matter-formation-continuum-report.md` §10) |
| Scalar-parent spatial numerical witnesses | angular/phase spectra, sampled symmetry candidates and domain comparisons | Four immutable prepared-$Q_C=256$ profiles; six eigenpairs for each of $H_1,H_2,L_0,L_1$; twelve inherited qualified charged radial embeddings | **Numerical boundary data / Tested finite-grid scalar angular and phase support / INCONCLUSIVE combined scalar-parent spatial qualification.** Independent banded/tridiagonal operators match all 96 eigenvalues. All four source-sector verdicts support; seven of eight spatial comparisons pass. The dipole nonsymmetry gap fails its domain comparison. These data establish no continuum monotonicity, nonlinear stability, physical coefficient selection or new §7 parameter (`computations/matter-formation-continuum-report.md` §11) |
| Finite stationary-solver boundary data | numerical domains, grids, seeds, tolerances, localization shells, symmetry bases, gauge quotients, and stopping rules | $\rho=1$, $c=0$, and a fixed gauge representative on $\partial\Omega_R$; no-flux scale interval; fixed $Q_C$. The Cartesian branch uses $R=4$, $N\in\{17,21,25,29\}$, and $R=5,N=21$; its finest constrained $C_4$ quotient has dimension $77000$. The continuum scalar campaign prepares $Q_C\in\{4,16,64,256\}$ on $R=12$ with 192, 384 and 768 radial cells and an $R=24$, 768-cell domain control; four coarse seeds are frozen. The $Q_C=16$ spatial campaign uses the fixed-charge $\ell=0$ tangent, $\ell=1$ amplitudes and carrier-phase operator on four immutable fields | **Numerical boundary data / Tested finite-grid spectra / Tested conditional continuum static binding / INCONCLUSIVE combined smooth-branch spatial stability.** These choices carry no physical parameter status and remain outside the §7 count. The Cartesian carrier is ultraviolet-dominated. Scalar binding is independently reproduced at prepared $Q_C=16$ and $256$, while $Q_C=4$ spreads in the tested basins and $Q_C=64$ misses residual qualification. Physical creation, infinite-domain existence, scale-dependent and nonlinear stability remain open (`foundations/particle-stationary-action-closure.md` §§6–8; `computations/matter-formation-continuum-report.md`) |
| $U_{\rm top},B_{\rm top},\alpha,\beta$ | independent compact-target field, degree, and static-action coefficients | $U_{\rm top}\in SU(2)_{\rm top}$, $B_{\rm top}\in\mathbb Z$, $\ell_{\rm top}=\sqrt{\beta/\alpha}$, $\mathcal E_{\rm top}=\sqrt{\alpha\beta}$; the dimensionless comparison sets $\alpha=\beta=1$ | **Hypothesized independent comparison input / Derived conditional degree and scaling identities.** The $\alpha,\beta$ physical scales remain undetermined. The shifted-angle radial result qualifies the supplied comparison model only; no density bridge, particle identification or §7 parameter is added (`computations/matter-formation-continuum-report.md` §§18.1–18.7) |
| $z,\ U_B,\ B,\ e_B,\ f_B,\ I_0$ | added normalized complex doublet, compact chiral field, integer degree, two effective action coefficients and collective inertia scale | $z^\dagger z=1$, $U_B\in SU(2)$, $B\in\pi_3(SU(2))=\mathbb Z$, $[e_B]=1$, $[f_B]=E$, $I_0=\pi/(3e_B^3f_B)$; two measured masses give $e_B=5.416264578979231$, $f_B=64.29440244394192\ \mathrm{MeV}$ and length unit $0.5666480972477047\ \mathrm{fm}$ | **Hypothesized added effective field and action / Mapped two-target/two-coefficient calibration / Derived conditional degree and collective-coordinate identities / Tested finite-domain radial stationarity and relaxation.** The odd-degree Finkelstein–Rubinstein sign and $Q=I_3+B/2$ are supplied rules. These quantities belong to a conditional colour-neutral baryon benchmark, add no §7 parameter and do not select a canonical Cassi completion (`foundations/particle-stationary-action-closure.md` §8.15; `computations/matter-formation-continuum-report.md` §§30–31) |
| $\mathbf n,\ \kappa,\ \mu;\ (e_B,f_B)_{\rm massive}$ | added $O(4)$ chiral orientation field, quartic action coefficient, massive vacuum-mode scale, and effective two-coefficient calibration | $\mathbf n\in S^3$, $\kappa=1$, $\mu=0.5266577616452649$ selected by the $M_\pi=138.039\ \mathrm{MeV}$ normalization, $e_B=4.842429173417474$, $f_B=54.126511603191005\ \mathrm{MeV}$ selected by $M_N=938.918754\ \mathrm{MeV}$ and $M_\Delta=1232.0\ \mathrm{MeV}$, length unit $0.7528581473116732\ \mathrm{fm}$ | **Hypothesized added massive comparison action / Mapped three-mass calibration / Derived conditional action, stress, current, vacuum-mass and finite-site topology identities.** The six target-bearing out-of-fit diagnostics contradict their inherited tolerances (electric radius $0.6815867\ \mathrm{fm}$, proton/neutron moments $1.966596/-1.237016$, moment ratio $1.589790$, $g_A=0.973224$, $g_{\pi NN}=16.882262$); the magnetic radius $0.953426\ \mathrm{fm}$ is reported without a frozen target. These are not additional §7 parameters or canonical Cassi predictions (`computations/matter-formation-continuum-report.md` §§32.1, 32.4; `runs/20260907_matter_formation_chiral_lattice/structure_recovery1/results.json`) |
| $(M_1,M_2,z)_{\rm lep}$ | resolved-flavour two-singlet thermal-leptogenesis benchmark | $z=\pi/4+i/2$ and $M_2/M_1=10$ are frozen supplied texture inputs; the observed $|\eta_B|$ calibrates $M_1=5.774318838589164\times10^{10}\ \mathrm{GeV}$, with $T_R=20M_2$ | **Calibrated heavy mass / supplied conditional texture and state / Tested selector boundary.** The benchmark reconstructs the two nonzero light-neutrino masses and yields the measured magnitude in the $\{e+\mu,\tau\}$ regime. Its rank-two light matrix has one exactly massless state and cannot equal the separate Mapped three-nonzero-mass spectrum. Exact conjugate textures give opposite $\eta_B$, while registered Cassi data select neither branch. This adds no canonical §7 parameter (`computations/matter-formation-continuum-report.md` §§83–84) |
| $N,\ a_i$ | gauge-invariant relative orientation and adjoint connection in the fixed-norm gauge sector | $N\in S^2$, $D_iN=\partial_iN+a_i\times N$ | **Derived conditional fixed-norm identities.** The physical relative target survives the gauge quotient; joint gauge cancellation leaves $N$ constant, while screening a nonconstant texture is a separate conditional question. These symbols add no §7 parameter (`computations/matter-formation-continuum-report.md` §§19.1–19.6) |
| $(p,q,\gamma,t,w)_{\rm rel}$ | retained dimensionless coefficients for the registered relative Hopf trial | $(p,q,\gamma,t,w)=\left(\frac18,\frac12,\frac12,\frac45,\frac{(1+\varphi)^2}{2}\right)$; $m_\parallel^2=p/\gamma=1/4$, $m_\perp^2=(p+q)/\gamma=5/4$, and $k_H/m_\parallel\ge\sqrt{27/32}$ in the declared stationary-scale test | **Selected numerical comparison inputs, not fitted physical parameters.** The trial has negative connection-amplitude curvature for the declared shape and supplies no general Hopf instability theorem or §7 parameter (`computations/matter-formation-continuum-report.md` §§19.3, 19.7–19.9) |
| Matter-formation qualification witnesses | compact radial, relative-orientation, soft-adjoint, conditional baryon and complete-mechanism qualification outputs | compact shifted-angle primary 34 checks and verifier 121; full variation $59/59$ exact identities and $8/8$ predicates; conditional baryon independent verifier $78/78$ checks; deterministic six-requirement adjudication | **Tested conditional comparison outputs.** The compact and baryon receipts qualify their stated finite-domain calculations, and the adjudication returns `FAIL` for complete physical matter formation. They do not select canonical action/state data, continuum all-sector stability or degree-zero production and are not §7 parameters (`computations/matter-formation-continuum-report.md` §§18.6, 19.6, 20.6, 30–31) |
...
| $\mathcal G_C$ | optional signed Qi-gravity coupling diagnostic | $\mathcal G_C=(\pi/\rho)[1+(\varphi^6-1)q]$; on $\varepsilon=0$, $\pi/\rho=\varphi^{-3}$, the dilute endpoint gives $\varphi^{-3}$, and the registered reference-density point gives $5\sqrt5/3\approx3.72678$ | **Derived conditional algebra / Hypothesized physical gravity mapping.** The composition attractor is the full $\varepsilon=0$ density line; $q=0$ names only its dilute endpoint. Equal $q$ can have opposite signs of $\pi/\rho$ and $\mathcal G_C$. Neither $q$ alone nor this diagnostic supplies an atomic response, source profile, metric, or measured $G/G_N$; no §7 parameter is added (`foundations/quantum-free-fall-correspondence.md` §§7,9.1) |
| $r_B,\ \mathcal R_{br}$ | operational gravitational-response ratio for body or preparation $B$, and differential ballistic/held response observable | $r_B=m_{g,B}/m_{i,B}$; $\mathcal R_{br}=\sqrt{-3\hbar\Delta\phi_b/(m_iT^3a_h^2)}=|r_b/r_r|$ in the ideal common-source limit | **Derived conditional external-potential observables / Hypothesized Cassi mapping.** These introduce no fitted or §7 parameter. A numerical prediction requires a frozen atom-to-field map and gravity completion; the published Quantum Galileo Interferometer ([doi:10.1126/sciadv.aec8045](https://doi.org/10.1126/sciadv.aec8045)) does not determine $q$ (`foundations/quantum-free-fall-correspondence.md`) |
| Canonical physical-$q$ domain | instantaneous nonnegative-density constraint and derived interval | $-\varphi\rho\leq\varepsilon\leq\rho$; $\rho^2/[(1+\varphi^2)\rho^2+\varphi^{-2}\rho_\star^2]\leq q\leq\rho^2/[\rho^2+\varphi^{-2}\rho_\star^2]$ for $\rho,\rho_\star>0$ | **Derived conditional** on the canonical instantaneous diagnostic. The lower bound is strictly positive at fixed nonzero density; $q\to0$ requires $\rho/\rho_\star\to0$. Equal $q$ need not determine composition, and $q(\langle E\rangle)\ne\langle q(E)\rangle$ in general. QFC1–QFC3 verify exact bounds and counterexamples. Optional memory diagnostics require a separate state/domain; no §7 parameter is added (`foundations/quantum-free-fall-correspondence.md` §§9.1,12) |
...
| $\theta_d$ | density-plane angle | $\operatorname{atan2}(E_I,E_Y)$ | **Derived** state variable of the canonical two-density pair; conversion relaxes it monotonically toward the $\varphi$-line with local rate $\lambda(1-q)\rho\varepsilon/(E_Y^2+E_I^2)$; it supplies no periodic $2\pi$ phase clock or fixed per-rung pitch. **Measured** at $N=32$, $\lambda=0.1$, $T=240$ in the gated mode: quartile local fixed ratios read $1.617049$, $1.617451$, $1.617700$ and $1.618028$ about the mean openness, and the openness-weighted ratio sits $2.76\times10^{-4}$ from $\varphi$ (`runs/two_fluid_phi_ray_relaxation/verification.json`) |
...
| Resolved-flavour two-singlet leptogenesis mass $M_1=5.774318838589164\times10^{10}\ \mathrm{GeV}$ | Calibrated inside the frozen $z=\pi/4+i/2$, $M_2/M_1=10$ benchmark | $M_1$ is the root that gives $|\eta_B|=6.100000000002\times10^{-10}$ under the resolved $\{e+\mu,\tau\}$ equations; independent Radau integration reproduces the mass to $1.49\times10^{-12}$ relative and the yield to $1.18\times10^{-12}$ relative. The conjugate coordinate gives the opposite sign, so the calibration fixes a magnitude and supplies no Cassi branch selector | Observed baryon-to-photon magnitude $5.8\times10^{-10}\le|\eta_B|\le6.4\times10^{-10}$ | `computations/qcd-whole-bubble-cp-selection-prereg.md`; `computations/matter-formation-continuum-report.md` §§83–84 | Calibrated |
...
| Proton stability ($\mathfrak s_p=91.4616$; coherence exponent $4505.58$ at $N_p^{\mathrm{budget}}=91.46$; candidate current $\hbar\mathcal J_{Q,1}/(K_{\mathfrak s}\rho)=0.0162173$) | Mapped endpoint with two distinct conditional mechanisms | The measured proton mass fixes $\mathfrak s_p$. The independent-step profile at the registered two-decimal budget coordinate gives the conditional $\varphi^{4505.58}$ cycle count, and the additional Compton-cycle map gives $\tau_p\approx10^{910}$ yr. The interscale action gives a zero-total-flow two-rail current and normalized energy $0.0509481$; selecting the endpoint in the unbiased $m=1$ closure requires the unmeasured ratio $\mathcal T_{\mathfrak s}/(K_{\mathfrak s}\rho)=5.57043\times10^{-4}$. Charged coherent and one-way open endpoint equations are Derived conditionally. Point-core flux gives an exact conditional exterior coefficient. The Hypothesized auxiliary adjoint $SU(2)_Q$ action supplies a smooth decoupled core, while the registered condensate confines flux and supplies no persistent pair. The source-free temporal branch supplies Derived conditional gauge covariance, Gauss, and fixed-charge stationary equations. Its first-order carrier law preserves an exactly empty sector. At one Mapped density-depletion coefficient, the stored Cartesian localized sequence has a matched six-mode constrained finite-grid spectrum but parity-concentrated carrier structure that contradicts a smooth-carrier interpretation. A continuum-consistent scalar calculation at the same coefficients supports prepared static binding at $Q_C=16$ and $256$, reproduced by independent collocation. The smooth $Q_C=16$ constrained spatial stability verdict is INCONCLUSIVE. The supplied temporal scalar parent supports finite-time radial condensation at signed charge $Q=256$ and continuum minimizer-set orbital stability under arbitrary small real-mediator/complex-carrier perturbations at every strictly bound charge; a fixed trial certifies binding at $Q=256$ and above the sufficient threshold $149.3602250815$. These are distinct from the prepared-$Q_C$ profiles. Membership or capture of the formed clouds by that set, their nonradial and infinite-time stability, additional field sectors, microscopic production, physical proton formation, endpoint normalization, scale tension, proton quantum numbers and decay rate remain open | Proton mass; proton-decay searches | 02-sm.md:101-106; 01-core.md:120-122; `foundations/proton-coherence-budget.md` §10; `foundations/endpoint-link-and-localization-boundary.md`; `foundations/point-core-flux-sector.md`; `foundations/nonabelian-magnetic-core-boundary.md`; `foundations/particle-stationary-action-closure.md`; `computations/particle-carrier-resolution-recovery-report.md`; `computations/particle-physical-hessian-precision-v2-report.md`; `computations/particle-localized-physical-hessian-report.md`; `computations/matter-formation-continuum-report.md` | Mapped endpoint and carrier coupling / Derived conditional algebra, empty-sector invariant and scalar continuum minimizer-set stability / Tested finite-grid spectrum and conditional smooth scalar binding / Hypothesized physical realization |
| $h_C=2.9598260763447164$ density-depletion coupling | Mapped numerical candidate; no physical calibration | The value is half the analytic retention-buffer reference and the first entry in the frozen ordered scan. The stored Cartesian branch has a matched finite-grid constrained spectrum and measured parity-concentrated ultraviolet carrier structure. At the same coefficients, a continuum-consistent scalar calculation with prepared $Q_C=16$ and $256$ gives static self-binding reproduced by independent collocation; the tested $Q_C=4$ profiles spread and $Q_C=64$ misses the residual qualification. The supplied temporal parent also has independently qualified finite-time radial condensation from diffuse clouds at signed charge $\mathcal Q_a=256$ and widths $4,8$ (`computations/matter-formation-continuum-report.md` §35). The same real-mediator/complex-carrier action has a continuum energy-minimizer set with nonlinear orbital stability at $Q=256$, with no claim of capture by that set for the formed clouds (§36). These populations and signed-charge witnesses select no physical charge or particle mass. The first-order action preserves the empty carrier sector | No observational anchor; fixed-charge numerical artifacts, independent continuum comparisons and radial initial-value trajectories | `foundations/particle-stationary-action-closure.md` §8.5–§8.7; `computations/particle-carrier-localization-report.md`; `computations/matter-formation-continuum-report.md` | Mapped |
...
| $\kappa_{\text{DE}} = 3\varphi^2 H_0$ | "Calibrated" (open-questions C1); 2σ tension, not resolved | Calibrated via the $w_0$ coupling anchored to DESI (2σ tension); $3\varphi^2 = K_{md}$ is a Wu Xing coefficient (Derived), but $\kappa_{\text{DE}}$ as a whole has no independent derivation—the DESI anchor supplies the normalization, with no additional fitted coefficient introduced afterward | DESI DR2 ($w_0 \approx -0.75 \pm 0.06$, $w_a$) | 03-cosmo.md:18-25, 157 | Calibrated |
...
**Row count: 48.** A row here records the quantity's epistemic status. Each
...
These declared scratch couplings are registered for reproducibility but remain outside the Fit-Status Ledger because no observation or selected numerical target fixes their values.

| Quantity | Current status | Selection record | Data anchor | Receipt | Epistemic tier |
|---|---|---|---|---|---|
