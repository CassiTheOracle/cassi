...
| $\Gamma_{\rm in},\Gamma_{\rm out},\Gamma_{\rm io},\mathsf C_{\rm io},K_{\rm io}$ | interior and exterior Yang/Yin coherence fibres, enlarged state, cross-domain coherence, and normalized cross-coherence | $\Gamma_{\rm io}=\begin{psmallmatrix}\Gamma_{\rm in}&\mathsf C_{\rm io}\\\mathsf C_{\rm io}^\dagger&\Gamma_{\rm out}\end{psmallmatrix}\succeq0$; for positive-definite local fibres, $\mathsf C_{\rm io}=\Gamma_{\rm in}^{1/2}K_{\rm io}\Gamma_{\rm out}^{1/2}$ with $\|K_{\rm io}\|_{\rm op}\le1$ | **Derived conditional interface geometry.** The singular case also requires the pseudoinverse support condition. The exterior fibre and physical meaning of the cross block remain Hypothesized/open. These state variables add no fitted numerical parameter and are not counted in §7 (`foundations/yin-yang-qi-dynamical-geometry.md` §7.4; `computations/cross_scale_coherence_interface_check.py`; `computations/cross_scale_coherence_interface_report.md`) |
| $V,L_a,R_a,\mathcal S_{\rm in},\mathcal S_{\rm out}$ | reciprocal two-domain coupling, left/right scale-transfer maps, and induced interface sources | $K_{a+1}=L_aK_aR_a^\dagger$; $\mathcal S_{\rm in}=-(i/\hbar)(V\mathsf C_{\rm io}^\dagger-\mathsf C_{\rm io}V^\dagger)$ with equal-and-opposite enlarged number exchange | **Derived conditional exchange and transfer algebra / Hypothesized physical inputs.** The relative-charge ledger also requires $Q_{\rm in}V=VQ_{\rm out}$. No $q$ dependence, carrier, reservoir, or stress map is assigned to these operators. Their values are unselected and not counted in §7; $T_\varphi$ is registered separately below (`computations/cross_scale_coherence_interface_check.py`; `computations/cross_scale_coherence_interface_report.md`) |
