...
| $\mu_{a,L},\bar\mu,\nu_V,\mathcal L_V,\lambda_{\mathrm{gi}},\lambda_{\mathrm{loc}},\lambda_f,\lambda_c,\lambda_{\mathrm{fib}},\kappa,\vartheta,C_*,C_{-1},m_*,r_f,r_c,t_f,\delta_c,\delta_v,\delta_f,R_{\mathrm{cmp}},M,A_{\mathrm{AT}},\rho,C_B$ in the pure-gauge comparison | exact finite-vacuum, coarse-marginal and conditional measures; conditional Poisson operator; gauge-invariant, local, fine, coarse and fibre Poincaré rates; $L^2$ score covariance and $H^{-1}$ minimum transport cost; recurrence coefficients; target physical mass, normalized rate and margin coordinates; Gaussian score-comparison factor; mixed-Hessian, tensorization, cover and boundary-density constants | $\Delta_{\mathrm{phys}}=(g^2/2a)\lambda_{\mathrm{gi}}$; $\lambda_{\mathrm{gi}}\ge\lambda_{\mathrm{loc}}/(A_{\mathrm{AT}}\rho)$ under (YM29)–(YM31); $\lambda_f\ge C_*^{-1}$ under (YM105)–(YM114); $\lambda_f\ge C_{-1}^{-1}$ under (YM126)–(YM132), with $\vartheta^2\le\kappa^2/\lambda_{\mathrm{fib}}$; the exact margin budget is (YM139); $r_f=2a_fm_*/g_f^2$, $r_c=r_f/2$, and $\kappa^2\le M^2/\lambda_{\mathrm{fib}}$ under the mixed-Hessian hypothesis | **Derived regulated identities / Derived conditional sufficient theorems**, `foundations/loop-to-bubble-projection-theorem.md` §§9.13–9.14, 9.20–9.21. These target-theory mathematical quantities introduce no fitted Cassi parameter. Their cutoff-, volume- and weak-coupling-uniform interacting-vacuum bounds remain open, and they do not change the §7 parameter count |
