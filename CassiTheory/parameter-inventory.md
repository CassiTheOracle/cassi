...
| $K_v,\kappa_v,U_v$ and the $\Upsilon_v$ normalization | primitive endpoint-section stiffness, rail-endpoint coupling, self-potential, and field-normalization convention in EL9 | Under the conditional raw-field density normalization $[\Upsilon_v]=[\psi_Y]=[\psi_I]=L^{-3/2}$ and a dimensionless scale coordinate, $[K_v]=\hbar L^2/T$, $[\kappa_v]=\hbar L^{3/2}/T$, $[U_v]=\hbar/(TL^3)$, and $[U_v']=\hbar/T$. The interaction closes at the action level as $[dt\,d^3x\,\kappa_v\Upsilon_v^*\psi_Y^*\psi_I]=T L^3(\hbar L^{3/2}/T)L^{-9/2}=\hbar$. A different endpoint or boundary-trace normalization rescales these coefficients together | **Hypothesized endpoint-action inputs.** EL9 establishes their allowed gauge-covariant roles and selects no values or normalization. They are M-type optional-extension inputs recorded descriptively in §9 and are not primary-counted in §7 while the endpoint action remains conditional (`foundations/endpoint-link-and-localization-boundary.md` §3.1) |
| $t_\Upsilon$ | conservative inter-vertex endpoint hopping coefficient | positive free coefficient with $[t_\Upsilon]=\hbar/T$ under the EL9 normalization | **Hypothesized Wilson-link action input.** The normalized checker value $t_\Upsilon=0.8$ is protocol data without physical calibration. The microscopic origin, physical value, and local scale-bulk completion remain open. This is an M-type optional-extension input recorded descriptively in §9 and is not primary-counted in §7 while the extension remains conditional (`foundations/endpoint-link-and-localization-boundary.md` §3.11) |
