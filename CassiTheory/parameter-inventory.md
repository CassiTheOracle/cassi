...
| $\mathbb W_P,T^{\rm closed}_{\mu\nu},J_\nu^{\rm io},\mathbb K_Q^{(2)},\mathbb M_Q,\mathbb G_Q,\mathbb P_Q(\omega)$ | multimode flux operator, total Hilbert stress, subsystem exchange density, physical fixed-charge Hessian, temporal and gyroscopic fluctuation operators, and mixed spectral pencil | $P=\operatorname{tr}(\mathbb W_P\rho_{\rm car})$; $\nabla^\mu T^{\rm closed}_{\mu\nu}=0$; $\nabla^\mu T^{\rm in}_{\mu\nu}=-J_\nu^{\rm io}$; $\mathbb P_Q=\mathbb K_Q^{(2)}-i\omega\mathbb G_Q-\omega^2\mathbb M_Q$ | **Derived conditional definitions and conservation/stability boundary / Tested one-point PA42 Hessian sign.** $\mathbb W_P$ and explicit stress components require a selected metric-dependent carrier and reservoir action. On the higher-precision Q1–Q4 background, the $13622$-dimensional strict-shell $C_4$ representation of $\mathbb K_Q^{(2)}$ has a nonnegative matched low spectrum within the frozen uncertainty; the global phase mode remains grid-scale. Domain and resolution convergence, $\mathbb M_Q$, $\mathbb G_Q$, and the PA43 mixed pencil remain open. These operators introduce no universal parameter or §7 count (`foundations/matter-completion-boundary.md` §§5, 7–10; `foundations/particle-stationary-action-closure.md` §8.6; `computations/particle-physical-hessian-precision-v2-report.md`) |
...
| $\alpha_{\mathfrak s},u_\rho,u_\varphi,\gamma_x,\gamma_{\mathfrak s},u_H,k_{Cx},k_{C\mathfrak s},e_C,h_C,u_C$ | independent static particle-action groups | $K_{\mathfrak s}\ell_Q^2/K_x$, $\lambda_\rho\rho_0\ell_Q^2/K_x$, $\lambda_\varphi\rho_0\ell_Q^2/K_x$, $v_Q^2/(\mu_xK_x\rho_0)$, $1/(\mu_{\mathfrak s}g_Q^2K_x\rho_0)$, $\lambda_Hv_Q^2/(g_Q^2K_x\rho_0)$, $K_{Cx}/K_x$, $K_{C\mathfrak s}\ell_Q^2/K_x$, $\varepsilon_{C,\rm out}\ell_Q^2/K_x$, $\eta_C\rho_0\ell_Q^2/K_x$, and $\lambda_C\rho_0\ell_Q^2/K_x$ | **Derived conditional dimensionless combinations / Frozen numerical test input.** The registered campaign sets $(1,4,4,1,1,4,1,1,0.75,1.50,1)$ in the displayed order. The higher-precision continuation returns `PASS—HIGHER-PRECISION BACKGROUND` at physical-gradient RMS $5.47125\times10^{-5}$. Its $13622$-dimensional strict-shell $C_4$ physical quotient has one near-zero global phase mode and five positive matched PA42 modes with no verified negative mode, giving `PASS—NONNEGATIVE C4 FINITE-GRID PA42 HESSIAN`. The phase mode remains grid-scale. The point has no physical calibration and adds no §7 parameter (`computations/particle-stationary-precision-v5-report.md`; `computations/particle-physical-hessian-precision-v2-report.md`) |
...
| $q_C,\widehat\omega_C,\widehat R,L_{\mathfrak s}$ | dimensionless fixed charge, Lagrange multiplier, spatial-domain size, and scale-interval length | $q_C=Q_C/(\rho_0\ell_Q^3)$, $\widehat\omega_C=\hbar\omega_C\ell_Q^2/K_x$, $\widehat R=R/\ell_Q$, $L_{\mathfrak s}=\mathfrak s_+-\mathfrak s_-$ | **Hypothesized boundary-value data / Derived rescalings / Frozen numerical test input.** The registered campaign sets $q_C=4$, $\widehat R=4$ on P/H, $\widehat R=5$ on D, and $L_{\mathfrak s}=1$. Five structural P arms pass Q1–Q4; the higher-precision `P:separated_core` endpoint has $\widehat\omega_C=0.9619139451720478$, fails carrier retention, and lacks domain/resolution qualification. Its one-point strict-shell $C_4$ PA42 low spectrum is nonnegative within the frozen uncertainty, while the global phase mode remains grid-scale. The values have no physical calibration and add no §7 parameter (`computations/particle-stationary-precision-v5-report.md`; `computations/particle-physical-hessian-precision-v2-report.md`) |
...
| Proton stability ($\mathfrak s_p=91.4616$; coherence exponent $4505.58$ at $N_p^{\mathrm{budget}}=91.46$; candidate current $\hbar\mathcal J_{Q,1}/(K_{\mathfrak s}\rho)=0.0162173$) | Mapped endpoint with two distinct conditional mechanisms | The measured proton mass fixes $\mathfrak s_p$. The independent-step profile at the registered two-decimal budget coordinate gives the conditional $\varphi^{4505.58}$ cycle count, and the additional Compton-cycle map gives $\tau_p\approx10^{910}$ yr. The interscale action gives a zero-total-flow two-rail current and normalized energy $0.0509481$; selecting the endpoint in the unbiased $m=1$ closure requires the unmeasured ratio $\mathcal T_{\mathfrak s}/(K_{\mathfrak s}\rho)=5.57043\times10^{-4}$. Charged coherent and one-way open endpoint equations are Derived conditionally. Point-core flux gives an exact conditional exterior coefficient. The Hypothesized auxiliary adjoint $SU(2)_Q$ action supplies a smooth decoupled core, while the registered condensate confines flux and supplies no persistent pair. The separate source-free temporal branch supplies Derived conditional gauge covariance, Gauss, and fixed-charge stationary equations. One dimensionless coefficient point has a Tested higher-precision Q1–Q4 finite-grid background and explicit physical quotient. Its matched low PA42 spectrum is nonnegative within the frozen uncertainty, while the global phase mode remains grid-scale. The field fails localization and carrier retention. Physical endpoint normalization, scale tension, coefficient calibration, domain and resolution convergence, a qualified supported composite, PA43, proton quantum numbers, and decay rate remain open | Proton mass; proton-decay searches | 02-sm.md:101-106; 01-core.md:120-122; `foundations/proton-coherence-budget.md` §10; `foundations/endpoint-link-and-localization-boundary.md`; `foundations/point-core-flux-sector.md`; `foundations/nonabelian-magnetic-core-boundary.md`; `foundations/particle-stationary-action-closure.md`; `computations/particle-stationary-precision-v5-report.md`; `computations/particle-physical-hessian-precision-v2-report.md` | Mapped endpoint / Derived conditional algebra and localization boundary / Tested one-point higher-precision background and nonnegative $C_4$ finite-grid PA42 Hessian / Hypothesized physical realization |
