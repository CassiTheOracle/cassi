...
| $\Delta_\square(x),\|Q_N\psi_j^{(x)}\|^2,\lambda_j(h_x),N(x)$ in the isolated-square radial comparison | exact class-function spectral gap, discarded probability, fixed-level eigenvalues and character cutoff schedule | $\Delta_\square(1)=2.8249709909463223$; for each fixed $j$, $\lambda_j(h_x)/\sqrt{x}\to4j+3$, while bare finite-section convergence to that limit and $\|Q_N\psi_j^{(x)}\|^2\to0$ hold iff $N(x)/x^{1/4}\to\infty$, equivalently $gN\to\infty$ | **Derived regulated spectral quantities and fixed controls**, `foundations/loop-to-bubble-projection-theorem.md` §§9.18.1–9.18.7. The exact energy-dependent Feshbach reduction is retained at finite scaled cutoff, while bare cutoff convergence fails for fixed or finite $N/x^{1/4}$. The scheduled $x$, $j$, and $N$ values are mathematical controls and add no fitted Cassi parameter or change to the §7 count |
