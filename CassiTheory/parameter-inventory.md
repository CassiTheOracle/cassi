...
| $n_s$: $N_e = 40$ window | "Gate correction Closed—1.0σ from Planck" | E-fold window (steps 20–60) labeled after the fact: 40 rungs = 19.25 e-folds by the ladder's own formula; the closed form is not reproduced by the repo's own script (its §7 integrates $N_{\text{eff}} = 43.22$); $r = 12/N_e^2 = 0.0075$ at the adopted window (the 0.003 reading would need $N_e = 63.2$—row 495); trajectory test 2026-08-06 (`computations/slow_roll_trajectory.py`): the gate slow-roll trajectory gives $(n_s, r) = (0.813, 0.188)$ (1 step = 1 e-fold) or $(0.914, 0.060)$ ($N_e = 40$ literal, 1 step = $\ln\varphi$ physical e-folds)—$n_s$ 12–36σ from Planck; $N_e = 40$ is a start-threshold choice, not a derived count; the two claimed numbers do not coexist on the trajectory | Planck 2018 $n_s = 0.9649 \pm 0.0042$ | 03-cosmo.md:53, 108-113; 01-core.md:182-186 | Mapped |
| $\Omega_{\text{DM}}/\Omega_b = \varphi^3 + 1 \approx 5.24$ | "zero free parameters" (observational-seti.md) | Base $\varphi^3 = \alpha_0^{-1} = \xi \cdot \sin^2\theta_W$ Derived conditional on the Weinberg-angle identification (2026-08-11: the literal rung-gap reading fails — span $\xi$(rung 6) − $\alpha_{\text{EM}}$(10.2) = −4.2, not 3); $+1$ hand-added term, NOT supported by the SPARC condensate fits (bound-baryon fraction 0.27–0.40 vs the implied 0.19) and double-counting $\Omega_b$ (the observed ratio already includes bound baryons) | $\Omega_{\text{DM}}/\Omega_b = 5.39$ (Planck) | 06-hyp.md:161-166 | Mapped ($+1$); base Derived conditional |
...
| CMB axis alignment $12.2°$ (C10) | "Predicted alignment" (observational-seti §4.2); registry C10: Derived (magnitude) / Calibrated (direction) | Magnitude: $2\pi/\varphi^7 = 12.40°$ (the pole-spiral closure ladder's 13-seed residual; exact identity $13/\varphi^2 = 5 - 1/\varphi^7$) matches the measured $12.22°$ at 1.5% — the closure ladder is the framework's documented sequence, no power scanned to fit; the axis DIRECTION is computed via spherical law of cosines from the measured CMB direction vectors (Calibrated); the boundary orientation is fitted to the measured axis post-hoc (mechanism Hypothesized) | Planck CMB dipole and quadrupole-octopole directions | 03-cosmo.md:81; refined-numeric-predictions.md §2.3; wake-geometry.md §3b | Calibrated (direction) / Derived (magnitude) |
...
**Row count: 40.** A row here does not settle the quantity's physics—it settles
