...
| $t_\Upsilon,\mathcal B,\mathcal W_{+\leftarrow-},I_{-\to+},I_c$ | inter-vertex endpoint hopping coefficient, oriented relative-connection integral, Wilson transporter, endpoint number current, and fixed-amplitude transport capacity | $\mathcal B=\int_{v_-}^{v_+}B_{\mathfrak s}\,d\mathfrak s$; $\mathcal W_{+\leftarrow-}=e^{-ig_Q\mathcal B}$; $I_{-\to+}=-(2t_\Upsilon/\hbar)\operatorname{Im}(\Upsilon_+^*\mathcal W_{+\leftarrow-}\Upsilon_-)$; $I_c=2t_\Upsilon u_-u_+/\hbar$; $[t_\Upsilon]=\hbar/T$, $[\mathcal B]=[\mathcal W]=1$, and $[I]=[n_v]T^{-1}$ under the EL9 normalization | **Hypothesized conservative action extension / Derived conditional covariance, conservation, capacity, and phase-curvature identities.** The IT1–IT6 receipt passes at one normalized point. The microscopic origin and physical value of $t_\Upsilon$, endpoint amplitudes, coupled rail solution, and local scale-bulk completion remain open; none of these quantities enters the §7 numerical totals (`foundations/endpoint-link-and-localization-boundary.md` §3.11) |
