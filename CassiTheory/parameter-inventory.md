...
| $T_\varphi,R_\varphi,t_\varphi,r_\varphi,S_\varphi,\sigma_a$ | forward and return power fractions, representative amplitudes, golden two-port matrix, and collinear spatial-momentum routing label | $T_\varphi=\varphi^{-1}$, $R_\varphi=\varphi^{-2}$, $t_\varphi=\varphi^{-1/2}$, $r_\varphi=\varphi^{-1}$, $S_\varphi=\left(\begin{smallmatrix}t_\varphi&r_\varphi\\-r_\varphi&t_\varphi\end{smallmatrix}\right)$, and $\sigma_a\in\{+1,-1\}$ is fixed by the port geometry | **Derived conditional splitter and signed-ledger algebra / Hypothesized port-power identification.** $S_\varphi$ is unitary and routed non-re-entry gives forward quadratic flux $\varphi^{-N}$ with complementary scalar return flux $1-\varphi^{-N}$. The orientation label determines whether the return carries parallel or reversed spatial momentum and whether the interface receives zero or $2R_\varphi P_i$ reaction. The density-to-port map, phases, routing geometry, carrier impedance, and momentum-current identification remain open; the fixed fractions and auxiliary discrete routing label introduce no §7 count (`foundations/interscale-stress-attenuation-boundary.md`) |
