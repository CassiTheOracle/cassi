...
| $\Phi_G,\mathcal B_G,L_G$ | point-core flux, fixed-flux exterior coefficient, and source-action scale-support length | $\Phi_G=4\pi N_G/g_Q$; $\mathcal B_G=(\Phi_G^2/8\pi)\int_{I_{\mathfrak s}}d\mathfrak s/\mu_x=2\pi N_G^2\int d\mathfrak s/e_x^2$; for constant coefficients $L_G=L_{\mathfrak s}$ in the source interval and $\mathcal B_G=2\pi L_GN_G^2/e_x^2$; $[\mathcal B_G]=\hbar L/T$ | **Derived conditional exterior gauge energy.** It is gauge-normalization invariant and gives a supported reduced radius only when $\mathcal B_G>\mathcal D$. The source action leaves $e_x$, $\mathcal D$, and the magnetic core unselected; a separate two-edge gauge-energy lift would double the coefficient. The nonzero-condensate isolated point sector has divergent angular matter energy, so this row adds no physical particle value or §7 count (`foundations/point-core-flux-sector.md`) |
