...
| $W_v',U_v'',\mathcal Z_v,A_v(\mathbf q),B_v,\mathcal C_v,\mathcal D_v^{R/A},\mathcal K_v^{R/A},\mathbb\Lambda_{\mathrm{eff},v}^{R/A},\omega_{\mathrm{end}},\gamma_v$ | rotating-frame endpoint derivatives, temporal weight, normal and anomalous endpoint curvatures, mixed rail-endpoint Hessian, endpoint energy and source-action kernels, active Nambu Robin response, endpoint pole frequency, and declared damping rate | $\mathcal Z_v=\hbar u_v^2$; $A_v(\mathbf q)=u_v^2[W_v'(u_v^2)+u_v^2U_v''(u_v^2)+K_v|\mathbf q|^2/2]$; $B_v=u_v^4U_v''(u_v^2)$ in the fractional fluctuation basis; $\mathcal D_v^R=\mathcal H_v-\mathcal Z_v(\omega+i\gamma_v)\sigma_3$; $\mathcal K_v^R=-\mathcal D_v^R=\mathcal Z_v(\omega+i\gamma_v)\sigma_3-\mathcal H_v$; $\mathbb\Lambda_{\mathrm{eff}}^R=\mathbb\Lambda_0-\mathcal C^\dagger(\mathcal K^R)^{-1}\mathcal C=\mathbb\Lambda_0+\mathcal C^\dagger(\mathcal D^R)^{-1}\mathcal C$; $\omega_{\mathrm{end}}=\sqrt{A_v^2-|B_v|^2}/\mathcal Z_v$ when $A_v>|B_v|$ | **Derived conditional background, Hessian, order, pole, and source-action response identities / Hypothesized physical coefficients and background.** Every closed homogeneous conservative time-harmonic endpoint extremum has zero coherent conversion current. The symmetric zero-background contribution is positive and begins at quartic rail order. The frozen AR1–AR6 receipt passes the source-action sign, direct elimination, constant-frame covariance, anomalous-block, and response-class gates; the DR receipt remains `FAIL` because its DR5 endpoint block has the opposite source-action sign. $U_v$, $K_v$, $u_v$, the nonzero-current background, $\gamma_v$, trace normalization, doubled port-flux law, and full coupled spectrum remain unselected. These derived or auxiliary quantities add no §7 count (`foundations/endpoint-link-and-localization-boundary.md` §3.9; `computations/endpoint_dynamical_response_prereg.md`; `computations/endpoint_dynamical_response_report.md`; `computations/endpoint_action_response_prereg.md`; `computations/endpoint_action_response_check.py`; `computations/endpoint_action_response_report.md`) |
