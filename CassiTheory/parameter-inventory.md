...
| $B_i,B_{\mathfrak s},\mu_x,\mu_m$ | relative Yang/Yin connection components and spatial/mixed curvature stiffnesses | $[B_i]=L^{-1}$, $[B_{\mathfrak s}]=1$, $[\mu_x]=T/(\hbar L)$, $[\mu_m]=TL/\hbar$; only $e_x^2=g_Q^2\mu_x$ and $e_m^2=g_Q^2\mu_m$ are internally identifiable | **Hypothesized interscale inputs.** A full time-local gauge completion requires temporal curvatures and a Gauss law; not counted in §7 (`foundations/interscale-current-soliton.md`) |
