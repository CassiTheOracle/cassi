...
| $\gamma_\varepsilon,\gamma_c$ | composition-imbalance and transverse-coherence relaxation rates in the minimal completely positive conversion lift | $\gamma_\varepsilon=\varphi^2\lambda(1-q)$; $\gamma_c=\gamma_\varepsilon/2$; at the gated reference state $(\gamma_\varepsilon,\gamma_c)=(\lambda/3,\lambda/6)$ | **Derived within the minimal lift.** The population equations reduce exactly to canonical conversion; the coherence half-rate depends on the declared two-jump off-diagonal completion and is not an independent parameter or §7 count (`foundations/geometric-manifold-completion.md` §§4.4–4.5) |
