...
| $g_f,a_f,b$ and induced $x_f,g_c,x_c$ in the pure-gauge comparison | supplied bare $SU(2)$ coupling, fine spacing and integer path length, with block-map coefficients | $g_f,a_f>0$; $x_f=2/g_f^4$; $g_c^2=b^2g_f^2$ and $x_c=x_f/b^4$ when $a_c=ba_f$; $\Delta_E=3g_f^2/(2a_f)$ on the girth-four graph; $\Delta E_{\mathrm{el},\partial}=2g_f^2/a_f$ inside the fixed fundamental-boundary fibre | **External target-theory inputs / Derived regulated spectrum, representation-support and unit identities**, `foundations/loop-to-bubble-projection-theorem.md` §§9.4–9.22. Numerical couplings, path lengths and representation cutoffs are fixed controls. No Cassi parameter, fitted value, physical mass prediction or change to the §7 count is introduced |
