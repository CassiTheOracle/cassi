...
| $\mathcal O_N,\vartheta,J_t,J_x$ | gauge-invariant common-phase observable and temporal/spatial stiffnesses | $\mathcal O_N=\Psi^T i\sigma^2(\Phi^a\sigma^a)\Psi$, $\vartheta=\arg\mathcal O_N$; $J(a,d)=4a\rho_0d v_Q^2\sin^2\beta/(a\rho_0+4d v_Q^2)$, $J_t=J(C_\Psi,C_\Phi)$, $J_x=J(K_x,1/\mu_x)$; $[\mathcal O_N]=L^{-4}$, $[J_t]=\hbar T/L^3$, $[J_x]=\hbar/(TL)$ | **Derived conditional composites / Tested linear-wave dynamics.** The supplied action has $\omega^2=(J_x/J_t)k^2+O(k^4)$, with $J_x/J_t=0.8649109511178478$ at the declared model-unit inputs. The common-phase identification is $\Theta\sim\Theta+\pi$; the local gauge stabilizer stays trivial. No physical normalization, quantum condensate or statistics is selected; no fit or §7 numerical parameter is added (`foundations/particle-stationary-action-closure.md` §4.3; `computations/matter-formation-continuum-report.md` §40) |
