...
| $\mathcal G_C$ | optional Qi-gravity coupling-magnitude diagnostic | $\mathcal G_C=(\pi/\rho)[1+(\varphi^6-1)q]$; on $\varepsilon=0$, $\pi/\rho=\varphi^{-3}$, the dilute endpoint gives $\varphi^{-3}$, and the registered reference-density point gives $5\sqrt5/3\approx3.72678$ | **Derived conditional algebra / Hypothesized physical gravity mapping.** The composition attractor is the full $\varepsilon=0$ density line; $q=0$ names only its dilute endpoint. $\mathcal G_C$ is not a selected atomic response, source profile, metric, or measured $G/G_N$ and adds no §7 parameter (`foundations/quantum-free-fall-correspondence.md` §7) |
