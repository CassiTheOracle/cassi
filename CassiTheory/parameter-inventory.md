...
| $\mathcal A,\mathcal B,\mathcal C,\mathcal D,\mathcal T_\star,K_\parallel,W;\ A,B,\mathcal E_{\mathcal Q}$ | reduced Derrick gradient, core, bulk, attraction, loop tension, phase stiffness, and winding data; illustrative Fibonacci-sector costs and dimensionless profile energy | free profile-dependent quantities with $\mathcal B>\mathcal D$ required for a small-radius barrier and $W\in\mathbb Z$ only under compact finite-energy boundary conditions; $A,B,\mathcal E_{\mathcal Q}$ have no selected values | **Conditional reduced-model inputs** in `foundations/interscale-current-soliton.md`; the positive candidate action does not derive $\mathcal D$, the loop geometry, compact winding, Fibonacci sector cost, or mass-profile normalization; not counted in §7 |
| $\mathfrak s_p,m,\delta_{\mathrm{end}},\kappa_{0,p},\alpha_{0,p},\mathcal T_{\mathfrak s}$ | Planck-to-proton scale endpoint, circuit winding, endpoint phase bias, endpoint mixing strengths/phases, and scale tension | $\mathfrak s_p=91.4616$ is Mapped from $m_p$; $m\in\mathbb Z$ only for a compact closed circuit; the remaining quantities are free. In the unbiased $m=1$ closure, selecting $\mathfrak s_p$ requires $\mathcal T_{\mathfrak s}/(K_{\mathfrak s}\rho)=5.57043\times10^{-4}$ | **Hypothesized proton scale-circuit inputs** in `foundations/interscale-current-soliton.md` §4.5 and `foundations/proton-coherence-budget.md` §10; the current and energy coefficients are Derived conditional on these inputs, while endpoint conversion, proton selection, and a decay rate remain open; not counted in §7 |
| $\widetilde I_{\mathfrak s},S_\pm,\delta_\pm$ | two-rail scale metric graph, endpoint gluing maps, and endpoint phases | two vertices and two edges; $b_1=1$; circumference $2L_{\mathfrak s}$; $S_\pm$ are gauge-covariant intertwiners represented by flux-unitary Yang/Yin boundary phases $\delta_\pm$ and accompanied by Kirchhoff current conservation | **Hypothesized object-sector topology and boundary completion / Derived graph algebra.** The quotient gives one compact internal scale cycle and recovers the existing circuit holonomy without forcing equal rail densities. The physical scale metric, charged endpoint fields or equivalent dressing, and phase biases remain open; no new numerical value or §7 count is introduced (`foundations/geometric-manifold-completion.md` §§2.4–2.5, 5) |
