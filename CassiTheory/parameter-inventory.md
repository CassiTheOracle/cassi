...
| $T_\varphi,R_\varphi,t_\varphi,r_\varphi,S_\varphi,\sigma_a;\ \Lambda_v,k_\star,\tau_\varphi,S_\Lambda(k)$ | forward and return power fractions, representative amplitudes, golden two-port matrix, spatial-momentum routing label; Hermitian endpoint coupling, design wave number, inverse-Cayley coefficient, and two-lead scattering family | $T_\varphi=\varphi^{-1}$, $R_\varphi=\varphi^{-2}$, $t_\varphi=\varphi^{-1/2}$, $r_\varphi=\varphi^{-1}$, $S_\varphi=\left(\begin{smallmatrix}t_\varphi&r_\varphi\\-r_\varphi&t_\varphi\end{smallmatrix}\right)$, $\sigma_a\in\{+1,-1\}$; for dimensionless $\mathfrak s$, $\Lambda_v$ is a Hermitian $2\times2$ auxiliary coupling with $[\Lambda_v]=[K_{\mathfrak s}]=\hbar/T$, while $k_\star$, $\tau_\varphi$, and $S_\Lambda$ are dimensionless; $S_\Lambda(k)=(ikK_{\mathfrak s}I-\Lambda_v)^{-1}(ikK_{\mathfrak s}I+\Lambda_v)$, $\tau_\varphi=r_\varphi/(1+t_\varphi)$, and matching the Hypothesized target at $k_\star$ requires the selected inverse coupling $\Lambda_\varphi=iK_{\mathfrak s}k_\star\tau_\varphi\left(\begin{smallmatrix}0&1\\-1&0\end{smallmatrix}\right)$ | **Hypothesized golden target / Derived conditional boundary and inverse-matching algebra / Hypothesized endpoint and port selection.** The quadratic action fixes the boundary flux; Hermitian endpoint data give unitary $S_\Lambda(k)$; the registered two-rail gluing is perfect transfer; inverse matching fixes the coupling required by the target at one $k_\star$, while a fixed local coupling gives a wave-number-dependent split. Routed non-reentry gives forward quadratic flux $\varphi^{-N}$ with complementary scalar return flux $1-\varphi^{-N}$. The physical endpoint field, gauge dressing, $k_\star$, density-to-port map, routing geometry, impedance, and momentum-current identification remain open. These auxiliary boundary quantities introduce no §7 count (`foundations/particle-stationary-action-closure.md`; `foundations/interscale-stress-attenuation-boundary.md`; `computations/interscale_port_matching_prereg.md`) |
...
| $\Upsilon_\pm,\kappa_\pm,K_\pm,U_\pm,\gamma_\pm,\mathcal J_{c,\pm}$ | charged coherent endpoint sections, link couplings, endpoint stiffnesses/potentials, one-way open rates, and critical currents | $\Upsilon_v\mapsto e^{-ig_Q\alpha(v)}\Upsilon_v$; $\mathcal J_{c,v}=2\kappa_v|\Upsilon_v|\sqrt{E_YE_I}/\hbar$; uniform circulation requires $\kappa_v|\Upsilon_v|\geq K_{\mathfrak s}|\Delta_m|/(2\varphi^{3/2}\mathfrak s_p)$; the open alternative gives $\gamma_-/\gamma_+=\varphi$ | **Hypothesized endpoint field or bath / Derived conditional covariance, source, capacity, and rate ratio.** The absolute couplings, potentials, rates, and endpoint normalization are unselected. The rate ratio is derived from the uniform donor densities and introduces no independent parameter or §7 count (`foundations/endpoint-link-and-localization-boundary.md`) |
