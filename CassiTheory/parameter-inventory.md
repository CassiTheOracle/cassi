...
| $\mathbb W_P,T^{\rm closed}_{\mu\nu},J_\nu^{\rm io},\mathbb K_Q^{(2)},\mathbb M_Q,\mathbb G_Q,\mathbb P_Q(\omega)$ | multimode flux operator, total Hilbert stress, subsystem exchange density, physical fixed-charge Hessian, temporal and gyroscopic fluctuation operators, and mixed spectral pencil | $P=\operatorname{tr}(\mathbb W_P\rho_{\rm car})$; $\nabla^\mu T^{\rm closed}_{\mu\nu}=0$; $\nabla^\mu T^{\rm in}_{\mu\nu}=-J_\nu^{\rm io}$; $\mathbb P_Q=\mathbb K_Q^{(2)}-i\omega\mathbb G_Q-\omega^2\mathbb M_Q$ | **Derived conditional definitions and conservation/stability boundary / Tested low-spectrum sign on one diffuse and one localized finite-grid field.** The independently matched lowest eigenpairs of the diffuse $h_C=1.50$ background's $13622$-dimensional strict-shell $C_4$ representation of $\mathbb K_Q^{(2)}$ contain no negative mode, while the field fails localization and retention. The localized $h_C=2.9598260763447164$ branch has a $77000$-dimensional fixed-charge $C_4$ physical quotient; its six independently matched lowest eigenpairs contain one numerically near-zero carrier-phase symmetry mode, no negative mode, and five positive modes. The symmetry mode retains unresolved odd-even spatial structure. $\mathbb W_P$, explicit stress components, localized Hessian resolution, perturbations beyond $C_4$, continuum qualification, $\mathbb M_Q$, and $\mathbb G_Q$ remain open. These operators introduce no universal parameter or §7 count (`foundations/matter-completion-boundary.md` §§5, 7–10; `foundations/particle-stationary-action-closure.md` §8.6; `computations/particle-physical-hessian-precision-v2-report.md`; `computations/particle-localized-physical-hessian-report.md`) |
...
| $\alpha_{\mathfrak s},u_\rho,u_\varphi,\gamma_x,\gamma_{\mathfrak s},u_H,k_{Cx},k_{C\mathfrak s},e_C,h_C,u_C$ | static particle-action dimensionless groups | Reference normalization fixes $\alpha_{\mathfrak s}=\gamma_x=\gamma_{\mathfrak s}=k_{Cx}=k_{C\mathfrak s}=u_C=1$, $u_\rho=u_\varphi=u_H=4$, and $e_C=0.75$. The diffuse fluctuation background uses $h_C=1.50$. The localized retained branch uses $h_C=2.9598260763447164$, the first candidate in a frozen ordered density-depletion scan; its finest artifact's six independently matched lowest constrained $C_4$ finite-grid PA42 eigenpairs contain one numerically near-zero carrier-phase symmetry mode, no negative mode, and five positive modes | **Hypothesized action coefficients / Mapped selected $h_C$ / Tested finite-grid branch and six-mode low-spectrum sign.** The selected coupling is uncalibrated and has its Fit-Status Ledger row in §10. The symmetry mode retains unresolved odd-even spatial structure. These optional-extension groups remain outside the counted §7 canonical classes (`foundations/particle-stationary-action-closure.md` §7; `computations/particle-localized-physical-hessian-report.md`) |
...
| Finite stationary-solver boundary data | numerical domains, grids, seeds, tolerances, localization shells, symmetry bases, gauge quotients, and stopping rules | $\rho=1$, $c=0$, and a fixed gauge representative on $\partial\Omega_R$; no-flux scale interval; fixed $q_C$; diffuse point $(R,N)=(4,17)$ plus $(5,21)$ comparison; localized branch at $R=4$ on $N=17,21,25,29$ grids and at $R=5,N=21$; localized Hessian on the $N=29$ field in the $77000$-dimensional fixed-charge $C_4$ physical quotient | **Numerical boundary data / Tested finite-grid localized branch and six-mode constrained PA42 low-spectrum sign.** These implementation choices carry no physical parameter status and remain outside the §7 count. Localized Hessian resolution, perturbations beyond $C_4$, and continuum qualification remain open (`foundations/particle-stationary-action-closure.md` §§6–8; `computations/particle-carrier-resolution-recovery-report.md`; `computations/particle-localized-physical-hessian-report.md`) |
...
| Proton stability ($\mathfrak s_p=91.4616$; coherence exponent $4505.58$ at $N_p^{\mathrm{budget}}=91.46$; candidate current $\hbar\mathcal J_{Q,1}/(K_{\mathfrak s}\rho)=0.0162173$) | Mapped endpoint with two distinct conditional mechanisms | The measured proton mass fixes $\mathfrak s_p$. The independent-step profile at the registered two-decimal budget coordinate gives the conditional $\varphi^{4505.58}$ cycle count, and the additional Compton-cycle map gives $\tau_p\approx10^{910}$ yr. The interscale action gives a zero-total-flow two-rail current and normalized energy $0.0509481$; selecting the endpoint in the unbiased $m=1$ closure requires the unmeasured ratio $\mathcal T_{\mathfrak s}/(K_{\mathfrak s}\rho)=5.57043\times10^{-4}$. Charged coherent and one-way open endpoint equations are Derived conditionally. Point-core flux gives an exact conditional exterior coefficient. The Hypothesized auxiliary adjoint $SU(2)_Q$ action supplies a smooth decoupled core, while the registered condensate confines flux and supplies no persistent pair. The source-free temporal branch supplies Derived conditional gauge covariance, Gauss, and fixed-charge stationary equations. A Mapped density-depletion coefficient supports a Tested localized, nodeless, carrier-retaining finite-grid branch across four same-domain resolutions and one larger-domain comparison. Its finest artifact's six independently matched lowest constrained $C_4$ finite-grid PA42 eigenpairs contain one numerically near-zero carrier-phase symmetry mode, no negative mode, and five positive modes; the symmetry mode retains unresolved odd-even structure. Localized Hessian resolution, perturbations beyond $C_4$, continuum qualification, PA43 dynamics, real-time formation, physical endpoint normalization, scale tension, proton quantum numbers, and decay rate remain open | Proton mass; proton-decay searches | 02-sm.md:101-106; 01-core.md:120-122; `foundations/proton-coherence-budget.md` §10; `foundations/endpoint-link-and-localization-boundary.md`; `foundations/point-core-flux-sector.md`; `foundations/nonabelian-magnetic-core-boundary.md`; `foundations/particle-stationary-action-closure.md`; `computations/particle-carrier-resolution-recovery-report.md`; `computations/particle-physical-hessian-precision-v2-report.md`; `computations/particle-localized-physical-hessian-report.md` | Mapped endpoint and carrier coupling / Derived conditional algebra and localization boundary / Tested resolution-consistent localized finite-grid branch and six-mode constrained $C_4$ finite-grid PA42 low-spectrum sign / Hypothesized physical realization |
| $h_C=2.9598260763447164$ density-depletion coupling | Mapped numerical candidate; no physical calibration | The value is half the analytic retention-buffer reference and the first entry in the frozen ordered scan. With all other dimensionless coefficients fixed, the direct carrier coordinate produces a localized, nodeless, retained stationary branch on four same-domain grids and one larger-domain grid. Its finest artifact's six independently matched lowest constrained $C_4$ finite-grid PA42 eigenpairs contain one numerically near-zero carrier-phase symmetry mode, no negative mode, and five positive modes; the symmetry mode remains spatially unresolved. The selection used numerical qualification rather than observed particle data | No observational anchor; stationary and Hessian finite-grid artifacts in `computations/particle-carrier-direct-coordinate-report.md`, `computations/particle-carrier-resolution-recovery-report.md`, and `computations/particle-localized-physical-hessian-report.md` | `foundations/particle-stationary-action-closure.md` §8.5–§8.6; `computations/particle-carrier-localization-report.md`; `computations/particle-carrier-resolution-recovery-report.md`; `computations/particle-localized-physical-hessian-report.md` | Mapped |
