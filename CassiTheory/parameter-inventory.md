...
| $q_C,\widehat\omega_C,\widehat R,L_{\mathfrak s}$ | dimensionless fixed charge, Lagrange multiplier, spatial-domain size, and scale-interval length | $q_C=Q_C/(\rho_0\ell_Q^3)$, $\widehat\omega_C=\hbar\omega_C\ell_Q^2/K_x$, $\widehat R=R/\ell_Q$, $L_{\mathfrak s}=\mathfrak s_+-\mathfrak s_-$ | **Hypothesized boundary-value data / Derived rescalings / Frozen numerical test input.** The registered campaign sets $q_C=4$, $\widehat R=4$ on P/H, $\widehat R=5$ on D, and $L_{\mathfrak s}=1$. Five structural P arms pass Q1–Q4; the selected `P:separated_core` arm has $\widehat\omega_C=0.961914$, fails carrier retention, and lacks domain/resolution qualification. The values have no physical calibration and add no §7 parameter (`computations/particle-stationary-q2-recovery-report.md`) |
