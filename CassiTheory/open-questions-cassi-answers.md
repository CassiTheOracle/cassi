...
separation under explicit support, retention, and matching inequalities. The
physical carrier map, reservoir, scale metric, endpoint normalization,
transverse carrier mode, full stationary solution, observation map, quantum
numbers, and temporal gauge dynamics remain open, so the DQ and GQ
physical-identification verdicts are unchanged.
...
Grand unified theories motivate proton-lifetime searches. Cassi has two distinct conditional descriptions. The coherence-budget extension assigns a Hypothesized independent failure probability to each scale step and yields the arithmetic cycle count $\varphi^{4506}$; its conversion to a lifetime remains conditional. The interscale-current extension places the Mapped proton coordinate at $\mathfrak s_p=91.4616$ and admits a closed two-rail current with $J_{\mathfrak s}=0$ and $J_Q\ne0$. A charge-$-g_Q$ endpoint section closes the circuit coherently when its capacity bound holds. A one-way Markov alternative closes the populations with $\gamma_-/\gamma_+=\varphi$ and damps endpoint coherence. The smooth zero-Chern endpoint sector has no finite Derrick radius. Point-core flux and the auxiliary adjoint branch supply a conditional smooth local core; the registered condensate confines it and supplies no persistent pair by itself. A neutral core-trapped carrier gives one reduced stable separation under support, retention, and matching inequalities. No transverse carrier mode, full stationary proton, particle quantum numbers, temporal gauge sector, or lifetime map is selected.

| **Cassi Answer** | A Planck-to-proton relative-current circuit has explicit conditional endpoint equations, an exact conditional point-flux exterior coefficient, a conditional auxiliary smooth-core branch, and a conditional neutral-carrier reduced support branch. The registered fields alone give no persistent composite. The carrier theorem supplies a finite length coordinate under explicit inequalities, while the full localized proton and lifetime remain open |
| **Mechanism** | Yang flows from Planck to the proton endpoint and Yin returns. At $E_Y/E_I=\varphi$, $\mathcal J_{Q,m}=K_{\mathfrak s}\rho\Delta_m/(\hbar\varphi^3\mathfrak s_p)$. Coherent turning has finite capacity; one-way open turning has rate ratio $\varphi$. Imposed point flux contributes $\mathcal B_G/R$. The auxiliary adjoint $SU(2)_Q$ branch regularizes its local core, while the nonzero fundamental condensate gives a London mass and confines unit flux into a spatial-winding-two tube. A neutral carrier contributes $A_C/L$ and has one reduced root when $A_C>C_Q$, $\hbar\omega_C<\varepsilon_{C,\rm out}$, and the root lies beyond core overlap |
| **Epistemic** | **Mapped** (proton coordinate) / **Derived conditional** (zero-total-flow current, endpoint covariance, capacity, rate ratio, point-flux coefficient, support inequality, auxiliary-core identities, condensate topology, confinement, registered-pair no-go, and carrier-support theorem) / **Hypothesized** (auxiliary $SU(2)_Q$ and carrier actions and inputs, physical endpoint normalization, scale tension, bound transverse carrier mode, full stationary composite, temporal gauge dynamics, proton identification, and decay rate). Neither candidate determines a physical lifetime |
| **Reference** | `foundations/proton-coherence-budget.md` §10; `foundations/interscale-current-soliton.md` §4.5; `foundations/endpoint-link-and-localization-boundary.md`; `foundations/point-core-flux-sector.md`; `foundations/nonabelian-magnetic-core-boundary.md`; `foundations/core-trapped-charge-support.md`; `computations/endpoint_link_localization_check.py`; `computations/point_core_flux_check.py`; `computations/magnetic_core_completion_check.py`; `computations/core_trapped_charge_check.py` |
...
**Visual:** The framework uses $\varphi$, canonical two-fluid notation, and the cascade to index mixed-tier claims. The geometric completion ansatz unifies the declared density, coherence, projective, and scale-circuit maps while preserving separate conservative and open-system dynamics. An auxiliary internal $SU(2)_Q$ branch regularizes the tested magnetic core, the registered condensate confines it, and a neutral carrier supplies one conditional reduced support branch. Unified microscopic physics and a covariant gravity completion remain open.

The current framework organizes cosmology, particle physics, gravity, and the Standard Model around shared notation. The geometric completion supplies a stratified metric-graph bundle with exact reduction to canonical density dynamics and a conditional completely positive coherence lift. Charged coherent and one-way open vertex sectors provide conditional endpoint equations. The full coherence fibre is contractible, the smooth object base has no first-Chern sector, and the minimal endpoint completion has no finite Derrick radius. Point-core Chern flux supplies an exact conditional exterior $1/R$ coefficient. An auxiliary adjoint $SU(2)_Q$ branch, distinct from electroweak $SU(2)_L$, supplies a regular local core and matches that coefficient. The registered nonzero fundamental condensate removes the isolated magnetic sector and confines flux. A neutral core carrier supplies one conditional reduced separation through exact support, retention, and matching inequalities. No microscopic identification or full stationary fixed-charge solution is selected.

| **Cassi Answer** | Mixed-tier sector organization plus a Hypothesized geometric completion ansatz, a Hypothesized auxiliary magnetic-core action, and a Hypothesized neutral-carrier action with exact conditional reductions; unified microscopic dynamics remains open |
| **Mechanism** | The stratified bundle connects canonical densities, loop coherence, projective geometry, a compact scale circuit, conditional endpoint sectors, and a point-flux exterior coefficient. The auxiliary adjoint core regularizes local flux, the registered condensate confines it, and the neutral carrier supplies a conditional inverse-length support term. Conservative, dissipative, gravity, and Standard Model dynamics remain distinct |
| **Epistemic** | **Hypothesized completion ansätze / Derived canonical reduction, conditional fibre geometry, endpoint boundaries, point-flux coefficient, auxiliary-core matching, condensate confinement, registered-pair no-go, and reduced carrier-support theorem**—microscopic matching, physical endpoint normalization, reservoir, transverse carrier mode, full stationary solution, temporal gauge dynamics, and covariant gravity remain open |
| **Reference** | `foundations/geometric-manifold-completion.md`; `foundations/endpoint-link-and-localization-boundary.md`; `foundations/point-core-flux-sector.md`; `foundations/nonabelian-magnetic-core-boundary.md`; `foundations/core-trapped-charge-support.md`; `foundations/physical-becoming-hierarchy.md` §7.1, §7.2; `foundations/unified-lagrangian.md`; `foundations/interscale-current-soliton.md`; `cassi-physics.md` |
