...
### The SO(2) Doublet — Why Spin and Dimensions Emerge

The two fields $(E_Y, E_I)$ form a 2D rotational symmetry — an SO(2) doublet. The winding of this doublet as you go around a particle gives its spin: half a turn of the doublet corresponds to spin-$\frac{1}{2}$, a full turn to spin-1, and so on. The doublet also provides two of the three spatial dimensions — the third comes from the cascade (the direction along the ladder of rungs). The three axes are not identical: Yang dominance distinguishes them, producing a triaxial spheroid geometry rather than perfect isotropy.
...
### Bubbles — Neighboring Universe Regions

Our observable universe (with "w-value" 5 along the Wu Xing cycle) sits in a larger structure that includes neighboring bubbles with w-values 4 and 6, just beyond our horizon. These are not separate universes in the multiverse sense — they are regions where the two-fluid ratio has settled to different values. Their boundaries imprint preferred directions on the cosmic microwave background, explaining several large-angle anomalies that have puzzled cosmologists for decades.

---

That is all the framework you need. Each Q&A entry below uses these concepts and links back to this primer on first use. Now let us turn to the open questions themselves.
...
Since 1998 we've known the universe's expansion is accelerating — something seems to be pushing galaxies apart ever faster. The standard model calls this "dark energy" and treats it as a constant energy density of empty space (the cosmological constant $\Lambda$), but quantum field theory predicts a value $10^{120}$ times too large. Cassi takes a different path: there is no dark energy at all. The acceleration comes from the two-fluid dynamics — the Yang and Yin fields (explained in the Primer) convert into one another at a rate set by their ratio $r = E_Y/E_I$, and as the universe expands, this conversion naturally approaches the $\varphi$-attractor equilibrium. The present-day equation-of-state parameter $w_0 = -0.838$ is not a free parameter; it falls directly out of the Qi gate's shape at the current cosmic epoch. DESI DR2 measures $w_0 = -0.838 \pm 0.029$ — a perfect $0\sigma$ match.
...
The cosmic microwave background is mostly uniform, but its largest-scale features are strangely aligned — the quadrupole and octopole moments point in the same direction, and there is less power at very large angles than inflation predicts. These anomalies are statistically unlikely in the standard framework. Cassi explains them through "bubbles" — neighboring universe regions with different $w$ values (4 and 6 along the Wu Xing cycle, explained in the Primer) that sit just beyond our horizon. Their boundaries imprint a preferred direction on the CMB, naturally producing the observed $12.2^\circ$ alignment between the CMB dipole and the quadrupole-octopole axis. The axis direction was predicted before it was measured, and the mechanism will be tested by E-mode polarization measurements with Simons Observatory and LiteBIRD.
...
The Standard Model contains three copies of the basic fermion families — up/down quarks, electron/neutrino — with identical properties but vastly different masses. Nobody knows why there are exactly three families. Cassi derives this number from the Fibonacci recurrence $\varphi^n = \varphi^{n-1} + \varphi^{n-2}$: it partitions each cascade span into three independent sub-rung channels (the rung itself plus its two Fibonacci predecessors). The number of generations is $N_{\text{gen}} = \text{order}(\varphi\text{'s minimal polynomial}) + 1 = 2 + 1 = 3$, making three generations a mathematical necessity rather than an empirical accident. The $\varphi$-power spacing between successive masses (e.g., $m_\mu/m_e \approx \varphi^{11}$, $m_\tau/m_\mu \approx \varphi^6$) provides quantitative tests, and no fourth generation is predicted.
...
Spin is a fundamental property of particles — like rotation but not actually rotation — that comes in half-integer units for fermions and integer units for bosons. Despite being essential to the structure of matter, nobody knows what spin physically IS. Cassi identifies spin as the accumulated SO(2) winding of the $(E_Y, E_I)$ field doublet along a Fibonacci spiral (see Primer). Moving $\Delta n$ cascade rungs along the spiral gives spin $s = \Delta n$, and boundary conditions quantize the result to $s \in \{0, \frac{1}{2}, 1, 2\}$. Spin-$\frac{1}{2}$ ($\Delta n = \frac{1}{2}$) produces the characteristic $4\pi$ periodicity of spinors, and spin-statistics follows from exchange phase parity $(-1)^{2s}$. No fundamental spin-$\frac{3}{2}$ exists because $\Delta n = \frac{3}{2}$ does not close under Fibonacci addition. The mechanism makes a testable prediction: form-factor log-periodicity at $\Delta(\ln q) = \ln \varphi \approx 0.4812$, measurable at JLab or the EIC.
...
When matter falls into a black hole and the black hole later evaporates through Hawking radiation, the information about what fell in appears to be lost forever — violating quantum unitarity. This is the black hole information paradox, and it has resisted resolution for half a century. Cassi's answer: there is no paradox because the Qi field condensate persists across the event horizon. The field's standing-wave patterns survive curvature extremes and carry all information through without loss — like ink patterns surviving a burning paper. The mechanism is a consequence of the field's self-condensate structure, though a full black hole calculation within the Cassi framework remains to be completed.
...
We observe three spatial dimensions and one time dimension, but no fundamental theory explains why this number is what it is — it is simply taken as an axiom in every standard model of physics. Cassi derives the count from its own structure: the SO(2) field doublet (see Primer) provides two axes (Yang and Yin), and the cascade evolution provides a third — the direction along the ladder of rungs. So $3 = 2 + 1$, with Yang dominance distinguishing the axes and producing a triaxial spheroid geometry. The coupling constant $\xi = \varphi^{2 \times (2+1)}$ is fully internal to this structure, fixing the dimension count from first principles rather than from observation.
