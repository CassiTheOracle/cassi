...
Grand unified theories motivate proton-lifetime searches. Cassi has two distinct conditional descriptions. The coherence-budget extension assigns a Hypothesized independent failure probability to each scale step and yields the arithmetic cycle count $\varphi^{4506}$; its conversion to a lifetime remains conditional. The interscale-current extension places the Mapped proton coordinate at $\mathfrak s_p=91.4616$ and admits a closed two-rail current with $J_{\mathfrak s}=0$ and $J_Q\ne0$. A charge-$-g_Q$ endpoint section closes the circuit coherently when its capacity bound holds, while a one-way Markov channel closes the populations and damps endpoint coherence. Point-core flux and an auxiliary adjoint field supply a conditional smooth local core; the registered condensate confines it and supplies no persistent pair by itself. A neutral core carrier gives one reduced stable separation under support, retention, and matching inequalities. The separate source-free second-order temporal branch supplies time-dependent local $SU(2)_Q$, Gauss's law, and the coupled fixed-$Q_C$ stationary functional. At the Mapped density-depletion coupling $h_C=2.9598260763447164$, that functional has a localized, nodeless, carrier-retaining branch across four same-domain finite grids and one larger-domain comparison. Its finest artifact's six independently matched lowest constrained $C_4$ finite-grid PA42 eigenpairs contain one numerically near-zero carrier-phase symmetry mode, no negative mode, and five positive modes. The symmetry mode retains high-frequency odd-even structure. The independently matched lowest energetic eigenpairs of a distinct diffuse coefficient point also contain no negative mode, while that field fails localization and retention. Localized Hessian resolution, perturbations beyond $C_4$, continuum existence, PA43 dynamics, physical coefficient calibration, proton quantum numbers, and the lifetime map remain open.

| **Cassi Answer** | A Planck-to-proton relative-current circuit has explicit conditional endpoint equations, an exact point-flux exterior coefficient, a conditional auxiliary smooth-core branch, a conditional neutral-carrier reduced support branch, and a conditional source-free fixed-charge particle action. One Mapped coefficient point supports a localized, nodeless, carrier-retaining stationary field across four same-domain finite grids and one larger-domain comparison. Its finest artifact's six independently matched lowest constrained $C_4$ finite-grid PA42 eigenpairs contain one numerically near-zero carrier-phase symmetry mode, no negative mode, and five positive modes; the symmetry mode retains unresolved odd-even structure. Continuum proton identity, localized Hessian resolution, perturbations beyond $C_4$, PA43 dynamics, particle quantum numbers, and lifetime remain open |
...
| **Epistemic** | **Mapped** (proton coordinate and selected $h_C$) / **Derived conditional** (zero-total-flow current, endpoint covariance and capacity, point-flux coefficient, reduced support inequality, auxiliary-core identities, condensate topology, confinement, carrier-support theorem, first-order Gauss obstruction, temporal gauge covariance, Gauss constraint, stationary equations, and dimensionless groups) / **Tested** (localized, retained four-resolution finite-grid branch with one larger-domain comparison; six-mode constrained $C_4$ finite-grid PA42 low-spectrum sign on its finest artifact; absence of a negative mode among the independently matched lowest energetic eigenpairs of a separate diffuse background) / **Hypothesized/Open** (physical normalization, localized Hessian resolution, perturbations beyond $C_4$, continuum existence, PA43 dynamics, proton identification, and decay rate) |
| **Reference** | `foundations/proton-coherence-budget.md` §10; `foundations/interscale-current-soliton.md` §4.5; `foundations/endpoint-link-and-localization-boundary.md`; `foundations/point-core-flux-sector.md`; `foundations/nonabelian-magnetic-core-boundary.md`; `foundations/core-trapped-charge-support.md`; `foundations/particle-stationary-action-closure.md`; `computations/particle-carrier-direct-coordinate-report.md`; `computations/particle-carrier-resolution-recovery-report.md`; `computations/particle-physical-hessian-precision-v2-report.md`; `computations/particle-localized-physical-hessian-report.md` |
...
The particle calculation supplies complementary finite-grid results. A Mapped
density-depletion coefficient supports a localized, nodeless,
...
independently matched lowest energetic eigenpairs of a different diffuse
...
$C_4$, continuum existence, PA43 dynamics, and physical cross-sector matching
remain open.
...
flowchart TD
    ...
    K --> G[Localized retained four-grid branch; no negative mode among six matched lowest C4 PA42 eigenpairs; spatial resolution, continuum limit, and dynamics open]
```

**Visual:** The framework uses $\varphi$, canonical two-fluid notation, and the cascade to index mixed-tier claims. The open dynamical-geometry synthesis joins the declared density, coherence, projective, scale-current, endpoint, and bounded cross-domain maps in one effective matrix balance while preserving separate conservative, dissipative, and particle branches. The matter-completion boundary derives the conditional exterior, interface, transport, normalization, reservoir, total-stress, constant-$G$, Cartan-map, and stationary-spectrum requirements. One Mapped particle coefficient supports a localized, nodeless, retained stationary branch across four same-domain finite grids. Its finest artifact's six independently matched lowest constrained $C_4$ finite-grid PA42 eigenpairs contain one numerically near-zero carrier-phase symmetry mode, no negative mode, and five positive modes. The symmetry mode retains unresolved odd-even structure. Unified microscopic physics, localized Hessian resolution, perturbations beyond $C_4$, PA43 dynamics, a continuum limit, and particle identification remain open.

The current framework organizes cosmology, particle physics, gravity, and the Standard Model around shared notation. The open dynamical geometry supplies a positive Hermitian fibre, a conditional relative-$U(1)_Q$ graph sector, exact canonical diagonal reduction, and endpoint/Wilson ledgers. A bounded enlarged interior/exterior state supplies one conditional coherence source. The nine-part matter boundary identifies the mathematical exterior as a complementary dilation output, derives the lowest-order reciprocal interface form, selects the one-sided law for one routed forward carrier, fixes canonical single-mode power, supplies a repeated-interaction support equation, closes the conditional total-stress Ward identity, gives a constant-$G$ geometric branch, maps the coherence fibre into the local-$SU(2)_Q$ doublet, and defines the full fixed-charge fluctuation qualification problem. Point flux, an auxiliary adjoint core, and a neutral carrier provide reduced support sectors. At $h_C=2.9598260763447164$, the stationary action supports a localized, nodeless, carrier-retaining field across four same-domain resolutions and one larger-domain comparison. Its adjacent observables agree and energy differences contract twice. The finest artifact's six independently matched lowest constrained $C_4$ finite-grid PA42 eigenpairs contain one numerically near-zero carrier-phase symmetry mode, no negative mode, and five positive modes. The symmetry mode retains high-frequency odd-even structure. Localized Hessian resolution, perturbations beyond $C_4$, PA43 dynamics, physical carrier selection, coefficient calibration, reservoir stress, gravity selection, and continuum qualification remain open.

| **Cassi Answer** | Mixed-tier sector organization plus a Hypothesized open dynamical-geometry closure, a bounded cross-domain coherence-source interface, auxiliary magnetic-core and neutral-carrier actions, a source-free temporal particle action, and a nine-part conditional matter boundary. One Mapped coefficient supports a localized, nodeless, carrier-retaining finite-grid branch across four same-domain resolutions and one larger-domain comparison. Its finest artifact's six independently matched lowest constrained $C_4$ finite-grid PA42 eigenpairs contain one numerically near-zero carrier-phase symmetry mode, no negative mode, and five positive modes; the symmetry mode retains unresolved odd-even structure. Unified microscopic dynamics, localized Hessian resolution, perturbations beyond $C_4$, continuum existence, PA43 dynamics, and particle identity remain open |
| **Mechanism** | The open matrix balance connects canonical populations and $q$, optional transverse coherence, relative-phase transport, scale currents, and endpoint channels. Its conversion flow preserves the positive cone and fixes the finite-density coherence-support boundary. The enlarged interior/exterior fibre and reciprocal cross block supply a conditional source. The matter-completion boundary adds the complementary-space exterior, bilinear interface, routed single-carrier exponent, stress-flux normalization, repeated-interaction support law, conserved total stress, constant-$G$ backreaction, Gram/Cartan particle map, and constrained energetic and mixed dynamical operators. The auxiliary adjoint core regularizes local flux, the registered condensate confines it, and the neutral carrier supplies conditional inverse-length support. The particle action supplies local $SU(2)_Q$, Gauss's law, and the fixed-$Q_C$ stationary functional. Conservative, exterior, reservoir, gravity, Standard Model, and particle realizations retain distinct source status |
| **Epistemic** | **Hypothesized integrated open-system, exterior, and geometric closure / Derived finite-state $q$ bound, exact canonical reduction, positive-cone preservation, local and cross-domain covariance, coherence-decay and support boundary, bounded cross-domain positivity, reciprocal exchange, routed transfer exponents from declared port factors, single-mode power normalization, repeated-interaction support equation, closed-action Ward identity, constant-$G$ source compatibility, Gram/Cartan map, endpoint ledgers, point-flux coefficient, auxiliary-core matching, condensate confinement, registered-pair no-go, reduced carrier-support theorem, first-order Gauss obstruction, temporal gauge covariance, Gauss constraint, stationary equations, and full fluctuation qualification operators / Mapped selected density-depletion coefficient / Tested finite algebraic closures, one localized retained four-resolution finite-grid stationary branch with one larger-domain comparison, and six-mode constrained $C_4$ finite-grid PA42 low-spectrum sign on its finest artifact / microscopic matching, physical endpoint normalization, exterior carrier identity, reservoir action and stress, gravity selection, localized Hessian resolution, perturbations beyond $C_4$, continuum qualification, PA43 dynamics, and particle identity remain open** |
| **Reference** | `foundations/yin-yang-qi-dynamical-geometry.md`; `foundations/matter-completion-boundary.md`; `computations/matter_completion_boundary_report.md`; `computations/dynamical_geometry_closure_report.md`; `computations/cross_scale_coherence_interface_report.md`; `foundations/geometric-manifold-completion.md`; `foundations/endpoint-link-and-localization-boundary.md`; `foundations/point-core-flux-sector.md`; `foundations/nonabelian-magnetic-core-boundary.md`; `foundations/core-trapped-charge-support.md`; `foundations/particle-stationary-action-closure.md`; `foundations/physical-becoming-hierarchy.md` §7.1, §7.2; `foundations/unified-lagrangian.md`; `foundations/interscale-current-soliton.md`; `computations/particle-carrier-direct-coordinate-report.md`; `computations/particle-carrier-resolution-recovery-report.md`; `computations/particle-physical-hessian-precision-v2-report.md`; `computations/particle-localized-physical-hessian-report.md`; `cassi-physics.md` |
...
- **Proton decay rate and localized sector.** The mapped proton coordinate and conditional circuit arithmetic do not determine a lifetime. Coherent and one-way open endpoint equations are explicit, while their physical normalization is unselected. Point-core Chern flux supplies a conditional exterior term, an auxiliary adjoint field smooths the local core, and the registered condensate confines flux. A neutral core carrier supplies one reduced stationary separation under support, retention, and matching inequalities. The source-free second-order charged-field branch defines the temporal action, Gauss constraint, fixed-$Q_C$ stationary equations, boundary inventory, and dimensionless variational class. At the Mapped coupling $h_C=2.9598260763447164$, that action has a localized, nodeless, carrier-retaining branch across four same-domain finite grids and one larger-domain comparison. Its finest artifact's six independently matched lowest constrained $C_4$ finite-grid PA42 eigenpairs contain one numerically near-zero carrier-phase symmetry mode, no negative mode, and five positive modes; the symmetry mode retains unresolved odd-even structure. Localized Hessian resolution, perturbations beyond $C_4$, continuum qualification, PA43 dynamics, particle identification, and a physical decay channel remain open.
