...
| C4 | **Inflation** | What drove it? Why did it end? What set $n_s$, $r$? | Cascade steps $n \approx 20$–$60$ are the inflationary epoch; Qi gate slow-roll drives expansion; gate engagement at $r = \varphi^{-1}$ (step $\sim 60$) provides graceful exit. $N_e = 40$ e-folds, $n_s = 0.967$, $r \approx 0.003$, $\alpha_s = -0.0013$. No inflaton. Full derivation: `cosmology/inflation-from-cascade.md` | Qi gate $(1-q)$ modulates $H$ during ratio evolution; wake-wave mechanism imprints $\varphi$-scaled perturbations. Gate closure replaces fine-tuned inflaton potential. Zero free parameters. | **Hypothesized** — mechanism (steps 20-60, gate exit) derived; $n_s$, $r$ predictions testable with CMB-S4/LiteBIRD | `cosmology/inflation-from-cascade.md` |
...
| Q3 | **Neutrino masses** | Why so small? Dirac or Majorana? Normal or inverted? | Seesaw scale at cascade step 20: $m_\nu \approx v_0 \cdot \varphi^{-12} \sim 0.1$–$1$ eV. Three mass eigenstates from Fibonacci triple-clustering with compressed sub-rung spacing ($\Delta_\nu \approx 1.5$–$3.5$ rungs). Predicts **normal ordering**, no sterile neutrinos. Full derivation: `foundations/neutrino-masses.md` | Same Fibonacci partitioning as three-generations (Q5), applied to compressed seesaw span ($N_\nu \approx 12$ vs $N_{\text{lep}} \approx 72$). Compressed span → mild hierarchy. $\varphi$-power spacing testable with JUNO/DUNE | **Hypothesized** — overall scale derived; specific $\Delta_{\nu,k}$ spacings require full cascade RGE | `foundations/neutrino-masses.md` |
...
| Q6 | **Matter-antimatter asymmetry** | Sakharov conditions; why $\eta \sim 10^{-9}$? | $\eta \approx \varphi^{-46} \approx 6 \times 10^{-10}$ from three derived mechanisms: (1) organized annihilation ($\S5.2$ of proton doc) eliminates all paired antimatter; (2) Yang-Yin imbalance at Wu Xing freeze-out (gap $g = 1-\varphi^{-5}$) leaves residual Yang excess; (3) cascade dilution through photon-producing rungs (8→54). All three Sakharov conditions satisfied by independently derived Cassi mechanisms. Full derivation: `foundations/baryon-asymmetry.md` | Freeze-out Yang-Yin ratio at GUT; organized annihilation probability O(1); cascade expansion dilutes to present-epoch $\eta$. $\varphi$-power scaling derived; specific exponent (-46) sets cascade freeze-out at step 54 (testable). | **Hypothesized** — all three Sakharov mechanisms derived; specific $\eta$ exponent is a cascade-step constraint pending full $w(a)$ dilution model | `foundations/baryon-asymmetry.md` |
...
| **Hypothesized** (mechanism proposed, test exists) | 22 | Hubble tension (C3), inflation (C4), horizon (C6), baryon asymmetry (C7), cosmic web (C9), CMB axis (C10), neutrinos (Q3), 3 generations (Q5), matter asymmetry (Q6), measurement (Q7), spin (Q10), 3+1 dimensions (G5), unification (F3), TOE (F4), JWST galaxies (T2), $\sigma_8$ (T3), $H_0$ tension (T4), consciousness (all M1–M5) |
| **Speculative** (framework-consistent, no test yet) | 1 | BH information (G2) |

**Total: 39 open questions mapped to Cassi answers.** Of those, 16 are derived — mathematical consequences of $\varphi$ and the PDE requiring no fitting. The remaining 23 have proposed mechanisms and testable predictions, with several (C1, C2, C4, C5, Q2, Q4, Q8, Q9, G4, T1, W1) already confirmed at the level of observational match or PDE verification.
