...
Grand unified theories motivate proton-lifetime searches. Cassi has two distinct conditional descriptions. The coherence-budget extension assigns a Hypothesized independent failure probability to each scale step and yields the arithmetic cycle count $\varphi^{4506}$; its conversion to a lifetime remains conditional. The interscale-current extension places the Mapped proton coordinate at $\mathfrak s_p=91.4616$ and admits a closed two-rail current with $J_{\mathfrak s}=0$ and $J_Q\ne0$. A charge-$-g_Q$ endpoint section closes the circuit coherently when its capacity bound holds. A one-way Markov alternative closes the populations with $\gamma_-/\gamma_+=\varphi$ and damps endpoint coherence. The smooth zero-Chern endpoint sector has no finite Derrick radius. Point-core flux and the auxiliary adjoint branch supply a conditional smooth local core; the registered condensate confines it and supplies no persistent pair by itself. A neutral core-trapped carrier gives one reduced stable separation under support, retention, and matching inequalities. A separate source-free second-order temporal branch supplies time-dependent local $SU(2)_Q$, Gauss's law, and the coupled fixed-$Q_C$ stationary functional. One registered numerical point has a Q2-qualified finite-grid primary background. The selected field fails localization and carrier retention; its finite-grid physical quotient and Hessian action pass structural checks, while the augmented-gradient preflight stops before an eigenspectrum. Physical coefficient calibration, domain and resolution convergence, the transverse carrier mode, a stationary proton, particle quantum numbers, the fluctuation spectrum, and the lifetime map remain open.

| **Cassi Answer** | A Planck-to-proton relative-current circuit has explicit conditional endpoint equations, an exact conditional point-flux exterior coefficient, a conditional auxiliary smooth-core branch, a conditional neutral-carrier reduced support branch, and a conditional source-free fixed-charge particle action. One registered coefficient point has a Q2-qualified finite-grid primary background. The field fails localization and carrier retention, and every domain plus high-resolution arm fails Q2. Its $13622$-dimensional finite-grid physical quotient and Hessian action pass independent structural checks, while the augmented-gradient preflight returns `INCONCLUSIVE—HESSIAN PREFLIGHT` before eigenvalue evaluation. The full localized proton and lifetime remain open |
...
| **Epistemic** | **Mapped** (proton coordinate) / **Derived conditional** (zero-total-flow current, endpoint covariance, capacity, rate ratio, point-flux coefficient, support inequality, auxiliary-core identities, condensate topology, confinement, registered-pair no-go, carrier-support theorem, first-order Gauss obstruction, temporal gauge covariance, Gauss constraint, stationary equations, and dimensionless groups) / **Tested** (one dimensionless stationary point; canonical-preimage continuation has independent-verifier `PASS` with scientific verdict `PASS—Q2-QUALIFIED PRIMARY BACKGROUND`; finite-grid physical-quotient construction and Hessian action pass independent preflight checks; augmented-gradient gate gives `INCONCLUSIVE—HESSIAN PREFLIGHT`) / **Hypothesized** (auxiliary fields, source-free temporal completion, physical endpoint normalization and coefficient calibration, bound transverse carrier mode, localized domain-converged composite, PA42 eigenspectrum, proton identification, and decay rate). Neither candidate determines a physical lifetime |
| **Reference** | `foundations/proton-coherence-budget.md` §10; `foundations/interscale-current-soliton.md` §4.5; `foundations/endpoint-link-and-localization-boundary.md`; `foundations/point-core-flux-sector.md`; `foundations/nonabelian-magnetic-core-boundary.md`; `foundations/core-trapped-charge-support.md`; `foundations/particle-stationary-action-closure.md`; `computations/particle-stationary-bvp-pre-registration.md`; `computations/particle_stationary_bvp.py`; `computations/verify_particle_stationary_bvp.py`; `computations/particle-stationary-bvp-report.md`; `computations/particle-stationary-q2-recovery-report.md`; `computations/particle-physical-hessian-report.md`; `computations/endpoint_link_localization_check.py`; `computations/point_core_flux_check.py`; `computations/magnetic_core_completion_check.py`; `computations/core_trapped_charge_check.py`; `computations/particle_action_closure_check.py` |
...
selection remain open. The particle campaign supplies a Q2-qualified
finite-grid primary background and an explicit physical quotient. Its Hessian
action passes independent structural checks, while the augmented-gradient
preflight stops before eigenvalue evaluation. Localization and domain and
resolution convergence remain open.
