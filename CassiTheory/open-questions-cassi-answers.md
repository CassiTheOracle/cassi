...
| **Epistemic** | **Mapped** (proton coordinate) / **Derived conditional** (zero-total-flow current, endpoint covariance, capacity, rate ratio, point-flux coefficient, support inequality, auxiliary-core identities, condensate topology, confinement, registered-pair no-go, carrier-support theorem, first-order Gauss obstruction, temporal gauge covariance, Gauss constraint, stationary equations, and dimensionless groups) / **Tested** (one dimensionless stationary point; the frozen run-time source at commit `474b4596` has an independent-verifier `PASS` with scientific verdict `INCONCLUSIVE—NUMERICAL QUALITY`) / **Hypothesized** (auxiliary fields, source-free temporal completion, physical endpoint normalization and coefficient calibration, bound transverse carrier mode, full stationary composite, fluctuation spectrum, proton identification, and decay rate). Neither candidate determines a physical lifetime |
| **Reference** | `foundations/proton-coherence-budget.md` §10; `foundations/interscale-current-soliton.md` §4.5; `foundations/endpoint-link-and-localization-boundary.md`; `foundations/point-core-flux-sector.md`; `foundations/nonabelian-magnetic-core-boundary.md`; `foundations/core-trapped-charge-support.md`; `foundations/particle-stationary-action-closure.md`; `computations/particle-stationary-bvp-pre-registration.md`; `computations/particle_stationary_bvp.py`; `computations/verify_particle_stationary_bvp.py`; `computations/particle-stationary-bvp-report.md`; `computations/endpoint_link_localization_check.py`; `computations/point_core_flux_check.py`; `computations/magnetic_core_completion_check.py`; `computations/core_trapped_charge_check.py`; `computations/particle_action_closure_check.py` |
...
| **Cassi Answer** | One explicit open dynamical-geometry closure with exact canonical reduction, conditional positive-coherence and endpoint ledgers, one bounded cross-domain coherence-source interface, and one conditional interscale momentum ledger; matching to the four established interactions remains open |
| **Mechanism** | The positive Hermitian Yang/Yin fibre carries the canonical populations and optional transverse coherence. A nonlinear two-jump conversion flow factorizes into a nonnegative $q$-dependent rate and a fixed GKSL generator, reproduces the canonical diagonal PDE, and strictly damps undriven finite-density $c$ when $\lambda>0$. A positive enlarged interior/exterior state with cross block $\mathsf C_{\rm io}$ and reciprocal coupling $V$ gives $S_c^{\rm ext}=(\mathcal S_{\rm in})_{IY}$, closes enlarged number and relative-charge ledgers, and obeys capacity and phase-sensitive support conditions. Its two-sided transfer $K_{a+1}=L_aK_aR_a^\dagger$ separates closed, one-sided routed, and symmetric routed attenuation. Relative-$U(1)_Q$ currents, endpoint sources, and a separately declared Wilson link close their own conditional ledgers. Mixed-stress boundary flux transfers momentum between scale windows only after a physical carrier-to-stress map is supplied. DG1–DG7 and EC1–EC7 pass; the frozen AR1–AR6 receipt passes, the incompatible DR receipt remains `FAIL` at DR5, and IT1–IT6 pass at one normalized point. The physical exterior, carrier, reservoir, total stress, geometry backreaction, local scale-bulk completion, and cross-sector matching remain unselected |
| **Epistemic** | **Hypothesized open-system, geometric, constitutive, exterior, and Wilson-link ansätze / Derived finite-state $q$ bound, exact canonical reduction, positive-cone preservation, conditional local and cross-domain fibre geometry, coherence-decay and support boundary, independent-frame covariance, reciprocal interface exchange, two-sided attenuation from declared port factors, source-capacity bound, endpoint and relative-charge ledgers, momentum-window conservation, frozen-link and first-order source-action responses, stationary spatial-flux boundary, Wilson transport and capacity, and source-action order/sign boundary**—physical realization and cross-sector matching remain open |
| **Reference** | `foundations/yin-yang-qi-dynamical-geometry.md`; `computations/dynamical_geometry_closure_report.md`; `computations/dynamical_geometry_closure_check.py`; `computations/cross_scale_coherence_interface_prereg.md`; `computations/cross_scale_coherence_interface_check.py`; `computations/cross_scale_coherence_interface_report.md`; `foundations/geometric-manifold-completion.md`; `foundations/physical-becoming-hierarchy.md` §7.2, §7.3; `foundations/unified-lagrangian.md`; `foundations/interscale-current-soliton.md`; `foundations/endpoint-link-and-localization-boundary.md` §§3.6–3.11; `foundations/interscale-stress-attenuation-boundary.md`; `computations/endpoint_dynamical_response_report.md`; `computations/endpoint_action_response_report.md`; `computations/endpoint_spatial_flux_report.md`; `computations/endpoint_intervertex_transport_report.md` |
...
flowchart TD
    ...
    J --> G[Microscopic action and P0 to P1 to P2 matching open]
```

**Visual:** The framework uses $\varphi$, canonical two-fluid notation, and the cascade to index mixed-tier claims. The open dynamical-geometry synthesis joins the declared density, coherence, projective, scale-current, endpoint, and bounded cross-domain maps in one effective matrix balance while preserving separate conservative, dissipative, and particle branches. Unified microscopic physics, a physical exterior, total stress and geometry backreaction, a qualified stationary matter solution, and a covariant gravity completion remain open.

The current framework organizes cosmology, particle physics, gravity, and the Standard Model around shared notation. The open dynamical-geometry synthesis supplies a positive Hermitian fibre, a conditional relative-$U(1)_Q$ graph sector, a nonlinear positivity-preserving conversion flow with exact canonical diagonal reduction, and endpoint/Wilson number and charge ledgers. DG1–DG7 pass. The minimal undriven lift strictly damps nonzero transverse coherence at finite density when $\lambda>0$, so a stationary phase-bearing state requires an identified support term. A bounded enlarged interior/exterior state supplies one conditional source from cross-coherence and reciprocal exchange; its positivity, independent-frame covariance, enlarged ledgers, two-sided attenuation cases, capacity bound, and phase controls pass EC1–EC7. The physical exterior and sustaining dynamics remain open. Point-core Chern flux supplies an exact conditional exterior $1/R$ coefficient. An auxiliary adjoint $SU(2)_Q$ branch supplies a regular local core, while the registered fundamental condensate confines its flux. A neutral core carrier supplies one conditional reduced separation. The separate local-$SU(2)_Q$ second-order charged-field branch supplies a source-free Gauss-compatible fixed-$Q_C$ stationary action. One registered coefficient point returns `INCONCLUSIVE—NUMERICAL QUALITY`; microscopic identification, coefficient calibration, full stress and backreaction, and a stationary field solution remain open.

| **Cassi Answer** | Mixed-tier sector organization plus a Hypothesized open dynamical-geometry closure, a bounded cross-domain coherence-source interface, auxiliary magnetic-core and neutral-carrier actions, and a separate source-free temporal particle action with exact conditional reductions; unified microscopic dynamics remains open |
| **Mechanism** | The open matrix balance connects canonical populations and $q$, optional transverse coherence, relative-phase transport, scale currents, and endpoint channels. Its nonlinear two-jump conversion flow preserves the positive cone by time reparametrization of a fixed GKSL flow and fixes the finite-density coherence-support boundary. An enlarged positive interior/exterior fibre and reciprocal cross block supply one conditional transverse source, close enlarged number and relative charge, and expose distinct attenuation exponents after left and right port factors are declared. The auxiliary adjoint core regularizes local flux, the registered condensate confines it, and the neutral carrier supplies conditional inverse-length support. A separate second-order charged-field branch supplies local $SU(2)_Q$, Gauss's law, and the fixed-$Q_C$ stationary functional. Conservative, exterior, reservoir, gravity, Standard Model, and particle-branch dynamics retain distinct source status |
| **Epistemic** | **Hypothesized integrated open-system, exterior, and geometric closure / Derived finite-state $q$ bound, exact canonical reduction, positive-cone preservation, local and cross-domain covariance, coherence-decay and support boundary, bounded cross-domain positivity, reciprocal exchange, two-sided attenuation from declared port factors, source capacity, endpoint ledgers, point-flux coefficient, auxiliary-core matching, condensate confinement, registered-pair no-go, reduced carrier-support theorem, first-order Gauss obstruction, temporal gauge covariance, Gauss constraint, stationary equations, and dimensionless groups / Tested DG1–DG7 and EC1–EC7 closures and one-point stationary campaign**—DG1–DG7 and EC1–EC7 pass; the particle point returns `INCONCLUSIVE—NUMERICAL QUALITY`; microscopic matching, physical endpoint normalization and coefficient calibration, physical exterior, reservoir, total stress, geometry backreaction, stationary solution, fluctuation spectrum, and covariant gravity remain open |
| **Reference** | `foundations/yin-yang-qi-dynamical-geometry.md`; `computations/dynamical_geometry_closure_report.md`; `computations/dynamical_geometry_closure_check.py`; `computations/cross_scale_coherence_interface_prereg.md`; `computations/cross_scale_coherence_interface_check.py`; `computations/cross_scale_coherence_interface_report.md`; `foundations/geometric-manifold-completion.md`; `foundations/endpoint-link-and-localization-boundary.md`; `foundations/point-core-flux-sector.md`; `foundations/nonabelian-magnetic-core-boundary.md`; `foundations/core-trapped-charge-support.md`; `foundations/particle-stationary-action-closure.md`; `foundations/physical-becoming-hierarchy.md` §7.1, §7.2; `foundations/unified-lagrangian.md`; `foundations/interscale-current-soliton.md`; `computations/particle-stationary-bvp-report.md`; `computations/particle_action_closure_check.py`; `cassi-physics.md` |
