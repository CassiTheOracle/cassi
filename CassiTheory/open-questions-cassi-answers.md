...
The particle calculation supplies two complementary finite-grid results. A
Mapped density-depletion coefficient supports a localized, nodeless,
carrier-retaining stationary branch across four same-domain resolutions and
one larger-domain comparison. A different diffuse coefficient point has an
explicit fixed-charge physical quotient and a nonnegative matched low
energetic spectrum; its near-zero global carrier-phase direction remains
concentrated at grid scale. The localized branch requires its own constrained
Hessian and mixed dynamical spectrum, while continuum existence and physical
cross-sector matching remain open.
...
- **Proton decay rate and localized sector.** The mapped proton coordinate and conditional circuit arithmetic do not determine a lifetime. Coherent and one-way open endpoint equations are explicit, while their physical normalization is unselected. Point-core Chern flux supplies a conditional exterior term, an auxiliary adjoint field smooths the local core, and the registered condensate confines flux. A neutral core carrier supplies one reduced stationary separation under support, retention, and matching inequalities. The source-free second-order charged-field branch defines the temporal action, Gauss constraint, fixed-$Q_C$ stationary equations, boundary inventory, and dimensionless variational class. At the Mapped coupling $h_C=2.9598260763447164$, that action supports a localized, nodeless, carrier-retaining stationary field across four same-domain finite grids and one larger-domain comparison; adjacent observables agree and the absolute energy drift decreases twice. A different diffuse coefficient point has one nonnegative matched low energetic spectrum, but its global phase direction remains concentrated at grid scale and its Hessian does not establish stability of the localized field. Continuum existence, the localized branch's constrained energetic and mixed dynamical spectra, physical coefficient calibration, particle quantum numbers, real-time formation, and decay data remain open. See `computations/particle-carrier-resolution-recovery-report.md` and `computations/particle-physical-hessian-precision-v2-report.md`.
