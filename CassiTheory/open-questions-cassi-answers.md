...
Under homogeneous conversion-only dynamics with advection, diffusion, and other closures held out, the ratio $r = E_Y / E_I$ approaches $\varphi \approx 1.618$. This is the conditional $\varphi$-attractor. Cosmic acceleration and spatial flatness are separate model mappings applied to the attractor; each requires its own dynamical and observational closure. The dynamical half is now measured in the canonical solver. In the ungated mode at $N=32$, $\lambda=0.1$, $\Delta t=0.002$ and $T=30$, four declared compositions off the ray end with $E_Y/E_I$ within $5.7\times10^{-4}$ of $\varphi$, and all four volume-mean imbalance-decay rates equal $0.2618034$ against $(1+\varphi)\lambda=0.2618034$ with conservation residuals at or below $1.1\times10^{-15}$—an equality that witnesses the declared block's own trace and cannot be read as evidence for the ratio, since for any rank-one conversion block whose entries carry the ratio the spectrum, the characteristic polynomial and the relaxation rate are functions of $a+b$ alone and the ratio lives in the null vector—while the composition reading above is the half that carries the ratio. The $q$-gated mode displaces the volume mean: the gated ray control ends $2.5\times10^{-3}$ below the ray at $T=30$ and the gated run that starts off-ray and reaches the ray inside its horizon ends $3.1\times10^{-4}$ below it at $T=240$, their volume-mean rates following $(1+\varphi)\lambda\Xi$ with the measured openness weighting $\Xi=\langle w\varepsilon\rangle/\langle\varepsilon\rangle$. The displacement is transient, not frozen: it peaks at $t\approx26.8$ s at $-2.5\times10^{-3}$ and is carried by the cells the gate weights down, the lowest-openness quarter of the field retaining $0.848$ of its deviation from $t=3.75$ to $30$ where its own openness permits $0.608$, and the long-horizon run is inside the $10^{-3}$ tolerance from $t=201$ s onward—so the declared horizon of $30$ s samples a ray-initialized run at its plateau, and the crossing near $t\approx58$ s obtained by extrapolating the stored rates is arithmetic rather than a measurement (`computations/two-fluid-phi-ray-relaxation-prereg.md`; `runs/two_fluid_phi_ray_relaxation/verification.json`, verdict `CONTRADICTS` on the gated-ray clause alone).
...
- **Interacting Yang–Mills fibre and continuum mass gap.** Finite regulated
  ...
  uses every ordered product of the eleven fundamental plaquette characters
  through degree four. Its source-bound receipt records `FAIL` with $32/32$
  checks and final ranks $861$, $863$, $863$, and $863$, leaving $Q$
  deficiencies $6$, $4$, $4$, and $4$ at
