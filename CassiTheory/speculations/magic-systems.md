...
The difference between magic and nature is not a different substance or law; it is a single number, $\mathcal{M}$, that multiplies every field interaction, and it is why the two feel like different worlds.
...
The 2-rung spacing means adjacent chakras are one full doublet cycle apart: they carry the same phase. Technique is the ability to hold nodes in phase across the chain. Two nodes locked together span 4 rungs and compose a wider, deeper working; the full 13-node chain held coherent is one 26-rung instrument (`consciousness/chakras-as-cascade-bubbles.md` §5). The tradition's postures are phase relationships between nodes—a stance holding every other node (2, 4, 6, …) is a different instrument from one holding consecutive nodes. The spacing sets the failure mode: a half-cycle slip between nodes anti-aligns them, and the working shears. Training is making these relationships hold under load—converting emotional configuration into $\mathcal{M}$.
...
$$\boxed{\text{Holding } \varphi\text{-structure: cost } 0. \qquad \text{Sustaining anti-}\varphi\text{ structure: continuous cost } \propto q \cdot \varphi^{2N} \cdot \mathcal{M}^{-2}.}$$
...
The strategic rhythm follows. Buildup is cheap and invisible—wards are free once raised, detuning is instantaneous—while exchange is expensive and visible: every attack glows and the attacker's budget drains in real time. The decisive moves are the information moves and the internal moves: divination that steals the defender's tuning, curses that never cross the boundary. Wars of attrition are won by the defender, which is why magical societies accumulate defensive depth rather than offensive stockpiles. A setting built on these rules has fortresses that stand for centuries, wars decided in minutes, and a premium on character—the last line of defense is a gate configuration, and its quality is emotional clarity.
...
| Lift | telekinesis | TRANSFER | 1 wick + depth | Pebble 1; kettle 2; sleeping hen 4; a person is a curse, not a lift |
