...
The Wu Xing bubble at cascade step $n = 285$ (~191 Mpc) is the coherence length of the Wu Xing number $w = 5$—the scale over which the cosmological initial conditions remain constant, set by the comoving horizon when the Qi gate first engaged (`foundations/dimensionful-cascade.md` §6). Adjacent bubbles sit at $\varphi$-spaced intervals in the chord lattice, separated by voids at the $C = -1$ sites of the condensation field; the observable universe is 98% inside the bubble, which spans the last seven rungs ($285 \to 292$) of the ladder. The boundary is a level set of the condensation field—a $\varphi$-detuned interface between the bubble's interior structure and the exterior lattice.
...
- `foundations/dimensionful-cascade.md`—292-step ladder, Wu Xing bubble at $n \approx 285$, megacascade extension
...
- `two-fluid/run_trauma_crossover.py` and `run_trauma_crossover_low.py`—φ-phased drain crossover, onset specificity (2026-07-31)
