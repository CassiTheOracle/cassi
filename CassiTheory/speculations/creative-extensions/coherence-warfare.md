...
A condensed pattern is armor-plated against chaos. The proton's coherence budget is $\varphi^{4506} \approx 10^{942}$ cycles; even the most violent imaginable environment cannot dent it, because the budget is *structural*: each of the 92 supporting rungs must fail in the same cycle for the pattern to dissolve. Suppressing the top fifty rungs entirely—reducing their coherence to zero, which no environment has ever done—still leaves $\varphi^{880} \approx 10^{184}$ cycles of stability (`foundations/proton-coherence-budget.md` §5.1). There is a **coherence floor** under every condensed pattern, and it is quadratic in depth:
