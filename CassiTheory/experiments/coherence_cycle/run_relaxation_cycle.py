#!/usr/bin/env python3
r"""Coherence Commons illustrative relaxation-oscillation model.

Implements the phenomenological toy mechanism in
`speculations/creative-extensions/coherence-commons.md` §8.2. It tests
whether one explicit fast-slow system can produce asymmetric booms, crashes,
recoveries, and eventual regime lock under an accumulation ratchet. It is
not calibrated to an economy and its state variables are not Cassi field
observables.

    eps * ds/dt = s - s^3/3 - D + I
    dD/dt       = gamma * (D_inf(H_peak) - D) + mu * s
    c(D)        = clip(1 - D/D_max, 0, 1)
    u(s,D)      = lam*c(D) - delta if s > 0 else -delta
    dH/dt       = u * H

Here D is a dimensionless depletion index, c is a normalized recoverable-
capacity proxy, u is an illustrative accumulation-return index, and H is an
accumulated-stock index. Neither c nor u is a measured Qi coherence or
Marxian profit rate.

The fast equation is FitzHugh-Nagumo-type: the linear term supplies positive
feedback, the cubic bounds it, depletion brakes expansion, and I supplies an
offset. The slow equation pits expansion-driven depletion against recovery.
The ratchet raises the long-run depletion target with the peak accumulated
stock, representing a persistent reproduction burden.

Every numerical claim made for this toy system is computed here. The folds
follow from the cubic, the lock threshold follows from the nullcline crossing
at the lower fold, and the verification block checks the simulation against
those model-internal values.
...
# ── Model parameters (dimensionless; illustrative constants, see §8.2) ───────
EPS   = 0.02    # market timescale vs recovery timescale (1/50)
I     = 0.7     # expansion offset
GAMMA = 1.0     # recovery rate; the slow unit is one recovery time
MU    = 2.0     # expansion-to-depletion coupling
LAM   = 1.0     # capacity-to-return-index coefficient
DELTA = 0.05    # accumulated-stock carrying-cost index
D_0   = 0.1     # baseline structural depletion
H_STAR = 12.0   # accumulated-stock index at the lock threshold
...
def capacity_fraction(D):
    """Normalized recoverable-capacity proxy, clipped to [0, 1]."""
    return min(1.0, max(0.0, 1.0 - D / D_MAX))


def return_index_of(s, D):
    """Illustrative accumulation-return index for this toy system."""
    return LAM * capacity_fraction(D) - DELTA if s > 0.0 else -DELTA
...
def rk4_step(s, D, H, H_peak, dt):
    ...
    return_index = return_index_of(s, D)
    H_new = H * math.exp(return_index * dt)
    return s_new, D_new, H_new


def footprint(H_peak):
    """Accumulated stock's persistent burden on the recovery target."""
...
def integrate(s0, D0, H0, t_end, dt, ratchet):
    ...
    Ds = np.empty(n + 1)
    Hs = np.empty(n + 1)
    return_indices = np.empty(n + 1)
    s, D, H = s0, D0, H0
    H_peak = H0
    ts[0], ss[0], Ds[0], Hs[0], return_indices[0] = 0.0, s, D, H, return_index_of(s, D)
    for k in range(n):
        s, D, H = rk4_step(s, D, H, H_peak, dt)
        if ratchet and H > H_peak:
            H_peak = H
        ts[k + 1] = ts[k] + dt
        ss[k + 1], Ds[k + 1], Hs[k + 1], return_indices[k + 1] = s, D, H, return_index_of(s, D)
    return ts, ss, Ds, Hs, return_indices


def cycle_stats(ts, ss, Ds, return_indices, dt, skip=0):
    """Per-cycle statistics from a time series.

    A cycle runs from one recovery jump (s crossing -1 upward) to the next.
    Returns period and phase durations plus the average and final values of
    the illustrative accumulation-return index.
    """
    ...
    for j in range(len(idx) - 1):
        i0, i1 = idx[j], idx[j + 1]
        seg_s, seg_D, seg_return = ss[i0:i1], Ds[i0:i1], return_indices[i0:i1]
        t0, t1 = ts[i0], ts[i1]
        ...
        stats.append({
            ...
            "crash": crash_dur,
            "bust": bust_t,
            "return_avg": float(np.mean(seg_return)),
            "return_end": float(seg_return[-1]),
...
def main():
    ...
    ts, ss, Ds, Hs, return_indices = integrate(s0, D0, H0, 40.0, dt, ratchet=False)
    stats = cycle_stats(ts, ss, Ds, return_indices, dt)
    n_cycles = len(stats)
    ...
    ts2, ss2, Ds2, Hs2, return_indices2 = integrate(s0, D0, H0, 26.0, dt, ratchet=True)
    stats2 = cycle_stats(ts2, ss2, Ds2, return_indices2, dt)
    ...
    # Panel 3: toy return-index path and bust lengthening
    ax4 = fig.add_axes([0.395, 0.09, 0.56, 0.33])
    ax4.set_title("toy-model path: return index falls, depressions lengthen",
                  loc="left", fontsize=11.5, fontweight="bold", color=YANG_BRIGHT)
    ks = np.arange(1, len(stats2) + 1)
    return_avgs = np.array([st["return_avg"] for st in stats2])
    busts = np.array([st["bust"] for st in stats2])
    ax4.plot(ks, return_avgs, color=YANG_PEAK, lw=2.2, marker="o", ms=4,
             label="$\\langle u \\rangle_k$: average return index of cycle $k$")
    ax4.axhline(-DELTA, color=YANG_MID, lw=1.0, ls=(0, (4, 3)))
    ax4.text(0.6, -DELTA - 0.014, "carrying-cost floor  $u = -\\delta$",
             fontsize=8, color=YANG_MID)
    ax4.set_ylim(-0.09, 0.35)
    ax4.set_xlabel("cycle index  $k$", fontsize=9.5)
    ax4.set_ylabel("$\\langle u \\rangle_k$", fontsize=10, color=YANG_PEAK)
    ...
    ax4.annotate("the last recovery fails:\nregime parks at the fold, $u = -\\delta$",
                 xy=(ks[-1], return_avgs[-1]), xytext=(ks[-1] - 4.6, -0.055),
    ...
    print(f"  average return index of the settled cycle:  {st['return_avg']:.4f}")
    capacity_peak = capacity_fraction(D_MIN)
    return_peak = LAM * capacity_peak - DELTA
    return_crash = LAM * capacity_fraction(D_MAX) - DELTA
    print(f"  boom opens at c = {capacity_peak:.4f}, u = {return_peak:.4f};  "
          f"boom ends at u = {return_crash:.4f} (== -delta: "
          f"{'PASS' if abs(return_crash + DELTA) < 1e-9 else 'FAIL'})")
    # capacity squeeze: the return index crosses zero before the crash
    D_zero = D_MAX * (1.0 - DELTA / LAM)
    print(f"  capacity squeeze: u crosses zero at D = {D_zero:.4f}, "
    ...
    print(f"  per-cycle average return index:  " +
          "  ".join(f"k={k}:{st['return_avg']:.4f}" for k, st in
                    list(enumerate(stats2, 1))[::2]))
    return_first, return_last = stats2[0]["return_avg"], stats2[-1]["return_avg"]
    print(f"  toy-model tendency: <u> falls {return_first:.4f} -> {return_last:.4f} across cycles "
          f"({'PASS' if return_last < return_first else 'FAIL'})")
    ...
    H_max = float(Hs2.max())
    print(f"  accumulated stock peaks at H = {H_max:.2f} (threshold H_star = {H_STAR:.0f});  "
          f"after regime lock it decays at -delta: H_end = {Hs2[-1]:.2f}")
    # terminal lock: no boom, return index at -delta, accumulated stock decaying
    tail = ss2[-int(3.0 / dt):]
    no_boom = bool((tail < 1.0).all())
    h_decay = Hs2[-1] < H_max
    print(f"  terminal state: no boom in the last 3 slow units: {'PASS' if no_boom else 'FAIL'}, "
          f"u = {return_indices2[-1]:.4f} == -delta: {'PASS' if abs(return_indices2[-1] + DELTA) < 1e-9 else 'FAIL'}, "
          f"stock decaying: {'PASS' if h_decay else 'FAIL'}  (s = {ss2[-1]:.3f}, parked on the fold)")
    ...
    # u(H) within the first boom: use the first full detected cycle and walk
    # the depletion phase from the last pre-crash D-minimum to the crash trigger
    st = stats2[0]
    t0, t1 = st["t_start"], st["t_start"] + st["boom"] + 0.01
    seg = (ts2 >= t0) & (ts2 <= t1)
    Dseg, sseg, Hseg, return_seg = Ds2[seg], ss2[seg], Hs2[seg], return_indices2[seg]
    ...
    D_b, return_boom, H_b = Dseg[drain], return_seg[drain], Hseg[drain]
    return_falls = bool((np.diff(return_boom) <= 1e-9).all())
    D_rises = bool((np.diff(D_b) >= -1e-9).all())
    positive_return = return_boom > 0.0
    rho = float(np.corrcoef(H_b[positive_return], return_boom[positive_return])[0, 1])
    h_rises = bool((np.diff(H_b[positive_return]) > 0.0).all())
    h_gain = float(H_b[-1] / H_b[0])
    squeeze = float((return_boom < 0.0).mean())
    print(f"\nwithin the first boom (depletion phase):  D strictly rising: {'PASS' if D_rises else 'FAIL'}, "
          f"u strictly falling: {'PASS' if return_falls else 'FAIL'}")
    print(f"  accumulated stock grows while u falls: {'PASS' if h_rises else 'FAIL'} "
          f"(u vs H correlation {rho:+.4f}; H gains {h_gain:.2f}x over the boom)—"
          f"a model-internal result, not an empirical profit-rate estimate")
    print(f"  the squeeze: u < 0 for the last {squeeze * 100:.1f}% of the boom before the crash "
          f"while accumulated stock is still expanding")
