...
`computations/particle_physical_hessian_precision_v2_manifest.json`. Preflight
...
### 5.3 JSON receipt boundary

The primary converts result-tree scalar leaves to built-in Python values before
strict JSON serialization. NumPy scalar values use `.item()`; zero-dimensional
NumPy arrays and PyTorch tensors use `.item()` after tensor detachment and CPU
transfer. Dictionaries, lists, and tuples are traversed recursively. A
non-scalar array or tensor at the JSON boundary is an implementation failure.
The final encoder retains `allow_nan=False`. The operator calculation, mode
archive, eigenvalues, and diagnostics are complete before this representation
conversion.
...
`computations/verify_particle_physical_hessian_precision_v2.py` must not import the primary
...
- `computations/particle_physical_hessian_precision_v2.py`

Independent verifier:

- `computations/verify_particle_physical_hessian_precision_v2.py`

Frozen source manifest:

- `computations/particle_physical_hessian_precision_v2_manifest.json`

Frozen run directory:

- `runs/20260903_particle_physical_hessian_precision_v2/`
