...
SCHEMA = "cassi.matter-formation.cp-odd-pair-qball.v2.tail-diagnostic"
RADIUS = 32.0
DT = 0.002
T_FINAL = 48.0
SAVE_INTERVAL = 4.0
...
def main() -> int:
    ...
    # From d(ux*py-uy*px)/dt, the interaction contribution is
    # dQ/dt = 4*pi*integral[kappa*(v/r)*(ux**2-uy**2) dr].
    positive_arrays = independent_arrays["positive"]
    late_array = positive_arrays["times"] >= 24.0
    radius = positive_arrays["r"]
    spacing = float(radius[1] - radius[0])
    charge_source_rates = 4.0 * math.pi * np.sum(
        KAPPA
        * positive_arrays["v"][late_array]
        / radius[None, :]
        * (positive_arrays["ux"][late_array] ** 2 - positive_arrays["uy"][late_array] ** 2),
        axis=1,
    ) * spacing
    bulk_source_rate_max = float(np.max(np.abs(charge_source_rates)))
    bulk_source_integral = float(
        np.trapezoid(charge_source_rates, positive_arrays["times"][late_array])
    )
    ...
    payload = {
        ...
        "bulk_charge_source": {
            "max_abs_rate": bulk_source_rate_max,
            "integrated_rate": bulk_source_integral,
            "post_pulse_source_is_not_zero": bulk_source_rate_max > 1.0e-12,
            "charge_drift_is_not_explainable_by_outer_flux_alone": bulk_source_rate_max > 1.0e-12 and drift >= 0.05,
        },
        "controls": {
