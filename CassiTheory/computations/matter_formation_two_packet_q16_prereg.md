# Two-Packet Threshold Probe at Total Charge 16

## Status: Preregistered conditional calculation—September 2026

## Abstract

This calculation asks whether two initially separated, inward-moving charged carrier packets can produce a persistent localized remnant when their total signed charge is fixed at $Q=16$. The charge is selected just above the searched stationary onset bracket in the supplied real-mediator, complex-carrier action. The calculation compares a two-packet collision with an equal-charge single preparation and an uncoupled control. It measures only the declared axisymmetric scalar model. General three-dimensional stability, gravitational capture, physical size, quantum particle identity, spin, statistics, and a whole-bubble production mechanism remain outside the calculation.

## 1. Scope and lineage

The execution class is `two_packet_threshold_probe`. The scientific question is whether the smallest two-packet preparation tested at the stationary onset scale forms a remnant under the fixed wave-capture criteria. This is a new preparation question with its own source archive and stopping rule. It does not modify the frozen spatial-convergence campaign.

The scientific output path is `runs/20260911_matter_formation_two_packet_q16`. The primary runner creates it exclusively, snapshots this protocol, the declarative preparation specification, both runner/verifier sources, and every declared action source. The verifier writes `verification.json` and a sibling independent-state archive. A failed or partial execution remains preserved.

The result cannot establish the global minimum charge, a physical length map, gravitational capture, or a minimum packet count. A positive two-packet result would establish only a conditional remnant for this preparation at this total charge.
...
Every preparation has $f=1$ and $\dot f=0$. The single arm is a Gaussian of width $w=4$ centred at the origin with carrier frequency $\Omega_\infty$. The pair arms are equal Gaussian envelopes of width $w=4$ centred at $\zeta=\pm12$, with inward wave number $k=1$ and

$$
\omega=\sqrt{\Omega_\infty^2+8k^2}.
$$

The antiphase arm multiplies the left packet by $-1$. The uncoupled arm uses the pair preparation with $h_C=0$ throughout. Every field is normalized once from the full single or superposed field to total signed charge $Q=16$. The two separated packets therefore carry equal nominal charge share $Q/2=8$ before interaction; the normalization is applied to the complete superposition, including its measured overlap.

The pair is preparation-eligible only when its normalized envelope overlap is at most $0.01$, its origin-centred initial core charge fraction is at most $0.10$, and its initial energy satisfies $E\ge\Omega_\infty|Q|$. The single arm is a same-total-charge preparation control and is not treated as a collision.
...
```json
{
  "schema": "matter-formation-two-packet-q16-protocol-v1",
  "execution_class": "two_packet_threshold_probe",
  "output": "runs/20260911_matter_formation_two_packet_q16",
  "preparation": {
    "total_charge": 16.0,
  ...
  "arms": {
    "all": ["single16", "pair16", "antiphase16", "uncoupled16"],
    "coupled_candidates": ["pair16", "antiphase16"],
    "comparison": ["single16", "pair16", "antiphase16", "uncoupled16"],
    "independent": ["single16", "pair16", "antiphase16", "uncoupled16"]
  },
  "geometry": {
    "core_radius": 8.0,
...
A two-packet formation candidate must be a coupled pair, pass preparation eligibility, pass numerical qualification, pass every persistence condition, and pass both spatial ($G0\to G1$) and time-step ($G0\to T1$) comparisons. The single and uncoupled arms are controls; neither can establish packet formation.
...
2. With all numerical and evidence checks passing, at least one fully compared coupled candidate plus a numerically qualified uncoupled control that fails formation gives **EMERGES—conditional two-packet bound remnant at total charge 16**.
3. With all numerical and evidence checks passing and no fully compared coupled candidate, the result gives **DOES NOT EMERGE in the specified Q16 two-packet calculation**.

The execution ends at $t=48$. It does not extend time, change phase, select another packet width, change the charge, alter a grid, relax a tolerance, or omit a failed arm. A negative or inconclusive result completes this preregistered calculation.
...
- `computations/matter_formation_minimum_droplet_prereg.md`—supplied scalar action and stationary onset calculation.
- `computations/matter_formation_wave_capture.py`—primary finite-volume observables and Yoshida-composed evolution.
...
- `computations/matter_formation_neutral_packets.py`—axisymmetric finite-volume geometry and operator used by the primary runner.
