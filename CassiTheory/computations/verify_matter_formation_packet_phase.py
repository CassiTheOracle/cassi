#!/usr/bin/env python3
"""Independently verify the fixed-charge relative-phase basin probe."""
...
PRIMARY_SOURCE = COMPUTATIONS / "matter_formation_packet_phase.py"
...
SCHEMA = "matter-formation-packet-relative-phase-verification-v1"
PRIMARY_SCHEMA = "matter-formation-packet-relative-phase-primary-v1"
...
def assemble_independent(grid: independent_dynamics.IndependentGrid, arm: str) -> tuple[torch.Tensor, torch.Tensor, float, float]:
    ...
    if config["kind"] == "single":
        ...
        complex_field = right * torch.exp(1j * phase_right) + left_multiplier * left * torch.exp(1j * phase_left)
...
def preparation_contract(receipt: dict[str, Any]) -> dict[str, Any]:
    ...
    checks = {"row_key_set": observed == expected, "row_count": len(rows) == len(expected), "total_charge": True, "nominal_packet_charge": True, "arm_metadata": True, "center_metadata": True, "width_metadata": True, "wave_metadata": True, "phase_metadata": True, "relative_phase_metadata": True, "orientation_metadata": True, "rule_control_metadata": True}
    for row in rows:
...
def run_smoke() -> int:
    ...
    for arm in ARMS:
        ...
        records.append({"arm": arm, "charge": observed, "center": ARM_SPECS[arm]["center"], "wave_number": ARM_SPECS[arm]["wave_number"], "phase_sign": ARM_SPECS[arm]["phase_sign"], "relative_phase": ARM_SPECS[arm]["relative_phase"], "orientation": ARM_SPECS[arm]["orientation"], "coupling": coupling})
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--input", type=Path, default=ROOT / "runs" / "20260912_matter_formation_packet_phase")
