#!/usr/bin/env python3
"""Run the preregistered fixed-charge momentum-orientation basin probe."""
...
from matter_formation_momentum_orientation_spec import (  # noqa: E402
    ARM_SPECS,
    ARMS,
    COUPLED_CANDIDATES,
    CENTER,
...
PREREG = COMPUTATIONS / "matter_formation_momentum_orientation_prereg.md"
SPEC_SOURCE = COMPUTATIONS / "matter_formation_momentum_orientation_spec.py"
...
VERIFIER_SOURCE = COMPUTATIONS / "verify_matter_formation_momentum_orientation.py"
SCHEMA = "matter-formation-packet-momentum-orientation-primary-v1"
...
def run_smoke() -> int:
    ...
    for arm in ARMS:
        ...
        records.append({"arm": arm, "charge": charge, "center": metadata["center"], "wave_number": metadata["wave_number"], "phase_sign": metadata["phase_sign"], "orientation": metadata["orientation"], "overlap": metadata["initial_overlap"]})
...
def run_campaign(output: Path) -> dict[str, Any]:
    ...
    uncoupled = indexed[("G0", "uncoupled_inward")]
    ...
    if not comparison_pass:
        ...
        verdict = "EMERGES—conditional bound remnant in the declared momentum-orientation basin"
    else:
        verdict = "DOES NOT EMERGE in the specified momentum-orientation calculation"
    receipt = {
        "schema": SCHEMA,
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--output", type=Path, default=ROOT / "runs" / "20260911_matter_formation_momentum_orientation")
