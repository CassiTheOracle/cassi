"""Run the preregistered pair-frequency-resonant incoming-shell formation calculation.

Run from the CassiTheory root:
    python computations/matter_formation_closed_cp_incoming_reservoir_v11.py \
        --output-dir runs/20260913_matter_formation_closed_cp_incoming_reservoir_v11
"""
...
DEFAULT_OUTPUT = ROOT / "runs/20260913_matter_formation_closed_cp_incoming_reservoir_v11"
SCHEMA = "cassi.matter-formation.closed-cp-incoming-reservoir.v11"
...
def initial_state(
    ...
    carrier = reservoir_amplitude * envelope * np.cos(phase) * r
...
def instantaneous_metrics(
    ...
    reservoir_gradient_r = gradient(wr, spacing)
    reservoir_gradient_i = gradient(wi, spacing)
    ...
    reservoir_energy_total = float(4.0 * math.pi * np.sum(reservoir_energy_density) * spacing)
    reservoir_energy_centroid = float(
        np.sum(r * reservoir_energy_density) / max(np.sum(reservoir_energy_density), np.finfo(float).tiny)
    )
    return {
        ...
        "reservoir_energy": reservoir_energy_total,
...
def archive_run(
    ...
    np.savez_compressed(
        ...
        r=r,
        times=checkpoints,
        reservoir_energy=np.asarray([row["reservoir_energy"] for row in checkpoint_rows]),
        reservoir_energy_centroid=np.asarray(
            [row["reservoir_energy_centroid"] for row in checkpoint_rows]
        ),
...
def main() -> int:
    ...
    reservoir_centroid_initial = positive_rows[0]["reservoir_energy_centroid"]
    reservoir_centroid_min = min(
        row["reservoir_energy_centroid"] for row in positive_rows if row["time"] <= OVERLAP_END
    )
