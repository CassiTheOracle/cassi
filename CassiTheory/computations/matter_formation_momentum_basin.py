#!/usr/bin/env python3
"""Run the preregistered fixed-charge incoming-momentum basin probe."""
...
VERIFIER_SOURCE = COMPUTATIONS / "verify_matter_formation_momentum_basin.py"
SCHEMA = "matter-formation-packet-momentum-primary-v1"
...
def run_smoke() -> int:
    ...
    for arm in ARMS:
        ...
        records.append({"arm": arm, "charge": charge, "center": metadata["center"], "wave_number": metadata["wave_number"], "overlap": metadata["initial_overlap"]})
...
def run_campaign(output: Path) -> dict[str, Any]:
    ...
    uncoupled = indexed[("G0", "uncoupled_k10")]
    ...
    if not comparison_pass:
        ...
        verdict = "EMERGES—conditional bound remnant in the declared incoming-momentum basin"
    else:
        verdict = "DOES NOT EMERGE in the specified packet-momentum calculation"
    receipt = {
        "schema": SCHEMA,
        ...
        "preparation": {
            ...
            "width": WIDTH,
            "center": CENTER,
            "wave_numbers": list(WAVE_NUMBERS),
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--output", type=Path, default=ROOT / "runs" / "20260911_matter_formation_momentum_basin")
