#!/usr/bin/env python3
"""Audit the reserved-plaquette Feshbach receipt without assembling matrices."""
...
PRIMARY_SOURCE = ROOT / "computations/verify_yang_mills_volume_adapted_feshbach_v2.py"
...
PRIMARY = ROOT / "runs/yang_mills_volume_adapted_feshbach_v2/verification.json"
DEFAULT_OUTPUT = ROOT / "runs/yang_mills_volume_adapted_feshbach_v2/verification-independent.json"
...
def audit() -> dict[str, Any]:
    ...
    result: dict[str, Any] = {
        "schema": "cassi.yang-mills.volume-adapted-feshbach-v2.independent.v1",
        ...
        "sources": {
            ...
            "v1_source": {"path": V1_SOURCE.relative_to(ROOT).as_posix(), "sha256": digest(V1_SOURCE)},
            "v1_protocol": {"path": V1_PROTOCOL.relative_to(ROOT).as_posix(), "sha256": digest(V1_PROTOCOL)},
            "v1_primary_receipt": {"path": V1_PRIMARY_RECEIPT.relative_to(ROOT).as_posix(), "sha256": digest(V1_PRIMARY_RECEIPT)},
            "v1_independent_receipt": {"path": V1_INDEPENDENT_RECEIPT.relative_to(ROOT).as_posix(), "sha256": digest(V1_INDEPENDENT_RECEIPT)},
    ...
    record_check(result, "primary status and complete checks",
                 ...
                 and primary.get("classification") == "SUPPORTS_FINITE_VOLUME_RESERVED_PLAQUETTE_FAMILY"
    ...
    expected_bindings = {
        ...
        "v1_primary_receipt": V1_PRIMARY_RECEIPT,
        "v1_independent_receipt": V1_INDEPENDENT_RECEIPT,
    ...
    record_check(result, "fixed reserved-plaquette schedule",
    ...
    for graph, columns, reserved_index in (("small", 2, 1), ("large", 11, 10)):
        local = basis.get(graph, {}).get("local", {})
        singular = [float(value) for value in local.get("singular_values", [])]
        record_check(result, f"{graph} reserved source rank and singular values",
                     local.get("column_count") == columns
                     and local.get("reserved_plaquette_index") == reserved_index
    ...
    record_check(result, "scientific classification arithmetic",
                 primary.get("classification") == "SUPPORTS_FINITE_VOLUME_RESERVED_PLAQUETTE_FAMILY"
...
def main(argv: list[str] | None = None) -> int:
    ...
    required = (PROTOCOL, SOURCE, PRIMARY_SOURCE, V1_SOURCE, V1_PROTOCOL, V1_PRIMARY_RECEIPT,
                V1_INDEPENDENT_RECEIPT, VOLUME_BRIDGE_SOURCE, EXACT_SOURCE, LARGE_SOURCE,
                SCIENTIFIC_PROTOCOL, RECOVERY_PROTOCOL, LARGE_RECEIPT, PRIMARY)
    if not all(path.exists() for path in required):
        raise FileNotFoundError("protocol, sources, v1 receipts, volume protocols, primary or receipt is missing")
    result = audit()
