...
PRIMARY = ROOT / "computations/matter_formation_closed_cp_reservoir_v5.py"
PROTOCOL = ROOT / "computations/matter-formation-closed-cp-reservoir-v5-prereg.md"
SCHEMA = "cassi.matter-formation.closed-cp-reservoir.v5.verification"
RADIUS = 48.0
N_PRIMARY = 384
N_FINE = 768
DT_PRIMARY = 0.003
...
def make_initial(
    ...
    pv = -sign * amplitude * np.exp(-0.5 * (r / RESERVOIR_WIDTH) ** 2) * (
        1.0 - (r / RESERVOIR_WIDTH) ** 2
    )
    return ux, uy, v, px, py, pv
...
def main() -> int:
    ...
    if primary.get("schema") != "cassi.matter-formation.closed-cp-reservoir.v5":
