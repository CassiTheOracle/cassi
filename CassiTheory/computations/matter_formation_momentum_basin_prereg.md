# Incoming-Momentum Basin Probe at Fixed Total Charge 16

## Status: Preregistered conditional calculation—September 2026

## Abstract

This calculation tests whether the inward wave number of two charged packets changes their ability to leave a persistent localized remnant in the supplied real-mediator, complex-carrier action. Total charge, packet separation, packet width, phase convention, coupling, domain, grid ladder, final time, and acceptance predicates are fixed. The momentum ladder is $k\in\{0.5,1.0,1.5\}$ at packet centres $\zeta=\pm12$. The calculation measures only the declared axisymmetric scalar model. General three-dimensional stability, gravitational capture, physical size, quantum particle identity, spin, statistics, and a whole-bubble production mechanism remain outside the calculation.

## 1. Scope and lineage

The execution class is `packet_momentum_basin_probe`. It is an incoming-preparation intervention at fixed total signed charge $Q=16$ and fixed centre separation $c=12$; it is a fresh calculation distinct from the completed Q16 and packet-separation probes. It does not modify either receipt or retune a completed campaign.

The scientific output path is `runs/20260911_matter_formation_momentum_basin`. The primary runner creates it exclusively, snapshots this protocol, the declarative preparation specification, both runner/verifier sources, and every declared action source. The verifier writes `verification.json` and a sibling independent-state archive. A failed or partial execution remains preserved.

A positive result would establish only a conditional remnant for one declared momentum and fixed preparation. It would not establish a global charge minimum, an optimal momentum, a minimum packet count, physical size, or gravitational capture.
...
Every preparation has $f=1$ and $\dot f=0$. The `single16` control is a Gaussian of width $w=4$ centred at the origin with carrier frequency $\Omega_\infty$. Each pair is the sum of equal Gaussian envelopes of width $w=4$ centred at $\zeta=\pm12$, with inward wave number $k$ and

$$
\omega(k)=\sqrt{\Omega_\infty^2+8k^2}.
$$

The packet phases point inward. The `antiphase_k10` arm multiplies the left packet by $-1$. The `uncoupled_k10` arm uses the $k=1$ pair with $h_C=0$ throughout. Every preparation is normalized once from the full single or superposed field to total signed charge $Q=16$. The nominal charge share of a separated pair is $Q/2=8$ before interaction.

The group-speed estimate for the declared carrier is $v_g(k)=8k/\omega(k)$, so all three pair arms reach the origin before $t=32$: their centre-crossing estimates $12/v_g(k)$ range from approximately $9.69$ to $26.49$. This is a timing check, not an acceptance predicate.
...
```json
{
  "schema": "matter-formation-packet-momentum-protocol-v1",
  "execution_class": "packet_momentum_basin_probe",
  "output": "runs/20260911_matter_formation_momentum_basin",
  "preparation": {
    "total_charge": 16.0,
  ...
  "arms": {
    "all": ["single16", "pair_k05", "pair_k10", "pair_k15", "antiphase_k10", "uncoupled_k10"],
    "coupled_candidates": ["pair_k05", "pair_k10", "pair_k15", "antiphase_k10"],
    "comparison": ["single16", "pair_k05", "pair_k10", "pair_k15", "antiphase_k10", "uncoupled_k10"],
    "independent": ["single16", "pair_k05", "pair_k10", "pair_k15", "antiphase_k10", "uncoupled_k10"]
  },
  "geometry": {
    "core_radius": 8.0,
...
A momentum-arm formation candidate must be a coupled pair, pass preparation eligibility, pass numerical qualification, pass every persistence condition, and pass both spatial ($G0\to G1$) and time-step ($G0\to T1$) comparisons. The single and uncoupled arms are controls.
...
2. With all numerical and evidence checks passing, at least one fully compared coupled momentum arm plus a numerically qualified uncoupled control that fails formation gives **EMERGES—conditional bound remnant in the declared incoming-momentum basin**.
3. With all numerical and evidence checks passing and no fully compared coupled momentum arm, the result gives **DOES NOT EMERGE in the specified packet-momentum calculation**.

The execution ends at $t=48$. It does not extend time, change separation, select another momentum, change packet width or charge, alter a grid, relax a tolerance, or omit a failed arm. A negative or inconclusive result completes this preregistered calculation.
...
- `computations/matter_formation_packet_separation.py`—source-bound fixed-charge separation primary runner.
