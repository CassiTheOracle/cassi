#!/usr/bin/env python3
"""Run the preregistered total-charge-16 two-packet threshold probe."""
...
from matter_formation_two_packet_q16_spec import (  # noqa: E402
    ARM_SPECS,
    ARMS,
    CENTER,
    CORE_RADIUS,
...
PREREG = COMPUTATIONS / "matter_formation_two_packet_q16_prereg.md"
SPEC_SOURCE = COMPUTATIONS / "matter_formation_two_packet_q16_spec.py"
...
VERIFIER_SOURCE = COMPUTATIONS / "verify_matter_formation_two_packet_q16.py"
SCHEMA = "matter-formation-two-packet-q16-primary-v1"
...
def assemble_primary(grid: CylindricalGrid, arm: str) -> tuple[torch.Tensor, torch.Tensor, float, dict[str, Any]]:
    """Assemble the primary preparation from the declarative specification."""
    config = ARM_SPECS[arm]
    pair = bool(config["pair"])
    uncoupled = bool(config["uncoupled"])
    omega = dynamics.OMEGA_INF if not pair else math.sqrt(dynamics.OMEGA_INF**2 + OMEGA_OFFSET_SQUARED)
    zeta = grid.axial
    if config["kind"] == "single":
        ...
        phase_right = WAVE_NUMBER * (zeta[None, :] + CENTER)
        phase_left = -WAVE_NUMBER * (zeta[None, :] - CENTER)
...
def run_row(output: Path, grid_name: str, grid: CylindricalGrid, dt: float, arm: str) -> dict[str, Any]:
    ...
    persistent = bool(
        late
        and complete_observables
        and min(float(value) for value in core_fraction_values) >= dynamics.RETAINED_FRACTION
        and max(float(value) for value in binding_values) < dynamics.BINDING_RATIO_MAX
...
def run_smoke() -> int:
    ...
    for arm in ARMS:
        ...
        records.append({"arm": arm, "charge": charge, "coupling": coupling, "overlap": metadata["initial_overlap"]})
...
def run_campaign(output: Path) -> dict[str, Any]:
    ...
    for arm in ARMS:
        comparisons.append(dynamics.compare_rows(indexed[("G0", arm)], indexed[("G1", arm)], "space"))
        comparisons.append(dynamics.compare_rows(indexed[("G0", arm)], indexed[("T1", arm)], "time"))
    coupled_candidates = [
        indexed[("G0", arm)]
        for arm in ("pair16", "antiphase16")
        if indexed[("G0", arm)]["formation"]
    ]
    ...
    for candidate in coupled_candidates:
        ...
        required = [
            ...
            if item["left"] in {f"G0_{arm}", f"G1_{arm}", f"T1_{arm}"}
            or item["right"] in {f"G0_{arm}", f"G1_{arm}", f"T1_{arm}"}
        ]
    ...
    uncoupled = indexed[("G0", "uncoupled16")]
    ...
    if not comparison_pass:
        ...
        verdict = "EMERGES—conditional two-packet bound remnant at total charge 16"
    else:
        verdict = "DOES NOT EMERGE in the specified Q16 two-packet calculation"
    receipt = {
        "schema": SCHEMA,
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--output", type=Path, default=ROOT / "runs" / "20260911_matter_formation_two_packet_q16")
