#!/usr/bin/env python3
"""Independent DOP853 verification for the G3 point-core continuation."""
...
PRIMARY = ROOT / "computations" / "matter_formation_autonomous_shell_g3_point_core.py"
ACTION_PREREG = ROOT / "computations" / "matter-formation-autonomous-shell-g3-point-core-prereg.md"
PREREG = ACTION_PREREG
SCHEMA = "cassi.matter-formation.autonomous-shell-point-core-verification.v1"
PRIMARY_SCHEMA = "cassi.matter-formation.autonomous-shell-point-core.v1"
V = 1.0
LAMBDA = 0.25
G = 3.0
RADIUS = 16.0
...
def run(input_dir: Path, output_path: Path) -> dict[str, Any]:
    ...
    verdict = "CAPTURED—conditional autonomous point-core radial formation" if numerical_pass and candidate_pass else "DOES NOT EMERGE—conditional autonomous point-core radial formation" if numerical_pass else "INCONCLUSIVE"
    receipt = {
        "schema": SCHEMA, "numerical_pass": numerical_pass, "verdict": verdict, "complete_physical_matter_formation": False,
        "autonomous_point_core_radial_mechanism": bool(numerical_pass and candidate_pass),
        ...
        "scope": "finite-box spherical kappa=-1 covariance with self-consistent scalar backreaction from a prescribed finite-energy initial shell using the point-core 1/r radial connection; no continuum or all-sector claim",
