#!/usr/bin/env python3
"""Run the preregistered closed CP-odd reservoir formation calculation.

Run from the CassiTheory root:
    python computations/matter_formation_closed_cp_reservoir_v1.py \
        --output-dir runs/20260912_matter_formation_closed_cp_reservoir_v1
"""
...
PROTOCOL = ROOT / "computations/matter-formation-closed-cp-reservoir-v1-prereg.md"
SOURCE = Path(__file__).resolve()
DEFAULT_OUTPUT = ROOT / "runs/20260912_matter_formation_closed_cp_reservoir_v1"
SCHEMA = "cassi.matter-formation.closed-cp-reservoir.v1"
...
def initial_state(
    ...
    if sign < 0:
        uy = -uy
        py = -py
    v = sign * reservoir_amplitude * np.exp(-0.5 * (r / RESERVOIR_WIDTH) ** 2) * r
    return ux, uy, v, px, py, np.zeros_like(r)
...
def outgoing_laplacian(values: np.ndarray, momentum: np.ndarray, spacing: float) -> np.ndarray:
    return static_laplacian(values, spacing) - momentum / spacing
...
def rhs(
    ...
    ax = outgoing_laplacian(ux, px, spacing) - coefficient * ux + coupling * (v / r) * uy
    ay = outgoing_laplacian(uy, py, spacing) - coefficient * uy + coupling * (v / r) * ux
    av = outgoing_laplacian(v, pv, spacing) - M_A * M_A * v + coupling * ux * uy / r
...
def energy(state: tuple[np.ndarray, ...], r: np.ndarray, spacing: float, coupling: float) -> float:
    ...
    ah_v = static_laplacian(v, spacing)
    gradient_energy = -0.5 * spacing * float(ux @ ah_x + uy @ ah_y + v @ ah_v)
    local = (
        0.5 * (px * px + py * py + pv * pv)
        + 0.5 * M * M * rho / (r * r)
        - 0.5 * LAMBDA * rho * rho / (r**4)
        + 0.5 * G * rho * rho * rho / (r**6)
        + 0.5 * M_A * M_A * v * v / (r * r)
        - coupling * v * ux * uy / (r**3)
    )
...
def instantaneous_metrics(
    ...
    support_density = (
        0.5 * (px * px + py * py + gx * gx + gy * gy + M * M * rho / (r * r))
        + 0.5 * G * rho * rho * rho / (r**6)
    )
    core = r <= CORE_RADIUS
    ...
    return {
        ...
        "source_rate": float(
            4.0 * math.pi * coupling * np.sum((v / r) * (ux * ux - uy * uy)) * spacing
        ),
        "boundary_charge_rate": float(
            4.0 * math.pi * (-ux[-1] * py[-1] + uy[-1] * px[-1])
        ),
        "boundary_energy_rate": float(-4.0 * math.pi * (px[-1] ** 2 + py[-1] ** 2 + pv[-1] ** 2)),
    }
...
def main() -> int:
    ...
    runs = {
        "vacuum_control": archive_run(output, "vacuum_control", N_PRIMARY, DT_PRIMARY, +1, 0.0, KAPPA),
