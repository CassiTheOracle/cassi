#!/usr/bin/env python3
"""Independently reconstruct the pair-frequency-resonant autonomous receipt."""
...
PRIMARY = ROOT / "computations/matter_formation_closed_cp_pair_reservoir_v10.py"
PROTOCOL = ROOT / "computations/matter-formation-closed-cp-pair-reservoir-v10-prereg.md"
SCHEMA = "cassi.matter-formation.closed-cp-pair-reservoir.v10.verification"
RADIUS = 48.0
N_PRIMARY = 384
N_FINE = 768
DT_PRIMARY = 0.003
...
def make_initial(
    ...
    carrier = amplitude * envelope * np.cos(RESERVOIR_WAVENUMBER * r) * r
...
def main() -> int:
    ...
    if primary.get("schema") != "cassi.matter-formation.closed-cp-pair-reservoir.v10":
    ...
    payload = {
        ...
        "reproduced_primary_verdict": (
            "CAPTURED—conditional pair-frequency-resonant closed CP-odd-reservoir vacuum-to-carrier formation"
            if scientific_pass
            else "DOES NOT EMERGE—conditional pair-frequency-resonant closed CP-odd-reservoir vacuum-to-carrier formation"
        ),
