#!/usr/bin/env python3
"""Audit the nested-cutoff Feshbach receipt without assembling matrices."""
...
PRIMARY_SOURCE = ROOT / "computations/verify_yang_mills_interacting_feshbach_cutoff_screen.py"
...
PRIMARY = ROOT / "runs/yang_mills_interacting_feshbach_cutoff/verification.json"
DEFAULT_OUTPUT = ROOT / "runs/yang_mills_interacting_feshbach_cutoff/verification-independent.json"
...
def audit() -> dict[str, Any]:
    ...
    result: dict[str, Any] = {
        "schema": "cassi.yang-mills.interacting-feshbach-cutoff-screen.independent.v1",
    ...
    record_check(result, "primary status and complete checks",
                 ...
                 and primary.get("summary", {}).get("rows") == 40
                 and primary.get("summary", {}).get("checks") == 403
                 and primary.get("summary", {}).get("passing_checks") == 403,
    ...
    record_check(result, "fixed family schedule",
    ...
    record_check(result, "row schedule and family summary",
                 len(rows) == 40 and seen == expected_keys
                 and primary.get("family_summary") is not None
                 and all(primary["family_summary"].get(coupling, {}).get("rows") == 10
                         for coupling in COUPLINGS),
    ...
    for coupling in COUPLINGS:
        ...
        for key in ("min_certified_root", "min_root_to_gap_ratio",
            ...
            summary_matches = summary_matches and (
                (left is None and right is None) or (left is not None and right is not None and close(left, right))
            )
...
def main(argv: list[str] | None = None) -> int:
    ...
    required = (PROTOCOL, PRIMARY_SOURCE, REFERENCE, PREDECESSOR_SOURCE, PREDECESSOR_RECEIPT, PRIMARY)
    if not all(path.exists() for path in required):
        raise FileNotFoundError("protocol, source, reference, predecessor or primary receipt is missing")
    result = audit()
