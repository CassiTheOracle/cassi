...
- `construction_positive_above_step_floor`—every declared initial field has a positive minimum whose margin over the solver's step floor of $10^{-3}$ exceeds a factor of one; the measured minima and margins are recorded. Amended in invocation 1 of §6: the original bullet here required every initial field's minimum to exceed $0.1$, a threshold derived from the incorrect sentence of §1.3, and it failed on the deepest declared composition ($0.0242$) although that composition's construction is exactly the declared one and no clamp exists anywhere in it.
- `construction_declared_state`—every declared initial state equals its declared composition: $R(0)$ against the declared $R_0$ and $\langle\rho\rangle(0)$ against $1+\varphi^{-1}$, each to $10^{-12}$ relative. Added in invocation 1 of §6 as the direct check the replaced threshold was standing in for.
