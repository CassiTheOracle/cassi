#!/usr/bin/env python3
"""Run the frozen v5 higher-precision particle continuation campaign."""
...
class PrecisionV5Failure(RuntimeError):
    """Raised when a frozen v5 source requirement fails."""


def source_shell_receipt(arrays: dict[str, np.ndarray]) -> dict[str, Any]:
    """Measure the fixed-shell residual directly on the source arrays."""
    n = arrays["x"].shape[0]
    shell = np.ones((n, n, n), dtype=np.bool_)
    shell[1:-1, 1:-1, 1:-1] = False
    psi_inf = np.array(
        (campaign.stationary.PHI**-0.5, campaign.stationary.PHI**-1.0),
        dtype=np.float64,
    )
...
def load_source(device: torch.device) -> tuple[dict[str, torch.nn.Parameter], dict[str, Any]]:
    if campaign.sha256_file(campaign.SOURCE_ARTIFACT) != campaign.SOURCE_ARTIFACT_SHA256:
        raise PrecisionV5Failure("source artifact hash mismatch")
    ...
    if not isinstance(source_arm, dict):
        raise PrecisionV5Failure("selected source arm is absent")
    if source_arm.get("artifact_sha256") != campaign.SOURCE_ARTIFACT_SHA256:
        raise PrecisionV5Failure("selected source receipt hash mismatch")
    if source_results.get("primary_verdict") != "PASS—Q2-QUALIFIED PRIMARY BACKGROUND":
        raise PrecisionV5Failure("selected source receipt verdict mismatch")
    if source_verification.get("pass") is not True or source_verification.get("mismatches"):
        raise PrecisionV5Failure("selected source verification is not clean")
    ...
    if schema_failures:
        raise PrecisionV5Failure(f"source artifact schema failure: {schema_failures}")
    shell_receipt = source_shell_receipt(arrays)
    if not shell_receipt["pass"]:
        raise PrecisionV5Failure(f"source shell mismatch: {shell_receipt}")
    ...
    if (
        ...
        raise PrecisionV5Failure(f"source reconstruction failure: {reconstruction}")
    ...
    if not all(row["pass"] for row in scalar_checks.values()):
        raise PrecisionV5Failure(f"source scalar mismatch: {scalar_checks}")
    return raw, {
