#!/usr/bin/env python3
"""Independently reconstruct the finite-mode autonomous pulse receipt."""
...
PRIMARY = ROOT / "computations/matter_formation_closed_cp_reservoir_v7.py"
PROTOCOL = ROOT / "computations/matter-formation-closed-cp-reservoir-v7-prereg.md"
SCHEMA = "cassi.matter-formation.closed-cp-reservoir.v7.verification"
RADIUS = 48.0
N_PRIMARY = 384
N_FINE = 768
DT_PRIMARY = 0.003
...
def make_initial(
    ...
    scale = amplitude / RESERVOIR_AMPLITUDE
...
def rk4(
    ...
    k2 = derivative(combine(state, k1, dt / 2.0), r, dr, coupling)
    k3 = derivative(combine(state, k2, dt / 2.0), r, dr, coupling)
...
def scientific_predicates(runs: dict[str, dict[str, Any]]) -> tuple[dict[str, bool], dict[str, Any]]:
    ...
    fit_coefficients, fit_omega, fit_residual = reservoir_coefficients()
    controls = {
        ...
        "fit_mode_frequencies": [float(value) for value in fit_omega],
        "fit_coefficient_count": int(fit_coefficients.size),
    }
...
def main() -> int:
    ...
    if primary.get("schema") != "cassi.matter-formation.closed-cp-reservoir.v7":
