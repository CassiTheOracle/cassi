...
RUN_DIR = ROOT / "runs" / "20260902_particle_stationary_q2_recovery_v2"
RESULTS_PATH = RUN_DIR / "results.json"
PREFLIGHT_PATH = RUN_DIR / "preflight_verification.json"
PREREG_PATH = ROOT / "computations" / "particle_stationary_q2_recovery_v2_prereg.md"
VERIFIER_PATH = ROOT / "computations" / "verify_particle_stationary_q2_recovery_v2.py"
        ...
        for basin in source.BASINS:
            ...
            try:
                ...
                field_values = {
                    ...
                    if name not in raw_gradient_names
                }
                source_field_values = {
                    name: value
                    for name, value in arm["diagnostics"].items()
                    if name not in raw_gradient_names
                }
                ...
                for name in raw_gradient_names:
                    reconstructed_value = values.get(name)
                    source_value = arm["diagnostics"].get(name)
                    if (
                        not isinstance(reconstructed_value, (int, float))
                        or not math.isfinite(float(reconstructed_value))
                        or reconstructed_value < 0.0
                        or not isinstance(source_value, (int, float))
                        or not math.isfinite(float(source_value))
                        or source_value < 0.0
                    ):
                        local_failures.append(
                            f"{name} is not a finite nonnegative receipt value"
                        )
...
def run() -> dict[str, Any]:
    ...
    if not PREFLIGHT_PATH.exists():
        raise FileNotFoundError(
            "Run verify_particle_stationary_q2_recovery_v2.py --preflight first"
        )
