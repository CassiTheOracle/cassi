#!/usr/bin/env python3
"""Run the preregistered fixed-charge packet-width basin probe."""
...
VERIFIER_SOURCE = COMPUTATIONS / "verify_matter_formation_packet_width.py"
SCHEMA = "matter-formation-packet-width-primary-v1"
...
def run_smoke() -> int:
    ...
    for arm in ARMS:
        ...
        records.append({"arm": arm, "charge": charge, "center": metadata["center"], "wave_number": metadata["wave_number"], "phase_sign": metadata["phase_sign"], "orientation": metadata["orientation"], "width": metadata["width"], "overlap": metadata["initial_overlap"]})
...
def run_campaign(output: Path) -> dict[str, Any]:
    ...
    if not comparison_pass or not rule_control["pass"]:
        ...
        verdict = "EMERGES—conditional bound remnant in the declared packet-width basin"
    else:
        verdict = "DOES NOT EMERGE in the specified packet-width calculation"
    receipt = {
        "schema": SCHEMA,
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--output", type=Path, default=ROOT / "runs" / "20260911_matter_formation_packet_width")
