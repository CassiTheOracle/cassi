#!/usr/bin/env python3
"""Independent verifier for matched-in-vacuum regulated formation."""
...
PRIMARY = COMPUTATIONS / "matter_formation_fermion_vacuum_bag_matched_in_vacuum.py"
...
SCHEMA = "cassi.matter-formation.fermion-vacuum-bag-matched-in-vacuum-verification.v1"
PRIMARY_SCHEMA = "cassi.matter-formation.fermion-vacuum-bag-matched-in-vacuum.v1"
CACHE_TOLERANCE = 1.0e-12
MIDPOINT_CORRECTORS = 1

_spec = importlib.util.spec_from_file_location("cassi_matter_matched_verifier_base", BASE_PATH)
...
def matched_basis(
    ...
    """Independently construct the initial-profile finite-box vacuum."""
    n = sigma.size
...
def independent_evolve(
    ...
    global _cache_sigma, _cache_dt, _cache_dr, _cache_coupling, _cache_full, _cache_half, _reference_sigma
    _cache_sigma = None
    _cache_dt = None
    _cache_dr = None
    ...
    diagnostics: list[dict[str, float]] = [
        _base.observables(
            sigma, pi, modes, modes0, basis, h0, radius, dr, volume, area, source0, _base.G_YUKAWA
        )
    ]
    for step in range(1, steps + 1):
        sigma, pi, modes = midpoint_step(
            sigma, pi, modes, dt, dr, volume, area, radius, b0,
            _base.G_YUKAWA, source_enabled
        )
        if step % sample_steps == 0 or step == steps:
            ...
            diagnostics.append(
                _base.observables(
                    sigma, pi, modes, modes0, basis, h0, radius, dr,
                    volume, area, source0, _base.G_YUKAWA
                )
            )
...
_original_snapshot = _base.snapshot_sources
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"matched-in-vacuum verifier failed before receipt: {exc}")
        return 1
    print(
        json.dumps(
            {
