...
PROTOCOL = ROOT / "computations" / "yang-mills-su2-schwinger-prereg.md"
SOURCE = Path(__file__).resolve()
DEFAULT_OUTPUT = ROOT / "runs" / "yang_mills_su2_schwinger_bridge" / "verification.json"
...
def schwinger_row(n_max: int, g2: float) -> dict[str, Any]:
    ...
    # The eigenvector matrix is reconstructed a second time by eigh so the
    # row stores the same spectral basis used to form the correlator.
    values, vectors = np.linalg.eigh(h)
    ...
    checks = [
        ...
        {
            "name": "perron_ground_coefficients_positive",
            "passed": bool(float(np.min(ground)) > 0.0),
...
def run(output: Path) -> dict[str, Any]:
    ...
    record: dict[str, Any] = {
        "schema": "cassi.yang-mills.su2-schwinger-bridge.v1",
