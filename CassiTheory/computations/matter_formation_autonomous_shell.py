#!/usr/bin/env python3
"""Primary autonomous incoming-shell vacuum-to-bound-state calculation."""
...
SCHEMA = "cassi.matter-formation.autonomous-shell.v6"
V = 1.0
LAMBDA = 0.25
G = 3.0
RADIUS = 16.0
...
def dense_hamiltonian(sigma: np.ndarray, dr: float) -> np.ndarray:
    ...
    if float(np.max(np.abs(h - h.conj().T))) > 1.0e-13:
        raise FormationError("non-Hermitian radial Hamiltonian")
    return h
...
def initial_state(n: int, dr: float, homogeneous: bool, zero_covariance: bool) -> dict[str, Any]:
    ...
    h0 = dense_hamiltonian(sigma0, dr)
    ...
    if int(np.count_nonzero(negative)) != n or int(np.count_nonzero(positive)) != n:
        raise FormationError("initial spectrum is not split into N plus N states")
    ...
    source0 = (np.sum(np.abs(modes0[:n]) ** 2 - np.abs(modes0[n:]) ** 2, axis=1) * dr / volume)
    return {"radius": radius, "volume": volume, "area": area, "sigma0": sigma0, "pi0": pi0, "h0": h0, "modes0": modes0, "modes": modes, "source0": source0, "metric": metric}


def normal_ordered_source(modes: np.ndarray, modes0: np.ndarray, dr: float, volume: np.ndarray) -> np.ndarray:
    n = volume.size
    current = np.sum(np.abs(modes[:n]) ** 2 - np.abs(modes[n:]) ** 2, axis=1)
    baseline = np.sum(np.abs(modes0[:n]) ** 2 - np.abs(modes0[n:]) ** 2, axis=1)
    return dr * (current - baseline) / volume


def scalar_acceleration(sigma: np.ndarray, modes: np.ndarray, state: dict[str, Any], source_enabled: bool) -> np.ndarray:
    source = normal_ordered_source(modes, state["modes0"], state["dr"], state["volume"]) if source_enabled else 0.0
    return scalar_laplacian(sigma, state["dr"], state["volume"], state["area"]) - LAMBDA * (sigma * sigma - V * V) * sigma - G * source


def matrix_hamiltonian_apply(modes: np.ndarray, sigma: np.ndarray, dr: float) -> np.ndarray:
    n = sigma.size
    radius = (np.arange(n, dtype=np.float64) + 0.5) * dr
    radial_connection = 1.0 / np.sqrt(radius * radius + CORE_SMOOTH * CORE_SMOOTH)
    upper = modes[:n]
    lower = modes[n:]
    result_upper = G * sigma[:, None] * upper - derivative_matrix(lower, dr) - radial_connection[:, None] * lower
    result_lower = derivative_matrix(upper, dr) - radial_connection[:, None] * upper - G * sigma[:, None] * lower
    return np.concatenate((result_upper, result_lower), axis=0)


def rhs(sigma: np.ndarray, pi: np.ndarray, modes: np.ndarray, state: dict[str, Any], source_enabled: bool) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    return pi, scalar_acceleration(sigma, modes, state, source_enabled), -1j * matrix_hamiltonian_apply(modes, sigma, state["dr"])


def rk4_step(sigma: np.ndarray, pi: np.ndarray, modes: np.ndarray, dt: float, state: dict[str, Any], source_enabled: bool) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    k1 = rhs(sigma, pi, modes, state, source_enabled)
    k2 = rhs(sigma + 0.5 * dt * k1[0], pi + 0.5 * dt * k1[1], modes + 0.5 * dt * k1[2], state, source_enabled)
    k3 = rhs(sigma + 0.5 * dt * k2[0], pi + 0.5 * dt * k2[1], modes + 0.5 * dt * k2[2], state, source_enabled)
    k4 = rhs(sigma + dt * k3[0], pi + dt * k3[1], modes + dt * k3[2], state, source_enabled)
    return (
...
def total_energy(sigma: np.ndarray, pi: np.ndarray, modes: np.ndarray, state: dict[str, Any]) -> tuple[float, float, float, float]:
    current_h = dense_hamiltonian(sigma, state["dr"])
    dynamic = float(np.real(np.trace(modes.conj().T @ current_h @ modes)) * state["dr"])
...
def instantaneous_metrics(sigma: np.ndarray, pi: np.ndarray, modes: np.ndarray, state: dict[str, Any]) -> dict[str, float]:
    n = state["radius"].size
    dr = state["dr"]
    values, vectors = np.linalg.eigh(dense_hamiltonian(sigma, dr))
    positive = values > 1.0e-10
    negative = values < -1.0e-10
    p = vectors[:, positive] / math.sqrt(dr)
    q = vectors[:, negative] / math.sqrt(dr)
    coefficients = dr * (p.conj().T @ modes)
    particles = p @ coefficients
    ...
    hole_number = float(n - np.sum(np.abs(holes) ** 2))
    ...
    if bound_count:
        ...
        bound_hole_coeff = dr * (bound_hole.conj().T @ modes)
        bound_density = np.sum(np.abs(bound[:n]) ** 2 + np.abs(bound[n:]) ** 2, axis=1)
        bound_occupation = float(np.sum(np.abs(bound_coeff) ** 2))
        bound_hole_occupation = float(bound_count - np.sum(np.abs(bound_hole_coeff) ** 2))
    else:
        bound = np.zeros((2 * n, 0), dtype=np.complex128)
        bound_density = np.zeros(n, dtype=np.float64)
        bound_occupation = 0.0
        bound_hole_occupation = 0.0
    core = state["radius"] < CORE_RADIUS
    pair_core = float(np.sum(state["dr"] * density[core]) / pair_number) if pair_number > PAIR_EPS else 0.0
    pair_rms = math.sqrt(float(np.sum(state["radius"] ** 2 * density) * dr / pair_number)) if pair_number > PAIR_EPS else 0.0
    ...
    return {
        ...
        "bound_hole_occupation": bound_hole_occupation,
        "bound_hole_gap": abs(bound_occupation - bound_hole_occupation),
        "pair_core_probability": pair_core,
        "pair_rms": pair_rms,
        "bound_core_probability": bound_core,
        "bound_rms": bound_rms,
...
def archive_arrays(output: Path, arm: str, times: list[float], sigmas: list[np.ndarray], pis: list[np.ndarray], modes: list[np.ndarray]) -> str:
    arrays = {"time": np.asarray(times, dtype=np.float64), "sigma": np.asarray(sigmas, dtype=np.float64), "pi": np.asarray(pis, dtype=np.float64), "u_re": np.asarray([x.real for x in modes], dtype=np.float64), "u_im": np.asarray([x.imag for x in modes], dtype=np.float64)}
...
def evolve_arm(output: Path, arm: str, grid: str, homogeneous: bool, source_enabled: bool, zero_covariance: bool) -> tuple[dict[str, Any], dict[str, str]]:
    ...
    diagnostics: list[dict[str, float]] = [instantaneous_metrics(sigma, pi, modes, state)]
    for step in range(1, steps + 1):
        ...
        if step % sample_steps == 0 or step == steps:
            ...
            diagnostics.append(instantaneous_metrics(sigma, pi, modes, state))
...
def snapshot_sources(output: Path) -> dict[str, str]:
    ...
    result: dict[str, str] = {}
    for path in (SELF, ACTION_PREREG, PREREG):
...
def run(output_dir: Path) -> dict[str, Any]:
    ...
    controls = (
        ("static_vacuum", "G1", True, False, False),
        ("source_off", "G1", False, False, False),
        ("zero_covariance", "G1", False, False, True),
    )
    for arm, grid, homogeneous, source_enabled, zero_covariance in controls:
    ...
    controls_rows = [row for row in rows if not row["candidate"]]
    by_grid = {row["grid"]: row for row in candidate_rows}
    resolution: list[dict[str, Any]] = []
    for left_grid, right_grid in (("G0", "G1"), ("G1", "G2")):
        left, right = by_grid[left_grid], by_grid[right_grid]
    ...
    static = next(row for row in controls_rows if row["arm"] == "static_vacuum")
    zero = next(row for row in controls_rows if row["arm"] == "zero_covariance")
    ...
    receipt = {
        "schema": SCHEMA,
        "identities": {"primary": {"path": relative(SELF), "sha256": canonical_sha(SELF)}, "action_prereg": {"path": relative(ACTION_PREREG), "sha256": canonical_sha(ACTION_PREREG)}, "prereg": {"path": relative(PREREG), "sha256": canonical_sha(PREREG)}, "archived_source_sha256": archived},
...
def main() -> int:
    ...
    try:
        ...
        print(f"autonomous-shell primary failed before receipt: {exc}")
        return 1
