...
from verify_matter_formation_parent_spatial import json_safe
...
HEADING = "### 11.7 Receipt qualification supplement: pre-execution criteria\n"
ORIGINAL_HEADING = "### 11.5 Extended-domain calculation: pre-execution criteria\n"
...
N = 256.0
DEFAULT_OUTPUT = ROOT / "runs/20260906_matter_formation_spatial_domain_v2"
...
def write_json(path: Path, value: dict) -> None:
    if has_nonfinite(value):
        value["numerical_pass"] = False
        value["verdict"] = "INCONCLUSIVE"
        value.setdefault("failures", []).append(
            "Nonfinite diagnostics are retained as null; inspect raw failed-field archives")
    clean = json_safe(value)
    with path.open("x", encoding="utf-8", newline="\n") as stream:
        json.dump(clean, stream, indent=2, ensure_ascii=False, allow_nan=False)
        stream.write("\n")
    value.clear()
    value.update(clean)
...
def freeze_inputs(output: Path) -> dict:
    ...
    original_start = text.index(ORIGINAL_HEADING)
    original_end = text.index("\n##", original_start + len(ORIGINAL_HEADING))
    original_hash = hashlib.sha256((text[original_start:original_end] + "\n").encode()).hexdigest()
    original_program = ROOT / "computations/matter_formation_spatial_domain.py"
    if original_hash != "5b37fa884dac6a9788f7cc9cbe5992dc8c14b2a030fa1d94deff1bb2991d65eb":
        raise ValueError("original frozen protocol mismatch")
    if sha(original_program, canonical=True) != "dcbfd57ffc03081d435f48e853b56a283bea5e555ee1994b95672504d4c8f518":
        raise ValueError("sealed spatial program mismatch")
    inputs["original_protocol_sha256"] = original_hash
    for name in (Path(__file__).name, original_program.name, "matter_formation_radial.py",
                 "matter_formation_parent_spatial.py", "verify_matter_formation_charged_stability.py",
                 "verify_matter_formation_parent_spatial.py"):
...
def retained_spectrum_check() -> dict:
    with np.load(SOURCE, allow_pickle=False) as archive:
        f, c = archive["f"].copy(), archive["c"].copy()
    grid = Grid.make(24.0, 768)
    diagnostics = stationary_diagnostics(f, c, grid, N)
    omega = diagnostics["omega"]
    band, _ = stationary_band(f, c, omega, grid)
    eta = max(5e-4, 10 * diagnostics["residual_f"], 10 * diagnostics["residual_c"])
    actual, _ = spatial_spectra(f, c, omega, grid, band, eta)
    verification = json.loads((SPATIAL_DIR / "verification.json").read_text(encoding="utf-8"))
    expected = next(row for row in verification["rows"] if row["id"] == "q256_R24_n768_refine")
    comparisons = []
    for name, operator in actual["operators"].items():
        for index, (left, right) in enumerate(zip(operator["eigenvalues"], expected["operators"][name]["eigenvalues"], strict=True)):
            comparisons.append({"operator": name, "index": index, "difference": abs(left - right),
                                "tolerance": 1e-10, "pass": bool(abs(left - right) <= 1e-10)})
    return {"source_sha256": SOURCE_HASH, "source_relaxed": False, "comparisons": comparisons,
            "pass": len(comparisons) == 24 and all(row["pass"] for row in comparisons)}


def run(output: Path) -> int:
    output.mkdir(parents=True, exist_ok=False)
    receipt = {"schema": "matter-formation-spatial-domain-v2", "rows": [], "comparisons": [],
               "numerical_pass": False, "verdict": "INCONCLUSIVE", "failures": []}
    try:
        receipt["inputs"] = freeze_inputs(output)
        receipt["retained_spectrum_check"] = retained_spectrum_check()
        if not receipt["retained_spectrum_check"]["pass"]:
            raise RuntimeError("retained unrelaxed spectral construction mismatch")
        with np.load(SOURCE, allow_pickle=False) as archive:
