#!/usr/bin/env python3
"""Measure degree-four plaquette-cyclic coverage of the centered Q sector.

Protocol:
``computations/yang-mills-plaquette-cyclic-coverage-prereg-v4.md``.

This is a finite C=1 screen on the recovered open 3x2x2 SU(2) graph.  It
retains every ordered product of the eleven normalized fundamental plaquette
multiplication operators through degree four, projects out the finite ground
state, and measures cumulative singular-value rank.  It does not infer
completeness of the full closed-Wilson-word algebra or any continuum claim.
"""
...
PROTOCOL = ROOT / "computations" / "yang-mills-plaquette-cyclic-coverage-prereg-v4.md"
...
INDEPENDENT_SOURCE = ROOT / "computations" / "verify_yang_mills_plaquette_cyclic_coverage_v4_independent.py"
...
DEFAULT_OUTPUT = ROOT / "runs" / "yang_mills_plaquette_cyclic_coverage" / "verification-v4.json"

COUPLINGS = (Fraction(1, 64), Fraction(1, 16), Fraction(1, 4), Fraction(1, 1))
MAX_DEGREE = 4
MATRIX_TOLERANCE = 1.0e-10
RANK_TOLERANCE = 1.0e-10
EXPECTED_WORD_COUNTS = (1, 11, 121, 1331, 14641)
EXPECTED_CUMULATIVE_WORD_COUNTS = (0, 11, 132, 1463, 16104)
...
def load_baseline() -> Any:
    """Load only the shared degree-three source-bound construction helpers."""
    spec = importlib.util.spec_from_file_location("cassi_ym_cyclic_v3", BASELINE_SOURCE)
...
def cyclic_word_rows_degree_four(
    omega: np.ndarray,
    operators: list[Any],
    ...
    for degree in range(MAX_DEGREE + 1):
        ...
        cumulative = (
            np.column_stack(projected_by_degree)
            if projected_by_degree
    ...
    return rows, np.column_stack(projected_by_degree)
...
def build_row(
    ...
    normalized_operators = [large.normalized_operator(row["matrix"], states) for row in plaquette_rows]
    degree_rows, projected_words = cyclic_word_rows_degree_four(omega, normalized_operators)
    final_rank = int(degree_rows[-1]["rank"])
    q_dimension = dimension - 1
    full_degrees = [item["degree"] for item in degree_rows if item["rank"] == q_dimension]
    first_full_degree = min(full_degrees) if full_degrees else None
    classification = (
    ...
    row_checks = [
        check(
            ...
            == EXPECTED_CUMULATIVE_WORD_COUNTS,
        ...
        check(
            ...
            projected_words.shape == (dimension, EXPECTED_CUMULATIVE_WORD_COUNTS[-1])
        ...
        check(
            f"finite_degree_four_boundary_x{row_key}",
            q_dimension == 867
            ...
            first_full_degree=first_full_degree,
...
def run(output: Path) -> dict[str, Any]:
    ...
    if not all(path.is_file() for path in required):
        ...
        raise FileNotFoundError(f"missing degree-four cyclic-coverage dependency: {missing}")
    if output.exists():
    ...
    plaquette_rows, operator, matrix_checks = baseline.build_plaquette_source(large, states, source_receipt)
    ...
    checks: list[dict[str, Any]] = [
        ...
        check("source_path", relative(SOURCE) == "computations/verify_yang_mills_plaquette_cyclic_coverage_v4.py"),
        ...
        check(
            ...
            and MAX_DEGREE == 4
            and EXPECTED_WORD_COUNTS == (1, 11, 121, 1331, 14641)
            and EXPECTED_CUMULATIVE_WORD_COUNTS == (0, 11, 132, 1463, 16104),
            couplings=[float(value) for value in COUPLINGS],
            max_degree=MAX_DEGREE,
            expected_word_counts=list(EXPECTED_WORD_COUNTS),
            expected_cumulative_word_counts=list(EXPECTED_CUMULATIVE_WORD_COUNTS),
        ),
    ]
    ...
    record: dict[str, Any] = {
        "schema": "yang_mills_plaquette_cyclic_coverage_v4",
        ...
        "baseline_protocol": relative(BASELINE_PROTOCOL),
        ...
        "graph": {"vertices": 12, "links": 20, "state_dimension_C1": len(states), "q_dimension": len(states) - 1},
        "plaquette_names": list(large.PLAQUETTE_NAMES),
        "couplings": [float(value) for value in COUPLINGS],
        "max_degree": MAX_DEGREE,
        "word_counts": list(EXPECTED_WORD_COUNTS),
        "cumulative_word_counts": list(EXPECTED_CUMULATIVE_WORD_COUNTS),
        "rows": rows,
        "checks": checks,
        ...
        "scope": "finite C=1 plaquette-cyclic operator coverage through degree four on the recovered open 3x2x2 SU(2) graph",
    }
