...
|---|---|---|
| `frozen_body_sha256` | `c0d2fe37b8b4ee7673b1ec1a7c658f2bedb4eae6ad360516057d5ee9cdc843fa` | this file from `## 1.` to just before `## 8.`, the text that carries every statistic, threshold, arm and decision rule |
| `executor_sha256` | `be9f651d085bb4ee5d8f62ddb4db0a1714eb58c1b2106025d52f7fe2224f0eac` | `computations/verify_loop_carrier_gate_load.py`, the complete executor frozen with this text; re-anchored with the body digest after the invocation—the executed value was `fdc242b3999c153ad77e0186f342bf70628505adb681a0a9c48bb4276c33a5f9` |
...
**The amendment of September 16, 2026, after the invocation.** Three corrections were made *inside* the body range—§6's steps row, §2.3's instrument symbol and §4.3's instrument symbol—each marked in place with what the pre-amendment text read and a pointer to §8.7, which records the correction, both body digests and the consequence. No arm, threshold, level, statistic or decision rule was touched: the executor parses none of the three corrected passages, and the arm table, the thresholds of §2.4, the oracles of §5.3 and the schedule of §6 are byte-identical to what ran. Re-anchoring the moved body required updating this executor's own `FROZEN_BODY_DIGEST` constant and its comment, and nothing else, so the executor's digest moved with it. The two rows above carry the standing values; the invocation's values were body `8f3fae24990b7b74632f08f3f6d4dde4e476e6d81f3ad8a5d903b37659c06268` and executor `fdc242b3999c153ad77e0186f342bf70628505adb681a0a9c48bb4276c33a5f9`, which is exactly what the receipt's `frozen_sources` records. So `check_binding` passes again on the amended tree, while the executor's self-check now reports the receipt's `protocol` and `probe` rows as `INCONSISTENT`—the check doing its job on artifacts that moved after the run, disclosed in §8.7 rather than repaired by rewriting the receipt.
...
both recorded, together with the per-level class membership that says whether the declared levels bracket each crossing at all. The pair $(\delta^\star,\ell^\star)$ is the answer the spent protocol's boundary left open. *(Corrected in place after the single invocation of September 16, 2026: the pre-amendment text read $(\delta^\star,q)$; §8.7 records the amendment and its digests.)*
...
`status=PASS` requires every gate and hence every feature; otherwise the run ends at `status=FAIL` with both verdicts null and no label issued. On `PASS`, fit A's label and fit B's label are the two verdicts, and the instrument $(\delta^\star,\ell^\star)$ of §2.3 is reported with them. *(Corrected in place after the single invocation of September 16, 2026: the pre-amendment text read $(\delta^\star,q)$; §8.7 records the amendment and its digests.)*
...
| Steps | $20\times600+100+1{,}834=13{,}934$ against the total cap $30{,}000$; the largest arm is the carried replication at $1{,}834$ against the per-execution cap $50{,}000$ *(corrected in place after the single invocation of September 16, 2026: the pre-amendment text read $19\times600+100+1{,}834=13{,}334$; §8.7 records the amendment, its pre- and post-amendment body digests, and why §3's arm count of $20$ six-hundred-step arms is the one the executor follows)* |
...
These are the digests as they stood at the invocation. §0's `frozen_body_sha256` row was re-anchored afterwards, when three slips inside §1–§7 were corrected in place; §8.7 records the amendment and both digests. Nothing here is a reading of the amended text.

**Execution note.** The self-check of §5 was run again *after* the invocation and passed at that time, reporting the receipt as present and consistent rather than as a failure of its own last item; this is the wart §0 records as repaired. After the in-place amendment recorded in §8.7 the same self-check reports the receipt's `protocol` and `probe` rows as `INCONSISTENT` and nothing else, because the receipt correctly records the digests of the body and the executor that ran: the binding was verified twice around the run on the artifacts that ran, and the amendment is a post-run edit disclosed rather than concealed. The executor changed only in its own body-digest constant, as §8.7 records.
...
**Stopping rule.** The receipt now exists, so a second invocation is refused in code before any arm is constructed, and no re-run was made. §0's digests were unchanged by this section, since §8 lies outside the frozen body; the body digest and the executed executor digest re-read after the run were the values §0 declared at that point, and §0's body row was re-anchored once afterwards for the amendment §8.7 records. The executor digest is the invocation's, unamended.

### 8.7 What this measures, what was amended, and what it does not
...
**Two slip sets in the frozen body, corrected in place after the invocation.** §6's steps row read $19\times600+100+1{,}834=13{,}334$ while §3 declares $20$ arms at $600$ steps, $13{,}934$ in total; the executor follows §3's arm table, gate 7 checked $13{,}934$ of $30{,}000$, and the receipt records the run's own number. And §2.3 and §4.3 both named the instrument $(\delta^\star,q)$ where §2.3's own definition is $(\delta^\star,\ell^\star)$; the receipt's instrument block carries both crossings under their own names. Both were corrected *in place* after the invocation at the user's instruction, each with a marker quoting the pre-amendment text and pointing here, because a reader of §6 and §4.3 should not be left with a wrong number and a wrong symbol. The consequence is stated rather than hidden: the body digest moved from `8f3fae24990b7b74632f08f3f6d4dde4e476e6d81f3ad8a5d903b37659c06268` (the value the receipt's `frozen_sources.protocol.sha256` records, and the value §0 carried at the invocation) to `c0d2fe37b8b4ee7673b1ec1a7c658f2bedb4eae6ad360516057d5ee9cdc843fa`, and the executor's digest moved with it from `fdc242b3999c153ad77e0186f342bf70628505adb681a0a9c48bb4276c33a5f9` to `be9f651d085bb4ee5d8f62ddb4db0a1714eb58c1b2106025d52f7fe2224f0eac` because it carries the body digest in its own `FROZEN_BODY_DIGEST` constant; the constant and its comment are the executor's only change, and the arm table, thresholds, oracle literals and schedule are byte-identical to the ones that ran. §8.1's source table quotes the digests as they stood at the invocation, and §0 is re-anchored to the standing values so `check_binding` passes again. Because the receipt must keep recording what actually ran, the self-check now reports exactly one problem—`INCONSISTENT ['protocol', 'probe']` on the receipt's rows. That is the check detecting two artifacts that moved after the run; the alternative, editing the receipt to make it agree, would destroy the record the two of them are.
