# Closed CP-Odd Reservoir Formation of a Non-Topological Carrier—Matched Finite-Volume Ledger
...
The reservoir begins in the finite-energy coherent state

$$
 a(0,r)=\sigma A\exp\left(-\frac{r^2}{2W^2}\right),\qquad
 \dot a(0,r)=0,
$$
...
$\kappa=0$. The source-free vacuum control sets $A=0$. The controls use the
same carrier draw and the same radial discretization as the primary arm.
...
+\frac12m^2\frac{u_{x,i}^2+u_{y,i}^2}{r_i^2}\\
&-\frac{\lambda}{2}\frac{(u_{x,i}^2+u_{y,i}^2)^2}{r_i^4}
+\frac{g}{2}\frac{(u_{x,i}^2+u_{y,i}^2)^3}{r_i^6}
+\frac12m_a^2\frac{v_i^2}{r_i^2}
-\kappa\frac{v_i u_{x,i}u_{y,i}}{r_i^3}
\Bigg]\Bigg\}.
\end{aligned}
$$
...
`computations/matter_formation_closed_cp_reservoir_v2.py` and
`computations/verify_matter_formation_closed_cp_reservoir_v2.py`. The verifier
