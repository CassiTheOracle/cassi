...
+\frac{\kappa}{2}a\,\operatorname{Im}(\Phi^2)
\right],
$$
...
$$
U(|\Phi|)=\frac12\left(m^2|\Phi|^2-\lambda|\Phi|^4+g|\Phi|^6\right).
$$
...
core, the ordinary signed charge

$$
Q=4\pi\int_0^R (u_x\dot u_y-u_y\dot u_x)\,dr
$$
...
second derivative supplies the regular origin condition. The evolved equations are
...
\ddot v&=v''-m_a^2v+\kappa u_xu_y/r.
\end{aligned}
\qquad
s=(u_x^2+u_y^2)/r^2.
$$
...
carrier at $t=0$. Let $e_n(r)=\sqrt{2/R}\sin(n\pi r/R)$ and
...
control uses the same draw with $A=0$. The negative arm uses the exact
CP-conjugate draw $u_x\mapsto u_x$, $u_y\mapsto-u_y$,
$\dot u_x\mapsto\dot u_x$, $\dot u_y\mapsto-\dot u_y$, together with
$a\mapsto-a$; it is not a second random realization.
...
The interaction energy $-4\pi\int_0^R\kappa(v/r)u_xu_y\,dr$ is included in the
full conserved action energy. The signed charge, core charge, rms radius and
exterior fraction use the displayed $Q$, with core $r\le6$ and exterior
$r\ge12$. A retained carrier remnant must satisfy all of:
For spatial support only, define the nonnegative carrier-support density
$s_{\Phi,+}=\tfrac12(\dot u_x^2+\dot u_y^2+u_x'^2+u_y'^2+m^2(u_x^2+u_y^2))
+\tfrac12g(u_x^2+u_y^2)^3/r^4$. The exterior support fraction is the integral
of $s_{\Phi,+}$ over $r\ge12$ divided by its integral over the full box. This
diagnostic is not the Hamiltonian energy and is not used in the energy-per-charge
criterion.
...
3. the late exterior support fraction from $s_{\Phi,+}$ is below $0.20;
...
The numerical calculation is `computations/matter_formation_cp_odd_pair_qball_v2.py`.
The independent reconstruction is
`computations/verify_matter_formation_cp_odd_pair_qball_v2.py`. The verifier
must rebuild the vacuum draw, finite-volume evolution, energy, charge, radial
...
parameters and the same vacuum modes evaluated through their analytic basis; it
is a resolution check, not a parameter scan.
...
finite-value, raw-state or independent-reconstruction failure is `INCONCLUSIVE`.
