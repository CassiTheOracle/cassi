...
The verifier refuses an existing output, manifest, staging path or source-snapshot directory. It reads each source once and writes those exact bytes to a staged snapshot tree. The complete tree and manifest are published by separate atomic renames; a successful return exposes both. A failure before publication removes every staging path and any snapshot tree published by that attempt; a manifest read-back failure retains the published manifest and snapshot tree for the `manifest-read` receipt.
...
After publication, the verifier reads the manifest back and requires its schema, snapshot root, source identities and receipt-local snapshot destinations to match the staged evidence. Failure to read or validate the published manifest gives receipt status `INCONCLUSIVE` with prerequisite stage `manifest-read` and retains the published evidence for inspection. Every current source and snapshot is then compared with those manifest records immediately after publication, after dependency imports and before scientific controls, and after the controls. A pre-control mismatch prevents scientific execution. A source-integrity or fixed-count mismatch gives receipt status `FAIL` with scientific classification `INCONCLUSIVE`. The outer runner requires exactly 70 named checks and separately requires the frozen verifier's declaration to be the integer 70, so the scientific schedule cannot lower or malform its own acceptance count. The result is `PASS` only when that count guard, every source-integrity comparison and both execution-module bindings succeed. Its scientific classification is `SUPPORTS-conditional compressible radiative-plasma closure`. A failed identity, conservation check, positivity check or rejection control gives `FAIL` and `CONTRADICTS`. A missing numerical dependency or source-read failure produces a source-bound `INCONCLUSIVE` receipt and stops interpretation.
