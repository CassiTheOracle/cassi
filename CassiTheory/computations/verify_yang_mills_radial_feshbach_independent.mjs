#!/usr/bin/env bun
/**
 * Source-independent reconstruction for the frozen continuous-SU(2) radial
 * Feshbach campaign.
 *
 * Usage:
 *   bun computations/verify_yang_mills_radial_feshbach_independent.mjs \
...
const PRIMARY_SCHEMA = "cassi.yang-mills.radial-feshbach.v1";
const RECEIPT_SCHEMA = "cassi.yang-mills.radial-feshbach.independent.v1";
const PROTOCOL_BASENAME = "yang-mills-radial-feshbach-prereg.md";
const PRIMARY_BASENAME = "verify_yang_mills_radial_feshbach.py";
const SELF_BASENAME = "verify_yang_mills_radial_feshbach_independent.mjs";
const EXPECTED_PROTOCOL_SHA256 = "ebf74019ad289f7619149baff4c1a82beefa3bac560aa8f6b43ce047c2dbb99c";
...
const FESHBACH_RESIDUAL_TOLERANCE = 5e-7;
...
function validatePrimaryEnvelope(primary) {
  ...
  if (primary.schema !== PRIMARY_SCHEMA) failures.push(`primary.schema must be ${PRIMARY_SCHEMA}`);
  if (primary.status !== "PASS") failures.push(`primary.status must be PASS, got ${String(primary.status)}`);
  if (!primary.protocol || typeof primary.protocol !== "object" || Array.isArray(primary.protocol) || primary.protocol.expected_protocol_sha256 !== EXPECTED_PROTOCOL_SHA256) {
    failures.push("primary.protocol must bind the frozen preregistration SHA-256");
  }
  if (!primary.inputs_manifest || typeof primary.inputs_manifest !== "object" || Array.isArray(primary.inputs_manifest)) {
    ...
    if (primary.inputs_manifest.schema !== "cassi.yang-mills.radial-feshbach.inputs.v1") failures.push("primary.inputs_manifest schema mismatch");
...
function validateCutoffRows(rows) {
  const failures = [];
  const map = new Map();
  if (!Array.isArray(rows)) return { failures: ["primary.cutoff_rows must be an array"], map };
...
function feshbachData(x, level, energy, N, terminal, referenceEnergy = energy) {
...
function checkManifestInputs(inputPath, identities, binding, primaryManifest) {
  ...
  try {
    ...
    if (data?.schema !== "cassi.yang-mills.radial-feshbach.inputs.v1") failures.push("adjacent inputs manifest schema mismatch");
...
function run(inputPath, outputPath) {
  ...
  let cutoffInfo = { failures: ["cutoff validation skipped"], map: new Map() };
    ...
    for (const x of X_VALUES) {
      ...
      for (const n of uniqueNs) {
        for (const energy of [...new Set([-1, -Math.sqrt(x), 0])]) {
          ...
          const slack = 2e-10 * Math.max(1, Math.abs(bounds.sigma), Math.abs(bounds.minimum));
          const inequalitiesPass = bounds.lower - slack <= bounds.sigma && bounds.sigma <= bounds.upper + slack && bounds.upper <= bounds.minimum + slack;
  ...
  try {
    for (const x of [4096, 65536]) {
      ...
      for (const j of LEVELS) {
        ...
        const computed = feshbachData(x, j, independent.eigenvalues[j], nIso, terminal);
        const primaryRow = feshbachInfo.map.get(`${xKey(x)}|${j}`);
        const primaryEigenvalue = primaryRow.eigenvalue;
        const primaryMargin = primaryRow.isolation_margin;
        const eigenvalueError = relativeError(computed.eigenvalue, primaryEigenvalue);
        const marginError = relativeError(computed.isolation_margin, primaryMargin);
        ...
        const row = {
          ...computed,
          primary_eigenvalue: primaryEigenvalue,
          primary_isolation_margin: primaryMargin,
          eigenvalue_relative_error: eigenvalueError,
          isolation_margin_relative_error: marginError,
          ...
          pass:
            ...
            && computed.self_energy >= computed.self_energy_lower - 2e-10 * Math.max(1, computed.self_energy)
            && computed.self_energy <= computed.self_energy_upper + 2e-10 * Math.max(1, computed.self_energy)
            ...
            && eigenvalueError < COMPARISON_TOLERANCE
            && marginError < COMPARISON_TOLERANCE,
        };
  ...
  const cfIndependentPass = checks.find((check) => check.name === "continued_fraction_derivative_bounds")?.passed === true && checks.find((check) => check.name === "finite_tail_linear_reconstruction")?.passed === true;
