...
SPATIAL_VERIFIER_SOURCE = COMPUTATIONS / "verify_matter_formation_wave_capture_spatial_convergence_repair.py"
...
SCHEMA = "matter-formation-spatial-convergence-primary-20260912-repair"
DEFAULT_OUTPUT = ROOT / "runs" / "20260912_matter_formation_wave_capture_spatial_convergence_repair"
GRID_SPECS = {
    "S0": (192, 0.5, 0.015625),
...
def run_smoke() -> int:
    base.run_smoke()
    baseline = bind_baseline()
    assert baseline["primary_receipt_sha256"] == BASELINE_RECEIPT_SHA256
    assert baseline["protocol_sha256"] == BASELINE_PROTOCOL_SHA256
    print(json.dumps({"smoke": "PASS", "baseline_bound": True, "baseline": baseline}))
    return 0
