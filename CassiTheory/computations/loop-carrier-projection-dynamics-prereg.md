...
**Stopping rule.** One invocation, spent. It finished inside its bound and wrote its receipt, so the further invocation §6 permits for an expired run is not available, and §5.3 forbids a re-run at another setting: this protocol has measured what it measured. What it establishes is that at this realization, these step sizes and these horizons the closure, transport, gate and mode energies hold at the arithmetic floor in thirteen of the fifteen arms — every arm except the two whose disagreement is injected by construction, the loop-varying gate of `covariance` and the split exterior velocity of `direction_split`, whose classes the receipt records as `above_budget_no_partner` — and that two of the four controls fail only because their declared reading is the terminal state of an attracting relaxation.
