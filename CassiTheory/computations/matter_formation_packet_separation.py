#!/usr/bin/env python3
"""Run the preregistered fixed-charge packet-separation basin probe."""
...
from matter_formation_packet_separation_spec import (  # noqa: E402
    ARM_SPECS,
    ARMS,
    COUPLED_CANDIDATES,
    CORE_RADIUS,
...
PREREG = COMPUTATIONS / "matter_formation_packet_separation_prereg.md"
SPEC_SOURCE = COMPUTATIONS / "matter_formation_packet_separation_spec.py"
...
VERIFIER_SOURCE = COMPUTATIONS / "verify_matter_formation_packet_separation.py"
SCHEMA = "matter-formation-packet-separation-primary-v1"
...
def run_smoke() -> int:
    ...
    for arm in ARMS:
        ...
        records.append({"arm": arm, "charge": charge, "center": metadata["center"], "overlap": metadata["initial_overlap"]})
...
def run_campaign(output: Path) -> dict[str, Any]:
    ...
    uncoupled = indexed[("G0", "uncoupled_c12")]
    ...
    if not comparison_pass:
        ...
        verdict = "EMERGES—conditional bound remnant in the declared separation basin"
    else:
        verdict = "DOES NOT EMERGE in the specified packet-separation calculation"
    receipt = {
        "schema": SCHEMA,
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--output", type=Path, default=ROOT / "runs" / "20260911_matter_formation_packet_separation")
