...
`computations/particle_stationary_precision_v4.py`. It uses the frozen
continuation engine in `computations/particle_stationary_precision_v3.py` with
only the v4 output paths, v4 manifest path, and source-validation rule (HP7)
...
`computations/verify_particle_stationary_precision_v4.py`. It does not import
the v4 primary driver. It parameterizes the independent v3 verifier with the
v4 output and manifest paths. Physical energy, gradient, virial, boundary,
...
endpoint scalars use the same tolerance (HP7).
...
`runs/20260902_particle_stationary_precision_v4/`:
...
| HP-D | Independent verification reproduces every checkpoint diagnostic, gate, artifact, and selection |
