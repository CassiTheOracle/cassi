# Fixed-Charge Momentum-Orientation Probe at Separation 12

## Status: Preregistered conditional calculation—September 2026

## Abstract

This calculation tests whether reversing the signed carrier momentum changes the outcome of two charged packets at fixed total charge and fixed separation. The packet centres remain at $\zeta=\pm12$, the packet width is $w=4$, and the carrier magnitude is $|k|=1$. The only changed preparation factor is the common phase-gradient sign: `pair_inward` sends the packets toward the origin, while `pair_outward` sends them away from the origin. The calculation measures only the declared axisymmetric scalar model. General three-dimensional stability, gravitational capture, physical size, quantum particle identity, spin, statistics, and a whole-bubble production mechanism remain outside the calculation.

## 1. Scope and lineage

The execution class is `packet_momentum_orientation_probe`. It is a fresh phase-sign intervention at fixed total signed charge $Q=16$, width $w=4$, centre separation $c=12$, and carrier magnitude $|k|=1$. The fixed-charge packet-separation receipt supplies the preparation lineage; this calculation does not modify or reinterpret that receipt.

The scientific output path is `runs/20260911_matter_formation_momentum_orientation`. The primary runner creates it exclusively, snapshots this protocol, the declarative preparation specification, both runner/verifier sources, and every declared action source. The verifier writes `verification.json` and a sibling independent-state archive. A failed or partial execution remains preserved.

A positive result would establish only a conditional remnant for one declared momentum orientation and fixed preparation. It would not establish a global charge minimum, an optimal orientation, a minimum packet count, physical size, or gravitational capture.

## 2. Fixed action and signed-momentum preparation
...
Every preparation has $f=1$ and $\dot f=0$. The `single16` control is a Gaussian of width $w=4$ centred at the origin with carrier frequency $\Omega_\infty$. Every pair is the sum of equal Gaussian envelopes of width $w=4$ centred at $\zeta=\pm12$, with

$$
\omega=\sqrt{\Omega_\infty^2+8k^2},\qquad |k|=1.
$$

The sign convention is fixed by the carrier field $z\propto e^{i(k\zeta-\omega t)}$ and the initialization $\dot z=-i\omega z$. For `phase_sign=+1`, the packet at $\zeta=-12$ has phase gradient $+1$ and moves toward increasing $\zeta$, while the packet at $\zeta=+12$ has phase gradient $-1$ and moves toward decreasing $\zeta$. This is `pair_inward`. `phase_sign=-1` reverses both gradients and is `pair_outward`.

The `antiphase_inward` arm uses the inward sign and multiplies the packet at $\zeta=-12$ by $-1$. The `uncoupled_inward` arm uses the inward sign with $h_C=0$ throughout. Every preparation is normalized once from the full single or superposed field to total signed charge $Q=16$. The nominal charge share of a separated pair is $Q/2=8$ before interaction.
...
```json
{
  "schema": "matter-formation-packet-momentum-orientation-protocol-v1",
  "execution_class": "packet_momentum_orientation_probe",
  "output": "runs/20260911_matter_formation_momentum_orientation",
  "preparation": {
    "total_charge": 16.0,
  ...
  "arms": {
    "all": ["single16", "pair_inward", "pair_outward", "antiphase_inward", "uncoupled_inward"],
    "coupled_candidates": ["pair_inward", "pair_outward", "antiphase_inward"],
    "comparison": ["single16", "pair_inward", "pair_outward", "antiphase_inward", "uncoupled_inward"],
    "independent": ["single16", "pair_inward", "pair_outward", "antiphase_inward", "uncoupled_inward"]
  },
  "comparison_observables": ["energy", "charge", "core_fraction", "core_rms", "core_energy", "shell_energy_fraction"],
  "binding_observable": "binding_ratio remains a formation predicate and is excluded from cross-resolution equivalence comparisons; stable observables carry the numerical comparison gate",
  "geometry": {
    "core_radius": 8.0,
...
The binding ratio remains a formation predicate. It is excluded from the equivalence comparison because its denominator becomes ill-conditioned when the retained core charge is below the $0.25$ persistence threshold. A candidate can form only if its rows satisfy the full persistence predicate, including the binding-ratio bound, on every declared grid.

A momentum-orientation formation candidate must be a coupled pair, pass preparation eligibility, pass numerical qualification, pass every persistence condition, and pass both spatial ($G0\to G1$) and time-step ($G0\to T1$) comparisons over the stable observable set. The single and uncoupled arms are controls.
...
The independent T1 reconstruction is compared to the matching primary T1 row at each archived time over the stable observable set. Every comparison also requires finite values for all declared observables, including the binding ratio. The maximum symmetric relative error over the stable set must remain below $0.05$.
...
2. With all numerical and evidence checks passing, at least one fully compared coupled orientation arm plus a numerically qualified uncoupled control that fails formation gives **EMERGES—conditional bound remnant in the declared momentum-orientation basin**.
3. With all numerical and evidence checks passing and no fully compared coupled orientation arm, the result gives **DOES NOT EMERGE in the specified momentum-orientation calculation**.

The execution ends at $t=48$. It does not extend time, change separation, change the carrier magnitude, change packet width or charge, alter a grid, relax a tolerance, or omit a failed arm. A negative or inconclusive result completes this preregistered calculation.
...
- `computations/matter_formation_momentum_basin_prereg.md`—fixed-charge magnitude ladder and comparison boundary.
