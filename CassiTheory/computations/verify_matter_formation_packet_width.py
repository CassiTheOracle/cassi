#!/usr/bin/env python3
"""Independently verify the fixed-charge packet-width basin probe."""
...
from matter_formation_packet_width_spec import (
    ARM_SPECS,
    ARMS,
    GRID_SPECS,
...
PREREG = COMPUTATIONS / "matter_formation_packet_width_prereg.md"
PRIMARY_SOURCE = COMPUTATIONS / "matter_formation_packet_width.py"
SPEC_SOURCE = COMPUTATIONS / "matter_formation_packet_width_spec.py"
...
SCHEMA = "matter-formation-packet-width-verification-v1"
PRIMARY_SCHEMA = "matter-formation-packet-width-primary-v1"
...
def preparation_contract(receipt: dict[str, Any]) -> dict[str, Any]:
    ...
    checks = {"row_key_set": observed == expected, "row_count": len(rows) == len(expected), "total_charge": True, "nominal_packet_charge": True, "arm_metadata": True, "center_metadata": True, "width_metadata": True, "wave_metadata": True, "phase_metadata": True, "orientation_metadata": True, "rule_control_metadata": True}
    for row in rows:
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--input", type=Path, default=ROOT / "runs" / "20260911_matter_formation_packet_width")
