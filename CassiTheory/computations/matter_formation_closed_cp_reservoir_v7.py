"""Run the preregistered finite-mode autonomous pulse formation calculation.

Run from the CassiTheory root:
    python computations/matter_formation_closed_cp_reservoir_v7.py \
        --output-dir runs/20260912_matter_formation_closed_cp_reservoir_v7
"""
...
PROTOCOL = ROOT / "computations/matter-formation-closed-cp-reservoir-v7-prereg.md"
SOURCE = Path(__file__).resolve()
DEFAULT_OUTPUT = ROOT / "runs/20260912_matter_formation_closed_cp_reservoir_v7"
SCHEMA = "cassi.matter-formation.closed-cp-reservoir.v7"
...
_RESERVOIR_COEFFICIENTS: tuple[np.ndarray, np.ndarray, float] | None = None


def reservoir_coefficients() -> tuple[np.ndarray, np.ndarray, float]:
    global _RESERVOIR_COEFFICIENTS
    if _RESERVOIR_COEFFICIENTS is not None:
        return _RESERVOIR_COEFFICIENTS
    fit_r = (np.arange(FIT_RADIAL_COUNT, dtype=np.float64) + 0.5) * RADIUS / FIT_RADIAL_COUNT
    fit_t = np.arange(0.0, T_FINAL + 0.5 * FIT_TIME_STEP, FIT_TIME_STEP)
    modes = np.arange(1, RESERVOIR_MODES + 1, dtype=np.float64)
    omega = np.sqrt(M_A * M_A + (modes * math.pi / RADIUS) ** 2)
    basis = math.sqrt(2.0 / RADIUS) * np.sin(
        modes[:, None] * math.pi * fit_r[None, :] / RADIUS
    )
    profile = RESERVOIR_AMPLITUDE * fit_r * np.exp(-0.5 * (fit_r / RESERVOIR_WIDTH) ** 2)
    envelope = np.where(
        fit_t < PULSE_DURATION,
        np.sin(math.pi * fit_t / PULSE_DURATION) ** 2,
        0.0,
    )
    target = envelope[:, None] * np.exp(1j * PULSE_FREQUENCY * fit_t[:, None]) * profile[None, :]
    time_modes = np.exp(1j * fit_t[:, None] * omega[None, :])
    design = (time_modes[:, :, None] * basis[None, :, :]).transpose(0, 2, 1).reshape(-1, RESERVOIR_MODES)
    coefficients = np.linalg.lstsq(design, target.reshape(-1), rcond=1.0e-10)[0]
    residual = float(np.linalg.norm(design @ coefficients - target.reshape(-1)) / max(np.linalg.norm(target), np.finfo(float).tiny))
    _RESERVOIR_COEFFICIENTS = coefficients, omega, residual
    return _RESERVOIR_COEFFICIENTS
...
def initial_state(
    ...
    if sign < 0:
        uy = -uy
        py = -py
    if reservoir_amplitude == 0.0:
        zeros = np.zeros_like(r)
        return ux, uy, zeros, zeros.copy(), px, py, zeros.copy(), zeros.copy()
    coefficients, omega, _fit_residual = reservoir_coefficients()
    basis = math.sqrt(2.0 / RADIUS) * np.sin(
        np.arange(1, RESERVOIR_MODES + 1, dtype=np.float64)[:, None]
        * math.pi
        * r[None, :]
        / RADIUS
    )
    scale = reservoir_amplitude / RESERVOIR_AMPLITUDE
    complex_field = scale * (coefficients @ basis)
    complex_momentum = scale * ((1j * omega * coefficients) @ basis)
    wr = np.real(complex_field)
    wi = sign * np.imag(complex_field)
    pwr = np.real(complex_momentum)
    pwi = sign * np.imag(complex_momentum)
...
def main() -> int:
    ...
    reservoir_coeffs, reservoir_omega, fit_residual = reservoir_coefficients()
    reservoir_coefficient_pairs = [
        [float(value.real), float(value.imag)] for value in reservoir_coeffs
    ]
    payload = {
        "schema": SCHEMA,
        ...
        "parameters": {
            ...
            "fit_radial_count": FIT_RADIAL_COUNT,
            "fit_time_step": FIT_TIME_STEP,
            ...
            "fit_relative_l2_residual": fit_residual,
            "fit_mode_frequencies": [float(value) for value in reservoir_omega],
            "reservoir_coefficients_real_imag": reservoir_coefficient_pairs,
        },
        "runs": runs,
        "controls": {
