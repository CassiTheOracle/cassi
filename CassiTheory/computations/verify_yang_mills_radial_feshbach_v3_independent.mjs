#!/usr/bin/env bun
/**
 * Source-independent reconstruction for the v3 continuous-SU(2) radial
 * Feshbach campaign.
 *
 * Usage:
 *   bun computations/verify_yang_mills_radial_feshbach_v3_independent.mjs \
...
const PRIMARY_SCHEMA = "cassi.yang-mills.radial-feshbach.v3";
const RECEIPT_SCHEMA = "cassi.yang-mills.radial-feshbach.independent.v3";
const REJECTED_PRIMARY_SCHEMA = "cassi.yang-mills.radial-feshbach.v2";
const MANIFEST_SCHEMA = "cassi.yang-mills.radial-feshbach.inputs.v3";
const REJECTED_MANIFEST_SCHEMA = "cassi.yang-mills.radial-feshbach.inputs.v2";
const PROTOCOL_BASENAME = "yang-mills-radial-feshbach-v3-prereg.md";
const PRIMARY_BASENAME = "verify_yang_mills_radial_feshbach_v3.py";
const SELF_BASENAME = "verify_yang_mills_radial_feshbach_v3_independent.mjs";
const EXPECTED_PROTOCOL_SHA256 = "40009de346204958d91158732cd5c3fdcc650e8b1717e75024518c6fe733ec48";
...
const REFERENCE_EIGENVECTOR_TOLERANCE = 1e-9;
...
function validatePrimaryEnvelope(primary) {
  ...
  if (primary.schema === REJECTED_PRIMARY_SCHEMA) failures.push(`v2 primary schema ${REJECTED_PRIMARY_SCHEMA} is rejected by the v3 verifier`);
  ...
  if (!primary.protocol || typeof primary.protocol !== "object" || Array.isArray(primary.protocol)) {
    ...
    if (primary.protocol.expected_protocol_sha256 !== EXPECTED_PROTOCOL_SHA256) failures.push("primary.protocol must bind the frozen v3 preregistration SHA-256");
    const amendment = primary.protocol.variable_ritz_amendment;
    if (!amendment
        || amendment.minimum_scheduled_cutoff !== null
        || amendment.finite_section_ritz_rule !== "q=min(3,N+1)"
        || amendment.reference_ritz_values !== ENERGY_LEVEL_COUNT
        || amendment.preserves_original_cutoff_schedules !== true
        || amendment.accepts_v2_receipts !== false) {
      failures.push("primary.protocol must declare the v3 variable-Ritz amendment");
    }
  }
  ...
  if (!primary.inputs_manifest || typeof primary.inputs_manifest !== "object" || Array.isArray(primary.inputs_manifest)) {
    ...
    if (primary.inputs_manifest.schema === REJECTED_MANIFEST_SCHEMA) failures.push(`v2 manifest schema ${REJECTED_MANIFEST_SCHEMA} is rejected by the v3 verifier`);
...
function expectedSchedules(x) {
  ...
  return { fixed: 8, C: Math.ceil(2 * q), iso: Math.ceil(4 * q), grow: Math.ceil(q * Math.log(2 + x)), half: Math.ceil(2 * Math.sqrt(x)) };
}

function finiteRitzCount(cutoff) {
  if (!Number.isInteger(cutoff) || cutoff < 0) throw new VerificationError("character cutoff must be a nonnegative integer");
  return Math.min(ENERGY_LEVEL_COUNT, cutoff + 1);
}
...
function validateCutoffRows(rows, spectrumMap) {
  const failures = [];
  const map = new Map();
  ...
  rows.forEach((row, index) => {
    ...
    const validX = isFiniteNumber(row.x) && X_VALUES.includes(row.x);
    const validSchedule = SCHEDULES.includes(schedule);
    if (!validX) failures.push(`cutoff_rows[${index}].x is not frozen`);
    if (!validSchedule) failures.push(`cutoff_rows[${index}].schedule is not frozen`);
    const validCutoff = Number.isInteger(row.cutoff) && row.cutoff >= 0;
    const expectedCount = validCutoff ? finiteRitzCount(row.cutoff) : null;
    const expectedIndices = expectedCount === null ? [] : Array.from({ length: expectedCount }, (_, j) => j);
    const expectedTerminal = validX ? expectedReference(row.x, "2M_ref") : null;
    if (validX && validSchedule) {
      const key = `${xKey(row.x)}|${schedule}`;
      const expected = expectedSchedules(row.x)[schedule];
      if (!validCutoff || row.cutoff !== expected) failures.push(`cutoff_rows[${index}] cutoff mismatch for ${key}`);
      if (map.has(key)) failures.push(`duplicate cutoff row ${key}`);
      else map.set(key, row);
    }
    if (!Number.isInteger(row.ritz_count) || row.ritz_count !== expectedCount) failures.push(`cutoff_rows[${index}].ritz_count must equal min(3,N+1)`);
    if (!Array.isArray(row.level_indices) || row.level_indices.length !== expectedCount || row.level_indices.some((value, j) => value !== expectedIndices[j])) failures.push(`cutoff_rows[${index}].level_indices must equal 0..q-1`);
    if (!Array.isArray(row.eigenvalues) || row.eigenvalues.length !== expectedCount || row.eigenvalues.some((v) => !isFiniteNumber(v)) || row.eigenvalues.some((v, j) => j > 0 && !(v > row.eigenvalues[j - 1]))) failures.push(`cutoff_rows[${index}].eigenvalues must contain q ordered finite values`);
    if (!Array.isArray(row.relative_errors) || row.relative_errors.length !== expectedCount || row.relative_errors.some((v) => !isFiniteNumber(v) || v < 0)) failures.push(`cutoff_rows[${index}].relative_errors must contain q nonnegative finite values`);
    ...
    if (!Number.isInteger(row.reference_terminal) || row.reference_terminal !== expectedTerminal) failures.push(`cutoff_rows[${index}].reference_terminal must equal the doubled reference terminal`);
    ...
    if (!Array.isArray(retained) || retained.length !== expectedCount || retained.some((v) => !isFiniteNumber(v) || v < -1e-12 || v > 1 + 1e-12)) failures.push(`cutoff_rows[${index}] retained-mode metrics must contain q probabilities`);
    if (!Array.isArray(discarded) || discarded.length !== expectedCount || discarded.some((v) => !isFiniteNumber(v) || v < -1e-12 || v > 1 + 1e-12)) failures.push(`cutoff_rows[${index}] discarded-mode metrics must contain q probabilities`);
    if (!Array.isArray(boundary) || boundary.length !== expectedCount || boundary.some((v) => !isFiniteNumber(v) || v < 0)) failures.push(`cutoff_rows[${index}] boundary-mode metrics must contain q nonnegative values`);
    if (!Array.isArray(massIdentity) || massIdentity.length !== expectedCount || massIdentity.some((v) => !isFiniteNumber(v) || v < 0 || v > 1e-10)) failures.push(`cutoff_rows[${index}] mass_identity_errors must contain q values at most 1e-10`);
    if (row.reference_eigenvectors_pass !== true) failures.push(`cutoff_rows[${index}].reference_eigenvectors_pass must be true`);
    if (row.pass !== true) failures.push(`cutoff_rows[${index}].pass must be true`);
    const arraysValid = expectedCount !== null
      && Array.isArray(row.eigenvalues) && row.eigenvalues.length === expectedCount && row.eigenvalues.every(isFiniteNumber)
      && Array.isArray(row.relative_errors) && row.relative_errors.length === expectedCount && row.relative_errors.every(isFiniteNumber)
      && Array.isArray(retained) && retained.length === expectedCount && retained.every(isFiniteNumber)
      && Array.isArray(discarded) && discarded.length === expectedCount && discarded.every(isFiniteNumber)
      && Array.isArray(boundary) && boundary.length === expectedCount && boundary.every(isFiniteNumber)
      && Array.isArray(massIdentity) && massIdentity.length === expectedCount && massIdentity.every(isFiniteNumber);
    if (validX && validSchedule && validCutoff && row.cutoff === expectedSchedules(row.x)[schedule] && row.ritz_count === expectedCount && row.reference_terminal === expectedTerminal && arraysValid) {
      const referenceRow = spectrumMap?.get(`${xKey(row.x)}|2M_ref`);
      if (!referenceRow || !Array.isArray(referenceRow.eigenvalues) || referenceRow.eigenvalues.length !== ENERGY_LEVEL_COUNT) {
        failures.push(`cutoff_rows[${index}] doubled-reference spectrum unavailable`);
        return;
      }
      const expectedRatio = row.cutoff / Math.pow(row.x, 0.25);
      const expectedLower = 4 * row.x * Math.pow(Math.sin(Math.PI / (2 * (row.cutoff + 2))), 2);
      const independentEigenvalues = finiteSectionEigenvalues(row.x, row.cutoff, expectedCount);
      const eigenvalueErrors = independentEigenvalues.map((value, j) => relativeError(value, row.eigenvalues[j]));
      const expectedRelativeErrors = row.eigenvalues.map((value, j) => relativeError(value, referenceRow.eigenvalues[j]));
      const relativeErrorFieldErrors = expectedRelativeErrors.map((value, j) => normalizedError(value, row.relative_errors[j]));
      const massIdentityFieldErrors = retained.map((value, j) => normalizedError(Math.abs(value + discarded[j] - 1), massIdentity[j]));
      const metricRows = expectedIndices.map((j) => {
        const vectorKey = `${xKey(row.x)}|${j}|${expectedTerminal}`;
        let data = vectorCache.get(vectorKey);
        if (!data) {
          data = referenceEigenvectorData(row.x, referenceRow.eigenvalues[j], expectedTerminal);
          vectorCache.set(vectorKey, data);
        }
        let independentRetained = 0;
        let independentDiscarded = 0;
        for (let n = 0; n <= expectedTerminal; n += 1) {
          const probability = data.values[n] * data.values[n];
          if (n <= row.cutoff) independentRetained += probability;
          else independentDiscarded += probability;
        }
        const independentBoundary = Math.sqrt(row.x) * Math.abs(data.values[row.cutoff + 1]);
        return {
          j,
          retained_probability: independentRetained,
          primary_retained_probability: retained[j],
          retained_normalized_error: normalizedError(independentRetained, retained[j]),
          discarded_probability: independentDiscarded,
          primary_discarded_probability: discarded[j],
          discarded_normalized_error: normalizedError(independentDiscarded, discarded[j]),
          boundary_residual: independentBoundary,
          primary_boundary_residual: boundary[j],
          boundary_normalized_error: normalizedError(independentBoundary, boundary[j]),
          reference_eigenvector_normalized_residual: data.normalizedResidual,
        };
      });
      const metricPass = metricRows.every((metric) =>
        metric.retained_normalized_error < COMPARISON_TOLERANCE
        && metric.discarded_normalized_error < COMPARISON_TOLERANCE
        && metric.boundary_normalized_error < COMPARISON_TOLERANCE
        && metric.reference_eigenvector_normalized_residual < REFERENCE_EIGENVECTOR_TOLERANCE);
      ...
      const reconstructionPass = eigenvalueErrors.every((error) => error < COMPARISON_TOLERANCE)
        && relativeErrorFieldErrors.every((error) => error < 1e-12)
        && massIdentityFieldErrors.every((error) => error < 1e-12)
        && massIdentity.every((error) => error <= 1e-10)
        && metricPass
        && ratioError < 1e-12 && lowerFormulaError < 1e-12
        && lowerBoundPass && primaryLowerBoundPass
        && row.reference_eigenvectors_pass === true && row.pass === true;
      reconstructionRows.push({
        x: row.x,
        schedule,
        cutoff: row.cutoff,
        ritz_count: expectedCount,
        level_indices: expectedIndices,
        eigenvalues: independentEigenvalues,
        primary_eigenvalues: row.eigenvalues,
        eigenvalue_relative_errors: eigenvalueErrors,
        expected_primary_relative_errors: expectedRelativeErrors,
        primary_relative_error_field_errors: relativeErrorFieldErrors,
        mass_identity_field_errors: massIdentityFieldErrors,
        metric_rows: metricRows,
        ratio_N_x_quarter: expectedRatio,
        primary_ratio_relative_error: ratioError,
        lower_bound_RF20: expectedLower,
        primary_lower_bound_relative_error: lowerFormulaError,
        lower_bound_slack: independentEigenvalues[0] - expectedLower,
        primary_lower_bound_slack: row.eigenvalues[0] - row.lower_bound,
        tolerance: RF20_SLACK,
        metric_comparison_tolerance: COMPARISON_TOLERANCE,
        pass: reconstructionPass,
      });
      if (!reconstructionPass) failures.push(`cutoff_rows[${index}] independent finite-section/RF20/cardinality/metric reconstruction failed`);
    }
  });
...
function stabilizedSturmPivot(pivot, pivotFloor) {
  if (!isFiniteNumber(pivot) || !isFiniteNumber(pivotFloor) || !(pivotFloor > 0)) {
    throw new VerificationError("Sturm pivot stabilization requires a finite pivot and positive floor");
  }
  if (Math.abs(pivot) >= pivotFloor) return pivot;
  // Preserve the side of every nonzero pivot.  An exact zero uses the
  // conventional negative infinitesimal so the Sturm count is one-sided.
  return pivot > 0 ? pivotFloor : -pivotFloor;
}

function tailSturmCount(x, start, terminal, value) {
  const pivotFloor = 1e-12;
  let count = 0;
  let pivot = stabilizedSturmPivot(kValue(start) + 2 * x - value, pivotFloor);
  if (pivot < 0) count += 1;
  for (let n = start + 1; n <= terminal; n += 1) {
    pivot = stabilizedSturmPivot(kValue(n) + 2 * x - value - (x * x) / pivot, pivotFloor);
    if (pivot < 0) count += 1;
  }
  return count;
}
...
function buildReceipt(status, checks, failures, payload) {
  const receipt = {
    ...
    protocol: {
      ...
      variable_ritz_amendment: {
        minimum_scheduled_cutoff: null,
        finite_section_ritz_rule: "q=min(3,N+1)",
        reference_ritz_values: ENERGY_LEVEL_COUNT,
        preserves_original_cutoff_schedules: true,
        accepts_v2_receipts: false,
      },
      x_values: [...X_VALUES],
      levels: [...LEVELS],
      schedules: [...SCHEDULES],
      ...
      reference_eigenvector_tolerance: REFERENCE_EIGENVECTOR_TOLERANCE,
...
function basePayload(inputPath) {
  return {
    ...
    scope: {
      included: ["isolated continuous-SU(2) class-function radial equation", "frozen six-x reference and variable-Ritz finite-section reconstruction", "right-tail Weyl continued fraction, derivative, bounds, and N_iso diagnostics"],
...
function run(inputPath, outputPath) {
  ...
  if (primary && typeof primary === "object") {
    ...
    cutoffInfo = validateCutoffRows(primary.cutoff_rows, spectrumInfo.map);
  ...
  if (primaryUsable) {
    try {
      ...
      for (const x of X_VALUES) for (const j of LEVELS) {
        ...
        const pass = Math.abs(data.norm - 1) <= 1e-12 && data.normalizedResidual < REFERENCE_EIGENVECTOR_TOLERANCE;
  ...
  payload.measured_summary = {
    ...
    cutoff_ritz_value_count: payload.cutoff_reconstruction_rows.reduce((total, row) => total + row.ritz_count, 0),
    cutoff_metric_max_normalized_error: payload.cutoff_reconstruction_rows.length
      ? Math.max(...payload.cutoff_reconstruction_rows.flatMap((row) =>
          row.metric_rows.flatMap((metric) => [
            metric.retained_normalized_error,
            metric.discarded_normalized_error,
            metric.boundary_normalized_error,
          ])))
      : null,
    cutoff_metric_reference_residual_max: payload.cutoff_reconstruction_rows.length
      ? Math.max(...payload.cutoff_reconstruction_rows.flatMap((row) =>
          row.metric_rows.map((metric) => metric.reference_eigenvector_normalized_residual)))
      : null,
