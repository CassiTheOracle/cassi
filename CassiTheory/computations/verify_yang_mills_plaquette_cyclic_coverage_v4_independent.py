#!/usr/bin/env python3
"""Audit the serialized degree-four plaquette-cyclic coverage receipt.

This verifier does not import the primary implementation.  It reconstructs the
word-count, rank, singular-value, nullity and finite-classification arithmetic
from the primary JSON receipt.  It is an arithmetic receipt audit, not a
second Hamiltonian reconstruction.
"""
...
PRIMARY_SOURCE = ROOT / "computations" / "verify_yang_mills_plaquette_cyclic_coverage_v4.py"
...
PRIMARY_RECEIPT = ROOT / "runs" / "yang_mills_plaquette_cyclic_coverage" / "verification-v4.json"
...
DEFAULT_OUTPUT = ROOT / "runs" / "yang_mills_plaquette_cyclic_coverage" / "verification-independent-v4.json"
...
def run(primary_path: Path, output: Path) -> dict[str, Any]:
    if not PROTOCOL.is_file() or not PRIMARY_SOURCE.is_file() or not primary_path.is_file():
        raise FileNotFoundError("missing degree-four protocol, primary source, or primary receipt")
    if not BASELINE_SOURCE.is_file() or not BASELINE_PROTOCOL.is_file():
        raise FileNotFoundError("missing degree-three baseline source or protocol")
    ...
    checks: list[dict[str, Any]] = [
        ...
        check(
            ...
            primary.get("schema") == "yang_mills_plaquette_cyclic_coverage_v4"
        ...
        check(
            ...
            and tuple(primary.get("cumulative_word_counts", [])) == EXPECTED_CUMULATIVE_WORD_COUNTS
            ...
            cumulative_word_counts=primary.get("cumulative_word_counts"),
            ...
            [
                ...
                check(
                    ...
                    and all(
                        ...
                        and all(np.isfinite(np.asarray(singular_values[degree], dtype=float)))
                        and all(
                            singular_values[degree][position] >= singular_values[degree][position + 1]
    ...
    record: dict[str, Any] = {
        "schema": "yang_mills_plaquette_cyclic_coverage_independent_v4",
        ...
        "scope": "independent arithmetic audit of the finite degree-four plaquette-cyclic coverage receipt; no second Hamiltonian solve",
