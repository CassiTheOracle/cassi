#!/usr/bin/env python3
"""Independently verify the Q16 two-packet threshold probe."""
...
PRIMARY_SOURCE = COMPUTATIONS / "matter_formation_two_packet_q16.py"
...
SCHEMA = "matter-formation-two-packet-q16-verification-v1"
PRIMARY_SCHEMA = "matter-formation-two-packet-q16-primary-v1"
...
def assemble_independent(grid: independent_dynamics.IndependentGrid, arm: str) -> tuple[torch.Tensor, torch.Tensor, float, float]:
    """Assemble the preparation independently from the declarative specification."""
    config = ARM_SPECS[arm]
    pair = bool(config["pair"])
    uncoupled = bool(config["uncoupled"])
    omega = independent_dynamics.OMEGA_INF if not pair else math.sqrt(independent_dynamics.OMEGA_INF**2 + OMEGA_OFFSET_SQUARED)
...
def mutation_control(input_dir: Path, row: dict[str, Any]) -> bool:
    ...
    return bool(
        ...
        and abs(after_energy - before_energy) > 1.0e-8
        and abs(after_energy - float(row["trace"][0]["energy"])) > 1.0e-8
    )
...
def preparation_contract(receipt: dict[str, Any]) -> dict[str, Any]:
    ...
    for row in rows:
        ...
        checks["nominal_packet_charge"] = checks["nominal_packet_charge"] and (
            (expected_nominal is None and observed_nominal is None)
            or (observed_nominal is not None and abs(float(observed_nominal) - expected_nominal) <= 1.0e-12)
        )
        arm = row.get("arm")
...
def run(input_dir: Path, output_path: Path) -> dict[str, Any]:
    ...
    first_row = receipt["rows"][0]
    mutation = mutation_control(input_dir, first_row)
    ...
    method_comparisons = [
        independent_dynamics._method_comparison(rows_by_arm.get(item.get("arm")), item)
...
def run_smoke() -> int:
    ...
    for arm in ARMS:
        ...
        records.append({"arm": arm, "charge": observed, "coupling": coupling})
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--input", type=Path, default=ROOT / "runs" / "20260911_matter_formation_two_packet_q16")
