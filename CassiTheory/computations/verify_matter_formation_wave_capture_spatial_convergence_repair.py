...
PREREG = COMPUTATIONS / "matter_formation_wave_capture_spatial_convergence_repair_prereg.md"
PRIMARY_SOURCE = COMPUTATIONS / "matter_formation_wave_capture_spatial_convergence_repair.py"
...
BASELINE_RECEIPT_SHA256 = "a202793ea2e6f91e8412461e723625ecaaf049819a85e0bf406ed1bfa52c92c4"
...
SCHEMA = "matter-formation-spatial-convergence-verification-20260912-repair"
...
def run(input_dir: Path, output_path: Path) -> dict[str, Any]:
    ...
    if receipt.get("schema") != "matter-formation-spatial-convergence-primary-20260912-repair":
...
def run_smoke() -> int:
    base.run_smoke()
    assert BASELINE_RECEIPT.is_file()
    assert sha256(BASELINE_RECEIPT) == BASELINE_RECEIPT_SHA256
    assert baseline_source_archive_binding()
    for path, expected in BASELINE_SOURCE_SHA256.items():
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--input", type=Path, default=ROOT / "runs" / "20260912_matter_formation_wave_capture_spatial_convergence_repair")
