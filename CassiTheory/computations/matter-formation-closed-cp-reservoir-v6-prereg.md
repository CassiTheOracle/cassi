# Autonomous Complex CP-Odd Reservoir Formation of a Non-Topological Carrier—Matched Finite-Volume Ledger
...
dynamical field with a finite-energy outgoing coherent packet; no
time-dependent source, damping term, field reset or parameter scan is used.
The positive and negative arms are the same initial draw under the declared
CP transformation.

The test asks whether the coupled action can produce a localized carrier
remnant after the reservoir has left the formation region. Charge creation
while the reservoir overlaps the carrier is measured as an internal
...
the stated outgoing reservoir preparation.
...
=(1,2,1.25,1,0.5,1,0.8,8),
\qquad
\Omega=\sqrt{m_a^2+K^2}=\sqrt2.
$$
...
outgoing packet preparation with $K=m$; $\Omega$ is its free
mass-plus-wavenumber frequency and is not fitted.
...
`computations/matter_formation_closed_cp_reservoir_v6.py` and
`computations/verify_matter_formation_closed_cp_reservoir_v6.py`. The verifier
rebuilds the vacuum draw, complex reservoir packet, finite-volume evolution,
ledgers, observables and all predicates without importing the primary program.
