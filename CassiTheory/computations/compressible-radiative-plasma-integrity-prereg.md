...
After publication the manifest is read back and its schema, snapshot root, source identities and receipt-local snapshot destinations are required to match the staged evidence. Every current source and snapshot is then compared with those manifest records immediately after publication, again after dependency imports and before scientific controls, and after the controls. A pre-control mismatch prevents scientific execution. A source or execution-module path, size or hash mismatch gives receipt status `FAIL` with scientific classification `INCONCLUSIVE`.
