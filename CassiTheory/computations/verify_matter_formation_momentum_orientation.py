#!/usr/bin/env python3
"""Independently verify the fixed-charge momentum-orientation basin probe."""
...
PRIMARY_SOURCE = COMPUTATIONS / "matter_formation_momentum_orientation.py"
...
SCHEMA = "matter-formation-packet-momentum-orientation-verification-v1"
PRIMARY_SCHEMA = "matter-formation-packet-momentum-orientation-primary-v1"
...
def preparation_contract(receipt: dict[str, Any]) -> dict[str, Any]:
    ...
    checks = {"row_key_set": observed == expected, "row_count": len(rows) == len(expected), "total_charge": True, "nominal_packet_charge": True, "arm_metadata": True, "center_metadata": True, "wave_metadata": True, "phase_metadata": True, "orientation_metadata": True}
    for row in rows:
...
def run_smoke() -> int:
    ...
    for arm in ARMS:
        ...
        records.append({"arm": arm, "charge": observed, "center": ARM_SPECS[arm]["center"], "wave_number": ARM_SPECS[arm]["wave_number"], "phase_sign": ARM_SPECS[arm]["phase_sign"], "orientation": ARM_SPECS[arm]["orientation"], "coupling": coupling})
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--input", type=Path, default=ROOT / "runs" / "20260911_matter_formation_momentum_orientation")
