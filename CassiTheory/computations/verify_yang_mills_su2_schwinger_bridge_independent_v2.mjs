...
const PROTOCOL = join(ROOT, "computations", "yang-mills-su2-schwinger-prereg-v2.md");
const PRIMARY_SOURCE = join(ROOT, "computations", "verify_yang_mills_su2_schwinger_bridge_v2.py");
const DEFAULT_INPUT = join(ROOT, "runs", "yang_mills_su2_schwinger_bridge", "verification-v2.json");
const DEFAULT_OUTPUT = join(ROOT, "runs", "yang_mills_su2_schwinger_bridge", "verification-independent-v2.json");
...
function rowScale(a) {
  return Math.max(...a.map((row) => row.reduce((sum, value) => sum + Math.abs(value), 0)));
}
...
function reconstruct(nMax, g2) {
  ...
  return {
    ...
    sign_tolerance: 256 * Number.EPSILON * (nMax + 1) * Math.max(1, rowScale(h)),
    times,
  };
}
...
function rowClose(expected, actual) {
  ...
  if (!closeEnough(expected.sign_tolerance, actual.checks[3].tolerance)) return false;
...
function main() {
  ...
  const checks = [];
  checks.push(check("primary_schema", primary.schema === "cassi.yang-mills.su2-schwinger-bridge.v2"));
  checks.push(check("primary_verdict", primary.verdict === "PASS"));
  checks.push(check("protocol_path", primary.protocol === "computations/yang-mills-su2-schwinger-prereg-v2.md"));
  checks.push(check("protocol_hash", primary.protocol_sha256 === sha256(PROTOCOL)));
  checks.push(check("primary_source_path", primary.source === "computations/verify_yang_mills_su2_schwinger_bridge_v2.py"));
  ...
  const record = {
    schema: "cassi.yang-mills.su2-schwinger-bridge.independent.v2",
    verdict: passed ? "PASS" : "FAIL",
    classification: "FINITE_REGULATOR_SCHWINGER_BRIDGE_INDEPENDENT",
    protocol: "computations/yang-mills-su2-schwinger-prereg-v2.md",
    protocol_sha256: sha256(PROTOCOL),
    source: relative(ROOT, SOURCE).replaceAll("\\", "/"),
    source_sha256: sha256(SOURCE),
    primary_source: "computations/verify_yang_mills_su2_schwinger_bridge_v2.py",
