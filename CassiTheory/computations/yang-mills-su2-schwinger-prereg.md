...
\Delta t=\tfrac12.
\]
The correlator at \(t+\Delta t\) is therefore also retained. The fundamental character is the sole observable.
...
4. strict positivity of the Perron ground-vector coefficients;
...
## 3. Finite-regulator positivity and observables

The off-diagonal entries of \(H_{N,g^2}\) are strictly negative on the connected nearest-neighbour character chain. After the ground-state phase is fixed, Perron–Frobenius positivity requires every ground coefficient to be strictly positive. The primary records the minimum coefficient and the independent implementation reconstructs the same condition.
...
`runs/yang_mills_su2_schwinger_bridge/verification.json`

The independent receipt is:

`runs/yang_mills_su2_schwinger_bridge/verification-independent.json`
...
Normalized matrix errors use
\[
\operatorname{err}_{\mathrm{mat}}(A,B)
=\frac{\|A-B\|_2}{\max\{1,\|B\|_2\}},
\]
and scalar errors use the corresponding absolute-difference formula. The primary tolerance is \(10^{-11}\); the independent comparison tolerance is \(10^{-9}\).
