#!/usr/bin/env python3
"""Independently reconstruct the closed CP-odd reservoir formation receipt."""
...
PRIMARY = ROOT / "computations/matter_formation_closed_cp_reservoir_v1.py"
PROTOCOL = ROOT / "computations/matter-formation-closed-cp-reservoir-v1-prereg.md"
SCHEMA = "cassi.matter-formation.closed-cp-reservoir.v1.verification"
RADIUS = 48.0
N_PRIMARY = 384
N_FINE = 768
DT_PRIMARY = 0.003
...
def make_initial(
    ...
    if sign == -1:
        uy = -uy
        py = -py
    v = sign * amplitude * np.exp(-0.5 * (r / RESERVOIR_WIDTH) ** 2) * r
...
def wave_operator(values: np.ndarray, momentum: np.ndarray, dr: float) -> np.ndarray:
    return static_operator(values, dr) - momentum / dr
...
def derivative(
    ...
    return (
        ...
        py,
        pv,
        wave_operator(ux, px, dr) - coefficient * ux + coupling * v * uy / r,
        wave_operator(uy, py, dr) - coefficient * uy + coupling * v * ux / r,
        wave_operator(v, pv, dr) - M_A * M_A * v + coupling * ux * uy / r,
    )
...
def total_energy(
    ...
    quadratic = -0.5 * dr * float(
        ux @ static_operator(ux, dr) + uy @ static_operator(uy, dr) + v @ static_operator(v, dr)
    )
    local = (
...
def metrics(
    ...
    return {
        ...
        "source_rate": 4.0 * math.pi * coupling * float(np.sum((v / r) * (ux * ux - uy * uy)) * dr),
        "boundary_charge_rate": 4.0 * math.pi * float(-ux[-1] * py[-1] + uy[-1] * px[-1]),
        "boundary_energy_rate": -4.0 * math.pi * float(px[-1] ** 2 + py[-1] ** 2 + pv[-1] ** 2),
    }
...
def rerun(
    ...
    archive = {key: [] for key in ("ux", "uy", "v", "px", "py", "pv")}
...
def main() -> int:
    ...
    if primary.get("schema") != "cassi.matter-formation.closed-cp-reservoir.v1":
    ...
    expected = {
        "vacuum_control": (N_PRIMARY, DT_PRIMARY, +1, 0.0, KAPPA),
