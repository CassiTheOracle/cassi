...
| `hypotheses/exoplanet-phi-spacing.md` | Hypothesized—August 2026 | Disk mechanism = the bubble-shell ring ladder ($r_k = R\varphi^{-k}$, Derived conditional); DSHARP gap-ratio test pending an auditable receipt; confirmed-catalog Kepler period-ratio classifier **INDETERMINATE** ($z_{\rm win}=1.087$, 46/562 in support) and scientific verdict **INCONCLUSIVE** because the $\varphi^{3/2}$ window overlaps the conventional wide-of-2:1 excess; channel reading leaves disk gas coherence-coupled while detached orbital bodies need not preserve the ladder |
