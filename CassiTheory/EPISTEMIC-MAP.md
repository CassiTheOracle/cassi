...
| `foundations/matter-completion-boundary.md` | Derived conditional boundary / Hypothesized physical realization / Mapped carrier coupling / Tested reduced receipts, prepared binding, finite-grid spectra and parent correspondences—September 2026 | Nine conditional sectors connect the exterior to the fixed-charge fluctuation problem. Empty-sector preservation, the Cartesian ultraviolet obstruction and separate prepared scalar binding delimit the first-order evidence. A Hypothesized parent reproduces Gaussian pair production, has a restricted neutral stationary-localization obstruction and supports finite-grid radial charged curvature, with INCONCLUSIVE aggregate radial-domain qualification. Its selected population-256 angular and phase sectors support on four grids, while one failed domain comparison leaves combined scalar-parent spatial qualification INCONCLUSIVE. Physical production, quantum content, normalization and full dynamics remain open |
...
| Document | Status | Summary |
|----------|--------|---------|
| `foundations/unified-lagrangian.md` | Hypothesized extended action and candidate common lapse / Derived conditional relative conversion-clock identity—September 2026 | Optional extended action assembly around the canonical real-density pair and rank-one conversion; the displayed Dirac chiral-scalar projection and mixing expressions are excluded by dimensional, conjugacy, and Hermiticity obstructions, so no physical $\kappa_s$ interaction follows. $\mathcal R=(1-q)/(1-q_0)$ is the exact relative conversion-clock rate. Candidate physical time assigns this factor to one common lapse, $d\tau_{\mathrm{phys}}(x)/d\tau_\star=(1-q(x))/(1-q_\star)$. A constant common lapse cancels from the ideal free-fall phase when duration is expressed in the same physical clock; CT-2 therefore requires resolved lapse contrast and independent clocks. Clock-versus-kinetics factorization remains open |
