...
QMQ3=`SUPPORTS`. The empirical action, interacting vacuum, renormalized Dirac
sea, nonradial thermal formation, confinement, observable nucleon map and
baryogenesis remain supplied or open; QMQ4 and QMC6 return `FAIL`. Physical
matter formation remains **Hypothesized/Open**
...
| `turbulence/navier-stokes-deformation-covariance.md` | Derived exact covariance, inverse-metric, projection and conditional continuation identities / Open active Rayleigh-quotient bound—September 2026 | The Constantin–Iyer forward-deformation covariance obeys a closed parabolic equation, and its harmonic inverse cancels vortex stretching while completing viscosity into a nonnegative covariant square. A projection theorem identifies weighted enstrophy with retained sampled initial-vorticity energy and accumulated dissipation with the endpoint regression residual; 38 source-bound exact checks pass. Homogeneous extension excludes coercivity from positivity, determinant or unrestricted deformation algebra alone, while periodic shear has inactive-plane covariance and active distortion one. A uniform initial-$H^3$-controlled bound on the data-conditioned active quotient would imply continuation and remains open. |
