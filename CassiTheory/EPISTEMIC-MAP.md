...
| `foundations/loop-to-bubble-projection-theorem.md` | Derived conditional projection and population spectrum; Derived regulated pure-gauge identities, exact isolated-square radial Feshbach transfer and weak-coupling character-cutoff theorem, exact fixed-link three-sphere geometry, two-scale conditional-score recurrence, $H^{-1}$ transport-score margin transfer, residual-recovery Gramian and score-penalty separation, exact fixed-boundary gauge-fibre support, conditional full-holonomy block theorem, and exact bare-cylindrical refinement obstruction; Hypothesized microscopic physical identification—September 2026 | The shared-loop law projects to the canonical PDE and fills the affine bubble volume. In the separate source-free $SU(2)$ comparison, exact finite gaps are true-vacuum gauge-invariant Poincaré rates. Haar path blocking is Haar-isometric and has exact path-length-weighted electric compression, while magnetic characters exclude the bare cylindrical full-Hamiltonian block; the isolated-square radial problem has an exact Feshbach pencil and sharp bare-cutoff condition $gN\to\infty$. A round spatial $S^3_R$ has free coexact frequency $2/R$, which vanishes as $R\to\infty$ and is not the interacting Hamiltonian gap. Fixed-metric $SU(2)$ links have exact character spectrum; the auxiliary one-link Wilson Bakry–Émery curvature bound is positive only for concentration $\beta<2$, with no identification of $\beta$ to the Hamiltonian coupling or exact vacuum. The factor-two exact-vacuum recurrence has an all-function $L^2$ score form and a sharper conditional-Poisson form $\lambda_f\geq C_{-1}^{-1}$. Its exact margin budget shows that nonzero minimum transport cost consumes both coarse and vertical margin. The residual Gramian identifies the optimal tensorization constant and shows that finite kernel rigidity can coexist with a vanishing recovery floor; the transported score remains a separate upper penalty. The 118/90 geometry, 86/32 transport and 58/30 recovery controls pass. Uniform interacting-vacuum recovery, transport, fibre and coarse-margin bounds, continuum existence, a regulator-independent mass gap and Cassi identification remain open |
