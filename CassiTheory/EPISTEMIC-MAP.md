...
| `foundations/unified-lagrangian.md` | Hypothesized extended action / Derived conditional relative conversion-clock identity—August 2026 | Optional extended action assembly around the canonical real-density pair and rank-one conversion; $\mathcal R=(1-q)/(1-q_0)$ is the exact relative conversion-clock rate, while its use as a universal breath/proper-time lapse remains Hypothesized; Dirac/particle, GR, SM, and cross-coupling sectors remain conditional; the named C-class/framework convention $\lambda=0.1$ is Asserted while the implementation class default is $\lambda=0.02$; $\lambda=1/(2w)$ is a Hypothesized Wu Xing linkage, and $c,\hbar,G$ remain external |
...
| `foundations/quantum-measurement-derivation.md` | Derived conditional (regulated quantum mechanics); Hypothesized (CassiFI physical identification)—August 2026 | Finite CassiFI configuration-space quantization; linear Schrödinger evolution, tensor-product entanglement, current-guided actual configuration, topological retention records, and unique local equivariant Born density. Quantum equilibrium remains an explicit statistical postulate; the sodium-nanoparticle limit has $R_\ell=1$ |
