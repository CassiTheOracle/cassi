...
registered Cassi inputs unchanged. CP selection returns `DOES NOT EMERGE`; the
fitted cascade endpoint and microscopic Cassi origin remain open.
...
| `foundations/loop-to-bubble-projection-theorem.md` | Derived conditional projection and population spectrum; Derived regulated pure-gauge identities, exact isolated-square radial Feshbach transfer and weak-coupling character-cutoff theorem, exact fixed-boundary gauge-fibre support, conditional full-holonomy block theorem, and exact bare-cylindrical refinement obstruction; Hypothesized microscopic physical identification—September 2026 | The shared-loop law projects to the canonical PDE and fills the affine bubble volume. In the separate source-free $SU(2)$ comparison, exact finite regulated gaps are gauge-invariant Poincaré rates and a conditional full-holonomy theorem isolates sufficient local-rate, tensorization and cover estimates. A normalized-Haar path-holonomy pullback is an exact endpoint-gauge-covariant electric isometry, but four unresolved plaquette characters on the fixed $2\times2$ refinement give $\|Qh_fJ1\|_2=2x_f$ and exclude that bare map as a full-Hamiltonian intertwiner. The isolated-square class-function operator has an exact energy-dependent Feshbach pencil and fixed-window isolation. Its fixed low bare-cutoff levels converge with vanishing discarded mass exactly when $N/x^{1/4}\to\infty$, equivalently $gN\to\infty$; fixed and finite-scaled cutoffs fail as bare weak-coupling limits. With all eight outer links fundamental, the internal Gauss fibre has exactly 14 states once $j_{\max}\geq1$ and conditional electric spacing $2g^2/a$; plaquette multiplication exits the fixed boundary sector. Interacting boundary-sector recoupling, volume-uniform resolvent control, thermodynamic and continuum limits, a regulator-independent mass gap and Cassi microscopic identification remain unresolved. |
...
| `foundations/matter-completion-boundary.md` | Derived conditional nine-sector boundary, microscopic non-identifiability, continuum minimizer-set stability, chiral-zero obstruction, regular baryon current, finite-cutoff confinement and CP-selection theorem / Mapped carrier and baryon normalizations / Calibrated resolved-flavour leptogenesis scale / Tested conditional empirical history and whole-bubble selector boundary / Hypothesized Cassi microscopic completion—September 2026 | The empirical Standard Model plus two-singlet branch connects a gauge-projected thermal state to observable nucleons and is robust to the tested coarse initial-state changes. Exact CP conjugation reverses $\eta_B$ with identical registered Cassi data; CP selection is `DOES NOT EMERGE`. The minimal rank-two benchmark also differs from the separate Mapped three-nonzero-mass spectrum. Complete Cassi-origin matter formation remains `FAIL`. |
...
| `turbulence/cassi-fluid-phase-current-hydrodynamics.md` | Derived conditional current and topology identities / Tested rotational, memory and first-order coercivity boundaries / Open microscopic viscosity and arbitrary-flow closure—September 2026 | A normalized doublet gives Mermin–Ho vorticity; a positive global chart has zero integrated helicity, while a full Hopf doublet and two fixed scale bands realize nonzero-helicity sectors. Scalar diffusion equals viscosity for one fixed-winding Beltrami family and fails generally by a nonzero commutator. Exact closed-mode elimination supplies recurrent memory and exterior-state dependence, not a derived positive coefficient; 227 checks pass. A separate 84-check analysis proves that bounded first-order positive-doublet energy need not control enstrophy or the critical scalar-Beltrami residual. |
...
| `foundations/baryon-asymmetry.md` | Hypothesized Cassi mechanism and CP selector / Mapped $\eta$ exponent / Calibrated resolved-flavour two-singlet scale / Tested whole-bubble selector boundary—September 2026 | The empirical comparator reproduces the observed magnitude and survives QCD under supplied inputs. Coarse initial differences wash out, while exact CP conjugation reverses the sign with identical registered Cassi data. CP selection returns `DOES NOT EMERGE`; the fitted cascade endpoint and microscopic Cassi origin remain open. |
