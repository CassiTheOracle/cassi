...
| `foundations/qi-as-spatial-spacing-signal.md` | Derived conditional (phase/coherence structure) / Hypothesized (channel-transmission claim) | Qi is the spatial-spacing signal: coherence ripples differently than it moves; the condensation clumping is the lattice; the ring ladder is a phase ladder; matter carries the spacing only through coherence-coupled channels (gas/condensate), not gravity-dominated matter channels (orbital dynamics)—resolving P53 SUPPORTS (coherence channel) vs P54 INDETERMINATE (matter channel) as consistent |
