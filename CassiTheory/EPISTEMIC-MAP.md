...
| `foundations/particle-stationary-action-closure.md` | Hypothesized temporal completion / Derived conditional action, scalar-reduction, parent-vacuum, dilation and fixed-charge identities / Mapped carrier coupling / Tested prepared binding, finite-grid spectra and parent correspondences—September 2026 | The first-order carrier preserves an empty sector. Separate prepared scalar binding leaves smooth spatial stability INCONCLUSIVE. A positive-inertia parent has verified Gaussian pair correspondence, an exact classical global-vacuum condition and a neutral stationary-dilation obstruction. Its rank-one charge penalty gives independently verified positive radial curvature in 24 frozen embeddings; aggregate radial-domain qualification remains INCONCLUSIVE. The normalization campaign supports conditional one-mass normalization nonuniqueness across three $a$ values, while the scalar electron-core assignment and chiral-scalar identity are CONTRADICTS; physical particle identity remains open |
...
| `foundations/matter-completion-boundary.md` | Derived conditional boundary and non-identifiability / Hypothesized physical realization / Mapped carrier coupling / Tested reduced receipts, prepared binding, finite-mode production and conditional formation dynamics; nonlinear comparison INCONCLUSIVE—September 2026 | Nine conditional sectors connect the exterior to the fixed-charge fluctuation problem. The evidence distinguishes exact empty-sector preservation, a Cartesian ultraviolet obstruction, prepared scalar binding, homogeneous finite-mode fermion excitation, continuum and energetic restrictions, periodic carrier amplification, faster spatial mediator instability and prepared local charge separation. The frozen nonlinear comparison is INCONCLUSIVE. Scalar and Dirac parents give the same slow carrier equation with different spin and statistics, so a selected action, state, normalization, continuum-localized formation and particle discriminator remain required |
