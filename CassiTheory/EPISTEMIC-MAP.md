...
| `turbulence/navier-stokes-replica-coherence.md` | Derived exact replica, covariance, temporal-rank and conditional continuation identities / Open recovered-envelope and cross-scale bounds—September 2026 | Two independent stochastic Cauchy replicas recover enstrophy through overlap and centred covariance through disagreement. The covariance source is twice viscous palinstrophy, and its Duhamel representation retains every source time and subsequent directional deformation. Global coherence obeys a replicator-diffusion law. Volume preservation gives a sharp instantaneous-source determinant bound; applying the determinant root after accumulation gives a stronger temporal-rank functional and sharpened enstrophy envelope. An exact rank-two periodic control has zero instantaneous determinant functional while its accumulated covariance becomes full rank on an open set; the separate ABC Beltrami heat control has zero production remainder. Periodic shear retains its common missing direction. The continuation bridge is explicit: a bounded enstrophy envelope gives $u\in L^4_tL^6_x$ by the periodic kinetic-energy identity and Sobolev embedding, so Prodi–Serrin supplies the finite-endpoint step. An indexed common-noise kernel diffuses only in the center coordinate; an absolute diagonal bound would control the active quotient, while increment bounds leave spatially constant extension unbounded. Sixty source-bound symbolic and exact-control checks pass; a separate source-bound Galerkin endpoint-control schedule passes 8 of 8 checks for zero and positive finite-mode remainders, near-rank full-dimensional controls, a common $H^3$ bound, and critical scaling. A source-bound 36-run short-time trajectory probe passes every prescribed check, with positive integrated remainders for $u_b$ and $u_{0.1}$ at $N=2,4,8$ and zero shear and ABC heat controls; direct strain agreement and timestep/grid refinements also pass. Uniform production-relative recovery, signed shell-stretching, recovered-envelope, cutoff-uniform Galerkin time-integral, and arbitrary-data regularity bounds remain **UNRESOLVED**. |
