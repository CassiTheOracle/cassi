...
- **Mapped**—placement (cascade step, exponent, offset, candidate, normalization) selected or fitted to data; the fit is recorded in the Fit-Status Ledger (`parameter-inventory.md` §10).
...
tier and does not upgrade it. Use one of the five evidential tiers defined above for claims; reserve Creative for exploratory applications outside the ladder.
...
The particle sector's continuum evidence is conditional static self-binding
at selected prepared $Q_C=16$ and $256$, reproduced by independent
...
electromagnetic charge, or physical projection interaction follows.
The obstruction is witnessed by $B_R=L^\dagger R$ and
$B_L=R^\dagger L=B_R^*$: real nonnegative values are equal, so a nonzero
$\varphi$ ratio is impossible. Scalar and Dirac $2\pi$ transformations are
$+1$ and $-1$, respectively, subject to the topological-quantization caveat;
...
coefficient, parent action normalization and interacting formation mechanism
remain unselected (`computations/matter-formation-continuum-report.md` §8).
...
The fixed-signed-charge energy and rank-one radial Hessian correction are
**Derived conditional** bounds (§8.10). All 24 frozen embeddings have
...
independently matched eigenvalues. Seven of eight spatial comparisons pass;
...
The selected positive-inertia scalar action also has a **Tested** finite-time
radial condensation result. Supplied signed charge $\mathcal Q_a=256$ starts in
diffuse Gaussian carrier clouds of widths $w=4,8$ (distinct from prepared
population $Q_C=256$) with real mediator $f=1$;
the coupled evolution retains a charged core inside $r<8$ through $t=48$.
Independent RK4 means over $32\le t\le48$ are 0.7476513 and 0.5637353,
versus 0.0545181 and 0.1537575 in matched $h_C=0$ controls. Across every
sampled late time, minima are 0.7459683 and 0.5203347, while maximum central
...
energy ledger are **Tested**, with 1,185 independent checks passing. Its
sudden-source continuum divergences, specified static subtraction, leading
two-body no-binding bound and local static energy unboundedness are **Derived
conditional** on the declared model. Dynamical renormalization, nonlocal
...
sector supplies a physical relative $S^2$ at fixed nonzero norms. Joint gauge
...
stationary size. The full spatial calculation is **Derived conditional** with
verdict `SUPPORTS—full-variation identities and the hard-Hopf soft-amplitude
obstruction`: every exact hard-norm nonzero-Hopf adjoint in the stated soft
action has negative amplitude first variation, and the included hard
Finkelstein–Rubinstein loops contract there. General amplitude-relaxed
metastability remains open
(`computations/matter-formation-continuum-report.md` §§18–20).

For the supplied scalar temporal parent, the periodic Hill/Floquet reduction,
spatial mediator dispersion, signed-charge continuity law and exact empty
sector are **Derived conditional**. Periodic carrier amplification, the faster
spatial mediator instability and prepared local charge separation are
**Tested**. The frozen nonlinear comparison is `INCONCLUSIVE`; its post-hoc
period-sampled reconstruction diagnoses phase aliasing without altering the
verdict (`computations/matter-formation-continuum-report.md` §§25–28).

The scalar/Dirac slow-sector equivalence is a **Derived conditional**
non-identifiability result. The two parents share the registered Schrödinger
carrier equation and differ in spin and canonical statistics. A microscopic
action, state-selection rule, physical normalization, continuum-localized
forming evolution and particle discriminator remain additional inputs.
...
Delta masses. The finite-domain stationary hedgehog, conservative relaxation
of broadened prepared $B=1$ data and six particle comparisons are **Tested**.
Two comparisons support their thresholds and four absolute observables
contradict the 10-percent criterion. Independent reconstruction passes all 78
checks. The resulting `ADOPT—Mapped conditional leading colour-neutral chiral
baryon benchmark` verdict applies only to the supplied model. The
six-requirement physical-completion adjudication returns `FAIL`
(`computations/matter-formation-continuum-report.md` §§30–31).

The matter-completion boundary also records a separate massive bubble-lattice
comparison in §14. An added $O(4)$ field on $S^3$ uses the periodic primitive
geometry and a pion-selected $\mu$; nucleon and Delta references map the two
massive-model coefficients and its length unit. The stationary radial profile
has finite radial energy with a $9.11\times10^{-12}$ virial residual, but
all six target-bearing out-of-fit diagnostics contradict their inherited
thresholds. Its conserved stress/current belong to the added action; no
canonical Cassi stress exchange follows. The finite-site space
...
renormalization and physical particle identity remain open.
The degree-zero excitation returns `DOES NOT EMERGE` through $T=4$ under a
qualified signed-preimage calculation. The geometric controls, admissibility,
net-degree conservation and numerical comparisons pass, but no retained
sample contains a resolved degree-$+1$/degree-$-1$ pair for all 16 regular
values. The verdict is confined to the supplied action, impulse and sampled
interval (`computations/matter-formation-continuum-report.md` §32.7). Physical
matter formation remains **Hypothesized/Open**.

The independently mapped step-95 QCD length is **Mapped/Rejected** as a
size-first normalization of the massive comparison. Its
$\ell_{95}=1.1543452099944254\ \mathrm{fm}$ value misses the empirical
isoscalar radius by $50.1099\%$, fixing the analytic verdict to
`CONTRADICTS` and the assignment to `REJECT`. The imposed-length profile
problem has one independently reproduced conditional root, but its withheld
nucleon and Delta masses miss observation by $58.2132\%$ and $44.3648\%$.
The calculation supplies neither a physical particle size nor a formation
mechanism (`computations/matter-formation-continuum-report.md` §77;
`foundations/matter-completion-boundary.md` §22).

The normalized-chiral topology-change obstruction is **Derived conditional**
on the selected $O(4)$ field and normalized-Skyrme action, with its exact
local formulas and anisotropic lower bound independently **Tested**. A
generic nondegenerate winding-changing zero has inverse-distance quartic
energy, while the formation code's max-normalization has a different leading
coefficient and therefore no regulator-independent continuum limit.
QZO1–QZO4 return `PASS`, QZO5 returns `CONTRADICTS`, and complete physical
matter formation remains **Hypothesized/Open**
(`computations/matter-formation-continuum-report.md` §78;
`foundations/matter-completion-boundary.md` §23).

The regular quark–meson carrier identity is **Derived conditional** on its
empirical two-flavour action, while the four-grid regulator extrapolation,
two-sided shooting endpoint, fixed-$B$ binding and reduced barrier are
independently **Tested**. The exact vector current remains defined at the
chiral zero. The continuum-qualified nodeless level is
$42.74439864\pm0.20\ \mathrm{MeV}$, the qualified binding margin exceeds
$291\ \mathrm{MeV}$ and the fixed-family conservative reduced barrier is
$18.2800415\ \mathrm{MeV}$. QMQ1=`PASS`, QMQ2=`SUPPORTS` and
QMQ3=`SUPPORTS`.

The finite-cutoff chromodielectric extension is **Derived conditional** as an
action-level confinement mechanism and independently **Tested** at its
dimensionless verification point. It defines the two-flavour sea, gives
infinite-volume divergent energy for every nonzero total Cartan charge and
admits a finite color-neutral three-arm flux witness. RCF1–RCF4 return
`PASS`; RCF5=`ADOPT`.

The supplied empirical action's first joint valence-plus-field minimization is
independently **Tested**. Its compact three-colour endpoint fails stationarity,
exterior-tail, virial and low-dielectric formation criteria, while a lower
topological boundary branch prevents a ground-state interpretation. The
independent-cell radial follow-up is also **Tested**: its first bounded trial
loses every positive nodeless occupied state, so the fixed empirical action
does not yet possess a regulator-qualified unconstrained radial baryon. A
separate renormalized Standard Model plus two-singlet history is
**Calibrated/Tested** from a gauge-projected thermal state through an observable
nucleon map. Its resolved-flavour benchmark gives the observed baryon
magnitude, while exact CP-conjugate data give the opposite asymmetry with all
registered Cassi inputs unchanged. The phase-bearing extension contains a
CP-odd integrated spatial helicity, but its positive homogeneous action leaves
opposite signs degenerate and has no negative linear mode. CP and handedness
selection return `DOES NOT EMERGE`; the fitted cascade endpoint, nonlinear
domain selection, baryon-production coupling and microscopic Cassi origin
remain open
(`computations/matter-formation-continuum-report.md` §§79, 81–86;
`foundations/matter-completion-boundary.md` §§24, 26–30).

The regular polynomial Gram stabilizer is **Derived conditional** as a
nonnegative chiral-zero-finite static operator. Its Cartesian reconstruction,
radial energy, three-grid stationarity and amplitude-collapse path are
independently **Tested**. RPS1–RPS2 return `PASS`, RPS3–RPS5 return `FAIL`,
and the operator decision is RPS6=`REJECT`. The verdict is scoped to the
frozen physical point, hedgehog class and numerical protocol
...
|----------|--------|---------|
| `foundations/cascade-suppression-formula.md` | Derived conditional product algebra / Hypothesized uniform input and signal or routed-port maps—September 2026 | The fixed-point Yang density fraction is exactly $\varphi^{-1}$; the uniform attenuation family declares that value as its per-step input. A routed two-port branch realizes $\varphi^{-N}$ for a quadratic forward flux with complementary return flux, while reciprocal stress and closed coherent branches obey different composition laws. Physical observable and routing identifications remain open. |
...
| `foundations/loop-to-bubble-projection-theorem.md` | Derived conditional projection and population spectrum; Derived regulated pure-gauge finite-regulator theorems, exact fixed-graph character-cutoff form limit, volume-uniform local cutoff-density bound and global-norm obstruction, conditional fixed-regulator thermodynamic and reflection-positive Euclidean subsequences, exact fixed-graph anisotropic Wilson transfer-to-Hamiltonian limit, universal two-loop Wilson asymptotic scale relation and necessary renormalized volume/gap criterion, exact fixed-boundary gauge-fibre support, conditional vacuum-measure recurrences and block theorems, exact bare-cylindrical refinement obstruction, and conditional local Wilson-word algebra/GNS vacuum-orthogonal density with the uniform-form transfer criterion; unresolved retained bowtie fibre specification with expected receipt absent; Tested conditional scalar-sector wound-loop mechanism; Hypothesized microscopic physical identification—September 2026 | The shared-loop law projects to the canonical PDE and fills the affine bubble volume. The pure-$SU(2)$ branch supplies fixed-regulator operator and measure results from character-cutoff removal through conditional thermodynamic and Euclidean subsequences, and it identifies the anisotropic Wilson transfer generator with the Kogut–Susskind Hamiltonian on every fixed finite graph. The universal two-loop asymptotic scale gives $g_0\to0$, $N F_W(g_0)\to\infty$ and finite positive bounds on $a\Delta/F_W$ as necessary continuum trajectory and gap conditions. The full centered local gauge-invariant Wilson-word algebra is dense in the finite invariant cylinder algebra and in the vacuum-orthogonal GNS subspace, with 36/36 primary and 41/41 independent finite controls. The exact-block cutoff study is finite/cutoff evidence only: its 20 Ritz rows and 180 boundary rows are `INCONCLUSIVE`, its sign-changing projected Ritz density has `unrestricted_conditional_gap: 0`, and the verifier does not compute the transport score. Its source provenance is unresolved and the independent file is arithmetic/self-check only. Section 9.34 isolates the remaining exact lower-form, strong-recovery, scale-matching and continuum-representation obligations; none is constructed. Section 9.45 carries the corrected matter-formation tube functional into a closed wound loop: the tube binds above a measured line-density threshold, the transported $Q=256$, $w=1$, $R=8$ trial is bound by $72.12081046$ with positive non-symmetry transverse modes, and the radial derivative $+8.6921788545$ with every reduced minimum at $R_*<1.85$ places the stationary loop inside core overlap. Accepted finite steps retain source-bound primary and independent controls, while interacting weak-coupling estimates, identification of an exact RG image with the full local algebra, full-sequence and phase control, the nontrivial continuum construction, Osterwalder–Schrader/Wightman reconstruction, a regulator-independent positive mass gap and Cassi identification remain open |
| `foundations/geometric-manifold-completion.md` | Hypothesized completion ansatz / Derived canonical reduction and conditional fibre geometry / Mapped selected carrier coupling / Tested finite-grid spectrum and conditional smooth scalar binding—September 2026 | The stratified bundle joins positive-matrix fibres, projective shell, affine bubble image and two-rail scale graph. Conditional core and carrier support remain distinct from microscopic matter. The Cartesian branch fails its smooth-carrier diagnostic; separate prepared scalar binding is reproduced, with INCONCLUSIVE constrained spatial stability |
...
| `foundations/particle-stationary-action-closure.md` | Hypothesized temporal and physical completion / Derived conditional action, scalar-reduction, parent-vacuum, dilation, fixed-charge, formation-dynamics, non-identifiability and regular quark-current identities / Mapped carrier coupling and conditional baryon benchmark / Tested prepared binding, finite-grid spectra, parent correspondences, conditional temporal dynamics, radial baryon relaxation, quark–meson regulator extrapolation, fixed-family binding and interacting radial-baryon rejection—September 2026 | The first-order carrier preserves an empty sector; its Cartesian branch is ultraviolet-dominated, while a positive-inertia parent supplies conditional finite-charge localization and fixed-charge minimizer-set stability. Scalar and Dirac parents prove the measured inverse map many-to-one. The empirical quark–meson action supplies explicit Dirac quarks, exact baryon current and a continuum-qualified fixed-family endpoint. A finite-cutoff chromodielectric completion supplies action-level confinement, but its first joint valence-plus-field radial endpoint fails stationarity, exterior-tail, virial and formation-basin criteria and is not the lowest reached branch. The empirical microscopic action, physical normalization, unconstrained nonradial formation, spectrum, observable nucleon map, baryogenesis and Cassi origin of QCD remain supplied or open. |
| `foundations/matter-completion-boundary.md` | Derived conditional nine-sector boundary, microscopic non-identifiability, continuum minimizer-set stability, chiral-zero obstruction, regular baryon current, finite-cutoff confinement and CP-selection theorem / Mapped carrier and baryon normalizations / Calibrated resolved-flavour leptogenesis scale / Tested conditional empirical history, whole-bubble selector, phase-current handedness, smooth vacuum-to-bound-state and autonomous finite-core radial formation boundaries / Hypothesized Cassi microscopic completion—September 2026 | The empirical Standard Model plus two-singlet branch connects a gauge-projected thermal state to observable nucleons and is robust to the tested coarse initial-state changes. Exact CP conjugation reverses $\eta_B$ with identical registered Cassi data. The optional phase-bearing extension supplies CP-odd spatial helicity, but its positive homogeneous action leaves opposite signs degenerate and linearly stable. CP and handedness selection return `DOES NOT EMERGE`; the smooth vacuum-to-bound-state result is conditional on a prescribed regulated background. The source-bound G3 continuation adds a self-consistent finite-core spherical $\kappa=-1$ radial formation witness with independent control and resolution agreement. Its coupling and regulator are supplied, and continuum removal, all angular sectors, renormalized backreaction, whole-bubble preparation, physical normalization and particle identity remain open. The rank-two benchmark also differs from the separate Mapped three-nonzero-mass spectrum. Complete Cassi-origin matter formation remains `FAIL`. |
...
| `foundations/core-trapped-charge-support.md` | Hypothesized auxiliary carrier / Derived conditional thin-tube support boundary / Mapped selected carrier coupling / Tested finite-grid spectrum, conditional smooth scalar binding, and wound-loop radius boundary—September 2026 | The neutral carrier supplies exact conserved population, transverse binding and inverse-length self-repulsion under explicit inequalities. Empty carrier data remain empty. Cartesian localization has ultraviolet structure; separate prepared $Q_C\in\{16,256\}$ scalar binding is reproduced, with INCONCLUSIVE smooth $Q_C=16$ spatial stability. Closing the relaxed tube into a wound loop gives the measured threshold $\lvert Q/w\rvert_{\rm bind}=41.1173021501$ and a bound transported $R=8$ trial whose radial derivative drives the loop inward, so the stationary radius lies inside core overlap at these coefficients (CC59), (CC60) |
| `foundations/cassi-first-principles.md` | Derived PDE; **C / Asserted Qi definition**; Asserted single-channel $g(q)$ input—September 2026 | Two-fluid PDE in primary nonnegative density fields $E_Y,E_I$ with $\rho=E_Y+E_I$; optional positive-root lift $\Psi^{(+)}=(\sqrt{E_Y},\sqrt{E_I})$ supplies amplitude-plane diagnostics; the canonical $q$ rational form and bare $\varphi^{-2}$ floor are constitutive choices evaluated in dimensionless/reference-normalized solver variables, with physical-density use requiring an external $\rho_\star$; the $\varphi$-line is a composition attractor while $q$ varies with density; optional gravity limits require a separate metric and matter map |
...
| `foundations/sector-coupling-derivation.md` | Derived conditional sector, fermionic, formation-dynamics, non-identifiability, regular quark-current and finite-cutoff confinement identities / Tested finite-mode production, continuum, scalar-vacuum, conditional dynamical restrictions and interacting radial-baryon rejection / Calibrated electroweak anchor / Hypothesized physical fermion coupling—September 2026 | Conditional on $\delta=3$, $\kappa_{s,\mathrm{scale}}=\varphi^{-6}/v_0^2\approx0.92\ \mathrm{TeV}^{-2}$ and $M_{s,\mathrm{scale}}\approx1.04\ \mathrm{TeV}$ are arithmetic. The chiral-scalar projection has dimensional, conjugacy and Hermiticity obstructions. The separate scalar mass-source model has verified finite-mode production and feedback; its continuum witness establishes ultraviolet and binding restrictions. Scalar and Dirac slow sectors establish microscopic non-identifiability. An empirical quark–meson model supplies a regular baryon current and localized fixed-family endpoint, while a finite-cutoff chromodielectric extension supplies action-level confinement. Their first joint radial endpoint fails stationarity, exterior-tail, virial and formation-basin qualification. Physical matching, state selection, renormalization, unconstrained nonradial formation, spectrum, baryogenesis and Cassi origin of QCD remain open (`computations/matter-formation-continuum-report.md` §§14–29, 79, 81–82). |
...
| `gravity/three-body-analytical.md` | Derived conditional on the selected $d = 3$ computational/physical domain and displayed PDE force sign—September 2026 | The well-separated-blob reduction has a state-dependent internal coupling-magnitude coefficient and retained conversion-driven masses. A body-dependent physical response requires a separate matter-state map; an attractive branch requires a separate Hypothesized force or metric closure |
| `gravity/quantum-gravity.md` | Derived conditional on the noise–signal identification, cascade-dephasing family and selected $d=3$ domain; Derived conditional obstruction to the Gaussian's standard positive spectral interpretation / Hypothesized two-fluid quantization, composite graviton and interacting completion—September 2026 | The nonzero-$\sigma$ Gaussian cannot be a standard unsubtracted positive physical scalar covariance. Auxiliary/regulator uses need separately qualified observables. Free Euclidean damping and the UV-convergent radial prototype at supplied $k_{\mathrm{IR}}>0$ do not close interactions, gauge structure, Lorentzian unitarity, GR matching or horizon physics; the observed-GW reading of the implemented dispersion is rejected |
| `foundations/quantum-free-fall-correspondence.md` | Derived conditional external-potential correspondence / Hypothesized Cassi atomic state, gravity-response, and common-lapse mappings—September 2026 | Derives the ideal QGI phase, local-acceleration identifiability boundary and constant-lapse cancellation; proves the canonical physical-$q$ interval, equal-$q$ response-diagnostic nonuniqueness and coarse-graining mismatch. QFC1–QFC4 pass independently, including the Gaussian's conditional spectral obstruction. Forty-three requirements separate material, gravity, clock, apparatus and interacting-quantum closure without assigning an atomic $q$ |
...
| `turbulence/navier-stokes-depletion-dynamics.md` | Derived filtered identities and instantaneous obstructions / Conditional continuation estimate—September 2026 | Exact coarse/fine strain split, two-sided instantaneous response and a viscous-only absorption counterexample; 1,012 checks pass. The frozen matter comparison identifies coercive structure; a separate applicability note treats conditional linear exterior response of the **Hypothesized** interscale action. Data-controlled cumulative depletion and arbitrary-data regularity remain **UNRESOLVED** |
| `turbulence/navier-stokes-strain-departure.md` | Derived conditional estimates, helical reduction and cumulative mixing obstruction / Open arbitrary-data critical work—September 2026 | Energy-coupled departure deadline and necessary defect; 71 checks. Critical-transfer bounds and exact radial spectral-spread dissipation yield a cumulative positive-production continuation criterion; the cyclic datum generates positive production immediately and the scalar-budget obstruction remains; 134 checks. Forced source budgets, conditional absorption and vanishing-source magnification pass 215 checks in each separate evidence record. An exact globally smooth periodic mixing family excludes amplitude-linear bounds for cumulative critical transfer, spread dissipation and positive spread production while retaining a finite nonlinear class-specific critical-transfer bound; 601 checks. The signed curl spectrum yields the critical residual $\omega-Hu/(2K)$: its finite squared $L^3$ time integral controls enstrophy and radial positive production. The 84-check follow-up constructs bounded first-order positive-doublet energy with unbounded enstrophy and critical residual. Initial-$H^3$ control, whole-field dynamical exclusion and arbitrary-data regularity remain open; no unforced singularity or audit of the announced forced proof |
| `turbulence/navier-stokes-second-order-field-energy.md` | Derived field and Navier–Stokes identities / Derived conditional connection-sector map / Open arbitrary-data residual bound—September 2026 | The continuous-time continuum Yang/Yin family associated with the live shader's source-free coupling has a nonnegative weighted energy. For $c_s^2>0$ and $\omega_0^2>0$ it has a static constant-density zero mode; at $c_s^2=0$ the static density-kernel degeneracy is spatially varying. The runtime uses an extent-dependent unit-coefficient finite-difference operator and has no exact finite-time-step conservation claim. A dimensionful Abelian one-color, temporal-gauge, charge-free and scale-flat field-particle restriction maps the surviving connection-sector functional to a multiple of $\|u_t\|_2^2+c^2\|\omega\|_2^2$. The original unforced equation gives an exact balance; finite $L_t^2L_x^3$ signed time–curl residual work implies continuation. The audit-qualified verifier passes 110 of 110 checks; the uniform bounded-data residual estimate remains open. |
| `turbulence/navier-stokes-adaptive-metric.md` | Derived exact weighted balances and conditional continuation reduction / Open active-distortion bound—September 2026 | Scalar, material, terminal-adjoint and forward-parabolic SPD metrics cancel vortex stretching in weighted enstrophy exactly. Positive propagation and the unscaled determinant floor are analytic smooth-interval consequences; 32 parent and 16 forward checks cover the fixed controls and algebraic substitutions. The local extensional and covariance controls contradict coercivity inferred from positivity or determinant alone, without classifying a Navier–Stokes-specific estimate. A uniform initial-$H^3$-controlled bound on Euclidean-to-active weighted enstrophy would continue every smooth periodic solution and remains **UNRESOLVED**. |
| `turbulence/navier-stokes-deformation-covariance.md` | Derived exact covariance, inverse-metric, projection and conditional continuation identities / Open active Rayleigh-quotient bound—September 2026 | Under stated compact-interval stochastic assumptions, the Constantin–Iyer forward-deformation covariance obeys a closed parabolic equation, and its inverse cancels vortex stretching while completing viscosity into a nonnegative covariant square. A projection theorem identifies weighted enstrophy with retained sampled initial-vorticity energy and accumulated dissipation with the endpoint regression residual. The audit-rechecked source-bound schedule passes 40 exact symbolic and fixed-control checks, including the citation-derived Stratonovich–Itô conversion; stochastic-flow and Navier–Stokes trajectory integration remain outside its executable scope. The finite-ensemble projection and inverse-Jensen group is **SUPPORTS**, while two scalar checks are only prototypes for the analytic endpoint bridge. Homogeneous extension excludes coercivity from positivity, determinant or unrestricted deformation algebra alone, while periodic shear has inactive-plane covariance and active distortion one. A uniform bound on the data-conditioned active quotient over every mean-zero, divergence-free initial-$H^3$ data ball would imply continuation and remains open. |
| `turbulence/navier-stokes-active-deformation-occupation.md` | Derived exact seeded-covariance, occupation and conditional continuation identities / Open uniform active-dose bound—September 2026 | The second moment of the stochastic Cauchy field seeded by $\omega_0\omega_0^{\mathsf T}$ obeys a closed common-noise equation, dominates enstrophy and equals an exponential directional-strain occupation selected by initial vorticity. Its logarithmic production uses only orientation anisotropy aligned with strain. A reachable-direction Khasminskii bound and summable positive active dose imply continuation. The source-bound schedule has 40 general polynomial components, finite fixtures, local controls and exponent checks; analytical stochastic-flow, heat-kernel, Khasminskii and restart steps remain outside executable scope. Generic energy-class-strain-only occupation control and geometric-spacing-only dose decay are **CONTRADICTS**. The uniform all-data active-dose bound and arbitrary-data regularity remain **UNRESOLVED**. |
| `turbulence/navier-stokes-replica-coherence.md` | Derived exact replica, covariance, temporal-rank and conditional continuation identities / Open recovered-envelope and cross-scale bounds—September 2026 | Two independent stochastic Cauchy replicas recover enstrophy through overlap and centred covariance through disagreement. The covariance source is twice viscous palinstrophy, and its Duhamel representation retains every source time and subsequent directional deformation. Global coherence obeys a replicator-diffusion law. Volume preservation gives a sharp instantaneous-source determinant bound; applying the determinant root after accumulation gives a stronger temporal-rank functional and sharpened enstrophy envelope. The source-bound analytic schedule passes 60 of 60 checks. The short-time 36-run and longer-horizon 48-run finite-mode Galerkin receipts pass their prescribed checks, with positive rank-two and near-rank remainders and zero shear/ABC heat controls. The independent signed-mode audit passes 10 of 10 checks, reproduces the four $N=2$ long rows, and agrees with direct convolution, alias-free grid products, and the saved production integrals to relative differences of $5.43\times10^{-17}$, $3.02\times10^{-15}$, and $1.34\times10^{-15}$. The paper derives a critical pointwise remainder bound, a global fixed-cutoff trajectory estimate with $N^{5/2}$ growth, and a cutoff-uniform remainder estimate on the local $H^3$ lifespan. The prescribed-horizon cutoff-uniform production bound and arbitrary-data regularity remain UNRESOLVED. |
| `turbulence/navier-stokes-covariance-recovery-rate.md` | Derived exact initial-layer asymptotic / Open recovery-only continuation bound and global regularity—September 2026 | The active rank-deficient periodic control has global leading coefficient $c_4=8\nu^4f\ge0$, with $f>0$ on an open region. Its normalized accumulated covariance volume grows as $\mathcal K(t)=6\nu^{4/3}\langle f^{1/3}\rangle t^{4/3}+o(t^{4/3})$, while seeded stretching grows by $\frac12t+O(t^2)$; the production-to-recovery ratio diverges as $t^{-1/3}$. The fixed integrated-rate schedule passes 6 of 6 checks. The occupation term required for a production-relative estimate and arbitrary-data regularity remain UNRESOLVED. |
| `turbulence/navier-stokes-near-rank-recovery-obstruction.md` | Derived exact near-rank determinant-scale obstruction / Open production-relative recovery and global regularity—September 2026 | A uniformly bounded smooth periodic family has full-rank initial vorticity-gradient source almost everywhere for every positive perturbation, while $A_\varepsilon\sim A_*\varepsilon^{2/3}$ and seeded production remains exactly $1/4$. The recovery-only coefficient diverges across the family; its source-bound schedule passes 7 of 7 checks. Production-relative occupation control and arbitrary-data regularity remain **UNRESOLVED** |
| `turbulence/navier-stokes-helical-dynamic-depletion.md` | Tested finite helical families / Universal sign depletion contradicted / Open scale-uniform direction-strain estimate—September 2026 | Eight pre-registered Fourier–Galerkin families measure the signed stretching production at every recorded state. The exact Beltrami control preserves $P$ at roundoff; the wide, narrow, tight-pitch, two-scale and opposite-handed tube families are positive at every reported checkpoint from $t=1/8$ onward, the opposite-handed pair with normalized helicity near $10^{-18}$, while the two mode families stay nonpositive. The 24-run matrix passes all integrity and refinement checks and classifies the universal sign statement `CONTRADICTS` for the declared families; the scale-uniform direction-strain estimate and global regularity remain open. |
| `turbulence/cassi-fluid-feasibility.md` | Derived conditional mechanical, thermal, and phase-current identities / Tested solver, rotational and phase-coercivity boundaries / Open physical-fluid completion—September 2026 | First-order pressure, quantum and counterflow stress with supplied mass; scalar positivity and relative entropy. The native-force study passes 246 checks across 28 trajectories and rejects physical replacement of those sectors because the periodic mean accelerates. A selected reacting capillary/thermal model conserves momentum and energy, produces nonnegative entropy and preserves homogeneous canonical conversion; 395 checks across 27 model trajectories plus an independent matrix reference support that closure. A separate 227-check phase-current reduction derives rotational and helical classes; its 84-check helical follow-up contradicts static first-order phase-energy coercivity for enstrophy and critical residual. Material calibration, microscopic viscosity, whole-field dynamical concentration control, arbitrary-flow closure and global regularity remain open. |
| `turbulence/cassi-fluid-phase-current-hydrodynamics.md` | Derived conditional current and topology identities / Tested rotational, memory, first-order coercivity and homogeneous-handedness selection boundaries / Open microscopic viscosity, nonlinear handed-domain formation and arbitrary-flow closure—September 2026 | A normalized doublet gives Mermin–Ho vorticity; a positive global chart has zero integrated helicity, while a full Hopf doublet and two fixed scale bands realize nonzero-helicity sectors. Scalar diffusion equals viscosity for one fixed-winding Beltrami family and fails generally by a nonzero commutator. Exact closed-mode elimination supplies recurrent memory and exterior-state dependence. A separate 84-check analysis excludes static first-order phase energy as an enstrophy or critical-residual bound. The whole-bubble follow-up identifies spatial helicity as CP-odd, but the registered positive action leaves opposite helicities degenerate and its homogeneous operators nonnegative; handedness selection returns `DOES NOT EMERGE`. |
| `turbulence/cassi-radiative-material-closure.md` | Derived conditional LTE transfer, conservation and entropy identities / Tested numerical kernels / Hypothesized Cassi material map—September 2026 | Established Planck–Kirchhoff emission and piecewise-gray M1 moments couple to the selected thermal material through an equal-and-opposite four-force. The exact slab, realizability, conservation, entropy and diffusion identities are derived under supplied coefficients. The comprehensive result passes 33 of 34 checks and rejects $\Delta t=0.01$ for one source-accuracy target; the separate 9-check qualification supports $\Delta t=0.001$ subcycling. Temperature, physical density, opacity, ionization and electromagnetic current remain external or open. |
| `turbulence/compressible-radiative-plasma-closure.md` | Derived conditional / Tested reference controls and integrity qualification—September 2026 | Conservative compressible mass, momentum and material-energy equations supply pressure work and Rankine–Hugoniot shocks; constrained species and level populations connect versioned atomic data to line and continuum transfer; contraction, accretion and nuclear mass defect enter a complete stellar energy ledger; discrete ordinates preserve crossing beams. The source-snapshotted schedule passes 70 of 70 checks, and a separate 36-check integrity qualification covers the species-level mass constraint, defensive state construction, thermodynamic identities, physical-frequency Doppler normalization, rejection boundaries, exchange cancellation, complete stellar and nuclear ledgers, and missing-prerequisite classification. Cassi material identification, evaluated-data qualification, production finite-volume convergence and live CassiCosmos integration remain open. |
...
| Document | Status | Summary |
|----------|--------|---------|
| `foundations/unified-lagrangian.md` | Hypothesized extended action and candidate common lapse / Derived conditional relative conversion-clock identity, slow-sector non-identifiability, regular baryon current and finite-cutoff confinement / Tested finite-mode production, conditional temporal dynamics, interacting radial-baryon rejection and homogeneous phase-current handedness boundary—September 2026 | Optional extended action assembly around the canonical real-density pair and rank-one conversion. The displayed Dirac chiral-scalar projection and mixing expressions fail dimensional, conjugacy and Hermiticity requirements. A separately supplied scalar mass-source parent has verified finite-mode fermion production, while scalar and Dirac parents share the registered slow carrier equation with different spin and statistics. The empirical quark–meson action supplies regular explicit Dirac content, and the chromodielectric completion supplies action-level confinement. Their first joint radial endpoint fails stationarity, localization, virial and formation-basin criteria. The separate phase-bearing action supplies CP-odd helicity but leaves opposite signs degenerate and homogeneously stable; it has no microscopic baryon-production coupling. The microscopic action, physical normalization, unconstrained nonradial solution, observable nucleon map, baryogenesis and Cassi origin of QCD remain supplied or open. $\mathcal R=(1-q)/(1-q_0)$ is the exact relative conversion-clock rate; its candidate common-lapse interpretation still requires resolved lapse contrast and independent clocks. |
| `standard-model/cp-violation.md` | Hypothesized particle-sector CP/chiral map and nonlinear handed-domain selection / Mapped $\delta_{\text{CKM}}$ and strong-CP span / Tested leptogenesis CP-selection and homogeneous phase-current handedness no-go boundaries / Incomplete Yukawa-determinant $J_{\text{CP}}$ candidate—September 2026 | The canonical real-density sector supplies no intrinsic CP-odd datum. The Mapped quark-sector CKM phase has no derived map to the leptonic Casas–Ibarra coordinate; exact conjugate textures produce opposite baryon yields. The optional phase-bearing extension supplies CP-odd integrated spatial helicity, but the registered positive action leaves opposite helicities degenerate and has no negative homogeneous mode. CP and handedness selection from registered data return `DOES NOT EMERGE`. |
...
| `foundations/neutrino-masses.md` | Hypothesized mechanism / Mapped offsets / Calibrated resolved-flavour two-singlet comparison / Tested rank and CP-selection boundary—September 2026 | The selected mapped coordinate span is $n=8\rightarrow20$ and the physical GUT anchor is $n\approx13.3$. The Mapped spectrum has three nonzero masses; the minimal two-singlet matter-history comparator has rank two and one exactly massless light state, so one unified microscopic completion cannot realize both without another mass source or a changed model. |
...
| `foundations/baryon-asymmetry.md` | Hypothesized Cassi mechanism, nonlinear handed-domain selection and microscopic CP coupling / Mapped $\eta$ exponent / Calibrated resolved-flavour two-singlet scale / Tested whole-bubble leptogenesis and phase-current selector boundaries—September 2026 | The empirical comparator reproduces the observed magnitude and survives QCD under supplied inputs. Coarse initial differences wash out, while exact CP conjugation reverses the sign with identical registered Cassi data. The phase-bearing extension supplies CP-odd spatial helicity, but its positive homogeneous action leaves opposite signs degenerate and has no baryon-production coupling. CP and handedness selection return `DOES NOT EMERGE`; the fitted cascade endpoint and microscopic Cassi origin remain open. |
...
| `foundations/interscale-current-soliton.md` | Hypothesized action and Wilson-link extension / Derived conditional endpoint, first-order source-action, phase-current, Wilson-transport, localization, smooth-core, confinement, and carrier-support boundaries / Mapped selected carrier coupling / Tested finite-grid spectrum, rotational, phase-coercivity, homogeneous-handedness selection controls, and conditional smooth scalar binding—September 2026 | Complex Yang/Yin fields give conditional continuity and counterflow identities; the separate Wilson link closes the circuit conditionally. Point flux, auxiliary core and neutral carrier supply reduced support sectors. The particle action has empty-sector preservation, an ultraviolet Cartesian branch and independently reproduced prepared scalar binding, with `INCONCLUSIVE` smooth constrained spatial stability. Section 12 derives positive collective phase inertia, one branch after admissible longitudinal connection variation, causal exterior memory with initial-state forcing, and a conditional Mermin–Ho rotational projection. The 84-check helical follow-up excludes static first-order phase energy as an enstrophy or critical-residual bound. The whole-bubble calculation identifies a CP-odd helicity observable but finds equal-energy opposite signs and no negative homogeneous mode; selection returns `DOES NOT EMERGE`. Physical scale boundaries, nonlinear handed-domain formation, coupling to microscopic production and particle identity remain open (`computations/matter-formation-continuum-report.md` §§37, 86). |
| `foundations/interscale-stress-attenuation-boundary.md` | Hypothesized physical carrier and Wilson-link coefficient / Derived conditional stress, frozen-link, first-order source-action, stationary spatial-flux, and Wilson-transport boundaries—September 2026 | Conditional scale-window force is the difference of mixed-stress boundary fluxes. The time-completed quadratic action supplies the canonical boundary flux, and Hermitian Robin data give a unitary $S_\Lambda(k)$ family. A frozen charged endpoint background supplies a gauge-covariant rail-rail Hessian; AR1–AR6 pass and the incompatible DR receipt remains `FAIL`. Every closed homogeneous EL9 extremum has zero coherent current, while stationary spatial flux supports only zero-mean source structure on closed domains. The separately declared Wilson link closes the homogeneous circuit conditionally, carries the scale-edge relative-charge current, and has finite capacity; IT1–IT6 pass at one normalized point. A reciprocal displacement ladder remains conservative, and the routed branch gives $\varphi^{-N}$ forward quadratic flux only when complementary return ports are removed from coherent re-entry. The physical stress carrier, current-to-stress map, $t_\Upsilon$, local scale-bulk completion, nonzero-current background, damping channel, doubled port-flux law, and full coupled spectrum remain open |
| `foundations/rung-offset-mechanism.md` | Derived envelope quantization; Hypothesized phase-to-rung mapping and selection; Empirical catalog (μ/Jψ placements Mapped—38-state scan, ledger) | The wake envelope's special positions and pool-cell quantization are derived; $\delta n$ is a Hypothesized coordinate mapping of the local phase lag; the exact relaxation-angle bound is $\lvert\Delta\theta_d\rvert\leq\operatorname{atan}(\varphi)\approx1.017$ rad; the mapped $\pm0.162$-rung reading remains outside the PDE derivation; sector edges at half-rungs; uniform 38-state baseline |
| `foundations/phi-rg-formalism.md` | Hypothesized—September 2026 | Discrete $\varphi$-RG ansatz; microscopic coarse graining, running-coupling derivations, and the momentum-to-Qi-state map remain open |
...
| `particles/matter-organization.md` | Synthesis of Derived conditional relations, Mapped scale assignments and Hypothesized physical realization—September 2026; forces, lattice pools and the n/p/e trio, with empty-sector and numerical formation boundaries |
