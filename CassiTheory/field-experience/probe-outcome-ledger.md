...
physical-scaling controls. The source-independent JavaScript verifier passes
**84 checks** and reconstructs every frozen formula from the protocol and
primary receipt.

| Control or claim | Decisive result | Classification and scope |
|---|---|---|
| Spatial Poincaré geometry | A closed simply connected spatial three-manifold is topologically $S^3$; the round radius-$R$ regulator has first coexact one-form frequency $2/R$ | **ADOPT** for the stated topology and round regulator; the frequency vanishes as $R\to\infty$, so topology supplies no continuum mass |
| Fixed $SU(2)$ link spectrum | In the Casimir metric generated by $i\sigma_A/2$, characters have eigenvalues $n(n+2)/4$ and the first scalar link rate is $3/4$ | **ADOPT** as exact finite-link geometry |
| Wilson-weight pointwise curvature | For one plaquette with concentration $\beta\geq0$, the Bakry–Émery lower bound is $(2-\beta)/4$ | **CONTRADICTS** extension of this pointwise curvature proof through $\beta=2$ and into weak coupling; it is a method boundary, not a proof that the true spectral gap closes |
...
| Frozen finite controls | The primary and independent receipts pass 118/118 and 84/84 checks with maximum recorded errors below their frozen tolerances | **SUPPORTS** the implementation and normalization controls; no sampled row establishes a uniform analytical estimate |
...
- `computations/yang-mills-poincare-geometry-prereg.md`—frozen spatial/link geometry, conditional-score recurrence and physical-scaling protocol.
- `computations/verify_yang_mills_poincare_geometry.py`—118-check source-bound geometry and recurrence receipt generator.
- `computations/verify_yang_mills_poincare_geometry_independent.mjs`—84-check source-independent spectrum, connection and recurrence reconstruction.
