...
- `runs/20260922_curvature_budget/curvature_budget_receipt.json`—amended invocation at `status=PASS`: the same frozen-field measurement with the material channels reported, the enstrophy identity verified as a material statement along the carried trajectory and the geometric channels' evolution gap recorded (gitignored run artifact).
- `computations/diagnose_curvature_budget_channels.py`—development diagnostic that isolated the gap by channel: the magnitude channel integrates at round-off, the curvature channel at half its measured change, the width channel at a fiftieth of its measured change.
- `computations/verify_navier_stokes_tube_modulus_transport.py`—six-check transport verifier on the exact Arnold–Beltrami–Childress flow: the frame closure and the enstrophy identity at round-off, flux conservation through a material element, the cross-section scale's $D_\tau\log a=-\ell/2$ and the integrated modulus identity, each with a step-refinement pair, the viscous cap at every sample, the curvature transport of (19) against finite differences of the curvature field to $3.1\times10^{-10}$, the rotation part of the velocity gradient dropping out at $4.6\times10^{-16}$, and the margin's rate against the width-weighted critical norms of (21) at ratio $0.309$ worst and $0.165$ mean with the terms of (20) summing to the rate to $2.7\times10^{-20}$.
