...
PREREG = ROOT / "field-experience" / "toroidal-coherence-survival-pre-registration.md"
VERIFIER = ROOT / "field-experience" / "verify_toroidal_coherence_survival.py"
...
def main() -> None:
    ...
    run_dir = args.output or ROOT / "runs" / f"{stamp}_toroidal_coherence_survival"
    run_dir.mkdir(parents=True, exist_ok=False)

    sources = {
        str(path.relative_to(ROOT)).replace("\\", "/"): sha256_file(path)
        for path in (Path(__file__).resolve(), PREREG, VERIFIER)
    }
    ...
    if not initialization_ok:
        invalid = {
            "probe": "toroidal_coherence_survival",
