...
def evaluate_preflight(preflight: dict[str, Any]) -> tuple[dict[str, bool], str]:
    closed = preflight["closed_initial"]
    untwisted = preflight["untwisted_initial"]
    gates = {
        "G1": analytic_closure_error() <= 1e-12 and preflight["closure_error"] <= 1e-12,
        "G2": (
            abs(closed["winding_y"] - 2.0) <= 0.05
            and abs(closed["winding_i"] + 3.0) <= 0.05
            and closed["coherence_y"] >= 0.20
            and closed["coherence_i"] >= 0.20
        ),
        "G3": (
            closed["helix_order"] >= 0.80
            and closed["opposition"] >= 0.80
            and untwisted["helix_order"] <= 0.20
        ),
        "G4": (
...
def main() -> None:
    ...
    errors: list[str] = []
    max_error = [0.0]
    if results.get("probe") != "toroidal_coherence_survival_v2":
    ...
    compare_tree(preflight["closure_error"], analytic_closure_error(), "preflight.closure_error", errors, max_error)
    if recomputed_arms:
        ...
        gates, verdict = evaluate_preflight(preflight)
