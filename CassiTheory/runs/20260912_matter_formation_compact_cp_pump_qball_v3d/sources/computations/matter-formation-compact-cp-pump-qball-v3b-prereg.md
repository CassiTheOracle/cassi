# Compact CP-Pump Formation of a Non-Topological Carrier—Finite-Volume Ledger
...
The carrier energy ledger uses the symmetric finite-volume Hamiltonian paired
with the actual outgoing evolution operator. This version makes that
semidiscrete pairing explicit; the physical action, pump, vacuum, grid,
observables, and scientific thresholds are otherwise unchanged from the
compact CP-pump protocol.
...
The prescribed positive-pump background is
...
## 3. Observables and finite-volume ledger

The continuum carrier energy excludes the pump interaction and is
...
For the finite-volume ledger, let $A_h$ be the symmetric second-difference
operator obtained from the same stencil as the evolution after removing the
outer Sommerfeld term: the inner ghost is $-u_0$ and the outer static ghost is
$u_{N-1}$. The actual outgoing operator is
$A_hu-e_R p_{N-1}/\\Delta r$. The receipt evaluates the spatial quadratic as

$$
E_{\\mathrm{grad},h}=-\\frac12\\Delta r\\,u^T A_hu,
$$

for each real component. This is the finite-volume representation of the
positive gradient-energy term paired with the evolution stencil. The removed
outer term contributes the explicit energy flux
$-4\\pi(p_{x,N-1}^2+p_{y,N-1}^2)$, rather than being included in $A_h$.
...
The charge and energy ledgers are integrated with the same trapezoidal
checkpoint ledger for every arm. Define $S_Q$ as the maximum of one and the
absolute charge, source-charge cumulative and boundary-charge cumulative values
in the primary positive arm. Define $S_E$ analogously from full energy, pump
work cumulative and boundary-energy cumulative values. The ledger predicate is

$$
\\max_t|\\delta Q|/S_Q\\le10^{-6},\\qquad
\\max_t|\\delta E|/S_E\\le10^{-6}.
$$
...
7. the charge and energy ledger inequalities above pass in the primary arm.
...
all seven predicates, the source-free control, the CP arm, the flux/energy
ledger, and the independent raw-state reconstruction pass. A numerical receipt
with a failed scientific predicate returns
