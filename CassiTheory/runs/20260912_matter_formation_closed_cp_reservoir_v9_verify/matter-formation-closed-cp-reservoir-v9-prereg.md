# Carrier-Frequency-Resonant Autonomous Complex CP-Odd Reservoir Formation of a Non-Topological Carrier—Matched Finite-Volume Ledger
...
dynamical field with a finite-energy outgoing coherent packet. Its phase
frequency is fixed to the compact-pump carrier frequency $\Omega=m$,
which is the observed resonant frequency of the declared non-topological
carrier branch. The reservoir mass is lowered only enough to permit this
frequency to have nonzero outgoing group velocity. No time-dependent
...
The test asks whether this carrier-frequency-resonant autonomous packet can
produce a localized carrier remnant after the reservoir has left the
formation region. Charge creation while the reservoir overlaps the carrier
is measured as an internal Noether-source term. The post-overlap charge,
...
=(1,2,1.25,\tfrac12,0.5,\tfrac{\sqrt3}{2},0.8,8),
\qquad
\Omega=\sqrt{m_a^2+K^2}=m=1.
$$

The value $\Omega=m$ is fixed by the carrier-frequency resonance retained
from the compact-pump mechanism; $m_a=\tfrac12$ is the minimal simple
mass choice that permits the nonzero group velocity
$K/\Omega=\sqrt3/2$. These values are fixed before execution and are not
fitted or scanned. The outgoing packet is initialized directly in the
finite-volume state below, not imposed as a time-dependent source.
...
carrier-frequency-resonant outgoing packet with $K=\sqrt3/2$;
$\Omega=m=1$ is fixed by the declared carrier branch and is not fitted.
...
`computations/matter_formation_closed_cp_reservoir_v9.py` and
`computations/verify_matter_formation_closed_cp_reservoir_v9.py`. The verifier
...
trajectory. The reservoir initial state is the declared
carrier-frequency-resonant finite-energy packet and the carrier state is a
