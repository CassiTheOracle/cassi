...
- **The coherence budget is conditional.** At human scale the continuation gives $\varphi^{14196}\approx10^{2967}$ as a dimensionless product-model cycle count; a physical lifetime requires the unselected failure process and trial-frequency map (`foundations/proton-coherence-budget.md` §7). The Creative attack model assigns targeted coupling through the Hypothesized constitutive label $\mathcal M_i^{\mathrm{attack}}\in[0,1]$ (`foundations/proton-coherence-budget.md` §5.2). Its proposed defenses are $\varphi$-detuned boundaries that reduce that assumed overlap—the shield domain of `speculations/creative-extensions/coherence-warfare.md`.
