...
- The wake aggregation scratch layer (`two-fluid/run_two_strand_binding_suite.py`) is implemented and tested: $\chi_w=0$ is a bit-exact no-op vs the canonical solver; the bracket {300, 1000, 3000} is null—sub-critical no effect, super-critical collapse (NaN at t = 0.6/0.3/0.2), no binding window; E1 remains open with two design-iteration candidates (ρ-weighted source, sub-critical feedback bound). Records: `runs/20260807_binding_suite/`, `hypotheses/two-strand-five-channel-matter-organization.md` §3.4.
