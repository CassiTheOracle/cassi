...
- The wake aggregation scratch layer (`two-fluid/run_two_strand_binding_suite.py`) is implemented and tested: $\chi_w=0$ is a bit-exact no-op vs the canonical solver; the bracket {300, 1000, 3000} is null—sub-critical no effect, super-critical collapse (NaN at t = 0.6/0.3/0.2), no binding window; the recorded iterations (ρ-weighted source, sub-critical feedback bound) are rejected (§3.4)—the weight does not re-center the wake (hump shift 0.09 cells) and the $\varepsilon^3$ collapse boundary sits at $\chi_w^{\mathrm{crit}}\approx2.9$, the cap is a fitted threshold with no attractive term. Records: `runs/20260807_binding_suite/` (generated output under `runs/`, regenerated by the cited script), `hypotheses/two-strand-five-channel-matter-organization.md` §3.4.
- The Yin-excess pair branch (`two-fluid/run_two_strand_yin_excess_suite.py`) is implemented and tested: with $E_Y\leftrightarrow E_I$ exchanged in the initialization ($\Pi<0$ in every ridge; representable under the 1e-3 positivity floor, verified), the pair persists at finite separation through t = 40 (d 9.90 → 7.51 cells, no merge, no escape) while the Yang-excess counterfactual escapes (15.73, published record reproduced); the branch is transient—conversion erases the Yin excess at t ≈ 21.3—and no equilibrium coefficients are projected. Records: `runs/20260807_014428_two_strand_yin_excess/` (generated output under `runs/`, regenerated by the cited script), `hypotheses/two-strand-five-channel-matter-organization.md` §3.5.
