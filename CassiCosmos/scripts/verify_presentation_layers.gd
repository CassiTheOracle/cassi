...
func _check_particle_profile() -> void:
	...
	_check("particle presentation keeps the glowing-sphere radius cap",
			material != null and is_equal_approx(
					float(material.get_shader_parameter("max_pixel_radius")), 18.0))
