extends Node
## CassiQwen L16 — deterministic hidden-state full-field GPU receipt.
##
## This is deliberately a windowed, bridge-off probe. It consumes only the
## prepared CPU seed for the captured Qwen hidden state (unit direction lifted
## into EY/EI with the raw source norm carried as hidden_l2_norm), seeds the
## canonical mind engine through seed_full_field(), and records cumulative
## checkpoints through the frozen extended horizons. No model, bridge, network,
## hidden-state decoding, vector coordinate reinterpretation, or _clear_field
## is involved. The frozen source norm is never applied to the field; it is
## re-applied only by the decoder during analysis.

const PROTOCOL := "CassiQwen L16 hidden-state field observatory"
const VERSION := 1
const SEED_PATH := "res://_diag/cassi_qwen_hidden_state_field_seed.json"
const GPU_PATH := "res://_diag/cassi_qwen_hidden_state_field_gpu.json"
const GRID_N := 32
...
const CASE_IDS := [
	"hidden_canonical", "hidden_shuffled", "zero",
]
const CASE_BASIS := {
...
# The lifted field encodes only the unit direction u = h/||h||_2 at unit
# amplitude, so the signed volume signal must have ||signal||_2 ~ 1. The
# source norm r = ||h||_2 is carried immutably in the seed as hidden_l2_norm
# and is validated as a finite positive scalar here; only the decoder uses it.
const UNIT_DIRECTION_TOL := 1.0e-3
...
func _prepare_seed(seed: Dictionary, prepared: Array) -> bool:
	...
	# Source norm: the immutable ||h||_2 carried for the decoder. It must be a
	# finite positive scalar (the captured hidden row has nonzero L2 norm).
	var norm_value: Variant = seed.get("hidden_l2_norm", null)
	if not (norm_value is int or norm_value is float):
		return false
	var source_norm: float = float(norm_value)
	if is_nan(source_norm) or is_inf(source_norm) or source_norm <= 0.0:
		return false

	# Capture provenance: SHA-256 of the capture artifact, 64 lowercase hex.
	var capture_hash: Variant = seed.get("capture_sha256", null)
	if not (capture_hash is String):
		return false
	var hash_text: String = str(capture_hash)
	if hash_text.length() != 64 or not _is_hex(hash_text):
		return false
	...
	for case_value in seed_cases:
		...
		if case_id == "zero":
			...
			# The lifted payload is the unit direction u = h/||h||_2 at unit
			# amplitude: the signed volume must be unit-norm.
			var signal_l2: float = _l2_norm(signed_field)
			if is_nan(signal_l2) or is_inf(signal_l2):
				return false
			if absf(signal_l2 - 1.0) > UNIT_DIRECTION_TOL:
				return false
		by_id[case_id] = {
			"id": case_id,
			"basis": basis_label,
...
func _is_hex(text: String) -> bool:
	for i in range(text.length()):
		...
		var lower: bool = code >= 97 and code <= 102
		var upper: bool = code >= 65 and code <= 70
...
func _l2_norm(values: PackedFloat32Array) -> float:
...
func _write_and_quit(receipt: Dictionary, message: String) -> void:
	if message != "":
		print("[CassiQwenHiddenState] ", message)
	...
	if file == null:
		...
		print("[CassiQwenHiddenState] failed to write ", tmp_path)
	else:
		file.store_string(JSON.stringify(receipt, "\t"))
		file.close()
		# Atomic replace: the final path only ever holds a complete receipt.
		var destination_ok: bool = true
		if FileAccess.file_exists(GPU_PATH):
			destination_ok = DirAccess.remove_absolute(
		...
		if not destination_ok:
			receipt["finite"] = false
			receipt["verdict"] = "INVALID"
			print("[CassiQwenHiddenState] failed to finalize ", GPU_PATH)
		else:
			print("[CassiQwenHiddenState] ", JSON.stringify({
