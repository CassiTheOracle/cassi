...
func _ready() -> void:
	...
	if sim == null:
		...
		get_tree().quit(1)
		return
	# PIN the grid FFT/Poisson battery against the campaign defaults
	# (meshless/tree/φ-aspect/dual now default on): the round-trip and
	# solve gates run the CUBE single-lattice spectral-Poisson chain.
