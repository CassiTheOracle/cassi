extends Node
## verify_subsonic_step — §5.3 of research/sound_coherence_note.md (TIER-1):
##   "Sharp subsonic threshold. With f_subsonic on, a pair at |v_t| slightly
##    BELOW h₀/dt passes the gate; slightly ABOVE always fails (the '>='
##    at cassi_particle_merge.glsl pass_best is an exact step). Confirms
##    the merge sound speed equals the coherence speed."
##
## The gate (pass_best): a pair qualifies only if (among distance, q_coh,
## order and binding) its TRANSVERSE relative speed |v_t| < c_s = h0/dt:
##   if (length(vt) >= pc.h0 / max(pc.dt, 1e-9)) continue;   // reject |v_t| >= c_s
##
## DESIGN (numbers re-derived here, printed in the gate details):
##   EXTENT = 37.5, N_GRID = 64 → h0 = 2·37.5/64 = 1.171875
##   DT = 0.001 → the subsonic threshold c_s = h0/dt = 1171.875.
##   Pair geometry like verify_merge_binding: separation d = 0.4 (< R_m
##   = 0.5·h0 = 0.586), EQUAL masses m, purely TRANSVERSE relative motion
##   along y (→ |v_t| = |v_rel| exactly).
##   To isolate the subsonic gate, the pair must be GRAVITATIONALLY BOUND at
##   BOTH speeds (so the ONLY blocker at the high speed is the subsonic gate).
##   Binding test ½μ|v_rel|²·d < g_n·m², with μ = m/2, uses the actual
##   pass_best push constant g_n = G_N = 1 (not the φ⁶-amplified bulk-force
##   coupling). At the UPPER speed (v = 1.05·c_s = 1230.47), this requires
##   m > v²d/(4·g_n).
##   v²·d = 1230.47²·0.4 ≈ 605,621; /(4·1.0) ≈ 151,405 → choose
##   m = 400,000 (≈2.64× margin, comfortably bound at both speeds).
##   f_virial = 0 (disabled to isolate the binding/subsonic gates);
##   f_order = 1, f_subsonic = 1.
##
## Gates:
##   G-S1  subsonic run: |v_t| = 0.95·c_s = 1113.28 < c_s → pair MERGES
##         (alive == 1, i.e. one survivor from the two).
##   G-S2  supersonic run: |v_t| = 1.05·c_s = 1230.47 >= c_s → pair does NOT
##         merge (alive == 2 — both free-stream past).
##   G-S3  (consistency) mass conserved across each run.
##
## Run (windowed console exe — NEVER --headless, which has no RD):
##   Godot_v4.7-stable_win64_console.exe --path <repo> res://scenes/verify_subsonic_step.tscn

const N := 2
const N_GRID := 64
const EXTENT := 37.5
const PHI: float = 1.618033988749895
const PHI_INV2: float = 0.3819660112501051
const RM_FRAC := 0.5
const H0 := 2.0 * EXTENT / float(N_GRID)      # 1.171875
const R_M := RM_FRAC * H0                     # 0.5859375
const Q_TH: float = PHI_INV2
const XI := 17.94427190999916                  # φ⁶
const G_N := 1.0
const DT := 0.001                              # c_s = H0/DT = 1171.875
const C_S := H0 / DT
const D_PAIR := 0.4                            # separation (< R_m)
const MASS := 400000.0                         # binds at both speeds for g_n=G_N=1
const MAX_CYCLES := 16
const HIGH_EY: float = PHI
const HIGH_EI := 1.0                           # q_coh ≈ 0.947
const HASH_NX := 128
const HASH_NY := 128
const HASH_NZ := 128
const CELL_W := (2.0 * EXTENT) / float(HASH_NX)
const HASH_TOTAL := HASH_NX * HASH_NY * HASH_NZ
const CELLS := N_GRID * N_GRID * N_GRID

var _rd: RenderingDevice
var _shader: RID
var _pipe: RID
var _us: RID
var _pos_buf: RID
var _vel_buf: RID
var _alive_buf: RID
var _mass_buf: RID
var _mom_buf: RID
var _cen_buf: RID
var _spin_buf: RID
var _fvel_buf: RID
var _mprev_buf: RID
var _site_dummy_buf: RID   # boxless site-read set (bindings 18-24) — unindexed dummy (boxless off)
var _ey_buf: RID
var _ei_buf: RID
var _best_buf: RID
var _sink_buf: RID
var _cc_buf: RID
var _cs_buf: RID
var _ch_buf: RID
var _cl_buf: RID
var _mc_buf: RID
var _pc := PackedFloat32Array()

var _input_pos := PackedFloat32Array()
var _input_vel := PackedFloat32Array()
var _input_mass := PackedFloat32Array()
var _checks := 0
var _failures := 0
var _t0 := 0


func _ready() -> void:
	_t0 = Time.get_ticks_msec()
	_rd = RenderingServer.create_local_rendering_device()
	if _rd == null:
		_check("local RenderingDevice acquired", false, "no RD — run windowed, not --headless")
		_finish()
		return
	_check("local RenderingDevice acquired", true)
	if not _load_pipeline():
		_finish()
		return
	_make_buffers()
	_upload_field()

	# print the derived threshold + actual Newtonian binding numbers up front
	print("[SubsonicStep] h0=%.6f  c_s=h0/dt=%.3f  R_m=%.4f" % [H0, C_S, R_M])
	print("[SubsonicStep] subsonic run v_t=%.2f (0.95·c_s); supersonic run v_t=%.2f (1.05·c_s)"
		% [0.95 * C_S, 1.05 * C_S])
	var qm := 0.0  # q_coh from the planted field; the binding gate uses pc.g_n
	var rho := HIGH_EY + HIGH_EI
	var eps := HIGH_EY - PHI * HIGH_EI
	qm = (rho * rho) / (rho * rho + PHI_INV2 + eps * eps)
	var g_n := G_N  # _fill_pc slot 16: the shader's actual pair-binding constant
	var v_up := 1.05 * C_S
	var req_m := v_up * v_up * D_PAIR / (4.0 * g_n)
	print("[SubsonicStep] q_mid=%.6f  g_n=%.4f  required m at v_up=%.1f → %.1f (chose %.0f, ×%.2f)"
		% [qm, g_n, v_up, req_m, MASS, MASS / req_m])

	# subsonic run: |v_t| = 0.95·c_s
	_build_input(0.95 * C_S)
	_upload_state()
	var merges_a := _run_merge()
	var alive_a := _alive_count()
	print("[SubsonicStep] subsonic run: v_t=%.2f → merges=%d alive=%d" % [0.95 * C_S, merges_a, alive_a])
	_check("G-S1: subsonic pair (|v_t|=0.95·c_s) merges (alive==1)",
		alive_a == 1, "alive=%d ½μv²d=%s g_n·m²=%s"
			% [alive_a, str(0.5 * (MASS / 2.0) * (0.95 * C_S) * (0.95 * C_S) * D_PAIR), str(g_n * MASS * MASS)])

	# supersonic run: |v_t| = 1.05·c_s
	_build_input(1.05 * C_S)
	_upload_state()
	var merges_b := _run_merge()
	var alive_b := _alive_count()
	print("[SubsonicStep] supersonic run: v_t=%.2f → merges=%d alive=%d" % [1.05 * C_S, merges_b, alive_b])
	_check("G-S2: supersonic pair (|v_t|=1.05·c_s) does NOT merge (alive==2)",
		alive_b == 2, "alive=%d (still bound per binding, but the subsonic gate blocks: |v_t|=%.2f ≥ c_s=%.2f)"
			% [alive_b, 1.05 * C_S, C_S])

	_finish()


func _load_pipeline() -> bool:
	var sf := load("res://compute/cassi_particle_merge.glsl") as RDShaderFile
	if sf == null:
		_check("merge shader loads", false)
		return false
	_shader = _rd.shader_create_from_spirv(sf.get_spirv())
	_pipe = _rd.compute_pipeline_create(_shader)
	_check("compute pipeline builds", _pipe.is_valid())
	return _pipe.is_valid()


func _make_buffers() -> void:
	_pos_buf = _rd.storage_buffer_create(N * 16)
	_vel_buf = _rd.storage_buffer_create(N * 16)
	_alive_buf = _rd.storage_buffer_create(N * 4)
	_mass_buf = _rd.storage_buffer_create(N * 4)
	_mom_buf = _rd.storage_buffer_create(N * 16)
	_cen_buf = _rd.storage_buffer_create(N * 16)
	_spin_buf = _rd.storage_buffer_create(N * 16)
	_fvel_buf = _rd.storage_buffer_create(CELLS * 16)
	_mprev_buf = _rd.storage_buffer_create(N * 4)
	_site_dummy_buf = _rd.storage_buffer_create(8192 * 16)   # boxless site set — unindexed (boxless off); sized like _ml_sites
	_ey_buf = _rd.storage_buffer_create(CELLS * 4)
	_ei_buf = _rd.storage_buffer_create(CELLS * 4)
	_best_buf = _rd.storage_buffer_create(N * 4)
	_sink_buf = _rd.storage_buffer_create(N * 4)
	_cc_buf = _rd.storage_buffer_create(HASH_TOTAL * 4)
	_cs_buf = _rd.storage_buffer_create(HASH_TOTAL * 4)
	_ch_buf = _rd.storage_buffer_create(HASH_TOTAL * 4)
	_cl_buf = _rd.storage_buffer_create(N * 4)
	_mc_buf = _rd.storage_buffer_create(64)
	var zero := PackedByteArray(); zero.resize(64); zero.fill(0)
	_rd.buffer_update(_mc_buf, 0, 64, zero)
	_us = _rd.uniform_set_create([
		_u(0, _pos_buf), _u(1, _vel_buf),
		_u(2, _alive_buf), _u(3, _mass_buf),
		_u(4, _mom_buf), _u(5, _cen_buf),
		_u(6, _ey_buf), _u(7, _ei_buf),
		_u(8, _best_buf), _u(9, _sink_buf),
		_u(10, _cc_buf), _u(11, _cs_buf),
		_u(12, _ch_buf), _u(13, _cl_buf),
		_u(14, _mc_buf), _u(15, _spin_buf),
		_u(16, _fvel_buf), _u(17, _mprev_buf),
		# Boxless site/query set (bindings 18-29). Dummies satisfy descriptor
		# validation; the boxless flag is off in this verifier.
		_u(18, _site_dummy_buf), _u(19, _site_dummy_buf),
		_u(20, _site_dummy_buf), _u(21, _site_dummy_buf),
		_u(22, _site_dummy_buf), _u(23, _site_dummy_buf),
		_u(24, _site_dummy_buf), _u(25, _site_dummy_buf),
		_u(26, _site_dummy_buf), _u(27, _site_dummy_buf),
		_u(28, _site_dummy_buf), _u(29, _site_dummy_buf),
	], _shader, 0)


func _u(binding: int, buf: RID) -> RDUniform:
	var u := RDUniform.new()
	u.uniform_type = RenderingDevice.UNIFORM_TYPE_STORAGE_BUFFER
	u.binding = binding
	u.add_id(buf)
	return u


## Uniform HIGH-q field everywhere → q_coh ≈ 0.947, q_ord = 1; isolates the
## subsonic gate (binding passes at both speeds by the Newtonian mass choice).
func _upload_field() -> void:
	var ey := PackedFloat32Array(); ey.resize(CELLS)
	var ei := PackedFloat32Array(); ei.resize(CELLS)
	for c in range(CELLS):
		ey[c] = HIGH_EY
		ei[c] = HIGH_EI
	# flow velocity field is zero (we seed velocity purely in the particles)
	var fvel := PackedByteArray(); fvel.resize(CELLS * 16); fvel.fill(0)
	_rd.buffer_update(_ey_buf, 0, ey.size() * 4, ey.to_byte_array())
	_rd.buffer_update(_ei_buf, 0, ei.size() * 4, ei.to_byte_array())
	_rd.buffer_update(_fvel_buf, 0, fvel.size(), fvel)


## One pair separated along x by D_PAIR, equal masses MASS, transverse
## relative velocity ±v_rel/2 along y (so |v_t| = |v_rel| exactly).
func _build_input(v_rel: float) -> void:
	_input_mass = PackedFloat32Array([MASS, MASS])
	var half := v_rel * 0.5
	_input_pos = PackedFloat32Array([
		5.0, 0.0, 0.0, 0.0,
		5.0 + D_PAIR, 0.0, 0.0, 0.0,
	])
	_input_vel = PackedFloat32Array([
		0.0, half, 0.0, 0.0,
		0.0, -half, 0.0, 0.0,
	])
	for i in range(N):
		_input_pos[i * 4 + 3] = _input_mass[i]


func _upload_state() -> void:
	_rd.buffer_update(_pos_buf, 0, _input_pos.size() * 4, _input_pos.to_byte_array())
	_rd.buffer_update(_vel_buf, 0, _input_vel.size() * 4, _input_vel.to_byte_array())
	_rd.buffer_update(_mass_buf, 0, _input_mass.size() * 4, _input_mass.to_byte_array())


func _fill_pc(pass_mode: float) -> void:
	_pc.resize(26)   # 26 floats: + boxless@24 + n_sites@25 (merge_boxless_prereg.md; both 0 here)
	_pc[0] = float(N)
	_pc[1] = PHI
	_pc[2] = PHI_INV2
	_pc[3] = Q_TH
	_pc[4] = R_M
	_pc[5] = EXTENT
	_pc[6] = EXTENT
	_pc[7] = EXTENT
	_pc[8] = float(N_GRID)
	_pc[9] = float(HASH_NX)
	_pc[10] = float(HASH_NY)
	_pc[11] = float(HASH_NZ)
	_pc[12] = CELL_W
	_pc[13] = CELL_W
	_pc[14] = CELL_W
	_pc[15] = pass_mode
	_pc[16] = G_N      # g_n: pass_best's Newtonian pair-binding/virial constant
	_pc[17] = XI
	_pc[18] = H0
	_pc[19] = DT
	_pc[20] = 1.0   # f_subsonic (the gate under test)
	_pc[21] = 0.0   # f_virial OFF (this fixture isolates binding + subsonic gates)
	_pc[22] = 1.0   # f_order
	_pc[23] = 0.0   # cyc_slot
	_pc[24] = 0.0   # boxless (site-direct read) — off in these tests → grid path
	_pc[25] = 0.0   # n_sites (nearest-site guard) — unused when boxless off


func _dispatch(pass_mode: float) -> void:
	_fill_pc(pass_mode)
	var cl := _rd.compute_list_begin()
	_rd.compute_list_bind_compute_pipeline(cl, _pipe)
	_rd.compute_list_bind_uniform_set(cl, _us, 0)
	_rd.compute_list_set_push_constant(cl, _pc.to_byte_array(), _pc.size() * 4)
	_rd.compute_list_dispatch(cl, int(ceil(float(N) / 256.0)), 1, 1)
	_rd.compute_list_end()
	_rd.submit()
	_rd.sync()


func _run_merge() -> int:
	_dispatch(0.0)   # reset
	var total := 0
	for cyc in range(MAX_CYCLES):
		_dispatch(1.0)   # fold
		zero_uints(_cc_buf, HASH_TOTAL)
		_dispatch(2.0)   # count
		var cc := _rd.buffer_get_data(_cc_buf, 0, HASH_TOTAL * 4).to_int32_array()
		var cs := PackedInt32Array(); cs.resize(HASH_TOTAL)
		var run := 0
		for c in range(HASH_TOTAL):
			cs[c] = run
			run += cc[c]
		_rd.buffer_update(_cs_buf, 0, cs.size() * 4, cs.to_byte_array())
		_rd.buffer_update(_ch_buf, 0, cs.size() * 4, cs.to_byte_array())
		_dispatch(3.0)   # fill
		_dispatch(4.0)   # best
		zero_uints(_mc_buf, 1)
		_dispatch(5.0)   # hop
		var merges := read_uint(_mc_buf)
		total += merges
		if merges == 0:
			break
	_dispatch(6.0)   # finalize
	return total


func zero_uints(buf: RID, count: int) -> void:
	var z := PackedByteArray(); z.resize(count * 4); z.fill(0)
	_rd.buffer_update(buf, 0, z.size(), z)


func read_uint(buf: RID) -> int:
	var d := _rd.buffer_get_data(buf, 0, 4)
	return int(d.decode_u32(0))


func _alive_count() -> int:
	var al := _rd.buffer_get_data(_alive_buf, 0, N * 4).to_float32_array()
	var cnt := 0
	for i in range(N):
		if al[i] > 0.5:
			cnt += 1
	return cnt


func _check(name: String, ok: bool, detail: String = "") -> void:
	_checks += 1
	if not ok:
		_failures += 1
	print("[%s] %s %s" % ["PASS" if ok else "FAIL", name, detail])


func _finish() -> void:
	var elapsed := Time.get_ticks_msec() - _t0
	print("[VerifySubsonicStep] checks=%d failures=%d elapsed=%d ms" % [_checks, _failures, elapsed])
	if _failures == 0:
		print("[VerifySubsonicStep] RESULT: PASS")
	else:
		print("[VerifySubsonicStep] RESULT: FAIL")
	get_tree().quit(0 if _failures == 0 else 1)
