...
vec3 presentation_coherence_emission(float coh, float sigma, float scheme) {
    float sigma01 = clamp(sigma, 0.0, 1.0);
    if (scheme > 0.5) {
        float hue = mix(0.70, 0.02, smoothstep(0.0, 1.0, coh));
        float lightness = clamp(0.085 + 0.10 * coh + 0.02 * sigma01, 0.07, 0.23);
        return presentation_hsl_to_rgb(vec3(hue, 0.82, lightness))
               * sigma * (0.38 + 0.42 * coh);
    }
    float lower_hue = mix(0.72, 0.56, smoothstep(0.0, 0.55, coh));
    float hue = mix(lower_hue, 0.93, smoothstep(0.45, 1.0, coh));
    float lightness = clamp(0.095 + 0.095 * coh + 0.015 * sigma01, 0.07, 0.22);
    return presentation_hsl_to_rgb(vec3(hue, 0.84, lightness))
           * sigma * (0.45 + 0.35 * coh);
}
...
vec4 integrate(vec3 ro, vec3 rd, float ta, float tb, out bool overflowed) {
    ...
    for(uint step=0u;step<limit;++step){
        ...
        vec3 emit;
        if (pc.coherence_mask > 0.5) {
            emit = presentation_coherence_emission(coh, sigma, pc.gradient_mask);
        } else {
