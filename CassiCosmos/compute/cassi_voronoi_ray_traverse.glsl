...
layout(set=0,binding=2,std430)readonly buffer Rays{vec4 rays[];};
...
bool connected(int a,int b,int words){return(adj[a*words+(b>>5)]&(1u<<uint(b&31)))!=0u;}
...
void main(){
 int q=int(gl_GlobalInvocationID.x),nr=int(pc.n_rays_f),ns=int(pc.n_sites_f),words=int(pc.words_f),cap=min(int(pc.max_seg_f),MAX_SEG);if(q>=nr)return;
 ...
 for(int step=0;step<cap;++step){
  ...
  for(int j=0;j<ns;++j){if(j==cur||!connected(cur,j,words))continue;vec3 r=site_world(j)-si;float den=dot(r,rd);if(den<=EPS_DEN)continue;float th=tc+(dot(r,r)-2.*dot(r,p-si))/(2.*den);if(!(th>tc+eps)||!(th<texit-eps))continue;if(th<best-eps||(abs(th-best)<=eps&&(nxt<0||j<nxt))){best=th;nxt=j;}}
