...
on the periodic square [0, L)^2. Two facts drive the design:
  ...
  - The spectral reference integrates each Fourier mode EXACTLY in time
    (per-mode eigenpair, psi(t) = sum_j c_j e_j cos(w_j t)), so the Voronoi
    finite-volume solver must converge to a time-continuous reference.

The Voronoi solver: static periodic mesh (3x3 tile replication of the seed
set; the central tile's cells are its periodic Voronoi cells), Green-Gauss
gradient reconstruction, symmetric face fluxes c2 (grad psi . n) l, cell-local
reaction, leapfrog time stepping. Same ICs, same L, same dt grid.

Gate verdicts (printed PASS/FAIL):
  V1 breather frequency matches OMEGA (both solvers, < 2%)
  V2 r(t) = <EY>/<EI> trajectories agree (spectral vs Voronoi, < 5%)
  V3 field snapshots agree (L2 shrinks as sites increase, < 5% at top count)
  V4 spectra agree in the resolved band (corr > 0.98)
...
# ─────────────────────────────────────────────────────────────────────────
# Voronoi finite-volume solver — static periodic mesh
# ─────────────────────────────────────────────────────────────────────────
def _lloyd_relax(seeds, L, iters=30):
    """Lloyd relaxation — move each seed to its cell centroid (AREPO's mesh
    regularization, done once on the static mesh): cells become near-regular
    hexagons, taming the discrete Laplacian's stiffness (jittered slivers
    make the leapfrog flux term unstable)."""
    n = len(seeds)
...
class VoronoiFV2D:
    """Static periodic Voronoi mesh via 3x3 tile replication. Cell state
    (psiY, psiI, piY, piI); Green-Gauss gradients; symmetric face fluxes;
    leapfrog. Face arrays are precomputed for vectorized stepping."""

    def __init__(self, seeds, L):
        self.L = L
        # Mesh regularization FIRST (AREPO's own discipline): relax the
        # jittered seeds to their cell centroids so the discrete Laplacian
        # is well-conditioned for the leapfrog flux term.
        seeds = _lloyd_relax(seeds.copy(), L)
        n = len(seeds)
        offs = np.array([[dx, dy] for dx in (-L, 0.0, L) for dy in (-L, 0.0, L)])
        all_seeds = (seeds[None, :, :] + offs[:, None, :]).reshape(-1, 2)
        vor = Voronoi(all_seeds)
        ...
        # cell areas from the regions (abs of the signed shoelace sum)
        areas = np.empty(n)
        for c, a in enumerate(central):
            reg = vor.regions[vor.point_region[a]]
            verts = np.array([vor.vertices[v] for v in reg])
        ...
        for (a, b), rv in zip(vor.ridge_points, vor.ridge_vertices):
            ...
            # its own outward normal). Emitting only one leaves half the mesh
            # face-less and the operator directed.
            if a_central:
    ...
    def step(self, psiY, psiI, piY, piI, dt):
        ...
        # reconstruction returns for the advective terms in the moving-mesh
    ...
    def dt_cfl(self):
        ...
        with h_min the smallest inradius proxy 2A/P per cell (well-conditioned
        after Lloyd relaxation; sliver jitter would make this tiny — the
        honest way to keep the flux term stable)."""
...
# ─────────────────────────────────────────────────────────────────────────
# ICs — smooth noise + ratio r0 = 1.5 (the sim's default initial_ratio)
# ─────────────────────────────────────────────────────────────────────────
def make_ics(N, L, rng, r0=1.5, amp=0.02):
...
def _breath_freq(r, dt):
    ...
    (the raw bins at dt=0.005, T=1.5 are 4.2 rad/s apart — too coarse)."""
    r_cent = r - r.mean()
    n = len(r)
...
def main():
    ...
    for n in site_counts:
        # jittered hexagonal seeds — well-shaped Voronoi cells
        cols = int(np.sqrt(n * np.sqrt(3.0) / 2.0)) + 1
        spacing = L / cols
        sx, sy = [], []
        for j in range(cols + 1):
            for i in range(cols + 1):
                px = (i + 0.5 * (j % 2)) * spacing + rng.uniform(-0.3, 0.3) * spacing
                py = j * spacing * np.sqrt(3.0) / 2.0 + rng.uniform(-0.3, 0.3) * spacing
        ...
        r_vor, d_vor, ey_v, ei_v = fv.solve(
            ...
            dt_use, len(t_v) - 1)
        # rasterize the final fields for L2 + spectra
    ...
    print("RESULT: %s" % ("ALL PASS" if (v1 and v2 and v3 and v4) else "FAILURES PRESENT"))
