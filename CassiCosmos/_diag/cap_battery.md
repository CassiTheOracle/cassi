...
| **c** — determinism in the compatibility regime | The filling structure, tracking OFF vs ON (the tracker no-ops — the re-centered filling's percentile-mid is EXACTLY zero at the seed-tracking) — the open pipeline must be bit-identical (max-diff == 0.0), in BOTH the closed-box (poisson) and the OPEN (tree) arms. | **WIRING FIXED (finish-B); the pin still blocked by the harness sequence + the mesh kernels** — the tracking wiring is now bit-identical (the matched-accounting probe `b_gatec_probe`: pos/hdr/rho max-diff 0.000000 in both arms; the battery's tree-arm pos 121.9 → 0.222). The remaining residuals are NOT the wiring: (1) the battery's ON canary runs 4 extra steps (the `_tracked and _cadence == 1` batch — OFF@600 vs ON@604 = the poisson's 0.049); (2) the canary reinit's unseeded field IC (the mesh sites sample a different random IC per canary — the tree pos ~0.17 + the field ~0.017); (3) the mesh rebuild's float-atomic centroid (the `OpAtomicFAddEXT` order-dependent sums — the mesh-driven field's run-to-run jitter ~0.002, a field-kernel matter). |
...
1. **The tree-arm compatibility canary (RESOLVED in the wiring — finish-B)**: the
   adaptive root was gated on the home_window FLAG, not the tracker's re-fit
   state — enabling the tracked window changed the tree's root half (the
   structure-rooted cube from the site lattice ≈ 0.97× the box vs the box
   cube) even when the tracker no-ops, so the tree arm was NOT bit-identical
   to the closed box in the compatibility regime (pos max-diff 121.9 over
   600 steps). FIXED: the root gates on the tracker's actual re-fit state
   (`_tree_worker_frame`'s window_refit + the engine's box_scale/origin
   check) + the worker cadence phase resets with the worker (`_tl_frame`).
   The battery's tree-arm pos is now 0.222; the matched-accounting probe
   proves the wiring itself is bit-identical (pos/hdr/rho 0.0 in both arms).
   The remaining residuals are the harness's 4-step comparison offset, the
   unseeded-IC canary premise, and the mesh rebuild's float-atomic centroid
   (a field-kernel nondeterminism) — see the gate-c row.
